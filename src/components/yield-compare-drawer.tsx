...
const COMPARE_ROW_DESCRIPTORS: readonly CompareRowDescriptor[] = [
  ...
  },
  {
    ...
    align: "right",
    render: (row) =>
      row.yieldStability !== null ? `${Math.round(row.yieldStability * 100)}%` : "—",
  },
  {
    key: "warnings",
    label: "Warnings",
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Side-by-side metrics for the {ids.length} coins you selected.
          </SheetDescription>
        </SheetHeader>
          ...
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr>
                <th
                  scope="col"
                  className="border-b border-border/60 py-2 pr-3 text-left text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground"
                >
                  Metric
                </th>
                {columns.map((column) => {
                  ...
                  return (
                    <th
                      key={column.id}
                      scope="col"
                      className="border-b border-border/60 px-2 py-2 text-left align-bottom"
                    >
            ...
            <tbody>
              {COMPARE_ROW_DESCRIPTORS.map((descriptor) => (
                <tr key={descriptor.key} className="border-b border-border/30 last:border-b-0">
                  <th
                    scope="row"
                  ...
                  {columns.map((column) => {
                    if (!column.row) {
                      return (
                        <td
                          key={column.id}
                          className="px-2 py-2 text-left text-xs italic text-muted-foreground"
                        >
                          {PLACEHOLDER}
                        </td>
                      );
