...
// Phase → thin tinted ring (static strings for the Tailwind scanner). Hues mirror
// PHASE_BADGE so the five readiness zones read as distinct.
...
export function UpcomingHorizonHero(): React.JSX.Element {
  return (
    <div className="h-full">
      {/* Wide layout (lg+): one count-scaled tinted ring per readiness stage, the
          packed dot cluster centered inside, columns split by hairlines. */}
      <div className="hidden h-full grid-cols-5 divide-x divide-border/50 lg:grid">
      ...
      {/* Narrow layout (below lg): phases stack as labelled lanes — a fixed label
          column, a hairline, then the dots spread to the right. */}
      <div className="divide-y divide-border/50 lg:hidden">
        {PHASE_ORDER.map((phase, i) => {
          ...
          return (
            <div key={phase} className="flex items-center gap-2 px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3.5">
