...
export const SAFETY_SCORES_SECTION_CONTENT = defineMethodologySectionContent({
  ...
  markdownParagraphs: [
    ...
    "Since methodology 9.48, a live composition with no mapped tracked-asset slices publishes no dependency edges rather than restoring older curated or manual weights. Dependency facts retain per-slice rejection reasons for unmatched identities, expired matching classifications, and explicitly reviewed non-links. Curated fallback applies only when no live composition exists; partial live mappings retain their live weights.",
