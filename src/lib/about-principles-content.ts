...
export const PRINCIPLES_AXIOMS: readonly PrincipleAxiom[] = [
  {
    id: "independence",
    title: "Independence is maintained through disclosed funding and editorial boundaries.",
  ...
  },
  {
    ...
    body: "Most trackers show what a stablecoin is worth right now. Pharos shows what could make it fail: backing, governance, freeze authority, dependency exposure, peg behavior, and exit liquidity. Price is the current outcome. Pharos measures the conditions that can change it.",
  },
  {
    id: "reserves-vs-freeze",
    title: "Reserves do not matter if you can be frozen.",
    body: "A reserve schedule records the balance sheet; transfer permissions depend on the issuer's controls. If the issuer can blacklist addresses, freeze contracts, or pause transfers, those reserves belong to a holder only until the issuer says otherwise. FreezeWatch exists for that reason, and blacklist exposure is a first-class input into every safety grade.",
  },
  {
    id: "por-theatre",
    title: "Real-time proof of reserves is theatre when the bank holds the keys.",
    body: "An attestation signed, hosted, and refreshed by the same entity that custodies the assets has no independent corroboration. Pharos treats issuer-controlled feeds as opaque until a third party with no commercial dependency on the issuer can corroborate the read.",
  },
  {
  ...
  },
  {
    ...
    body: "When a tracked stablecoin effectively dies, Pharos freezes the detail page rather than deleting it. The cemetery preserves failures as a research surface. A dashboard that omits its failures cannot be trusted with the ones that have not failed yet.",
  },
  {
