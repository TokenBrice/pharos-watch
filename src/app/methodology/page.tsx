...
function MethodologyDetails({
  ...
  return (
    ...
    >
      <summary className="cursor-pointer px-4 py-3.5 text-sm font-semibold text-foreground">{summary}</summary>
      <div className="space-y-6 border-t border-border/50 px-4 pb-5 pt-4">{children}</div>
    </details>
  );
}

function MethodologyFacts({ facts }: { facts: Array<{ label: string; value: string }> }) {
  return (
...
function WorkedExample({ children, summary }: { children: ReactNode; summary: string }) {
  return (
    <details data-methodology-worked-example="true" className="rounded-2xl border border-border/60 bg-background/80">
      <summary className="cursor-pointer px-4 py-3.5 text-sm font-semibold text-foreground">{summary}</summary>
      <div className="space-y-2 border-t border-border/50 px-4 pb-4 pt-4 text-sm text-muted-foreground leading-relaxed">
        {children}
      </div>
    </details>
  );
