    ...
    >
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p>These overrides set <code className="text-xs">priceSource = &quot;protocol-redeem&quot;</code> and <code className="text-xs">priceConfidence = &quot;high&quot;</code> when the quote validates against peg bounds, and they are applied after the GeckoTerminal probe so later market checks cannot overwrite them. Recovery scheduling treats a current price as usable only when it is positive and carries publishable source and observation-time provenance, and each live candidate has a bounded slice of the shared budget so one stalled wrapper cannot skip the remaining active gaps. Parent trust is judged on the composite&apos;s replay-safe core, so an agreeing non-replay-safe corroborator that joins a parent&apos;s consensus label can neither strip trust from a high-confidence parent nor upgrade one the gate would otherwise reject. Audited wrapper exceptions can use a fresh high-confidence same-run parent consensus for live pricing while keeping historical replay on replay-safe sources.</p>
          </div>
