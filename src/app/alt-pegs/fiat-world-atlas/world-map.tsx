...
function buildStyleBlock(fills: ReadonlyMap<string, { colorHex: string }>): string {
  const rules: string[] = [
    ...
    `.dark .fiat-world-map{--world-default-fill:oklch(0.28 0.014 248 / 1);--world-stroke:oklch(0.62 0.02 248 / 0.85)}`,
...
export function WorldMap({ items }: { items: readonly AltPegLinkHubItem[] }) {
  ...
  return (
    <div className="fiat-world-map relative w-full" aria-label="World map showing tracked fiat peg reference regions">
      <style>{styleBlock}</style>
      <div className="[&_svg]:h-auto [&_svg]:w-full" dangerouslySetInnerHTML={{ __html: WORLD_SVG }} />
    </div>
  );
}
