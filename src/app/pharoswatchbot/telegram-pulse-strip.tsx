  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs tabular-nums">
      <span className="text-muted-foreground">
        <span className="font-semibold text-foreground font-mono">{formatCount(data.activeWatchers)}</span> active watchers
      </span>
      <span className="hidden text-border sm:inline" aria-hidden="true">&middot;</span>
      <span className="text-muted-foreground">
        <span className="font-semibold text-foreground font-mono">{formatCount(data.coinSubscriptions)}</span> active coin follows
      </span>
...
export function TelegramPulseBoard({ className }: { className?: string }) {
  ...
  if (isLoading) {
    return (
      <div className={cn("grid gap-3 sm:grid-cols-3", className)} aria-label="Loading Telegram adoption metrics">
    ...
    return (
      <div className={cn("rounded-xl border border-border/60 bg-muted/25 px-4 py-3", className)}>
        <p className="text-xs text-muted-foreground">
          Live Telegram adoption metrics are temporarily unavailable. Bot links and setup commands still work.
        </p>
      </div>
    );
  }
    ...
    <section className={cn("space-y-3", className)} aria-label="Live Telegram adoption metrics">
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-border/60 bg-background/40 px-4 py-3">
          <p className="pharos-kicker">Watchers</p>
          <p className="mt-1 font-mono text-2xl font-semibold tabular-nums text-foreground">
            {formatCount(data.activeWatchers)}
          </p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">active chats with alerts enabled</p>
        </div>
        <div className="rounded-xl border border-border/60 bg-background/40 px-4 py-3">
          <p className="pharos-kicker">Coin follows</p>
          <p className="mt-1 font-mono text-2xl font-semibold tabular-nums text-foreground">
            {formatCount(data.coinSubscriptions)}
          </p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">explicit per-coin alert subscriptions</p>
        </div>
        <div className="rounded-xl border border-border/60 bg-background/40 px-4 py-3">
          <p className="pharos-kicker">Most watched</p>
          {topCoins.length > 0 ? (
      ...
      <div className="rounded-xl border border-border/60 bg-background/40 px-4 py-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="pharos-kicker">All-time watcher growth</p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              Cumulative current active watchers by the day each chat first subscribed.
            </p>
          </div>
          {latestHistoryPoint ? (
