...
describe("buildSelectorRows", () => {
  ...
  it("includes the selected non-USD universe and hashes that selected peg", () => {
    ...
    const result = buildSelectorRows(
      baseArgs({
        pegCurrency: "EUR",
        stablecoinsData: stablecoinsData([eurAsset]),
  ...
  it("uses a SHA-256 dataset hash over decision-affecting fields only", () => {
    ...
    const changedDecisionField = buildSelectorRows(
      baseArgs({
        reportData: reportData({
    ...
    const timestampOnly = buildSelectorRows(
      baseArgs({
  ...
  it("includes methodology versions in the dataset hash", () => {
    ...
    const changedMethodology = buildSelectorRows(
      baseArgs({
  ...
  it("preserves Yield altSources for engine venue/risk/freshness selection", () => {
    ...
    expect(sources.map((source) => source.sourceKey)).toEqual(["aave-v3-usdc", "curve-usdc"]);
  ...
  it("prefers Safety-gated report-card effective exit and maps raw inputs", () => {
    const result = buildSelectorRows(
      baseArgs({
        ...
        dexData: {
          "usdc-usd-coin": makeDexLiquidityData({
