...
export const DATA_SOURCE_GROUPS = [
  ...
  },
  {
    label: "DEX Data",
    sources:
      "DeFiLlama Yields & Protocols, protocol-native yield APIs and deterministic on-chain yield readers (Hashnote, Ondo, Midas NAV oracles, Re Protocol, Morpho, Pendle, Royco Dawn, Yearn Kong, Beefy, Aave V3, Compound V3, BIMA Earn, Curve scrvUSD current-rate, B.Protocol LQTY-only, Zephyr Scanner), vaults.fyi as an optional gated supplemental yield source that is disabled by default and rankable only for explicitly allowlisted vaults, Curve Finance API, The Graph, Fluid API + DexReservesResolver, Balancer API, Raydium API, Orca API, Jupiter direct-route quotes, Meteora API, PancakeSwap subgraphs, SUN.io SunSwap V2 pool census and SUN Smart Router proofs with TronGrid RPC state, shadow Uniswap V3 and PancakeSwap V3 QuoterV2/factory RPC reads, Aerodrome and Velodrome Sugar view contracts, GeckoTerminal, DexScreener; dead or deprecated DEX slugs such as Bunni are blocked from runtime pricing and liquidity inputs rather than treated as live venues",
  },
