...
export const DATA_SOURCE_GROUPS = [
  ...
  },
  {
    ...
    sources:
      "Issuer and protocol reserve APIs, dashboards, proof-of-reserve portals, issuer attestation indexes, and direct on-chain vault/accounting reads (including Ethena collateralization and proof-of-reserves measurements, plus live reserve composition feeds from providers such as 3Jane, Anzen, Falcon, Frankencoin, Hashnote, infiniFi, M0, Mento Reserve / analytics API, OpenEden, Origin, Blast, Nest Credit, Re, Resupply, Reserve Protocol, USDD, USD.AI, USD1 Chainlink bundle oracle, Accountable, Hyperbeat, Tether, Frax, Circle, First Digital Labs, Ripple, SG-FORGE, Paxos, Sky/MakerDAO, Chainlink PoR/NAV oracles, StraitsX, Kinesis, Quantoz, Yamato, Aave GHO, BIMA, SMARDEX, f(x), Asymmetry, JupUSD, Morpho vault liquidity, USDGO, Yield Optimizer, Yuzu, Solstice, River, Alloy, Zephyr Scanner, Spiko, United Stables, Polymarket PUSD vault reads, and Curve/Yield Basis reserve reads where available)",
  },
  {
  ...
  },
  {
    label: "DEX Data",
    sources:
      "DeFiLlama Yields & Protocols, protocol-native yield APIs and deterministic on-chain yield readers (Hashnote, Ondo, Midas NAV oracles, Re Protocol, Morpho, Pendle, Royco Dawn, Yearn Kong, Beefy, Aave V3, Compound V3, BIMA Earn, Curve scrvUSD current-rate, B.Protocol LQTY-only, Zephyr Scanner), vaults.fyi as an optional gated supplemental yield source that is disabled by default and rankable only for explicitly allowlisted vaults, Curve Finance API, The Graph, Fluid API + DexReservesResolver, Balancer API, Raydium API, Orca API, Jupiter direct-route quotes, Meteora API, PancakeSwap subgraphs, SUN.io SunSwap V2 pool census and shadow SUN Smart Router proofs with TronGrid RPC state, shadow Uniswap V3 and PancakeSwap V3 QuoterV2/factory RPC reads, Aerodrome and Velodrome Sugar view contracts, GeckoTerminal, DexScreener; dead or deprecated DEX slugs such as Bunni are blocked from runtime pricing and liquidity inputs rather than treated as live venues",
  },
