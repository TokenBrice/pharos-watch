Safety Score V9 is now fully released and functional on Pharos, covering every stablecoin we track. It is a ground-up rebuild of how Pharos thinks about stablecoin risk, and it reshuffles the results: USDT (Tether) and USDC (Circle) now sit at the top of the board, wrapped stablecoins can no longer outscore the assets they wrap, and every score now tells you exactly which evidence it stands on and who owes whatever is missing. If that first item just raised your eyebrow: good. The full story is below, and it starts with an admission about our old score. This is the release I teased in [our six-months post](/blog/pharos-at-six-months/) as "one version to rule them all," and it was worth the wait.

Here is what changed, why, and what it means for how you read a Pharos score.
...
Same three pillars for everyone, but the evidence feeding those pillars is tailored to what can actually kill each design. The result is that a V9 score means the same thing across radically different stablecoins: this is how safe this thing is, *given what it is*.
...
This is not theory; it is live on the board. Bucket Protocol's BUCK, a CDP stablecoin on Sui, posts a respectable Exit pillar, and an F: its oracle control topology is opaque, and that single critical signal caps the entire score at 39, whatever the other pillars say. The report card names the cap and the reason, in plain text, on the page.
