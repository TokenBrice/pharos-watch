export const meta = {
  name: 'docs-verify',
  description: 'Verify the whole /docs corpus against actual code; surface and adjudicate doc-vs-code discrepancies',
  phases: [
...
log(`Loaded ${loaded.docs.length} docs; verifying ${items.length} (api-reference.md skipped: generated + CI-current). deep=${items.filter((i) => i.depth === 'deep').length} light=${items.filter((i) => i.depth === 'light').length}`)

// ===========================================================================
// Pipeline: verify (sonnet) -> adjudicate (opus). No barrier between docs.
const perDoc = await pipeline(
  items,
  (item) => agent(buildVerifyPrompt(item), {
...
log(`Adjudication complete: ${confirmed.length} confirmed/revised findings across ${new Set(confirmed.map((c) => c.doc)).size} docs`)
