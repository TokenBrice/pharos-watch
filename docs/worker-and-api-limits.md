...
| API key default limiter          | `120 requests / 60 seconds` per key             | `shared/lib/ops-limits.ts`, `worker/src/lib/api-keys.ts`        | Every `/api/*` request (except `/api/telegram-webhook` and admin routes) requires a valid `X-API-Key`; the D1-backed `api_key_rate_limit` table enforces per-key quotas, and per-key overrides live in `api_keys.rate_limit_per_minute`. API-key auth/limiter dependency failures fail closed with `503` and `Retry-After: 60`; last-used metadata writes are best-effort. |
