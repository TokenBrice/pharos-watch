...
| Status-tracked cron jobs         | `47`                                            | `shared/lib/cron-jobs.ts`                                     | These are the jobs expected by `/api/status`. Latest additions include `cron-slot-sweeper` (cross-slot stale lease/slot reconciliation), `data-invariant-canary` (DB/cache-only structural checks behind `WORKER_CANARY_MODE`), `reserve-post-sync-watchdog` (named reserve drift/cache/stale-source accounting), `cron-staleness-watchdog` (quarter-hour freshness alerting), `project-tape` (DB-only `dewsPsiOffset` projection from existing source tables into `tape_events`), `telegram-pulse-snapshot` and `telegram-disambiguation-cleanup` (DB-only five-minute sidecars), `worker-repair-runner` (DB-only repair-task claim/defer/close loop behind `WORKER_REPAIR_RUNNER_MODE`), `prune-detail-cache`, `mint-burn-growth-watchdog`, `cron-duration-watchdog`, `telegram-inactive-cleanup`, `telegram-retention-cleanup` (`daily0300Utc`), and `yield-coverage-audit` (monthly). |
| Runtime jobs actually scheduled  | `47` status-tracked jobs, plus `2` budget-only scheduled surfaces | `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Runtime scheduling matches the shared status metadata set for `/api/status`. The additional budget-only surfaces are Telegram registration reconciliation on the five-minute Telegram slot and the `*/5 * * * *` digest-trigger poll, which executes pending manual requests under the existing `daily-digest` lease. |
...
| Live reserve sync internal run budget  | `11 minutes`        | `worker/src/cron/sync-live-reserves-config.ts`                    | Default cursoring budget; if the remaining budget drops below one adapter attempt, the untouched tail is marked deferred and resumed from cursor on the next run, leaving tail room for D1 cleanup and cron logging. Optional finalization cleanup/history pruning is skipped when the D1 tail budget is already exhausted, and the skip is recorded in cron metadata. |
...
| Telegram load simulation targets       | `500`, `1,000`, `5,000`, `10,000` active watchers | `scripts/ci/check-telegram-load.ts` | `npm run check:telegram-load` estimates drain time, per-invocation `estimatedCpuMs`, and D1 operations for single depeg, market-wide burst, DEWS+safety burst, admin broadcast, and Telegram 429 storm scenarios using the 3,600/run sender budget, 900/run pending drain, 3,664 pre-format chunk budget, Bot API broadcast pacing, p95 send latency, and D1 write cost. The CPU dimension (C102) reads `cpu_ms` from `worker/wrangler.toml`, caps the modeled format-count at the fresh budget plus allowance, and fails when the required 5,000-watcher scenario exceeds `0.5×cpu_ms`. The 5,000-watcher target is the milestone; 10,000 is exploratory headroom. |
...
- `sync-blacklist` materializes producer-owned blacklist gap snapshots into the existing D1 `cache` table only after an `ok` run with enough remaining snapshot budget. Runtime pressure still degrades the run when the subrequest budget is exhausted, a config scan is incomplete, or the skipped tail exceeds the bounded per-run tolerance (15% of configs, capped at 10 contracts); a smaller skipped tail remains visible in metadata but can still refresh producer snapshots. It writes both full summary metrics and core health/status metrics from one live full query. `/api/health`, `/api/status`, and `/api/blacklist-summary` prefer those producer snapshots, then fall back to the short request cache and finally the live query if the producer row is missing or stale.
- `/api/blacklist-summary` also has a producer-owned summary payload snapshot in the same D1 `cache` table. The handler preserves the public response shape, serves a fresh producer payload directly, serves a stale-but-present producer payload while refreshing it in `waitUntil`, and falls back to live construction only when no valid snapshot has ever been written. Freshness headers are based on the producer snapshot timestamp, so stale-but-served fallback snapshots still emit the normal `Warning` header once they exceed the blacklist summary max age.
