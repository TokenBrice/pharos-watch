...
| Status-tracked cron jobs         | `28`                                            | `shared/lib/cron-jobs.ts`                                     | These are the jobs expected by `/api/status`                                                                                                                 |
...
| Sim balances treasury snapshot        | `12 s` timeout/request, `1` retry, `1000 ms` between owner groups, `2` peak connections in daily 08:00 lane | `worker/src/lib/constants.ts`, `shared/lib/cron-jobs.ts`, `worker/src/cron/sync-treasury-stable-exposure.ts`   | Each owner group issues one full-balances read plus one stablecoin-only read, then waits before the next group so the free-tier path stays under the 5 RPS ceiling; launch scope is capped by reviewed owner-chain tuples |
