...
| Direct DEX API fetch phase            | `2` protocol fetches in parallel, `5` nested connection static peak, `15 s` request timeout, deterministic page caps (`50` default); Fluid ticker retries cap provider `Retry-After` sleeps at `5 s` per retry | `worker/src/cron/dex-liquidity/direct-api-policy.ts`, `worker/src/cron/dex-liquidity/direct-api-paginated.ts`, protocol fetchers, `shared/lib/cron-jobs.ts` | Runs inside the existing `sync-dex-liquidity` trigger; nested protocol fetchers can overlap inside the two-protocol cap, so `check:cron-connections` treats this trigger as 5/6 and headroom-full; page-cap errors include resume markers for large or drifting upstream responses |
