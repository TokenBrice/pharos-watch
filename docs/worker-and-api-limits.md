...
| Mint/burn runtime self-budget          | `9 minutes`; `60 seconds` minimum next-config window | `worker/src/cron/mint-burn/run-configs.ts`                        | Guardrail below the 10-minute cron wrapper timeout so the runner can stop before starting a config that would not leave enough persistence/logging tail room |
