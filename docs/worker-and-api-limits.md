...
| Cron expressions / trigger slots | `19`                                            | `worker/wrangler.toml`, `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Public status tooling groups around these trigger slots; the shared runner registry is the dispatch authority checked by `npm run check:cron-sync`; `check:cron-connections` models chained jobs with `connectionGroup` metadata. 2026-04-17: added `daily0300Utc` slot for `prune-status-probe-runs` (90d retention). 2026-04-19: added `dewsPsiOffset` (`26,56 * * * *`) so DB-only DEWS/PSI publication is not coupled to DEX-liquidity CPU usage. 2026-04-23: added `halfHourlyChartsOffset` (`16,46 * * * *`) so `sync-stablecoin-charts` no longer shares an invocation budget with `sync-dex-liquidity`. 2026-05-11: added `daily0810Utc` (`10 8 * * *`) so `discovery-scan` no longer shares the 08:05 lane. |
| Status-tracked cron jobs         | `33`                                            | `shared/lib/cron-jobs.ts`                                     | These are the jobs expected by `/api/status`. Latest additions: `telegram-degradation-watchdog` (five-minute lane alongside `dispatch-telegram-alerts`) and `telegram-inactive-cleanup` (`daily0300Utc`).                                                                                                                |
| Runtime jobs actually scheduled  | `33`                                            | `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Runtime scheduling matches the shared status metadata set. The additional `*/5 * * * *` digest-trigger poll is a runner slot that executes pending manual requests under the existing `daily-digest` lease, not a separate status job. |
| API key default limiter          | `120 requests / 60 seconds` per key             | `shared/lib/ops-limits.ts`, `worker/src/lib/api-keys.ts`        | Non-exempt `/api/*` requests require a valid `X-API-Key`; the no-key public exceptions are health, OG images, feedback, self-serve key request/verification, and the Telegram webhook. The D1-backed `api_key_rate_limit` table enforces per-key quotas, and per-key overrides live in `api_keys.rate_limit_per_minute`. Protected cacheable `GET` routes also keep a bounded isolate-local verified-key cache plus a bounded isolate-local fallback limiter so recently verified keys can ride through brief D1 auth/limiter outages; cold or unknown keys still fail closed with `503` and `Retry-After: 60`. Last-used metadata writes are best-effort. |
...
| Telegram group admin membership cache  | `5 minutes`         | `worker/src/lib/telegram-chat-member.ts`                          | `/subscribe`, `/unsubscribe`, `/set` in group/supergroup chats trigger one `getChatMember` lookup per webhook invocation (cached under `telegram:chat-member:<chat_id>:<user_id>`); soft-launch warnings additionally fetch `getChatAdministrators` (cached under `telegram:chat-admins:<chat_id>`). Both calls happen on the webhook ingress path, not inside the dispatch cron lane, so they do not draw from the 6-connection cron pool. |
...
| Address-price augmentation group      | `90 s` total budget, `5 s` timeout/request, `0` retries                      | `worker/src/lib/address-price-providers/index.ts` | Runs during primary pricing only for assets with missing prices or previous source depth below 3                  |
| DexScreener address augmentation      | `10` requests/run, `30` addresses/request                                    | `worker/src/lib/address-price-providers/index.ts` | Exact `/tokens/v1/{chain}/{addresses}` lane; no symbol search                                                     |
...
| GeckoTerminal address augmentation    | `5` requests/run, `30` addresses/request                                     | `worker/src/lib/address-price-providers/index.ts` | Public simple token-price lookup, separate from the serialized pool probe                                         |
...
| Moralis address augmentation          | `10` requests/run, `30` addresses/request                                    | `worker/src/lib/address-price-providers/index.ts` | Optional keyed EVM batch lookup                                                                                   |
