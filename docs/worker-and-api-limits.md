...
| Cron expressions / trigger slots | `20`                                            | `worker/wrangler.toml`, `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Public status tooling groups around these trigger slots; the shared runner registry is the dispatch authority checked by `npm run check:cron-sync`; `check:cron-connections` models chained jobs and budget-only scheduled surfaces with `connectionGroup` metadata. The 2026-07-10 reserve-recovery expression (`1,6,11,16,21,26,31,36,41,46,51,56 * * * *`) deliberately consumes the 20-trigger soft cap because recovery must remain independent of the interrupted reserve invocation. |
| Status-tracked cron jobs         | `48`                                            | `shared/lib/cron-jobs.ts`                                     | These are the jobs expected by `/api/status`. The latest addition is `reserve-recovery`, an isolated five-minute checkpoint reconciliation and suffix-resume job; other recent jobs include `cron-slot-sweeper`, `data-invariant-canary`, `reserve-post-sync-watchdog`, `cron-staleness-watchdog`, `project-tape`, Telegram sidecars, worker repair/prune/watchdog jobs, and the monthly yield coverage audit. |
| Runtime jobs actually scheduled  | `48` status-tracked jobs, plus `2` budget-only scheduled surfaces | `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Runtime scheduling matches the shared status metadata set for `/api/status`. The additional budget-only surfaces are Telegram registration reconciliation on the five-minute Telegram slot and the `*/5 * * * *` digest-trigger poll, which executes pending manual requests under the existing `daily-digest` lease. |
...
| Live reserve recovery poll / deadline  | `5 minutes` / `13 minutes`; `2/6` connection peak | `worker/src/handlers/scheduled/reserve-recovery.ts`, `worker/src/lib/scheduled-recovery-checkpoint.ts`, `shared/lib/cron-jobs.ts` | The isolated offset lane reconciles only abandoned four-hour reserve slots after their exact child lease expires, seals the old generation, and resumes unfinished queue items and sidecars under a new attempt number. Queue-hash mismatch fails closed. |
...
| Telegram dispatch overall deadline     | `14 minutes` hard timeout; `4 minutes` send-loop soft deadline | `worker/src/lib/cron-lease.ts`, `worker/src/handlers/scheduled/context.ts`, `worker/src/lib/telegram-constants.ts` | Dedicated five-minute Telegram lane timeout plus 30-second lease heartbeat; pending-drain and fresh-send loops stop starting Telegram batches near the four-minute mark, releasing pending claims or queueing the untouched fresh tail so slow Bot API runs yield the next trigger interval |
| Telegram eventless dispatch fast path  | 5-minute dispatch cadence with fan-out skipped on healthy no-change runs | `worker/src/cron/dispatch-telegram-alerts.ts` | Quiet runs still refresh snapshots and drain/clean due or expired pending rows, but skip subscriber fan-out, alert-job manifests, backoff reads, and fresh-send assembly when all alert families are unchanged and the safety source is healthy. |
...
| Durable alert broker retry peak | `1` webhook call at a time; `25` due deliveries per poll | `worker/src/lib/alert-broker.ts`, `worker/src/handlers/scheduled/five-minute-telegram.ts` | Runs serially inside `telegram-degradation-watchdog` after the 4-connection dispatch job releases its permits; failed and missing-target rows remain durable. |
...
| Telegram load simulation targets       | `500`, `1,000`, `5,000`, `10,000` active watchers | `scripts/ci/check-telegram-load.ts` | `npm run check:telegram-load` estimates drain time, per-invocation `estimatedCpuMs`, and D1 operations for single depeg, market-wide burst, DEWS+safety burst, admin broadcast, and Telegram 429 storm scenarios using the 3,600/run sender budget, 900/run pending drain, 3,600 pre-format chunk cap, Bot API broadcast pacing, p95 send latency, and D1 write cost. The CPU dimension (C102) reads `cpu_ms` from `worker/wrangler.toml`, caps the modeled format-count at the fresh budget, and fails when the required 5,000-watcher scenario exceeds `0.5×cpu_ms`. The 5,000-watcher target is the milestone; 10,000 is exploratory headroom. |
...
| Blacklist Etherscan pacing             | `3 requests/second`, serialized | `worker/src/cron/sync-blacklist.ts`, `worker/src/lib/evm-logs.ts` | Matches the live provider posture; recursive range splits remain sequential and deadline-bounded |
...
| Blacklist amount-recovery batch        | `100` rows / pass   | `worker/src/cron/blacklist/amount-recovery.ts`                    | Conservative per-run EVM recovery cap and Tron current-balance mirror cap on the 6-hour blacklist lane; statements are chunked through the shared D1 batch helper and stay under the sync subrequest/runtime budget |
...
- Migration `0179_measured_hot_query_indexes.sql` adds only the residual global cron-time, blacklist `chain_id` page, and yield-decision time-order indexes justified by the July 2026 Insights capture. The paired query changes use existing indexes for DDR first-publication probes, exact latest-stress short-circuits, Tape predecessor seeks, mint/burn time ranges and first-hour seeks, and pinned blacklist page plans. Follow [`d1-capacity-and-runtime-experiments.md`](./runbooks/d1-capacity-and-runtime-experiments.md) for before/after rows-read, query-plan, p95, and index-size evidence.
- D1 capacity samples are hourly and bounded. File-size states cross at 60%, 75%, and 90% of Cloudflare's 10 GB paid-plan ceiling. Exhaustion forecasts require at least three observations spanning 24 hours and never extrapolate a flat or shrinking trend.
...
| vaults.fyi supplemental yield         | `13` credits/run on the four-hour lane; `2,500` credits/UTC month            | `worker/src/cron/yield-sync/vaults-fyi.ts` | Reserves each run's allowance, dynamically throttles to the sustainable remainder, and reports actual/projected utilization before coverage exhausts |
...
| Birdeye address augmentation          | `10` Solana requests/run, `1000 ms` between requests                         | `worker/src/lib/address-price-providers/index.ts` | Optional keyed Solana-only targeted gap lookup; known no-price payloads count as `missing-quote` diagnostics rather than provider-shape failures |
