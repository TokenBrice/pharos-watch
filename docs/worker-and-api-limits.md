...
| Request attribution telemetry retention | `35 days`                                 | `worker/src/lib/request-source-attribution.ts`, `functions/lib/request-attribution.ts` | Total site-vs-external demand and worker-lane load buckets in `api_request_consumer_stats` / `site_data_request_stats` are pruned opportunistically |
