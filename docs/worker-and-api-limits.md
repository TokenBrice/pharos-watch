...
| API key revocation latency       | `≤5 minutes` normally; `≤15 minutes` during a D1 outage | `worker/src/lib/api-key-core.ts` (`API_KEY_AUTH_CACHE_TTL_MS`, `API_KEY_AUTH_CACHE_STALE_TTL_MS`) | Accepted operational bound, not a bug: revocation is immediate on the D1-backed path, but isolates may keep serving a recently verified non-self-serve key from the bounded auth cache for up to the 5-minute fresh TTL, and up to the 15-minute stale TTL only while D1 is unavailable. Self-serve keys are refused whenever revocation state cannot be rechecked. When revoking a leaked key, expect residual keyed traffic within this window and treat it as cache drain, not failed revocation. |
