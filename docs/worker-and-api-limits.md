...
| Telegram load simulation targets       | `500`, `1,000`, `5,000`, `10,000` active watchers | `scripts/check-telegram-load.ts` | `npm run check:telegram-load` estimates drain time and D1 operations for single depeg, market-wide burst, DEWS+safety burst, admin broadcast, and Telegram 429 storm scenarios. The 5,000-watcher target is the next milestone; 10,000 is exploratory headroom. |
