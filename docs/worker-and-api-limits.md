...
| Status-tracked cron jobs         | `34`                                            | `shared/lib/cron-jobs.ts`                                     | These are the jobs expected by `/api/status`. Latest additions: `telegram-disambiguation-cleanup` (DB-only five-minute sidecar) and `telegram-inactive-cleanup` (`daily0300Utc`).                                                                                                                |
| Runtime jobs actually scheduled  | `34`                                            | `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Runtime scheduling matches the shared status metadata set. The additional `*/5 * * * *` digest-trigger poll is a runner slot that executes pending manual requests under the existing `daily-digest` lease, not a separate status job. |
