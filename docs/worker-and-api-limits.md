...
| Status-tracked cron jobs         | `36`                                            | `shared/lib/cron-jobs.ts`                                     | These are the jobs expected by `/api/status`. Latest additions: `telegram-pulse-snapshot` and `telegram-disambiguation-cleanup` (DB-only five-minute sidecars) plus `telegram-inactive-cleanup` and `telegram-retention-cleanup` (`daily0300Utc`).                                                                                                                |
| Runtime jobs actually scheduled  | `36` status-tracked jobs, plus `2` budget-only scheduled surfaces | `shared/lib/cron-jobs.ts`, `shared/lib/scheduled-runner-registry.ts` | Runtime scheduling matches the shared status metadata set for `/api/status`. The additional budget-only surfaces are Telegram registration reconciliation on the five-minute Telegram slot and the `*/5 * * * *` digest-trigger poll, which executes pending manual requests under the existing `daily-digest` lease. |
