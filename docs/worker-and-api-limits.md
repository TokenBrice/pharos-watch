...
| Protocol-redeem live override stage   | `30 s` total budget, grouped circuit for external live RPC-backed providers  | `worker/src/lib/authoritative-price-sources/index.ts` | Local par and inherited tracked-base overrides are cache/local decisions; RPC-backed live providers stop when the grouped breaker is open |
