...
| GBP SONIA retained-fallback alert | `2` consecutive daily retained-fallback runs; `24 hours` re-alert cooldown | `worker/src/cron/fetch-tbill-rate.ts` | The first `gbp-sonia-compounded-index-failed-retained` run writes `cache["fetch-tbill-rate:gbp-retained-fallback-streak"]`; the second consecutive run sends the shared webhook alert and records a cron event. Fresh GBP market data resets the streak. |
