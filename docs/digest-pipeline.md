...
- **Max tokens:** 16000 for both jobs (thinking + visible output). The former 64k value guarded runaway thinking but did not bound spend. The measured Opus 5 `xhigh` configuration costs $0.354/day blended and $0.712/day at the measured worst-case retry rate; its 16k hard bound is $1.062/day, below the $1.15 ceiling. `stop_reason=max_tokens` remains a hard failure.
...
- **Voice guards:** a forbidden-tic list (21 anywhere-patterns plus closer-position bans on "worth watching / monitoring / bears watching"; the enforced source of truth is `FORBIDDEN_TICS_ANYWHERE` / `FORBIDDEN_TICS_CLOSER` in `worker/src/cron/daily-digest/voice-guards.ts`) fires a soft issue when hit. Opening-pattern fingerprint flags repeated "PSI [verb]" openings across the trailing 7 editions. A structural-repetition guard strips digits, compares five-word shapes against available recent copy, and soft-flags shapes present in at least two prior editions; it is advisory because recurring syntax is a voice defect, not a factual publication risk. Forward-look cue detector flags retrospective-only digests. Tone-cluster detector flags a register appearing 3+ times in the last 5 digests.
- **Quality gate:** parsed LLM output is validated for required fields, paragraph/word budget, title+text length, code fences, forbidden tics, opening/structural repetition, missing forward-look, repeated lead-family, tone-cluster, and recent title/tone/coin repetition. `unverifiable-movement-claim` is hard: "narrowed/widened from N bps" must trace to a previous-edition depeg fact, because the alternative is publishing a false observation; it takes the existing single corrective retry and blocks if unresolved. `price-bps-mismatch` stays soft at the measured 150 bps tolerance: the prompt now explicitly binds dollar price and bps to the same live observation, while making all mismatches hard would have doubled model cost on 4 of 6 measured candidate-model editions. Other soft checks are `title-symbol-streak` (same coin in three consecutive titles), `title-day-counting` (day/hour-count titles on repeat coverage), and title dedupe against a 30-edition trailing window. Sub-$50M depegs are suppressed as lead material after 48h (break-day-only coverage, owner-ratified floor). Hard issues receive exactly one corrective retry; unresolved hard issues block external delivery. Soft-only residual issues stay in cron metadata and never trigger retry or mark the operational cron lane degraded.
