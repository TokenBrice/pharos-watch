...
The current gate mode is shadow. It keeps `stripForbiddenDashes` active after the scan, records first-pass rule hits by id and field, retry eligibility, retry outcome, latency, and output-token use, and does not block an edition on a style finding. Daily and weekly telemetry is tracked separately.

Daily enforcement flips after a 30-edition window records at most one would-block event, with the corrective retry available for the editions counted. Weekly enforcement flips after seven consecutive clean daily editions have fed the weekly profile. The flip changes the `styleGateMode` value on the affected validation profile. At the flip, `stripForbiddenDashes` is removed. An unresolved hard style finding then blocks the edition after one corrective retry. Advisory findings never block.
