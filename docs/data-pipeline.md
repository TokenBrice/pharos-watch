...
- **Exact-address augmentation** only queries canonical chain+address deployments from `asset.address`, `contracts`, or `tradedContracts` for assets with missing prices or previous source depth below 3. These sources are soft, non-replay-safe, and non-depeg-authoritative; symbol search is retired.
