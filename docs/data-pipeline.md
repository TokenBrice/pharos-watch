...
47. **Exact stablecoin publication capability**: every published stablecoins run compares cache IDs to the active registry. Named unwaived omissions degrade the run and clear the downstream-safe cache capability, while any exception must carry an owner, reason, and future expiry. The data-invariant canary uses the same exact evaluator.
