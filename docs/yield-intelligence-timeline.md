...
- Decision-ledger alternatives migrate to a sibling `yield_source_decision_alternatives` table; the legacy `alternatives_json` blob continues to be written for one cycle of co-existence and a new nullable `retention_reason` column on `yield_source_decisions` tags each row as `trend` (source switch, anomaly, or rejected higher-confidence source) or `audit`. Audit-only rows are pruned at 30 days within the existing `pruneYieldTables` cleanup pass; trend-tagged rows persist for long-running analytics
