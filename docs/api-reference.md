...
Total documented public operations: **41**.
...
| GET | `/api/report-cards/v9-preview-412d818c031b7bc5` | Safety Score V9 feedback preview | Risk | X-API-Key | — | 200, 400, 401, 429, 503 |
...
`GET /api/report-cards/v9-preview-412d818c031b7bc5` is a temporary, owner-approved feedback endpoint for the unlisted `/v9-preview-412d818c031b7bc5/` page. It reads the same canonical shadow envelope and strict public projection, always returns `lifecycle: "shadow"`, and does not consult or alter the activation marker. The preview uses the standard cache profile and returns `503` when the canonical shadow envelope is unavailable or invalid. It exposes no admin diff, history, movement-review, or mutation data, has no V8 fallback, and does not change `/api/report-cards` or any active consumer. The page is absent from navigation and the sitemap and emits `noindex,nofollow`; the opaque path is not an access-control boundary.
