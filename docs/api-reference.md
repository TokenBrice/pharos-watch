...
| `supplySource`         | `string \| undefined`              | Supply data source: `"defillama"`, `"coingecko-fallback"`, or `"onchain-total-supply"` (used when a supplemental asset is normalized from on-chain total supply instead of an upstream market-cap field) |
