...
Unlike most numeric-query handlers, this endpoint uses the rejecting range policy: only a missing `days` falls back to `5000`. A non-numeric value returns `400 Invalid days: must be a number`, and any value outside `30..5000` — including `days=0` — returns `400 Invalid days: must be between 30 and 5000` rather than being clamped.
...
DEX liquidity scores, pool breakdowns, source-confidence metadata, and on-chain DEX price data for all tracked stablecoins. Liquidity scoring is republished every 2 hours by `sync-dex-liquidity` (its hourly `:16` slot refreshes on-chain DEX prices; the `:46` slot reuses the current scoring generation), and the endpoint freshness budget is 4 hours. Trend data is only returned when a trusted historical baseline exists.
...
Worker health check. Reports the most recent 15-minute public-health projection from the status self-check, including cache freshness, blacklist integrity, mint/burn freshness, and circuit-breaker states. Public responses use the realtime cache profile (`public, s-maxage=60, max-age=10`), so the projection can be up to about 15 minutes old; if no usable snapshot exists, the endpoint falls back to a live assessment.
...
Operator-only Telegram delivery lifecycle/suppression diagnostics and alert-broker details are exposed by `GET /api/status`, not this public health response.
...
| `blacklist.missingAmounts`                    | `number`                                                                  | Events where `amount_native` is null (should be 0)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `circuits`                                    | `Record<string, CircuitRecord>`                                           | Per-source circuit breaker states. Keys include `defillama-stablecoins`, `defillama-stablecoin-detail`, `defillama-coins`, `defillama-yields`, `defillama-protocols`, `coingecko-prices`, `coingecko-detail-platforms`, `coingecko-mcap`, `coinmarketcap-prices`, `dexscreener-prices`, `dexscreener-liquidity`, `dexscreener-search`, `treasury-rates`, `etherscan`, `alchemy`, `twitter-api`, `telegram-api`, `binance-prices`, `coinbase-prices`, `redstone-prices`, `curve-onchain`, `curve-liquidity-api`, `fx-realtime` |
...
| `healthy`                  | `boolean`                                                                  | `true` when `ageSeconds / maxAge ≤ 12.0`; status-page bands use `>8.0x` for degraded and `>12.0x` for stale. Caches with a per-key ratio override use that override's bands instead — `yield-data` is `{ degraded: 2.0, stale: 4.0 }`, so its `healthy` flips false above `2.0x`                                                                 |
...
For a route with declared tracked-stablecoin outputs, optional `outputDependencyResolution: { stablecoinId, resolutionState }` discloses the first output row in the same snapshot whose own `resolutionState` is not `resolved`. The field is omitted when the observed output rows are fully resolved. This is additive diagnostic context only: it does not change the upstream route's `resolutionState`, `routeStatus`, capacity or model confidence, score, or Safety Score V9 Exit eligibility. The field lives in the existing `details_json` payload, so the API addition requires no D1 migration and does not change the redemption methodology version.
...
| `outputDependencyResolution` | `{ stablecoinId: string; resolutionState: string } \| undefined`                                                                                             | First declared tracked output whose row is `missing-cache`, `missing-capacity`, `failed`, or `impaired` in the same snapshot; disclosure only, with no effect on the upstream score or confidence                             |
...
Identity-aware view of the same per-coin grade history. It returns every row the legacy endpoint returns plus the methodology-boundary rows that endpoint intentionally omits to stay a V8-compatible projection: a baseline row is written whenever the evaluating model, policy, or build identity cuts over, even when the grade does not move. Use v2 when you need to know which Safety Score model produced a point; use v1 when you only need the grade line.

The envelope differs from v1: v2 returns an object, not a bare array.
...
**Response:** object with `history` sorted by `date` ascending.
  ...
  "history": [
    {
      ...
      "safetyScoreIdentity": {
        ...
        "baseInputGenerationId": "report-cards-input:v1:<sha256>",
        "publicationGenerationId": "report-cards:v9:v1:<sha256>"
      }
    }
  ]
...
| Field                 | Type             | Description                                                                                                                                                     |
| --------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `schemaVersion`       | `2`              | Literal envelope version; a future shape change increments it rather than mutating this one                                                                     |
| `history[].date`      | `number`         | UTC day bucket (Unix seconds) when the row was recorded                                                                                                          |
| `history[].grade`     | `string`         | Safety Score letter grade at `date`                                                                                                                              |
| `history[].score`     | `number \| null` | Numeric score (0–100); `null` when grade is `NR`                                                                                                                 |
| `history[].prevGrade` | `string \| null` | Previous grade; always `null` on baseline rows                                                                                                                   |
| `history[].prevScore` | `number \| null` | Previous score; always `null` on baseline rows                                                                                                                   |
| `history[].transitionKind` | `string`    | `initial-baseline`, `organic-grade-change`, `methodology-boundary-baseline`, `rollback-baseline`, or `restoration-baseline`                                      |
| `history[].safetyScoreIdentity` | `object` | Publication identity that produced the row, discriminated on `model` (`v8` or `v9`). The `v9` variant adds `policyId` and `policyDigest`; both carry `methodologyVersion`, `evaluationBuildDigest`, `baseInputGenerationId`, and `publicationGenerationId` |

Only `organic-grade-change` rows describe an actual grade movement. Baseline rows carry `null` in both `prev*` fields by construction, so a client that infers a transition by diffing `prevGrade` against `grade` must skip them; compare consecutive `grade` values instead, or filter on `transitionKind`. Schemas live in `shared/types/safety-score-history.ts` and `shared/types/safety-score-publication.ts`.

---

### `GET /api/yield-rankings`

Cache-backed yield rankings written by `sync-yield-data`. The post-V9 publisher and API-time hydrator read the canonical current `report-cards:v9` publication through the shared safety loader, so Yield Intelligence stays aligned with `/api/report-cards/v9` without recomputing Safety Scores. A missing, held, stale, or incompatible V9 publication makes the safety snapshot explicitly unavailable; ranking rows remain available with the documented NR/default treatment and `safety-unrated` provenance. A rankings cache without a complete published `publication` contract is unavailable and returns `503`; the API does not infer publication or per-row generation/rank fields from legacy payloads. Royco Dawn structured-tranche rows use the attached underlying asset's V9 score before applying opportunity-level source risk. PYS remains benchmark-aware and source-risk-aware, and expired source or benchmark evidence returns a null PYS with the corresponding reason.
...
**Error responses:** `503` when the cached rankings payload is missing, unparseable JSON, parseable JSON that fails the `YieldRankingsResponseSchema` cache-read validation, or lacks a complete published `publication` contract. Invalid cached objects are not served because the endpoint cannot safely hydrate or trust their row shape or publication lineage.
...
| `benchmarks`    | `object \| null`         | Benchmark registry keyed by currency (`USD`, `USD_EFFR`, `EUR`, `CHF`, `GBP`, `JPY`, `MXN`, `BRL`, `AUD`, `CAD`, `RUB`, `TRY`; `SGD` is reserved without a fetcher and routes to USD fallback) with label, rate, freshness, fallback, and proxy metadata |
...
| `publication`   | `object \| null`         | Complete published publication metadata for successfully served rankings payloads; a legacy cache without this contract is rejected with `503`                                                                    |
...
Optional v8 fields are nullable and omittable. A valid publication contract is required for a served rankings payload, but its row-level publication-generation fields are preserved as authored and are not synthesized when omitted. Public source-risk values are nested under the `sourceRisk` object; flattened top-level fields such as `sourceRiskPenalty` or `rewardShare` are not part of the public API contract.

| Field                                    | Surface                     | Type                                                                                                                                                                                                              | Population status                                                                                                                                                                 |
| -------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `publication.generationId`               | rankings/history root       | `string \| null \| undefined`                                                                                                                                                                                     | Publisher generation identifier, e.g. `yield-1774526400`; required as part of the complete contract on served rankings payloads                                                     |
| `publication.updatedAt`                  | rankings/history root       | `number \| null \| undefined`                                                                                                                                                                                     | Unix seconds when the generation was computed; required as part of the complete contract on served rankings payloads                                                             |
| `publication.cutoffAt`                   | rankings/history root       | `number \| null \| undefined`                                                                                                                                                                                     | Latest history timestamp approved for public reads; required as part of the complete contract on served rankings payloads                                                        |
| `publication.schemaVersion`              | rankings/history root       | `number \| null \| undefined`                                                                                                                                                                                     | Payload-generation schema version; served rankings payloads require version `1`                                                                                                  |
| `publication.status`                     | rankings/history root       | `"staged" \| "published" \| "failed" \| null \| undefined`                                                                                                                                                        | Served rankings payloads require `published`; failed, staged, or CAS-skipped generations do not replace current `yield_data` rows                                                  |
...
| `benchmarkKey`            | `"USD" \| "USD_EFFR" \| "EUR" \| "CHF" \| "GBP" \| "JPY" \| "MXN" \| "BRL" \| "AUD" \| "CAD" \| "RUB" \| "TRY" \| "SGD" \| undefined`                         | Benchmark selected for this row's `excessYield` and any rate-derived APY logic (v8.13 expanded the set; `USD_EFFR` is the EFFR-linked key used by tokenized T-bill/MMF proxy rows; `SGD` is reserved without a fetcher and routes to USD fallback)                                               |
...
- `pause` — indefinitely pause chat-level alerts via the sentinel snooze; cleared with `clear-snooze`.
...
- `/pause [<duration>|off]` — Pause all alerts indefinitely (no argument) or for a supported snooze duration; `off` / `resume` clears the pause
- `/export` — Emit one lossless `pw3` portable watchlist token covering direct and preset subscriptions
- `/import <token>` — Decode a portable watchlist token and confirm the replacement through the bulk-confirm preview flow
- `/recap [on|off|<hour>]` — Show or configure the private daily watchlist recap and its local delivery hour
...
| `GET /safety-scores/map.json`, `GET /safety-scores/map.png` | `functions/safety-scores/map.json.ts`, `functions/safety-scores/map.png.ts` | Public KV-backed daily Safety Score map manifest (`no-store`) and poster image; `GET`/`HEAD` only, optional `?date=YYYY-MM-DD` archive. |
