...
Browser consumers should use same-origin `/_site-data/*` via the frontend helpers in `src/lib/api.ts`. In production, that Pages proxy targets `https://site-api.pharos.watch` through `SITE_API_ORIGIN`. Direct integrations and CI smoke should target `https://api.pharos.watch` and send `X-API-Key` for protected public reads, including `/api/telegram-pulse`; production Pages build-input syncs may instead read allowlisted `GET` endpoints through `https://pharos.watch/_site-data/*` with an allowed site caller header. When GitHub-hosted release egress is edge-blocked on those website-lane reads, the Pages release job can preserve validated checked-in static mirrors for digests, depeg events, and public datasets rather than treating the public API as the deployment availability dependency.
