...
The same static lane also serves the rolling public dataset mirrors at `https://pharos.watch/datasets/<topic>/latest.{csv,json,ndjson}`, plus one dated copy per refresh run at `https://pharos.watch/datasets/<topic>/<YYYY-MM-DD>.{csv,json,ndjson}`. Topic identifiers are a never-break external contract and are enumerated by `PUBLIC_DATASET_TOPICS` in `shared/lib/api-endpoints/datasets.ts`; the files themselves are regenerated by `scripts/maintenance/generate-public-datasets.ts`, which prunes dated copies older than 90 days. A dated file is a byte-identical copy of `latest` for the UTC snapshot date of the run that wrote it, so a date with no refresh run has no file and `latest` can be older than today; consumers should treat a missing dated URL as "no run", not as "no data". `https://pharos.watch/sheets/<topic>.csv` is a permanent `301` alias of that topic's `latest.csv` (`public/_redirects`) rather than a second written file, so a client that does not follow redirects gets no CSV. These URLs are unauthenticated, are advertised to crawlers as JSON-LD `DataDownload` targets (`src/lib/analytics-dataset-json-ld.ts`), and are served with `Access-Control-Allow-Origin: *` and a one-hour `Cache-Control` from `public/_headers`.
