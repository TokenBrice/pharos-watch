...
| GET | `/api/depeg-events` | Depeg events | Peg Monitoring | X-API-Key | `stablecoin?`, `limit?`, `offset?`, `active?` | 200, 400, 401, 429, 503 |
...
| `historySource`          | `"snapshot" \| "live-fallback"` | `snapshot` when at least two `telegram_watcher_lifecycle_daily` rows exist, or when no live fallback rows are available; `live-fallback` during the one-snapshot bootstrap window when subscriber-created-at aggregation yields older chart points |
...
| `lifecycleHistoryUpdatedAt` | `number \| null` | Unix seconds of the latest daily lifecycle snapshot when any snapshot row exists; can be non-null while `historySource="live-fallback"` during the one-snapshot bootstrap window; `null` when no snapshot history exists |
...
| `watcherHistory`         | `array`    | UTC day buckets. Snapshot-backed points preserve historical active counts and include `snapshotAt`; daily delta fields can be `null` when suppressed. During the initial one-snapshot bootstrap window, fallback points use current active watcher created-at aggregation and cumulative active watchers so the public chart keeps all available historical points. |
...
| `sourceRisk.deploymentPlace`  | ranking/history/source rows | `"native-wrapper" \| "issuer-savings" \| "lending-market" \| "strategy-vault" \| "lp-or-dex" \| "rwa-fund" \| "reward-program" \| "rate-derived" \| "price-derived" \| null \| undefined` | Optional sourced deployment-place label |
