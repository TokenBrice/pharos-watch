...
The additive P4a producer also writes optional same-notional route observations into the existing score-details envelope. Capability matrix `p4a.2` supports exact Raydium standard constant-product pools and Balancer weighted constant-mean pools when shaping retains normalized balances, token identities, each token's own USD reference, fees, weights, and the tracked input index. It simulates bounded curves at a 200 bps maximum cost and a 300 second settlement horizon. The DEX envelope accepts only `dex-amm` and `dex-orderbook` route families, and active replay rejects future observations or live observations older than twice the 30-minute producer interval (1 hour). CLMM/DLMM pools, stable/custom invariants, incomplete token-price models, aggregate TVL rows, and narrow CEX diagnostics remain explicitly unsupported or diagnostic-only; they are never converted into executable capacity from TVL.
