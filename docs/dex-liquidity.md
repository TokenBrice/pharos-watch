...
- **Current methodology version:** `v5.88`
...
Persistence is heap-bounded without weakening that publication fence. Candidate, history, and scoring-side writes construct and execute at most 25 D1 statements at a time; later batches are not prepared until the prior batch resolves. Challenger payloads additionally pack multiple pool rows into each statement up to D1's bind ceiling, then publish every complete asset's snapshot pointer in one atomic batch and clean superseded payloads with grouped asset IDs. A payload interruption therefore leaves the previous challenger snapshot set authoritative instead of exposing a mixed partial refresh. Price observations are derived one active asset at a time, staged in generation-keyed `dex_price_run_rows`, and exact-count validated before one atomic D1 batch checks an in-write current-generation fence and replaces the complete `dex_prices` table. The stage remains intact across ambiguous D1 retries, is removed only after the public replacement is verified, and otherwise expires after seven days through cleanup bounded to eight generations per run; cleanup protects both the in-flight generation and the generation named by the public `__global__` row. Depth-stability values are likewise written only to the current generation's private `dex_liquidity_run_rows` rows before one generation-guarded atomic update reaches the public table. A staging or final-batch error propagates to the cron and leaves each public price/depth surface wholly on its previous generation. Consumed pool/score maps are cleared as their downstream stages complete, and progress advances through generation, challenger, price, history, and depth substages so a platform interruption is attributable. The main public-table mirror and generation-state transition remain one final two-statement D1 batch after exact candidate coverage validation, so partial staging never becomes current and failed publication does not advance freshness. Superseded, failed, and rejected measured-execution generations, and completed `dex_liquidity_run_rows` generations no longer referenced by the public table, are retained for three days and then pruned oldest-first in bounded batches of sixteen generations per run inside the producing cron; generation-ledger rows are removed only after their data rows are gone, and `published` generations plus any target generation still referenced by a retained quote are never pruned regardless of age. Raw producer envelopes (`raw_quote_payload_json`) are persisted only for failed quotes, where they are the sole structured failure evidence; measured quotes carry their complete evidence in the validated profile's `quoteProof`, and the score-bearing evidence loader does not select the raw column.
...
| **SunSwap V2**           | `GET https://open.sun.io/apiv2/pools/scan`                                                                                            | Tron                                                                                                                                                             | `sunswap-v2`                           |                                           shadow-only | Exact pool/token identities, normalized balances, TVL, volume, token prices, and reviewed 0.3% fee; retained only for exact-execution target construction                                                                                                                                            |
...
During the scoring cron, the serialized direct API stage completes and releases provider-owned payloads before DeFiLlama/Curve loading and UniV3/Aerodrome subgraph enrichment begin, so those fetch families do not overlap inside Cloudflare's per-trigger connection budget. Fluid resolver enrichment and Slipstream Sugar reads use the scheduled runtime's configured `chainRpcs` map (Alchemy/dRPC when configured) instead of relying on module-level public RPC defaults.
...
DexScreener runs in the isolated `sync-dex-discovery` cron and populates `dex_pool_staging` for later merge during scoring. The discovery router queries all tracked deployments when earlier CoinGecko/GeckoTerminal stages find no pool, and otherwise queries only chains those providers do not cover. This covers 30+ chains including Solana, Berachain, Monad, MegaETH, Plume, and other exotic chains without loading per-contract responses into the scoring isolate. It records one aggregate outcome under `dexscreener-liquidity` per coin crawl instead of one breaker update per token request.
...
Route-observation selection considers the complete filtered retained-pool graph rather than the public top-10-by-volume display list. It prioritizes uninterrupted measured profiles first, current exact reserve models second, operationally interrupted last-known-good measured profiles third, and reviewed capability gates before diagnostic-only rows. This keeps a failed measured cycle from filling the bounded payload with aging last-known-good profiles while evicting a current exact fallback. A missing target or failed quote in the separate measured-execution lane does not attach a measurement gate to a pool that already carries an independently complete exact AMM model; any prior measured gate is cleared while unrelated capability gates remain intact. A pool that actually carries conflicting measured and exact execution evidence still fails closed. Selection then deduplicates by physical pool and applies the existing 10-route payload bound. Reviewed capability pools beyond that bound are counted as unsupported overflow so coverage cannot be declared complete while executable candidates are omitted. If selected multi-output pools would exceed the observation bound, the array is clipped deterministically; coverage fails closed only when clipping removes every emitted observation for a reviewed capability pool, while a represented pool does not become incomplete merely because an additional counter-asset route was clipped. An exact reserve simulation retains modeled realized cost only when the full requested notional executes; partial points defined by the 200 bps boundary and zero-capacity points preserve the bounded-unknown fallback. That modeled cost remains distinct from a proof-bearing measured `get_dy` quote. This is a route-only V9 candidate evidence change: aggregate liquidity metrics, price observations, retained target publication, the visible top-10 list, and V8 scoring remain unchanged.

Fully executable exact reserve-simulation capacity points also retain the realized execution cost from re-running the same invariant at the requested input and valuing its output with the captured token references. The projection is accepted only within the point's cost bound plus a narrow numerical tolerance. Zero capacity, an invalid recomputation, or partial capacity defined by bisection at the 200 bps request ceiling omits it and leaves V9 on the conservative fallback rather than mislabeling the bound as a realized cost. Curve reserve models continue to use the documented 10 bps fee upper bound because the pools endpoint does not expose the pool fee, so a separately pinned `get_dy` measurement may prove a lower realized cost. This additive V9 candidate field does not affect aggregate liquidity or V8 scoring.

The isolated `sync-cl-exit-depth` lane runs at `0,30 * * * *`, ten minutes before DEX scoring. It loads the latest published retained-pool target generation, pins one block per chain, verifies reviewed QuoterV2 and factory bytecode, proves each pool through the factory's exact `getPool` binding, and records a $1,000 marginal quote plus the TVL-tiered $100,000/$1 million/$10 million/$25 million ladder with bounded refinement. PancakeSwap target construction admits each direction only when the retained `token0Price` does not imply output worth more than `1.02x` the input under independent token references; incoherent directions remain in retained DEX evidence but do not become measured-execution failures. Uniswap V3 target construction resolves each leg's USD reference independently and, when the counter asset is untracked and has no direct reference, pool-implies the output reference from the subgraph candidate's decimal-adjusted spot price times the input leg's direct reference (the same convention the Uni V3 price indexer consumes, mirroring the Raydium pool-implied derivation). Pool-implied references must remain representable by the measured-execution pipeline's 1e8 fixed-point price encoding; smaller values revert to `target-unresolved` and emit a structured diagnostic with the raw pair spot prices instead of creating a guaranteed quote-validation failure. Identity failures still gate to target-unresolved. Tracked NAV tokens require a trusted live NAV at capture time and never fall back to a static fiat peg for quote sizing. The producer publishes target and quote generations atomically in D1. The following scoring run joins only a fresh published quote generation, projects a compact public profile, and publishes the next target generation after scoring; raw calldata, returndata, and failure evidence remain in D1. A mature fresh last-known-good profile may remain in the bounded route-only observation set when its physical pool rotates out of the current liquidity/display shortlist, but it never re-enters aggregate TVL, volume, visible pools, price consensus, target publication, or V8 liquidity scoring. QuoterV2 profiles must pass consumer validation of generation, identity, decimals, price, freshness, provenance, curve monotonicity, cost bracketing, and the `1.5x` retained-TVL capacity ceiling. Score-eligible deployments are the owner-ratified Uniswap V3 cohorts on Ethereum, Polygon, and Arbitrum; PancakeSwap V3 on Base, BSC, and Ethereum; plus the reviewed Aerodrome Slipstream cohort on Base. Fluid resolver measurements use the same producer and remain `activation-pending` under their own matrix and shadow requirements.
...
The same isolated job now captures a separate case-sensitive Solana target and quote generation for retained Raydium CLMM and Orca Whirlpool pools. Raydium quotes use the official Trade API and are accepted only when its single route returns the requested pool, input/output mints, exact raw input, exact output threshold, and CLMM `lastPoolPriceX64` state inside a bounded before/after Solana RPC slot window. Orca quotes use Jupiter's proven exact-in engine with `onlyDirectRoutes`, the current `Whirlpool` filter, and zero slippage; the consumer requires one 100% route whose `ammKey`, label, mints, raw amounts, and top-level `contextSlot` match the target and slot window. Historical persisted proofs using Jupiter's former `Orca V2` label remain schema-readable, but new quotes must prove `Whirlpool`. A best-route response for another physical pool is a failed exact quote, never substituted evidence. The serialized Solana producer admits 12 targets per half-hour run, enough to cover the current inventory nearly three times in a 72-hour evidence window without increasing connection concurrency. Both native adapters are registry-visible but remain shadow-only. Their registrations use the same reviewed active/shadow policy contract that the consumer join applies after full proof validation; only a future explicitly active registration can promote a valid profile into P4 capacity and physical-pool completeness. Shadow profiles retain `activation-pending`.

Tron SunSwap V2 uses a third native target/quote generation and remains shadow-only. The producer verifies the canonical V2 factory and pair runtime hashes, proves `factory.getPair(tokenIn, tokenOut)`, reads exact pair token order and reserves, and accepts only one direct `SUNSWAP_V2` route from SUN's documented Smart Router calculation service (`https://rot.endjgfsv.link/swap/routerUniversal`). The returned raw output and minimum output must exactly equal the reviewed 0.3% constant-product result from those reserves, with zero referral. TronGrid's JSON-RPC endpoint rejects numeric historical block quantities for `eth_call`, so all state reads explicitly use `latest` and are enclosed by before/after `eth_blockNumber` reads with a bounded 64-block window; this is a race-detecting bracket, not a same-block pin. The serialized lane admits the complete current 21-target inventory per half-hour generation because the native consumer reads one published generation rather than combining a multi-run rotation. It still waits one second before each Smart Router request, stops the admitted tail after the first `429`, resumes through the durable cursor, and preserves the seven-minute producer deadline with 20 seconds of final-request headroom. Any identity, hash, route, output, or block-window mismatch fails the quote. SunSwap census rows and measurements cannot affect scoring or price consensus, and matching retained rows keep `activation-pending` after replay validation until the registry policy is separately activated.

Base Aerodrome Slipstream targets come from the current Sugar RPC reader, which starts at the reviewed CL factory's live registry offset and preserves the exact pool address and signed `int24` tick spacing separately from the pool's dynamic fee. The producer pins the reviewed Aerodrome factory and QuoterV2 runtimes and validates the signed factory and quote calldata. The deployment entered shadow V9 route scoring on 2026-07-24 after complete repeated target rotation, clean endpoint/factory/pool-binding checks, current monotonic capacity curves within retained TVL, and independent historical-block quote reproduction. Consumer validation remains fail-closed when a fresh profile no longer matches the current target's identity, price, or TVL tolerances.

Optimism Uniswap V3 remains shadow-only until its own replay, executed-swap equivalence, independent cross-check, drift, and shadow packet is owner-approved. As of 2026-07-23, its only retained UniV3 row is a roughly $4.2K CoinGecko-sourced pool below the $10K primary admission threshold, so there is no current Optimism activation cohort; an eligible retained source row must appear before that packet can accumulate. Avalanche and Linea have candidate Uniswap deployments but no admitted retained-pool/source cohort and no equivalent evidence packet; Sonic has no reviewed official deployment. None are configured or score-eligible.
...
EVM admission rotates whole stablecoin cohorts through the existing durable cursor before quote work begins. Its phase-aware cohort estimate counts separate pool-binding, probe-notional, and bounded-refinement Multicall batches plus one reserved serialized revert-confirmation request per Quoter target. It admits up to 736 estimated requests and reserves 64 of the hard 800-request ceiling for block reads, deployment verification, additional revert confirmation, and adaptive batch fragmentation. When a cohort does not fit, admission keeps its cursor immediately before that cohort for the next run while packing any later whole cohorts that still fit. Non-admitted rows publish as `budget-deferred` so the quote generation still exactly covers its target generation; that deliberate deferral is healthy only when the next cursor is durably written and the current inventory can rotate completely within two half-hour runs. Attempted quote failures, an oversized single-coin cohort, cursor persistence failure, or a rotation longer than the one-hour evidence window remains degraded. If the hard runtime ceiling is nevertheless reached, only calls rejected by the shared budget are attributed to `request-budget-exhausted`; completed provider or execution failures retain their original reason. Solana and Tron remain activation-gated shadows: their per-target degradation is retained under nested metadata and `laneStatuses` without changing active EVM health, while an invocation error remains terminal and a non-durable shadow deferral, including a Tron rate-limit tail, remains degraded.
