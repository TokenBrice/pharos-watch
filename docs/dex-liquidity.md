...
The additive P4a producer also writes optional same-notional route observations into the existing score-details envelope. Capability matrix `p4a.2` supports exact Raydium standard constant-product pools and Balancer weighted constant-mean pools when shaping retains normalized balances, chain-scoped token identities, each token's own USD reference, fees, weights, and the tracked input index. It simulates bounded curves at a 200 bps maximum cost and a 300 second settlement horizon. The tracked input token must resolve to the stablecoin being scored, token identities must be distinct under the chain's casing rules, and modeled pool TVL (`sum(balance * referencePriceUsd)`) must stay within `0.5x-2x` of the retained pool TVL. A missing or invalid model, including a TVL reconciliation outside those bounds, emits no executable observation and records an `invalidExecutionModel:*` unsupported reason; aggregate TVL is never substituted as executable depth.

Exact DEX route coverage is complete only when the count of retained pools with at least one score-eligible modeled observation equals the total retained-pool count and no retained pool is unsupported. Aggregate observation count is not the completeness measure because one pool may emit multiple observations. Partial model coverage remains useful shadow evidence but cannot present the modeled subset as the holder's exhaustive DEX exit surface. Production report cards continue to use the legacy aggregate path; active fixed-input calibration can compare exact routes, while incomplete modeled coverage retains the aggregate DEX floor rather than allowing a modeled subset to replace it. The DEX envelope accepts only `dex-amm` and `dex-orderbook` route families, and active replay rejects future observations or live observations older than twice the 30-minute producer interval (1 hour). CLMM/DLMM pools, stable/custom invariants, incomplete token-price models, aggregate TVL rows, and narrow CEX diagnostics remain explicitly unsupported or diagnostic-only.
