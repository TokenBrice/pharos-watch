...
Safety Score v8.16 preserves those evidence fields, plus aggregate `dex_deployment_outcomes`, through its report-card snapshot loader. This does not change the standalone Liquidity Score. Liquidity / Exit separately classifies the retained evidence as reserve-based AMM simulation, generic TVL proxy, synthetic/fallback, or unobserved and applies conservative evidence ceilings. The stronger measured-executable-depth and direct-orderbook-depth classes are reserved for future producers that retain defensible execution observations; TVL is not labeled as executable slippage depth.
