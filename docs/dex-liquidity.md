...
Safety Score v8.16 preserves those evidence fields, plus aggregate `dex_deployment_outcomes`, through its report-card snapshot loader. This does not change the standalone Liquidity Score. Liquidity / Exit separately classifies the retained evidence as reserve-based AMM simulation, generic TVL proxy, synthetic/fallback, or unobserved and applies conservative evidence ceilings. Rows explicitly marked `legacy` remain neutral until current evidence is republished. The stronger measured-executable-depth and direct-orderbook-depth classes are reserved for future producers that retain defensible execution observations; TVL is not labeled as executable slippage depth.

The additive P4a producer also writes optional same-notional route observations into the existing score-details envelope. Capability matrix `p4a.2` supports exact Raydium standard constant-product pools and Balancer weighted constant-mean pools when shaping retains normalized balances, token identities, each token's own USD reference, fees, weights, and the tracked input index. It simulates bounded curves at a 200 bps maximum cost and a 300 second settlement horizon. CLMM/DLMM pools, stable/custom invariants, incomplete token-price models, aggregate TVL rows, and narrow CEX diagnostics remain explicitly unsupported or diagnostic-only; they are never converted into executable capacity from TVL.

Current rows expose `exitRouteObservations` and `exitRouteObservationCoverage`; daily history stores the bounded summary prospectively in `dex_liquidity_history.exit_route_summary_json`. Existing history is not backfilled or claimed to reconstruct old route capacity. The first isolated complete generation, `dex-liquidity-1783905029`, published 360 asset rows: 7 populated, 173 unsupported, and 180 unknown, with 21 exact observations. The fixed all-active decision table is committed at `shared/data/safety-score-v9/exit-route-calibration-v1.json`.
