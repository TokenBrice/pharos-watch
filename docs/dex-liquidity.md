...
| **PancakeSwap V3**       | Graph gateway -> official PancakeSwap subgraphs                                                                                       | BSC, Ethereum, Base                                                                                                          | `pancakeswap-v3-*`                     | 1bp 1.1x, 5bp 0.85x, 30bp+ 0.4x | TVL (`totalValueLockedUSD`), trailing 24h volume (sum of bounded `poolHourDatas.volumeUSD`), price (`token0Price`), balances (`totalValueLockedToken0/1`), fees (`feeTier`)              |
