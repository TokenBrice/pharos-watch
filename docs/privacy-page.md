...
Snapshot share links (`/screener/picker/?sid={sid}`) reference a server-side KV-backed JSON projection of client-generated selector output. It contains form answers, tracked stablecoin identities, shortlist/diagnostic fields, methodology versions, a dataset hash, and explicit `client-unverified` provenance; it does not contain an IP, browser fingerprint, wallet address, or account identifier. Pharos validates and normalizes the replay shape but does not reproduce its scores from canonical source data. Snapshot identifiers are **content-addressed** — two users with identical normalized output receive the same identifier, and anyone with the link can view the frozen artifact. KV entries start with a 90-day TTL; the first read returns the artifact only after confirming the full five-year retention extension.
