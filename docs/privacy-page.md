...
Telegram also uses the shared D1 `cache` table for short-lived chat-scoped bot state. `/forget` clears the caller's command cooldown/flood, chat-member/admin, group-welcome, and re-engagement warning cache keys immediately, along with that chat's alert-job target and dead-letter rows. Aggregate alert-job manifests and processed-update idempotency claims remain until their normal prune. The daily `telegram-retention-cleanup` job also removes stale chat-scoped cache keys in capped batches: 7 days for command cooldown/flood, chat-member/admin, and group-welcome keys, 30 days for re-engagement warning markers.
