...
Live FX rates are refreshed every 15 minutes by `sync-fx-rates.ts`. Frankfurter.app (ECB data) covers EUR, GBP, CHF, BRL, JPY, IDR, SGD, TRY, AUD, ZAR, CAD, CNY, PHP, and MXN. CNH, RUB, UAH, and ARS are filled from the secondary `fawazahmed0/exchange-api` path because Frankfurter/ECB does not cover the full set needed for peg evaluation. When `OPENEXCHANGERATES_API_KEY` is configured, the same cron also runs a real-time Open Exchange Rates cross-validation pass and can promote validated realtime quotes into the cached fallback-rate set for supported pegs.
