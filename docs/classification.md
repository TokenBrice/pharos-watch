...
On active multi-deployment assets, every Mint Authority control must use `deploymentRefs` to name its reviewed `native-issuance` deployment(s); a mutable `upgradeability` path must do the same for every native deployment whose mint logic it can replace. Active Mint Authority data must not use `role: "bridge-admin"`, `authorityType: "bridge"`, `routeChecks`, or `mintPath: "bridge-or-oft-synthetic"`. The three bridge-specific enum values remain readable only for historical records; `routeChecks` belongs on structured bridge controls. An asset with no local native issuance must instead carry the reviewed `mintAuthority.review.noLocalIssuance` exception for inherited parent issuance or external-only representations. [Stablecoin Data Registry](./stablecoin-data.md#mint-authority-and-bridge-risk-ownership) owns the exact authoring contract and deployment identity.
