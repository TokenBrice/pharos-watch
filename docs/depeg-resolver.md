...
| **K2** | Backing impairment | **Severe:** `dependencyImpaired` (V9 serial dependency with frozen/dead upstream when the V9 card is available; otherwise the legacy registry dependency weight ≥ 0.3 frozen fallback), **or** — for attested-reserve archetypes only (`fiat-cash` / `tbill` / `rwa-credit-fund` / `commodity-claim`) — very-high-risk reserves ≥ 30% paired with a severe/catastrophic below-peg fingerprint. **Elevated:** for those same attested-reserve archetypes, exotic collateral or high/very-high reserve concentration. For `cdp` and `synthetic-delta-neutral`, static concentration/exotic collateral alone never grades above elevated; severe requires observed impairment (`dependencyImpaired` or the measured impairment path above) |
