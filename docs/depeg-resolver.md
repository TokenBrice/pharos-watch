...
2. **Stratification, most-dependable-first.** A dependable wide band beats a precise band built on three incidents. The MVP strata, in order of how readily they are dropped:
   ...
   - **depth** — `minor` (≤250 bps) / `moderate` (250–1000) / `severe` (1000–2500) / `catastrophic` (>2500), validated by the monotonic depth→duration relationship in the corpus. Support-gated collapse runs `catastrophic`→`severe`→`moderate` when a bucket is thin (the catastrophic below-peg bucket is small enough that it routinely collapses into severe).
   ...
   The fallback order is: drop currency → drop the depth split → drop the structural split, reporting the actual stratum used. Finer splits (full 4-bucket depth at all times, 3-way structural class) are deferred until real-data counts justify the support.

3. **Landmark estimate at age `t`.** Among historical incidents in the stratum still open at age `t` and label-observable for a given horizon, compute the empirical resolution likelihood and the conditional remaining-time distribution. The headline is the **median time-to-repeg plus an interquartile band**; the secondary readout is per-horizon (6h / 24h / 7d / 30d) resolution probability.
