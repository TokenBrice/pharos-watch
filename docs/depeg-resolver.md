...
- **Pre-lock close reaper** (`closeRecoveredPreLockIncidents` in `worker/src/lib/depeg-resolver-incident-store.ts`, called directly from `worker/src/cron/compute-depeg-resolver.ts`): each run sets `closed_pre_lock_at` on unsealed active incidents whose **current** source event has an explicit recovery reason (`recovered-primary`, `recovered-dex`, or `recovered-native`) and is past `ended_at + DDR_INCIDENT_REOPEN_MERGE_WINDOW_SEC (6h) + DDR_PRE_LOCK_CLOSE_SETTLE_MARGIN_SEC_V1 (20min)`. Legacy rows w...
...
Every current generation-4 public review row carries its actual outcome natively: prediction and no-call rows via `actual.kind`, coverage rows via `actualOutcome` alongside `sourceEventState` (the same evidence in the public outcome vocabulary), and invalidated rows via their literal `invalidated` state. Consumers on the current payload therefore never re-derive an outcome from the source-event state.
During the 2026-09-06 Worker/Pages cutover, consumers tolerate generation-3 coverage rows that omit `actualOutcome` by deriving it from `sourceEventState`; generation-4 producer payloads continue to require and publish the field.

The cache-backed `GET /api/depeg-resolver-review` endpoint exposes the same review snapshot used by the UI. Headline stats are computed across the v2 policy universe, while public review rows are capped (`DDRR_PUBLIC_ROW_CAP = 400`) to keep the D1 cache row bounded; `_meta.incidentRowsTruncated` and `_meta.publicRowsTruncated` disclose truncation. `_meta.assessmentRowLimit` and `_meta.assessmentRowsTruncated` are retained response-shape compatibility fields from the pre-v2 reviewer — the flag is always `false` because the v2 review pipeline never truncates assessment rows; degraded responses zero both row limits. Missing or invalid snapshots return a degraded `200` with empty rows, matching DDR's public failure mode. Stale review snapshots keep their rows and mark `_meta.degraded=true` / `degradedReason="stale-cache"`.
...
The fenced slot requires at least 45 seconds of its two-minute execution window to remain before DDR starts; the absolute pre-quarter deadline still aborts overlong work.
