...
Repair-required events no longer halt the run. When the writer encounters an event that needs an explicit repair migration (ambiguous overlap with a canonical incident, or an unlinked event whose key maps to an existing incident), it quarantines that event for the run — no canonical incident, no fallback pseudo-incident, no public row — and continues resolving and publishing every other incident. The run is recorded as `degraded` with the quarantined event ids and reasons in `repairRequiredEvents` cron metadata, so the repair obligation stays visible without freezing DDR publication universe-wide until the migration ships (previously a single conflicted event failed every 15-minute run, alerting each time, until a hand-written repair migration landed).
