...
**Structural fragility** (static registry plus slow scores): `mintAuthority` posture / path plus mint-authority score band and recent reviewed mint-authority incident dates, optional issuer `windDownAnnouncedAt` / `windDownSourceUrl` (token-scoped; distinct from `frozenAt`), `mechanismArchetype`, governance flag, collateral quality, custody model, reserve risk, blacklistability, dependencies, redemption capacity and route family, and the V9 report-card safety grade / exit pillar as anchors when identified.
