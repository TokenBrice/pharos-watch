...
Migration `0178_historical_data_debt_closure.sql` is the reviewed closure artifact for BRLA events `90492`, `90493`, `90494`, and `90496`. It verifies the exact backfill event identities, requires both target incidents to remain unsealed, appends link/current-update authorizations and consumptions, records incident revisions, advances the two canonical current-event pointers, closes the four durable tasks, and invalidates both DDR projections for regeneration. The migration does not change DDR methodology or rewrite a public prediction.

Migration `0203_cngn_ddr_event_90548_link.sql` is the reviewed closure artifact for recovered cNGN live event `90548`. It verifies the event's exact NGN quote-domain identity, requires the event `90511` canonical incident to remain active and unsealed with event `90526` current, appends link/current-update authorization and consumption records, advances the canonical current-event pointer, closes the durable repair task, and invalidates both DDR projections for regeneration. The migration does not change DDR methodology or rewrite a public prediction.

Migration `0206_cngn_ddr_events_90573_90584_link.sql` is the reviewed follow-up closure artifact for recovered cNGN live events `90573`, `90576`, and `90584`. It verifies all three exact source-row fingerprints as one guarded batch, requires the same active and unsealed canonical incident to remain at event `90548`, appends authorized links plus ordered current-event revisions `90548 -> 90573 -> 90576 -> 90584`, closes the three durable repair tasks, and invalidates both DDR projections for regeneration. The repair-debt marker is removed only when it contains exactly those untruncated events and no active durable DDR repair task remains. The migration does not change DDR methodology or rewrite a public prediction.

Migration `0208_eurq_ddr_events_90589_90595_link.sql` is the reviewed closure artifact for EURQ live events `90589`, `90591`, `90594`, and `90595`. It verifies the three recovered rows exactly, verifies the immutable source identity of the current live tail, requires the event `90527` canonical incident to remain active and unsealed at event `90560`, appends authorized links plus ordered current-event revisions `90560 -> 90589 -> 90591 -> 90594 -> 90595`, closes the four durable repair tasks, and invalidates both DDR projections for regeneration. Event `90595` does not need to close before rollout: the migration never changes the source row, and its mutable peak and terminal observations remain owned by the depeg detector. The repair-debt marker is removed only when it contains exactly those four untruncated events and no active durable DDR repair task remains. The migration does not change DDR methodology or rewrite a public prediction.

Migration `0209_cngn_ddr_event_90599_link.sql` is the reviewed closure artifact for cNGN live reopen `90599`. It verifies the event's immutable source identity, the exact recovered predecessor `90584`, the prior authorized `0206` lineage, the deterministic repair task, and the same canonical incident remaining active and unsealed at event `90584`. It appends authorized link and current-update records plus revision `90584 -> 90599`, closes task `90599`, and invalidates both DDR projections for regeneration without mutating the source event. Peak and terminal observations remain detector-owned, so the event may recover naturally before rollout. The repair-debt marker is removed only when it contains exactly untruncated event `90599` and no active durable DDR repair task remains. The migration does not change DDR methodology or rewrite a public prediction.

Migration `0227_cngn_ddr_events_90718_90738_link.sql` is the reviewed follow-up closure artifact for cNGN live events `90718`, `90729`, and `90738`. It verifies the two recovered rows exactly, verifies the immutable source identity of the still-live event `90738`, requires the event `90511` canonical incident to remain active and unsealed at the authorized `0215` tail `90666`, and appends ordered revisions `90666 -> 90718 -> 90729 -> 90738`. It closes the three exact durable tasks and invalidates both DDR projections without mutating any source event. Event `90738` may close naturally before rollout because peak and terminal fields remain detector-owned. The repair-debt marker is removed only when it contains exactly those three untruncated events and no active durable DDR repair task remains. The migration does not change DDR methodology or rewrite a public prediction.
