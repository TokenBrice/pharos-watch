...
Every public review row carries its actual outcome natively: prediction and no-call rows via `actual.kind`, coverage rows via `actualOutcome` alongside `sourceEventState` (the same evidence in the public outcome vocabulary), and invalidated rows via their literal `invalidated` state. Consumers therefore never re-derive an outcome from the source-event state.
