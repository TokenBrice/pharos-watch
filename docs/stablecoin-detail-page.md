...
- `LongformScrollspyNav` pill order is: `overview` (Risk), `context`, `liquidity`, `activity`, `history`, `explore`. Child cards can still expose deep-link anchors such as `#report-card`, `#reserves`, `#mechanism-review`, `#mint-authority`, `#redemption`, `#price`, `#chart`, `#yield`, `#flows`, `#blacklist`, `#coin-timeline`, and `#explore-next` — plus the passport sub-anchors `#mechanism`, `#attestation`, `#jurisdiction`, and `#contracts` inside `KeyInfoCard` — but they are not top-level scrollspy pills.
