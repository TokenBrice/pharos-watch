...
| `MintAuthoritySection`        | Compact, descriptive-only mint-authority review. Always renders; the detail view model supplies an explicit not-reviewed/NR state when no compact review exists. Since Safety Score v8.0 the score feeds the Decentralization dimension through a penalty-only blend and surfaces in report-card raw inputs. |
...
| `src/lib/stablecoin-detail-mint-authority-view-model.ts`            | Reviewed and explicit not-reviewed Mint Authority states   |
