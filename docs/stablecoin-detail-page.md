...
Active pages render only an `sr-only` server-owned `h1` before the client island so the visible dossier begins with the coin hero. `StablecoinDetailSeoContent` remains the static read-only profile for quarantined and delisted records. The server shell passes `ExploreNextSection` and the data-derived FAQ into `StablecoinDetailClient`; the client renders them inside and immediately after the Explore zone, respectively.
...
| `ReportCardDetail`            | Overall Safety Score plus radar/dimension detail, contextual methodology hints, and a methodology footer line. Its header carries an inline coin lockup — logo · ticker · `Safety Score` — because this module is the most-screenshotted surface on the site and must name its subject without the surrounding page (owner call 2026-08-17); the lockup is omitted when no symbol is passed                                                                                                                                                                                                                                                                                                                                                                                                         |
