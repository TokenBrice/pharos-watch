...
`/api/` also renders a **Supporter Key** section between the free-grades card and the self-serve notice. It states the perk and its terms: one key per wallet that has donated at least $10 in receipt-date USD across the reconciled ledger on `/funding/` (every donation kind except `pool` counts, founder rows included), `tier="donor"`, exactly 10 requests per minute, no expiry, and no rotation by re-signing. It names the ledger reconciliation date, says a new donation counts only after the weekly reconciliation and the next release, says only externally-owned sender wallets can sign, and points a lost key at the `/feedback/` form for an operator rotation.
...
5. Reveal the plaintext token once in the shared one-time token panel, which keeps copy-to-clipboard, focus handling, and the unsaved-token navigation warning from the self-serve reveal.

Supporter claims are switched by `DONOR_KEY_CLAIMS_OPEN` in `shared/lib/public-api-contract.ts`, currently `false`. Paused means: the Worker answers `POST /api/donor-key-claims` with `403` before reading the body, and `/api/` renders the perk description with a "claims open with the next release" notice in place of the connect-and-sign control. Enabling sequence: deploy with the switch `false` (migration, Worker with the `DONOR_KEY_CLAIM_RATE_LIMIT` binding, Pages), flip the constant to `true` and release again, claim with a founder wallet on the live site and confirm the key authenticates at `10` requests per minute and that a replay answers `409`, then announce (the blog post is held until this step). To pause claims later, flip the constant back and release; never roll the Worker back to a version without the donor tier while donor keys exist. Both sides read the same value.
