...
2. Hero copy that makes the auth model explicit:
   ...
   - only a narrow no-key set remains on the public host (`health`, OG images, `feedback`, self-serve key request/verify, and `telegram-webhook` with Telegram secret auth)
