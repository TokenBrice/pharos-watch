...
The registry is the **single place** in the codebase for resolving any external provider ID to a canonical stablecoin entry. No code outside this module should do ad-hoc `geckoId` or `cmcSlug` matching.
...
// Per-provider lookup maps (computed from TRACKED_STABLECOINS at import time)
export const REGISTRY_BY_ID: Map<string, StablecoinMeta>;
export const REGISTRY_BY_LLAMA_ID: Map<string, StablecoinMeta>;
export const REGISTRY_BY_GECKO_ID: Map<string, StablecoinMeta>;
export const REGISTRY_BY_CMC_SLUG: Map<string, StablecoinMeta>;

// Extensible external ID resolver — the unified API for "I have an ID from provider X"
...
// Legacy migration resolver — for the 30-day dual-accept transition period
...
**Adding a new external data source in the future:**
1. Add an optional field to `StablecoinMeta` (e.g., `dexScreenerId?: string`)
2. Add a `REGISTRY_BY_XXX` map in the registry (3 lines)
3. Add the provider to `ExternalIdProvider` union and the `switch` in `resolveByExternalId`
...
- `BLUECHIP_SLUG_MAP` (`worker/src/lib/bluechip-slugs.ts`) — update ID values
- `SUMMARY_TIMEFRAME_OVERRIDES` (`src/lib/mint-burn-timeframes.ts:18`) — re-key
- `MAJOR_CENTRALIZED_IDS` (`src/hooks/use-portfolio.ts:240`) — update set values
- `COMMODITY_MEDIAN_EXCLUDES` (`shared/lib/peg-rates.ts:9`) — update `"gold-dgld"` to canonical ID
- `api-endpoints.ts` (`shared/lib/api-endpoints.ts`) — update 7 probe paths with hardcoded `"/api/stablecoin/1"`, `"/api/stablecoin-summary/1"`, and `"?stablecoin=1"`
