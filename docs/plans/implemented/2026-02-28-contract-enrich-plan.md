# Contract Enrichment Skill Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a `/contract-enrich` skill that discovers missing chain deployments via DefiLlama supply data and populates addresses via CoinGecko + block explorer verification.

**Architecture:** A single skill file (SKILL.md) with two phases — DL gap discovery, then CG address population with explorer verification. Follows the same patterns as the existing `/contract-populate` skill.

**Tech Stack:** Claude skill (SKILL.md), DefiLlama stablecoins API, CoinGecko coins API, agent-browser for explorer verification.
...
**Files:**
- Create: `.claude/skills/contract-enrich/SKILL.md`

**Step 1: Create the skill file**

Create `.claude/skills/contract-enrich/SKILL.md` with the full skill definition. The file has these sections:

1. **Frontmatter** — name, description, user_invocable: true
2. **Overview** — purpose and two-phase workflow summary
3. **DL_CHAIN_MAP** — DefiLlama chain name → internal chain ID (complete table)
4. **DL chains to skip** — dead/deprecated chains
5. **CG_PLATFORM_MAP** — copied from contract-populate (authoritative reference)
6. **CG platforms to skip** — copied from contract-populate
7. **Phase 1: Gap Discovery** — full step-by-step instructions
8. **Phase 2: Population** — per-gap CG lookup, explorer verification, write
9. **Adding new chains** — how to expand CHAIN_META (same as contract-populate)
10. **Quality standards & what NOT to do**
...
After creating the file, verify Claude Code can see it:
- The skill should appear in the available skills list as `contract-enrich`

**Step 3: Commit**

```bash
git add .claude/skills/contract-enrich/SKILL.md
git commit -m "feat: add /contract-enrich skill for DL-driven contract discovery"
```

---

### Task 2: Smoke-test the skill with a single coin

**Purpose:** Verify the skill works end-to-end by running it on one coin.

**Step 1: Run gap discovery for a single coin**

Invoke `/contract-enrich USDC` and verify:
- DL data is fetched successfully
- Chain matching works (DL names → internal chain IDs)
- Supply thresholds are applied correctly
- Gap report is printed with correct format

**Step 2: Run population for one gap**

Pick one gap from the report and let the skill populate it:
- CG address is fetched
- Block explorer verification succeeds
- Contract is written to stablecoins.ts
- Build passes
...
```bash
npm run build
git add src/lib/stablecoins.ts src/lib/chains.ts
git commit -m "feat: enrich USDC contracts via DL discovery"
```

---

### Task 3: Run batch enrichment across all tracked coins

**Step 1: Run full discovery**

Invoke `/contract-enrich` (no arguments) to run across all ~145 coins.

**Step 2: Review gap report**

Examine the report for:
- Reasonable gap counts (not too many false positives)
- Supply figures make sense
- Unknown DL chains are flagged appropriately

**Step 3: Proceed with population**

Approve the gaps and let the skill populate all addresses.
...
npm test
git add src/lib/stablecoins.ts src/lib/chains.ts
git commit -m "feat: batch enrich stablecoin contracts via DL discovery"
```
