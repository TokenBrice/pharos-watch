...
The page authors no navigation tree of its own. Three of its six tier columns are looked up from `NAV_GROUPS` by key (`markets`, `risk`, `tools`); the other three are the labeled columns of the `more` group (`research`, `watch`, `pharos`), and the closing quick-access section renders `QUICK_NAV_ITEMS` only. `Start Here` needs no hand-curated row because it arrives with the `pharos` column. Adding, removing, or reordering a nav item therefore reshapes this public page in the same change.
