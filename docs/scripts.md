...
| `scripts/check-worker-migrations.mjs`           | Replay every worker SQL migration against a throwaway SQLite database and enforce the frozen duplicate-prefix allowlist                             | `worker/migrations/*.sql`, `worker/migrations/MANIFEST.md`, Node 22+ `node:sqlite` or local `sqlite3` fallback                                                                                                                                                                                            | Exits non-zero on invalid migration SQL, ordering issues, or new duplicate migration prefixes                   |
...
- `pages_changed` turns on Pages build/browser-smoke/deploy when frontend/Pages/shared build paths or deploy infra changed.
- On `workflow_dispatch`, forces the full deploy path for safety.

### `test-merge-gate.mjs`

- Always runs the same non-negotiable validate core as CI: dependency audit, lint, worker-boundary, migrations, cron/doc sync, duplicate-export and redemption-backstop checks, unused-code, hotspot-ratchet, full tests, critical coverage, and worker type-checking.
- Adds `npm run build` and `npm run seo:check` only when the changed-file set touches export-critical files.
