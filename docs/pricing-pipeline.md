...
- **Current methodology version:** `v6.195`
...
| `phpm-mento`             | exact Celo Mento Broker PHPm -> USDm quote × trusted tracked USDm price         |
...
- **PHPm:** the Celo Mento route pins the Broker, BiPool Manager, exchange ID, PHPm/USDm token identities, and a 1,000 PHPm quote. Pool state and the Broker quote use the same Celo block, that block must be no older than five minutes, the USDm counter bucket must hold at least 1 million units, and the trusted USDm dependency must be current. Transport and empty-result failures use the dedicated `mento-broker` circuit.
...
The live override stage has a 10-second wall-clock budget. It first schedules circuit-backed routes so a half-open recovery probe cannot be starved by unrelated candidates, then schedules every remaining missing-price candidate before already-priced candidates and interleaves provider families inside each partition. A started circuit probe always finalizes success or failure, including a shared-budget abort; candidates never started by the budget remain neutral. Missing-only thin routes such as AZND are not enqueued over an existing usable price. Kava, JUSD, USX, AZND, and Mento each use dedicated circuits; identity mismatch, stale state, unavailable trusted dependencies, insufficient capacity, excessive impact or divergence, transport failure, and budget exhaustion all return no override. USX additionally tries independent public RPC operators on both reviewed chains while retaining the same two-route agreement requirement. The pipeline never substitutes nominal peg parity for a failed executable route.
...
As of 2026-07-16, the unresolved active rows include these intentionally fail-closed cases:
