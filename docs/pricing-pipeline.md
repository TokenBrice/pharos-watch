...
| DexScreener exact-address augmentation      | 1      | `worker/src/lib/address-price-providers/index.ts`                               | Targeted exact chain+address augmentation for missing or below-depth assets; no symbol search                                                   |
...
- **Exact-address price augmentation:** after the normal primary provider fetches, Pharos can query targeted exact-address providers for assets whose previous publication had fewer than 3 consensus sources or no price. The current augmentation providers are DexScreener exact token endpoint, DexPaprika, GeckoTerminal simple token price, Alchemy Prices, Moralis token prices, and Birdeye for Solana. Targets come only from canonical `asset.address`, `contracts`, or `tradedContracts` metadata and are matched by exact chain+address; symbol search remains confined to the older final DexScreener fallback pass.
- **Address-provider enablement:** unset `ADDRESS_PRICE_PROVIDERS_ENABLED` auto-enables the no-key providers plus any keyed provider with a configured credential (`ALCHEMY_API_KEY`, `MORALIS_API_KEY`, `BIRDEYE_API_KEY`). Operators can set a comma-separated allowlist or `none` to disable the group. Key-backed providers without credentials are skipped rather than attempted. Moralis uses 100-token batches but is capped at 3 requests per 15-minute sync so the default cadence stays inside Moralis's free 40k CU/day envelope; operators should exclude `moralis-address` from the allowlist when the account quota is exhausted or intentionally reserved for other jobs.
...
- Fallback/search lanes remain non-authoritative even when their source labels appear inside composite strings. `coinmarketcap`, `defillama-contract`, and CoinGecko mirror/low-volume-style sources are treated as list aggregators for independence checks; Jupiter, DexScreener exact/search/address, DexPaprika, Alchemy Prices, Moralis, Birdeye, GeckoTerminal address augmentation, and cached replay cannot satisfy single-source depeg authority.
