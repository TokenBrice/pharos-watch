...
- **Current methodology version:** `v2.11`
...
- **FX freshness semantics:** `fx-rates-meta` tracks usable cache freshness (`usableSyncAt`) separately from per-peg source freshness metadata (`sourceUpdatedAtByPeg`, `sourceCadenceByPeg`, `sourceDateByPeg`). Intraday sources (OXR, Chainlink, `gold-api.com`) still age on wall-clock seconds, while daily sources (ECB/Frankfurter and the secondary CNH/RUB/UAH/ARS feed) are evaluated against their expected publish cadence instead of a naive 6-hour clock. When the live FX fetches fail but those dated daily references are still within cadence, `sync-fx-rates.ts` now carries them forward as a successful live run. Cached fallback runs are reserved for cases where the job cannot refresh a live source and the carried-forward daily references are no longer cadence-valid, so non-USD and commodity validation cannot silently look fresh after a real upstream aging event.
...
- **GeckoTerminal probe observability:** `sync-stablecoins` now persists a `gtProbe` block into cron metadata, including `updatedCount`, coverage misses, upstream errors, public fallbacks, and per-transport attempt counts for CoinGecko on-chain vs public GeckoTerminal. Operators can inspect those fields in `cron_runs.metadata` without relying on live Wrangler tails.
