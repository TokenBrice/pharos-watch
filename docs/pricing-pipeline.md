...
- **DEX bridge overlap guard:** when at least one promoted per-protocol DEX bridge source is admitted for an asset, the overlapping `dex-promoted` aggregate is withheld so the same bridge observation family cannot self-confirm. If a promoted protocol candidate exists but is rejected, the overlapping aggregate `dex-promoted` lane is also withheld; the aggregate fallback can still enter only when no promoted protocol candidate exists for the asset.
