...
- **Current methodology version:** `v6.206`
...
- **GeckoTerminal probe transport:** the retained helper in `worker/src/lib/geckoterminal-price-probe.ts` prefers authenticated CoinGecko `/onchain/networks/.../tokens/.../pools` when `COINGECKO_API_KEY` is configured and the chain has a CoinGecko on-chain network mapping. If that path yields no usable pool, the helper can fall back to the public GeckoTerminal token-pools endpoint. This helper is not invoked by the production stablecoin sync.
...
- **GeckoTerminal probe self-budget:** when invoked in a separately isolated context, the serialized helper enforces a `90 second` wall-clock budget and reports unstarted candidates through `budgetExhausted` / `budgetSkipped`. The production `sync-stablecoins` path does not spend this budget.
- **GeckoTerminal probe isolation:** `sync-stablecoins` persists a `gtProbe` block with `inlineDisabled = true` and `isolationReason = "worker-memory-boundary"`. The optional live corroboration pass is not allowed inside the critical publication invocation after production analytics showed its response/evidence graph could exceed the Worker heap ceiling. The retained helper rejects declared or streamed bodies above 1 MB before parsing, but it must run only from a separately budgeted producer if reintroduced.
