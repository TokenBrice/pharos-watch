...
- **Direct native-peg downstream corroboration:** supported non-USD fiat assets with reliable CoinGecko native pairs can consult a fresh direct native-peg quote before downstream depeg logic trusts a derived `USD price / FX reference` move. This does not replace the published cached USD price; it hardens depeg routing when the direct native quote is back inside threshold or points the other way.
- **Historical native-peg replay:** supported non-USD fiat backfills now prefer direct CoinGecko native-fiat history and compare that series to the native `1.0` peg before falling back to USD-denominated market history. This prevents long synthetic depeg streaks created only by `USD price / FX reference` mismatch during replay.
...
- Supported non-USD fiat assets can require a fresh direct native-peg corroboration step before a derived USD/FX move is allowed to open, extend, or confirm downstream depeg state
