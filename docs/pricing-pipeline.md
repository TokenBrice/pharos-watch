...
2. If there is no `geckoId` but the asset has one unambiguous supported supply contract, try the same DefiLlama coins endpoint with an exact `chain:contract` key. Accepted quotes publish as `source: "defillama-contract"` and can be used to normalize the same on-chain total-supply fallback.
