...
- **Jupiter Price API V3 freshness semantics:** Jupiter fallback schema-validates the documented V3 quote object (`usdPrice`, `decimals`, `blockId`, and optional `priceChange24h` / `liquidity`). `blockId` is checked against a fresh Solana `getSlot` reference; optional `createdAt` is not used for freshness, and optional `liquidity` is treated as an extra guard only when present.
