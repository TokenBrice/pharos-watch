...
5. `pages-release`
   ...
   - waits for `upload-worker-version` when Worker promotion is required, then uses the uploaded Worker's preview URL for digest sync and local `/_site-data/*` proxying so CI rehearses the static export against the candidate API while validation and Worker promotion continue on their own runners
