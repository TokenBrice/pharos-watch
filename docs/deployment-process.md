...
5. `pages-release`
   ...
   - uses `SMOKE_UI_OVERFLOW_WORKERS=3` for the local smoke so the full local overflow route set remains covered with lower wall time
   - waits for the aggregate `validate / validate` job before Cloudflare Pages production publish
   - captures the current Cloudflare Pages production deployment id as a best-effort rollback target, publishes the already verified local artifact through Wrangler with the existing retry loop, and runs live public UI, ops, and transport smokes in parallel in the same job
