...
2. `validate`
   ...
   - after `npm run validate:prebuild`, runs independent Pages build/SEO, full-test, critical-coverage, and Worker typecheck groups in parallel through `scripts/run-validate-postbuild.mjs`; `npm run build` still precedes `npm run seo:check`, and `VALIDATE_POSTBUILD_SERIAL=1` restores the old serial shape for debugging
