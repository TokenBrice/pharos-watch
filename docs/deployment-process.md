...
2. For deploy-impacting diffs, run the shared validate pre-build command set from `scripts/lib/validation-lanes.mjs`. That ten-lane source owns prebuild/postbuild/Pages/Worker/smoke command ownership and validation-owned deploy-impact paths, including dependency/pricing audits, lint/typecheck, import boundaries, cycle detection across `shared/`, `worker/src`, and `src`, migrations, cron schedule/connection checks, documentation/generated-artifact checks, env contracts, redemption-backstop registry checks, CSP/static-header sync, unused code, hotspot ratchets, SQL-safety, stablecoin data validation, and supply-helper usage. Duplicate-declaration diagnostics belong to the root, test, and Worker TypeScript lanes. The local merge gate path-gates the two network-backed audits: `audit:deps` runs only when package metadata or lockfiles change, and `audit:pricing-providers` runs only when the pricing-provider audit or config changes; scheduled CI still covers fresh advisory/provider drift checks.
...
- `npm run test:merge-gate:discover` to run the same local plan in failure-discovery mode before the authoritative push gate
