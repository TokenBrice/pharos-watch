...
5. `pages-release`
   ...
   - calls `scripts/maintenance/rollback-pages-deployment.mjs` when `deploy-pages` succeeded but any fatal post-publish smoke (live public UI, Yield asset coherence, ops, or transport) failed; the overall workflow still surfaces as failed so the incident is visible
6. `smoke-ui-live`
