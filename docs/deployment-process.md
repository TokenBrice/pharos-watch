...
For combined deploys, the expected overlap starts after the aggregate validation result: `pages-release` and the gated Worker release path can run concurrently, then Pages waits for successful `deploy-worker` completion before production Pages publish. This keeps Cloudflare credentials behind validation while preserving Worker/Pages coordination; the consolidated Pages job reruns local artifact `smoke-ui` after worker promotion and before publishing Pages.
