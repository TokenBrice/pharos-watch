...
2. `validate`
   ...
   - the same reusable workflow also runs `validate-lts`, which installs Node 24.x and executes the targeted `npm run validate:lts` proof lane with the same deploy-surface flags as the main Node 25 validate job
