...
2. When `refresh_data=true`, refresh digests, confirmed depeg events, and public dataset mirrors through the Origin-gated `https://stablecoin-dashboard.pages.dev/_site-data` proxy into `site-api.pharos.watch`. The digest and depeg syncs reject removal of any published slug present in the checked-in snapshot. A failed fetch, invalid input, or archive shrink restores the committed snapshots and continues to the build with a job-summary warning.
