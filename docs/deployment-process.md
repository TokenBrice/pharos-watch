...
5. `pages-release`
   ...
   - retries the complete live Yield asset-coherence suite up to three times with bounded backoff after release-marker convergence, because custom-domain edges can briefly serve new HTML before every immutable chunk is available; every attempt retains the full HTTP, MIME, hydration, and runtime assertions, and persistent failure remains fatal
