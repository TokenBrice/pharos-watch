...
2. When `refresh_data=true`, refresh digests, confirmed depeg events, and public dataset mirrors through the Origin-gated `https://stablecoin-dashboard.pages.dev/_site-data` proxy into `site-api.pharos.watch`. The digest sync rejects archive shrink; the depeg sync carries previously published static rows forward when live reclassification would make them sub-threshold, and rejects any remaining published-slug loss. A failed fetch, invalid input, or archive shrink restores the committed digests, depeg events, dataset mirrors, and Sheets CSV snapshots; removes untracked dataset/Sheets artifacts from the rejected refresh; and continues to the build with a job-summary warning.
