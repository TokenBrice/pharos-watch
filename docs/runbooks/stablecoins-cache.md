...
- **Address-price provider headroom:** production pins `ADDRESS_PRICE_PROVIDERS_ENABLED=none` in `worker/wrangler.toml` while quarter-hour stablecoin sync runs under Worker heap pressure. Do not re-enable the optional exact-address provider group as part of cache remediation unless a fresh Worker headroom audit proves the scheduled run can complete with the group enabled. Reset provider breakers only after confirming the next sync will not immediately consume the same disabled or exhausted lane.
