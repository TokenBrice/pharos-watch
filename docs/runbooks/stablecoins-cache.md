...
- **Birdeye quota exhaustion:** production explicitly allowlists `dexpaprika-address,coingecko-onchain-address,alchemy-address,moralis-address` in `worker/wrangler.toml` while Birdeye compute units are exhausted. Deploy that exclusion before resetting `birdeye-prices`; otherwise the next stablecoin sync can immediately reopen the breaker. Re-enable `birdeye-address` only after quota is provisioned and a direct price request succeeds.
