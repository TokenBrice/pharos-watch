...
The webhook stores per-update claims in `telegram_processed_updates`. It inserts a `processing` row before command handling and marks it `processed` only after successful or terminal handled completion. Duplicate processed updates are ignored, while failed/non-terminal rows can be retried without a global high-watermark drop.
...
4. **Determine whether retry is safe.** Subscribe/unsubscribe/settings commands are idempotent enough for the user to resend. Ambiguous ticker replies and setup wizard callbacks may require clearing stale `telegram_pending_disambiguation` state first.
...
3. **Repair a stuck claim narrowly.** If a row is stuck in `processing` after the Worker failed before terminal handling, mark that specific `update_id` as `failed` or delete that one row so Telegram/user retry can process it. Do not clear the whole table.
