...
- `yieldHealth.status` is `"degraded"` (or `"stale"` once a supplemental, coverage-audit, or benchmark-feed surface passes its stale threshold) while `statusImpact` stays `"admin-watch"` for admin-watch yield diagnostics such as sparse safety coverage, stale supplemental cache, benchmark fallback, old coverage audit, or low source-risk evidence coverage; stale comparison anchors are exposed as field-level watch signals, but do not by themselves change the aggregate `yieldHealth.status`
