...
Behavior: dispatch aborts the run rather than producing a falsely-empty preset-subscriber list, so no snapshots are written and no per-coin alerts are sent for preset followers. See [`docs/telegram-alerts.md`](../telegram-alerts.md) section "Preset Watchlists" for the fail-closed contract.
...
```sql
SELECT preset_alias, COUNT(*) AS followers
FROM telegram_preset_subscriptions
GROUP BY preset_alias
ORDER BY followers DESC;
```

Check the safety-source cache TTL (the resolver's upstream dependency):

```bash
cd worker
npx wrangler tail stablecoin-api --format pretty
# Look for publish-report-card-cache run completions and `alert:safety-source-cache` writes.
curl -sS -H "CF-Access-Client-Id: $CF_ID" \
        -H "CF-Access-Client-Secret: $CF_SECRET" \
        https://api.pharos.watch/api/status | jq '.telegramBot.presetQueryFailures, .telegramBot.presetResolutionFailures'
```
...
2. **Stable stablecoins-cache miss.** Trigger or wait for the next `publish-report-card-cache` cron run; the resolver will succeed on the next dispatch tick once the cache is hot. Confirm via the `alert:safety-source-cache` health check above.
