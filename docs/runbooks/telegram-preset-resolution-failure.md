...
2. **Stablecoins cache available?** The resolver reads the strict `stablecoins` cache plus `TRACKED_STABLECOINS`. A missing or malformed stablecoins cache makes resolution fail closed. Check the `sync-stablecoins` cron and the `stablecoins` cache row before looking at safety-alert source state.
