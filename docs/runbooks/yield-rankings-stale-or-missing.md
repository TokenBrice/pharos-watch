...
- If the degraded reason is `safety-snapshot-unavailable:<reason>`, diagnose the upstream Safety Score V9 publication (`report-cards:v9`, `report-cards:v9:publication-health`, and the `compute-safety-score-v9` / `compute-safety-score-v9-workflow` runs). Nothing in the yield lane is broken and no D1 row needs repair: the yield runtime defers for exactly as long as no usable generation exists, and the next `:55` run publishes automatically once the accepted generation is readable inside its window. A reason of `safety-snapshot:v9-publication-held` inside the run's `fallbackMode` is not this case — the run published against the held accepted generation, and only the safety chain needs attention.
