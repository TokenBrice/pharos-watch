...
7. **Webhook drift.** The 5-minute Telegram lane reconciles the webhook automatically. Force a manual reset with `scripts/maintenance/register-telegram-webhook.sh` only if reconciliation is also failing.
