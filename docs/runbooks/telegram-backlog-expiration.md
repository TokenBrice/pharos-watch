...
- `telegramBot.oldestPendingDeliveryAgeSec > 6300` (105 minutes) means two-hour risk-alert rows are inside the final 15-minute TTL window. Launch, admin, and personalized-recap rows expire on their own schedules ([`docs/telegram-alerts.md`](../telegram-alerts.md#pending-delivery-queue) owns the full set), so evaluate those source types against their own expiry rather than the aggregate oldest-row age.
...
3. **Let scheduled expiry cleanup handle expired rows.** Never treat row age as expiry: a `created_at` cutoff is not `expires_at`, and filtering on it can cancel still-live risk, launch, or recap alerts. Scheduled cleanup evaluates explicit expiry (with the legacy two-hour fallback), dead-letters expired rows, and marks their targets expired before deletion.
4. **Size a dominant chat read-only.** Use the query in [`telegram-rate-limit-storm.md`](./telegram-rate-limit-storm.md). The filtered clear endpoint is retired, and a direct `DELETE` would skip dead-letter, target-state, recap-state, and audit bookkeeping. Let scheduled cleanup handle the rows; if an incident truly requires manual clear semantics, restore and review the retired bounded endpoint rather than improvising destructive SQL.
...
- [`telegram-bot-wide-outage.md`](./telegram-bot-wide-outage.md) for pause-state inspection, the audited-control requirement, and recovery.
