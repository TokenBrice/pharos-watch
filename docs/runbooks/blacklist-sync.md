...
- Tron-only historical amount gaps with a healthy `sync-blacklist` run do not establish a stuck cursor. Current-balance backfill and the next scheduled balance refresh cannot prove an event-time amount. Preserve null until exact historical evidence supports the guarded transfer-replay repair; never copy the current ledger or treat an unavailable amount as zero.
