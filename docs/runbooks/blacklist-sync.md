...
- Tron-only historical amount gaps with a healthy `sync-blacklist` run do not establish a stuck cursor. Current-balance backfill and the next scheduled balance refresh cannot prove an event-time amount. A freeze older than 15 minutes is replayed automatically from the token's confirmed transfer ledger when that ledger reconciles with a raw confirmed `balanceOf` read; a row that stays unresolved across several runs is one the replay refuses (unreconciled or truncated ledger, unmatched receipt, shared-millisecond transfer), so escalate with the guarded transfer-replay CLI instead of copying the current ledger or treating an unavailable amount as zero.
