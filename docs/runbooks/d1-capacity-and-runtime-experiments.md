...
The scheduled `status-self-check` lane refreshes the Cloudflare control-plane size observation and claims at most one bounded per-table growth snapshot per UTC day. A 60% crossing opens a warning watch, 75% makes public/admin health degraded, and 90% makes capacity health stale. A control-plane failure is logged and returned as a monitoring error without replacing the last cached assessment. Detailed capacity and table-growth assessments are admin-only through `/api/status.d1Usage.capacity` and `/api/status.d1Usage.tableGrowth`; public `/api/health` exposes D1 impact only through its overall status and machine-readable warnings. Ordinary status requests read the cached table snapshot and never run its table-count queries. An uninitialized observation cache remains compatible because the admin fields are nullable.
