...
- On Apr 8, 2026, PSI depeg identity and replay bounds were corrected without changing the live PSI compute version: legacy depeg IDs such as `ust-terra-classic` now canonicalize onto the current PSI universe before live/backfill price and supply matching, and historical replay now clamps same-day price-derived deviation to the event's recorded `peak_deviation_bps` instead of letting stale peg references inflate daily crises beyond the event's own peak.
