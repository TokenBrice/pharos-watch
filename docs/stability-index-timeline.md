...
- On Apr 7, 2026, the historical replay path was corrected without changing the live PSI compute version: completed-day backfills now treat any depeg overlapping the UTC day as active, keep `peak_deviation_bps` as a floor on the UTC start day when a coarse daily snapshot undercaptures an intraday shock, and preserve existing stored rows on days where archival replay inputs are missing instead of deleting those days during rebuilds.
