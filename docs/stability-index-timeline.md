...
- On Apr 8, 2026, PSI depeg identity and replay bounds were corrected without changing the live PSI compute version: legacy depeg IDs such as `ust-terra-classic` now canonicalize onto the current PSI universe before live/backfill price and supply matching, historical replay now clamps same-day price-derived deviation to the event's recorded `peak_deviation_bps` instead of letting stale peg references inflate daily crises beyond the event's own peak, replayed days whose restored daily price is already back inside the configured depeg threshold no longer count as active PSI contributors, and the historical restore path now repairs replay-critical `supply_history.price` coverage for restored USD rows plus PSI shadow assets so rebuilt history does not overuse peak fallback.
