...
| `asymmetry`                | `usdaf-asymmetry`                                                                                                              | `inputs.primary.kind = "http-json"` -> `https://app.asymmetry.finance/api/stats` (`usdaf.branch` plus top-level source timestamp)                                                                                                                                                                                                                                                                                  |
| `btcfi`                    | `btcusd-btcfi`                                                                                                                 | `inputs.primary.kind = "http-json"` -> `https://www.btcfi.one/api/getBtcfiMarket?isTestnet=false` + handler metadata                                                                                                                                                                                                                                                                                              |
| `chainlink-nav`            | `usyc-hashnote`, `tbill-openeden`, `usdy-ondo-finance`, `ousg-ondo-finance`, `mtbill-midas`                                    | `inputs.primary.kind = "onchain-evm"` -> Chainlink-style NAV oracle reads via Etherscan proxy RPC or configured public RPC (`latestRoundData()`, Ondo `getPrice()`, or Ondo `getAssetPrice(token)` + `getPriceData()` when available); USYC, TBILL, OUSG, and mTBILL use a 4-day `maxOracleAgeSec` to match business-day NAV cadence over weekends and market holidays |
...
| `frax-balance-sheet`       | `frax-frax`, `frxusd-frax`, `lusdfi-frax`                                                                                      | `inputs.primary.kind = "http-json"` -> Frax v2 balance-sheet APIs with `asOfTimestamp` and per-token USD asset values                                                                                                                                                                                                                                                                                            |
...
| `liquity-v1`               | `lusd-liquity`                                                                                                                 | `inputs.primary.kind = "onchain-evm"` -> Ethereum `eth_call` reads against Liquity v1 `TroveManager` (`getEntireSystemColl()` / `getEntireSystemDebt()`), plus optional live redemption-fee probe from the same contract                                                                                                                                                                                          |
| `m0`                       | `m-m0`, `musd-metamask`, `usdn-noble`, `usdsc-startale`, `ctusd-citrea`, `wm-m0`, `usdnr-nerona`, `usdk-kast`, `xo-exodus`      | `inputs.primary.kind = "http-json"` -> `https://protocol-api.m0.org/graphql` (`CollateralCurrent` plus latest collateral/update timestamp for source freshness)                                                                                                                                                                                                                                                    |
| `mento`                    | `cusd-celo`, `ceur-celo`                                                                                                       | `inputs.primary.kind = "http-html"` -> `https://reserve.mento.org/` (server-rendered `reserveComposition` plus embedded reserve-holding update timestamps)                                                                                                                                                                                                                                                         |
| `openeden-usdo`            | `usdo-openeden`                                                                                                                | `inputs.primary.kind = "http-json"` -> `https://prod-gw.openeden.com/usdo/sys/reserve-composition-last` with browser-style origin hints when the upstream gateway is origin-sensitive                                                                                                                                                                                                                              |
...
| `single-asset`             | `buidl-blackrock`, `meusd-mezo`, `feusd-felix`, `cetes-etherfuse`, `paxg-paxos`, `usdb-blast`                                  | `inputs.primary.kind = "onchain-evm"` or `http-json` -> single-asset probe with fixed 100% composition                                                                                                                                                                                                                                                                                                            |
...
| `superstate-liquidity`     | `ustb-superstate`                                                                                                               | `inputs.primary.kind = "onchain-evm"` + `https://api.superstate.com/v1/funds/liquidity` -> USTB NAV oracle reserve proof plus current Circle USD / USDC RedemptionIdle liquidity capacity                                                                                                                                                                                                                         |
| `tether`                   | `usdt-tether`                                                                                                                  | `inputs.primary.kind = "http-json"` -> `https://app.tether.to/transparency.json`                                                                                                                                                                                                                                                                                                                                  |
| `usd1-bundle-oracle`       | `usd1-world-liberty-financial`                                                                                                 | `inputs.primary.kind = "onchain-evm"` -> USD1 Chainlink bundle oracle `latestBundle()` / `latestBundleTimestamp()` plus USD1 ERC-20 total supply                                                                                                                                                                                                                                                                   |
...
| `worker/src/handlers/scheduled/*.ts`               | Per-trigger slot runners (quarter-hourly, isolated 30-min mint/burn lanes, half-hourly charts/DEX liquidity/DEWS/PSI, 2-hourly DEX discovery, 6-hourly blacklist, 4-hourly reserve slot, Telegram, and daily slots) |
