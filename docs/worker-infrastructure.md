...
| `chainlink-nav`            | `usdy-ondo-finance`, `ustb-superstate`, `ousg-ondo-finance`, `mtbill-midas`                                                     | `inputs.primary.kind = "onchain-evm"` -> Chainlink-style NAV oracle reads via Etherscan proxy RPC (`latestRoundData()`, Ondo `getPrice()`, or Ondo `getAssetPrice(token)` + `getPriceData()` when available); OUSG and mTBILL use a 4-day `maxOracleAgeSec` to match business-day NAV cadence over weekends and market holidays |
...
| `single-asset`             | `usyc-hashnote`, `buidl-blackrock`, `meusd-mezo`, `feusd-felix`, `cetes-etherfuse`, `paxg-paxos`, `usdb-blast`               | `inputs.primary.kind = "onchain-evm"` or `http-json` -> single-asset probe with fixed 100% composition                                                                                                                                                                                                                                                                                                            |
