...
The sibling daily `prune-detail-cache` job (`worker/src/cron/prune-detail-cache.ts`) owns the only DELETE path for `detail:<id>` rows in the `cache` table. Keyset-paginating the `detail:` prefix, it deletes a row when the id is absent from `WORKER_READABLE_IDS` in `shared/lib/stablecoins/worker-runtime-registry.ts` (orphan) or when the row is older than a week (stale, well past the 24-hour age at which a cached detail body stops being served and forces a synchronous refresh). Retiring or renaming a coin therefore drops its cached detail rows on the next daily run without any explicit cleanup step. The job covers nothing else: other `cache` keys, including the `detail-write-failure:<id>` markers, are not in its scan.
