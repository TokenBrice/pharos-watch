...
The paired Pages Functions contracts live in `functions/lib/ops-env.ts` and `functions/lib/site-api-env.ts`, with the same `required` / `optional` / `reserved` / `active` shape derived from that shared manifest. Worker runtime validation logs contract errors when Access bindings are only partially configured, when admin D1 status bindings are only partially configured, when `SITE_API_SHARED_SECRET` is missing, when `GITHUB_PAT` / `FEEDBACK_IP_SALT` are missing for `POST /api/feedback`, when `API_KEY_HASH_PEPPER` is missing, when `BANXICO_TOKEN` is missing for the official MXN CETES benchmark, or when the self-serve API key email verification bindings are only partially configured. It also flags a no-op `API_KEY_HASH_PEPPER_PREVIOUS` rotation (identical to `API_KEY_HASH_PEPPER`) and a partially configured Telegram bot-token / webhook-secret pair, including their `_PREVIOUS` rotation markers. The Pages ops-proxy contract now actively requires `CF_ACCESS_TEAM_DOMAIN` + `CF_ACCESS_OPS_UI_AUD` for inbound UI JWT verification. The Pages `site-data` `DB` binding is part of the required Pages site-data contract because selector-snapshot POST quotas fail closed without it; `/_site-data/*` public-read proxy requests continue without the binding but log that attribution telemetry is disabled.
...
The generation-fenced D1 target and quote tables are additive. Active and shadow targets/quotes use separate producer surfaces. A quote publication stores measured and real failure outcomes plus a target-manifest digest; manifest-proven `budget-deferred` outcomes are reconstructed on read instead of being written as dense rows. `sync-dex-liquidity-stage` loads sources and builds the exact pool graph hourly at `:10`, storing it as bounded chunks in `dex_liquidity_scoring_stages` / `dex_liquidity_scoring_stage_chunks`. The D1-only `sync-dex-liquidity` consumer at `:16` publishes DEX prices hourly, publishes liquidity score/history and active measured targets hourly, and publishes shadow target inventories once daily at `06:16 UTC`. When that source slot's stage is terminal (failed generation, or an errored stage run with no live stage lease) the `:16` consumer re-runs the stage for the exact slot under the stage job's lease and then publishes, so one failed hourly stage no longer leaves the DEX lanes degraded until the next hour; the `:46` bridge publishes the hour's still-unconsumed stage instead of skipping it as a cadence reuse, re-running it first when it is terminal or never started. On successful hourly publications, the serial D1-only turnover source of `cron-sentinel` compares the published per-coin route identities and evidence kinds with its compact prior-publication cache snapshot before V9 input preparation. The half-hour collector reuses the latest Solana target generation until the next daily refresh. The retained `:46` path reuses the exact current liquidity generation without rewriting DEX surfaces so half-hour Safety Score V9 input preparation remains fenced.
...
BNB Chain Uniswap V3 is also shadow-only. Its six-source Uniswap subgraph family is capped at five concurrent requests; BSC contributes execution candidates but not fee enrichment or DEX price consensus. A bounded target-only staging recovery independently pins the official factory, exact pool binding, token order, fee, slot state, balances, and the reviewed QuoterV2 deployment before a target can reach the daily shadow publication.

Hook-free Ethereum Uniswap V4 is active and score-eligible in this reviewed tranche. Hooked pools and all other V4 chains remain unsupported, and the shadow machinery is retained for future cohorts and rollback comparison.
...
Repair debt: DDR repair-required events are written to canonical `worker_repair_tasks`; the legacy DDR cache row remains only for later coordinated cleanup and is no longer read or written. The daily repair-debt source of `cron-sentinel` executes queued T1.2-safe repairs when enabled, while always reading due/stale backlog counts for visibility. `DDR_REPAIR_TASK_RUNNER_ENABLED` defaults to enabled when unset or empty; accepted on/off values are documented in the env table, and invalid non-empty values disable the runner with a structured warning. Producer reconciliation closes tasks that are no longer current, and retention prunes old terminal rows.

Data-invariant canaries: unset `WORKER_CANARY_MODE` defaults to `off`, and the checked-in Worker config now sets `status` after the shadow soak produced consecutive clean 7/7 cycles. The active registry now has eight checks, including blacklist identity completeness, which counts `blacklist_events` and `blacklist_current_balances` rows where both `config_key` and `contract_address` are `NULL`; any non-zero count is an error. `off` skips and writes no run, `shadow` records observed findings while the cron remains OK, `status` exposes current status-mode rows and returns degraded findings from the cron, and `alert` exposes current alert-mode rows and returns critical findings as a terminal error. In `off` or `shadow`, `/api/status.canaries` returns its empty/unknown compatibility shape without querying retained authoritative rows. The next promotion target remains `alert` only after a separate alert-routing acceptance review; roll back to `shadow` or `off` if status-mode findings are unexplained.

Reserve interruption recovery: unset `WORKER_RESERVE_RECOVERY_MODE` defaults to `off`, while the checked-in Worker config sets `recover`. Producer checkpoints are written in every mode; the five-minute recovery lane uses `off` to skip recovery scans and `recover` to claim and replay interrupted or abandoned checkpoint suffixes using the checkpoint's next-item pointer. Compatible queue hashes are filtered before bounded recovery claims, and malformed or stale checkpoints remain auditable without a second cache cursor.
...
| `TELEGRAM_WEBHOOK_SOURCE_RATE_LIMIT` | `RateLimit` | required | - | - | Cloudflare per-source pre-authentication rate limiter for Telegram webhook requests. |
| `TELEGRAM_MINI_APP_SESSION_SOURCE_RATE_LIMIT` | `RateLimit` | required | - | - | Cloudflare per-source pre-authentication rate limiter for Telegram Mini App session requests. |
| `TELEGRAM_MINI_APP_MUTATION_SOURCE_RATE_LIMIT` | `RateLimit` | required | - | - | Cloudflare per-source pre-authentication rate limiter for Telegram Mini App mutation requests. |
| `DONOR_KEY_CLAIM_RATE_LIMIT` | `RateLimit` | required | - | - | Cloudflare per-IP rate limiter for anonymous supporter (donor) API key claims, checked before the request body is read. |
| `SAFETY_GRADES_RATE_LIMIT` | `RateLimit` | required | - | - | Cloudflare per-IP rate limiter for public safety grades, checked before edge-cache reads. |
...
| `DWELLIR_API_KEY` | `string` | optional | - | - | Dwellir credential for supplemental multi-chain RPC endpoints during the provider trial; sent only as the X-Api-Key header. |
| `DWELLIR_MAX_CREDITS_PER_MONTH` | `string` | optional | - | - | Optional positive integer local cap for Dwellir JSON-RPC credit units consumed during one UTC month; unset or invalid values fall back to the built-in 20,000,000 credit default. |
| `ADDRESS_PRICE_PROVIDERS_ENABLED` | `string` | optional | - | - | Optional exact-address price-provider allowlist for the hourly corroboration step. Only `coingecko-onchain-address` is available, and an unset value enables no provider. |
...
| `TELEGRAM_OPERATOR_CHAT_ID` | `string` | optional | - | - | Private Telegram chat for operator-only alerts (cron freshness watchdog); operator alerts are suppressed when unset and never fall back to `TELEGRAM_CHAT_ID`. |
...
| `DDR_REPAIR_TASK_RUNNER_ENABLED` | `string` | optional | - | - | DDR repair-task runner control: `1`, `true`, `yes`, `on`, or `enabled` enables execution; `0`, `false`, `no`, `off`, or `disabled` disables it; unset or empty defaults to enabled; invalid non-empty values fail closed by disabling the runner and emitting a structured warning. |
| `WORKER_RESERVE_RECOVERY_MODE` | `string` | optional | - | - | Reserve interruption recovery mode. Unset or `off` skips recovery scans; `recover` claims and replays interrupted or abandoned reserve slots. |
...
| `WORKER_V9_WORKFLOW_MODE` | `string` | optional | - | - | Safety Score V9 Workflow pilot mode: `off` keeps the cron-only path; `shadow` creates a replay-safe shadow instance after the authoritative cron publication settles. |
...
Non-exempt `/api/*` requests on `api.pharos.watch` require a valid `X-API-Key`; missing or invalid keys return `401 Unauthorized`, and auth or limiter dependency failures fail closed with `503 Service Unavailable` and `Retry-After: 60`. The no-key exemptions are owned by the centralized endpoint registry in `shared/lib/api-endpoints/`: `GET /api/safety-grades`, `GET /api/health`, `GET /api/og/*`, `POST /api/feedback`, `POST /api/api-key-requests`, `POST /api/api-key-requests/verify`, `POST /api/donor-key-claims`, `POST /api/telegram-webhook`, `POST /api/telegram-mini-app/session`, and `POST /api/telegram-mini-app/mutate`.
...
- Valid keyed requests use the D1-backed `api_key_rate_limit` table with the per-key threshold stored in `api_keys.rate_limit_per_minute` (default `120/min`, self-serve `30/min`, donor `10/min` with no expiry).
- Protected cacheable `GET` edge-cache hits can use a narrow fast path when the key was recently verified in the same isolate and has the standard tier. That path still validates the presented token against the bounded fresh auth cache and applies the bounded isolate-local limiter before serving the cached response. Cold keys, donor and self-serve keys, expired auth-cache entries, edge-cache misses, cache-bypass routes, and non-`GET` requests keep the normal D1-backed auth and limiter path. After repeated D1 limiter failures, a 60-second isolate-local circuit opens only for protected cacheable reads and caps the fallback quota at the self-serve default; non-cacheable requests continue to fail closed with `503`.
- Requests already authorized for the `ops-api.pharos.watch` admin lane bypass the per-key limiter.
- `GET /api/safety-grades` is exempt from `X-API-Key` (`worker/src/api/safety-grades.ts`): a grade-only projection of the accepted V9 publication with the report-cards cache profile, so almost all traffic is an edge-cache hit. The `SAFETY_GRADES_RATE_LIMIT` native binding admits at most 120 requests per minute per source IP and Cloudflare location, before any edge-cache read. Missing or unavailable binding fails closed with `503`; exhausted allowance returns `429` with `Retry-After: 60`. Authenticated site-proxy and admin lanes retain their existing handling. The broad public-API zone WAF rule remains disabled; it is not the safety-grades control. Binding activation and an affected request must be verified against the deployed Worker version before claiming production remediation.
- `/api/api-key-requests` returns `403` before parsing the body while `SELF_SERVE_ISSUANCE_OPEN` (`shared/lib/public-api-contract.ts`) is `false`. When open, `/api/api-key-requests` and `/api/api-key-requests/verify` are exempt from `X-API-Key`, return no-store responses, and have their own request/verification throttles in `api_key_request_rate_limit_v2`; the per-token verification bucket is spent only after the token lookup proves the token exists, so unknown tokens mint no limiter rows. Successful issuance is additionally capped through `api_key_self_serve_issuance_limits`.
- `POST /api/donor-key-claims` (`worker/src/api/donor-key-claims.ts`) is exempt from `X-API-Key`. Claims are enabled for this release by `DONOR_KEY_CLAIMS_OPEN` (`shared/lib/public-api-contract.ts`). If paused (`false`), it returns `403` before anything else; when enabled, it spends `DONOR_KEY_CLAIM_RATE_LIMIT` (`10` per `60` seconds per client IP) before the body is read, returning `429` with `Retry-After: 60` on denial and `503` when the binding is absent. It then parses the Sign-In-With-Ethereum message, validates domain `pharos.watch`, URI `https://pharos.watch/api/`, and a 5-minute freshness window, recovers the externally-owned signer, and requires at least $10 in receipt-date USD from reconciled stablecoin donations in `shared/data/funding/donations.json`. The threshold is inclusive, so exactly $10 qualifies; non-stablecoin donations and `pool` rows are excluded, and founder stablecoin rows count under the same grade rule. Only stablecoins graded A+, A, A-, B+, B, or B- in the current non-held canonical accepted V9 Safety Score publication at claim time count. C/D/F/NR or missing grades do not count; an unavailable, missing, or held publication pauses claims with `503`. Values remain USD at receipt, not claim-time prices. Later grade changes do not revoke or alter an issued key. The reviewed symbol-to-canonical-ID mapping in `shared/lib/funding/donor-eligibility.ts` admits USDC, USDT, DAI, USDGLO, and CRVUSD; token contracts are verified during ledger curation and new assets require a reviewed allowlist update. Apply migration `0238_api_key_donor_claims.sql` before deploying this enabled Worker and matching Pages build; live acceptance follows `docs/api-page.md`. Issuance is one `db.batch` writing the `api_key_donor_claims` row and the `api_keys` row together, so a losing concurrent claim rolls back instead of orphaning a key. Responses are no-store and the plaintext token is returned once. Signatures, tokens, and wallet addresses are never logged.
- Keys with `tier="donor"` are forced onto the D1-backed auth and limiter path, like self-serve keys: `authenticateApiKeyFromFreshCache` reports them unavailable, so their `10/min` ceiling is global rather than per isolate and a D1 auth outage fails closed for them.
...
- API keys carry `api_keys.traffic_class` (`external` or `site`) so request attribution can treat website-owned automation separately from third-party consumers. `FEEDBACK_IP_SALT` remains scoped to feedback submission hashing only.
- The three Telegram POST exemptions pass through path-specific Cloudflare Rate Limiting bindings before body reads, Telegram auth, D1, or request attribution. There are no active Telegram-specific zone WAF rules. [`telegram-ingress-abuse-controls.md`](./runbooks/telegram-ingress-abuse-controls.md) owns the deployed posture, including the per-route ceilings and body caps; any WAF planning note there is non-authoritative, while `scripts/ci/cloudflare-account-state-manifest.json` is the account-state authority.
- Supporter key claims use one table, `api_key_donor_claims` (`address` primary key, unique `key_prefix`, `claimed_at`), written in the issuance batch. It stores no signature, no token, and no request metadata; it exists so a wallet cannot claim twice and so an operator can find the key issued for one address. Removing a donation row later does not revoke anything on its own, so the operator must also deactivate the matching `donor` key.
...
- retention is bounded to the latest `35` days; request-path flushes prune opportunistically, and the daily `prune-cron-history` pass provides the backstop
...
The daily `prune-cron-history` job owns cleanup for cron/producer/slot observability, quota, block-timestamp cache, canary, recovery-checkpoint, repair-task, self-serve limiter, and request-telemetry rows. High-growth retention deletes select oldest eligible `rowid`s in fixed-size statements and stop at an explicit per-invocation row or batch budget; cron metadata reports deleted counts and `retentionTruncated` when repair-task, canary, recovery-checkpoint, or self-serve limiter work remains. The self-serve limiter sweep drains oldest-first in bounded batches until the eligible backlog is empty or a 100,000-row per-run budget is reached, and it runs only here, never on an API-key request or verification path. `api_request_consumer_stats`, `site_data_request_stats`, and `api_key_request_stats` share the request-attribution authority's 35-day window. Each request table is deleted oldest-first in 10,000-row statements with a 100,000-row per-run ceiling; cron metadata reports each deleted count and whether a table reached that ceiling. The per-table rulings for the append-only product/audit tables it intentionally leaves alone live in [`worker/migrations/MANIFEST.md`](../worker/migrations/MANIFEST.md#append-only-retention-policy). Do not duplicate that inventory here.

The reviewed 2026-09-21 D1 baseline was `2,995,675,136` bytes against the 10 GB per-database ceiling, with 2,979,885 user rows across 157 expected application tables. Request telemetry accounted for 515,916 `api_request_consumer_stats` rows, 194,841 `site_data_request_stats` rows, and 176,155 `api_key_request_stats` rows. These values are a measurement baseline, not reclaimed capacity; the 10 GB ceiling itself is `D1_PAID_MAX_DATABASE_SIZE_BYTES` in `shared/lib/d1-capacity.ts`. D1 does not return free pages after a delete, so 48 hours after the first deployed prune, re-count all three tables with remote read-only `wrangler d1 execute --command` queries and compare `wrangler d1 info` `database_size`. Do not assume the bytes moved without that observation or a separately reviewed `VACUUM`-equivalent operation. Trend growth before considering retention for product archives such as `yield_source_decisions`, `yield_history`, or `worker_producer_history`.

The baseline numbers above are 2026-09-21 production captures, recorded with their producing commands so no capacity figure travels without them. `npx wrangler d1 info stablecoin-db --json` returned `database_size: 2995675136` and `num_tables: 158` (Wrangler's table count; the expected application-table roster in `worker/migrations/EXPECTED_SCHEMA.txt` is 157). The row counts come from four read-only batches of at most 40 scalar `(SELECT COUNT(*) FROM "<table>") AS "<table>"` columns covering all 157 expected tables, summing to 2,980,015 rows in that capture; the sibling same-day capture behind the 2,979,885 total above summed its own 157 counts (the live database moved 130 rows between the two snapshots). Raw captures: `agents/2026-09-21-holistic-review/evidence/live/ver-06/` (`d1-info.json`, `top-20-table-counts.json`, `table-counts-success-{1..4}.sql`) and `agents/2026-09-21-holistic-review/evidence/live/d1-row-counts-summary.json`. Beyond request telemetry, the largest growth surfaces were `supply_history` 245,994; `yield_source_decisions` 231,881; `yield_history` 212,840; `yield_source_decision_alternatives` 164,398; `worker_producer_history` 164,191; and `tape_events` 157,556. This reviewed budget is the D1 capacity context beside `CRON_GROWTH_HEADROOM_POLICY` in `shared/lib/cron-jobs.ts`.

The DDR publication family (`depeg_resolver_publication_snapshots`, `depeg_resolver_publication_snapshots_v2`, `depeg_resolver_publication_snapshot_refs`) is attributed in the daily table-growth snapshot: it is discovered through the `depeg_` family rule and carries an explicit `published_at` timestamp column in `worker/src/lib/status/d1-usage.ts`, so `/api/status.d1Usage.tableGrowth` reports its row counts, deltas, and oldest/newest publication for the archive decision. Since the reference rows landed, an unchanged quarter-hourly publication adds one narrow row to `depeg_resolver_publication_snapshot_refs` instead of a compressed payload row, so payload-table growth tracks content changes rather than the cron cadence.

The sibling daily `prune-detail-cache` job (`worker/src/cron/prune-detail-cache.ts`) owns the only DELETE path for `detail:<id>` rows in the `cache` table. Keyset-paginating the `detail:` prefix, it deletes a row when the id is absent from `WORKER_READABLE_IDS` in `shared/lib/stablecoins/worker-runtime-registry.ts` (orphan) or when an active coin's row (`WORKER_ACTIVE_IDS`) is older than a week (stale, well past the 24-hour age at which a cached detail body stops being served and forces a synchronous refresh). Non-active readable coins (frozen, quarantined, delisted) are exempt from the age rule: the detail API never refreshes them and serves their last row indefinitely, so deleting it would turn every later read into a permanent 404. Retiring or renaming a coin therefore drops its cached detail rows on the next daily run without any explicit cleanup step. The job covers nothing else: other `cache` keys, including the `detail-write-failure:<id>` markers, are not in its scan.

Every capped retention pass in the Worker runs through one lifecycle, `runCappedPruneFamily()` in `worker/src/cron/shared/capped-delete.ts`, which wraps the `deleteCapped()` batch loop. The caller supplies only data: the capped statements in the order they must run (each with its own SQL, binds, batch limit, and run budget) and the backlog probes issued afterwards. The lifecycle owns the parts that must never diverge between retention families — duration timing, statement sequencing, rethrowing a shutdown abort instead of reporting it, and capturing any other failure as bounded 500-character text next to the counts already collected, so a partial pass is still published. Mint/burn event and hourly retention plus its aggregation repair, Telegram high-growth retention, DEX liquidity generation and price-stage retention, DEX discovery staging cleanup, and measured-execution generation retention are all specs over it; a new prune adds a spec, never another envelope.
...
For endpoint descriptors marked `cacheKeyIgnoresQuery: true`, the edge-cache key drops the URL query string because the route handler does not consume query parameters. Parameterized routes retain their full query-bearing identity. This changes only cache lookup identity: per-request auth gates, attribution, `Vary`, CORS, and response cache-control handling remain in force.
...
| Standard           | `public, s-maxage=300, max-age=60`                             | stablecoin-charts, redemption-backstops, usds-status, daily-digest, digest-archive, stability-index, yield-rankings, mint-burn-flows, stress-signals, yield-adapter-manifest; `/api/report-cards/v9` uses this handler profile only while current, switches to `no-store` while held, and always bypasses edge cache |
...
- `GET /api/report-cards/v9` is the canonical Safety Score contract backed by `cache["report-cards:v9"]` and `cache["report-cards:v9:publication-health"]`. The reader validates and projects the accepted evaluator publication into the owned public schema with complete model, policy, build, base-input, and publication identity. Missing, malformed, or incomplete accepted state returns `503`; when individually valid publication and health rows disagree, the route serves the authenticated publication as explicitly held instead of hiding the board. It never falls back to V8 or recomputes a score. While held it serves the last accepted snapshot with `X-Safety-Score-Status: held`, accepted-timestamp freshness headers, and `Cache-Control: no-store`.
- `GET /api/yield-rankings` hydrates Safety Score fields only from a newer, complete, identity-compatible canonical V9 publication. Missing, held, or identity-incompatible V9 leaves ranking rows available but marks Safety unavailable/NR and nulls PYS where applicable; the handler never recomputes a Safety Score. The post-V9 publisher stages a yield publication generation, rechecks the active V9 identity before publishing, validates the rankings payload, and publishes the `yield-rankings` cache through CAS before replacing current `yield_data` rows. If the safety identity changed mid-run, the cache write fails, or CAS skips because a newer cache exists, the prior published D1 rows remain visible to downstream readers; successful cache writes then persist current/history rows and compact selected-source decisions as `published` for that generation.
...
- Pages Functions proxy same-origin `/_site-data/*` requests only to the exact HTTPS `SITE_API_ORIGIN=https://site-api.pharos.watch`. `resolveTrustedHttpsOrigin()` (`functions/lib/trusted-upstream-origin.ts`) rejects missing, non-HTTPS, credentialed, port-bearing, path-bearing, query-bearing, and foreign origins, and the proxy returns `500` before `SITE_API_SHARED_SECRET` is attached. The selector-snapshot Pages Function uses the same trusted origin and secret to recompute new share artifacts from canonical API responses. The lane accepts only `GET`, allowlists public-read routes through `shared/lib/site-data-lane.ts`, gates caller `Origin` / `Referer`, and forwards upstream `Age` / `Date` cache-age headers without adding a second Pages Cache API lifetime. Its deadline covers bounded response-body consumption, not response headers alone.
...
The worker fingerprints method + path + sorted query + body for a given action key. Replays return the stored response with `X-Idempotent-Replay: true`; conflicting reuse returns `409` with `X-Idempotency-Conflict: request-mismatch` and cannot replace the original intent's audit. Reservations have an owner/generation and a separate durable execution-start transition. An unstarted reservation may be taken over after 20 minutes. A started reservation is at-most-once: once `execution_started_at` is set, the absence of a terminal response cannot prove the effect did not commit, so after 24 hours the row is compare-and-swap recorded as terminally `execution_unknown` with a structured warning and every replay answers `503`. Re-execution is possible only when the action supplies a `reconcileAbandonedExecution` callback that proves the original effect never landed; without that proof the key is never re-run and operator reconciliation is required. Terminal writes remain owner/generation compare-and-swap operations. A thrown handler, an explicit unknown-certainty response, or any handler HTTP 5xx returns `503` with `error = "execution_unknown"` plus `X-Execution-Certainty: unknown`. Later requests with an explicitly stored `execution_unknown` key replay the same operator-reconciliation state without running the mutation again. Terminal rows age out of the table-wide seven-day prune; `PENDING` and `execution_unknown` rows are excluded, because an unknown row is a permanent reconciliation tombstone whose key must never reserve a fresh row and re-run after the TTL.
...
- `worker/src/api/backfill-yield-history.ts`
...
| --- | --- | --- | --- | --- |
| `shared/lib/client-ip-hash.ts` | `cachedSecret`<br>`cachedKey` | Pages and Worker client-IP hashing | Replaced when the supplied secret changes; otherwise resets on isolate recycle or deploy. | The runtime secret is authoritative; the non-extractable imported key is only a derived cache. |
...
| `shared/lib/editorial-style.ts` | `COMPILED` | Editorial style scanner | One compiled RegExp array per policy rule until isolate recycle; the policy is a build-time constant. | The fenced policy in docs/editorial-style.md and its generated module are authoritative; the cache only avoids recompiling fixed patterns. |
...
| `shared/lib/iana-local-time.ts` | `localFormatterCache` | IANA local-time scheduling helpers | At most 256 Intl.DateTimeFormat entries with case-insensitive timezone keys and oldest-entry eviction; unrecognized zones are never cached. | The subscriber timezone in D1 and the runtime timezone database are authoritative; cached formatters carry no business state. |
...
| `shared/lib/safety-score-v9/policy.ts` | `validatedPolicyEnvelopes`<br>`policyChainMaturityIdentities` | Safety Score V9 policy validation and chain-maturity identity | Weak object-identity markers; entries are garbage-collectable and disappear on isolate recycle. | The validated policy envelope, digest, and captured chain-maturity resolution identity are authoritative; the weak collections only attest local validation and preserve identity lookup. |
| `worker/src/api/telegram-webhook.ts` | `lastMissingBotTokenWarnAtMs` | Telegram webhook missing-token telemetry | Suppresses repeat warnings for 60 seconds; resets with the isolate. | The current TELEGRAM_BOT_TOKEN binding is authoritative; this timestamp only limits duplicate warning logs. |
...
| `worker/src/cron/depeg-resolver/utils.ts` | `v9DependencyImpairmentByCoin`<br>`v9MintPostureByCoin`<br>`v9MintPostureProjectionInstalled` | Depeg resolver V9 dependency and mint-posture projection | All three hydrated from the current V9 cards at resolver-run start; cleared when V9 publication is unavailable and reset with the isolate. The installed flag distinguishes a published 'no band' from an absent publication so K1 can fall back to the curated posture instead of failing open. | The current Safety Score V9 publication inputs and stablecoin registry statuses are authoritative; this only avoids repeated per-coin dependency and mint-component scans inside one run. |
| `worker/src/cron/dex-discovery/crawl-cosmos-pools.ts` | `cosmosRequestState` | Cosmos DEX discovery pacing | One last-request timestamp enforcing a 400ms pacing floor inside a discovery run; resets with the isolate and via the test-only reset hook. | Upstream Osmosis and Noble rate limits are authoritative; this state only spaces requests within one isolate. |
| `worker/src/cron/dex-discovery/crawl-horizon-pools.ts` | `horizonRequestState` | Stellar Horizon discovery pacing | One last-request timestamp enforcing the ~1s Horizon pacing floor inside a single discovery run; resets with the isolate and via the test-only reset hook. | Horizon's server-side rate limit is authoritative; this state only spaces requests within one isolate to stay under it. |
| `worker/src/lib/telegram/quiet-hours.ts` | `quietHoursTzFallbackLastLoggedAt` | Telegram quiet-hours telemetry | One fallback-log timestamp per timezone with a one-hour suppression window; resets with the isolate. | Runtime timezone resolution is authoritative; this state only suppresses duplicate logs. |
...
| `worker/src/lib/redemption-backstop/sources.ts` | `outputDependencyResolutionRuns` | Redemption output-dependency resolution | At most four snapshot timestamps retain row references while one serial redemption build converges; old runs are evicted and all state resets with the isolate. | The current redemption build inputs and persisted completed-run snapshot are authoritative; this state only reconciles rows built in either order. |
...
| `worker/src/lib/worker-version-first-seen.ts` | `scheduledVersionFirstSeenAttemptedInIsolate` | Scheduled Worker-version first-seen diagnostics | Set after the first scheduled marker attempt and reset on isolate recycle or deploy. | The write-once D1 worker-version-first-seen cache row is authoritative; this flag only suppresses repeat attempts in one isolate. |
...
| `worker/src/cron/reserve-adapters/request.ts` | `requestCacheStates` | Live-reserve adapter request cache | WeakMap keyed by each run's requestCache Map; LRU byte accounting lives only as long as the run's cache object and resets with the isolate. | Issuer endpoints are authoritative; cached bodies are per-run fetch dedup only, bounded to a 16 MiB budget. |
| `worker/src/lib/chain-registry.ts` | `RPC_AUTH_BY_ORIGIN` | RPC provider auth routing (Alchemy bearer, Dwellir X-Api-Key) | Repopulated by buildAlchemyRpcUrl/buildChainRpcs calls that supply a key and resets with the isolate; Dwellir entries are never cleared in-isolate, so a build without the key cannot break a concurrent scheduled slot. | Env ALCHEMY_API_KEY and DWELLIR_API_KEY are authoritative; the map only pairs key-free origins with their auth header. |
| `worker/src/lib/rpc-provider-budget.ts` | `pendingDwellirCredits` | Dwellir supplemental-RPC credit accounting | Credits recorded per JSON-RPC response item accumulate until the next flushDwellirCredits() drains them through a successful month-ledger compare-and-swap; a failed flush restores them, and isolate recycle drops the remainder. | The cache-table row rpc:dwellir:credits:v1:<YYYY-MM> is authoritative; the pending counter is a per-isolate write buffer only. |
...
| `worker/src/cron/dispatch-telegram-alerts.ts` | `COUNTED_STATEMENT_ORIGINALS` | Telegram planning rows-written attribution | WeakMap keyed by counted prepared statements; entries live only while a batch holds the statement and reset with the isolate. | D1 meta.rows_written on each executed statement is authoritative; the map only pairs counted wrappers with their originals for batch attribution. |
| `worker/src/lib/authoritative-price-sources/mento-broker.ts` | `validatedHeads` | Mento Broker Celo head reuse | WeakMap keyed by each price stage's LivePriceContext; a head is reused only while under 300 s old and dies with the context or isolate. | Celo RPC state is authoritative; every route rereads full broker state and rechecks the block's canonical hash before publishing. |
| `worker/src/lib/evm-rpc.ts` | `demotedSupplementalRpcOriginsByRun` | EVM RPC supplemental failover (Dwellir trial) | WeakMap keyed by each run's chainRpcs Map; demoted origins live only as long as that map object and are garbage-collectable, then reset with the isolate. | None: it only shortens supplemental failover within a single run, registry endpoints are never demoted, and no read result depends on it. |
| `worker/src/handlers/scheduled/context.ts` | `dwellirEnablementByChainRpcs` | Scheduled-runtime Dwellir enablement (provider trial) | WeakMap keyed by each scheduled runtime's chainRpcs Map; memoizes one budget + circuit read per runtime and dies with that map object or the isolate. | The Dwellir credit ledger row and the dwellir-evm circuit row are authoritative; the memo only prevents repeat reads within one runtime. |
...
One of those entries has a durable cross-isolate counterpart. `detailRefreshesInFlight` only deduplicates refreshes inside a single isolate, so concurrent refreshes of the same coin in different isolates are fenced through the `detail_cache_write_generations` table (`worker/src/lib/detail-cache-generation.ts`): each refresh bumps that coin's `generation` and takes a `claim_owner`, and the `detail:<id>` cache upsert only applies while that exact generation and owner are still current. A slower refresh that lost the race writes nothing and logs `stablecoin_detail.cache_write_superseded`; last claim wins, not last finisher, so a detail page that appears not to refresh may be a repeatedly superseded slow path rather than a failed write.

---

## Cron Scheduling

Fetch-heavy lanes are split across the expressions in `worker/wrangler.toml` so they do not compete with the quarter-hourly core pipeline for the trigger-wide fetch budget or share CPU budget with DB-only availability jobs. The DEX source lane is a single hourly `:10` physical expression, while the DEX publication lane retains the paired hourly `:16` + `:46` expressions for Cloudflare's hourly CPU class. `quarterHourly` (`0`, `15`, `30`, `45`), `v9SupplyAttributionOffset` (`8`, `23`, `38`, `53`), the run-scoped-heap-separated `depegResolverOffset` (`13`, `28`, `43`, `58`), `v9PublicationOffset` (`22`, `52`), `statusSelfCheckOffset` (`9`, `24`, `39`, `54`), `halfHourlyMintBurnCritical` (`4`, `34`), and `halfHourlyMintBurnExtended` (`18`, `48`) use the same hourly-alias form where their logical cadence remains sub-hourly. Expensive source work occurs only at `:10`, price publication at `:16`, and score publication at every hourly `:16`; `:46` only reuses the current generation for the dependent half-hour V9 preparation. `[limits].cpu_ms = 300000` remains the repo cap, while Cloudflare's [Workers limits](https://developers.cloudflare.com/workers/platform/limits/) give sub-hourly expressions a 30-second CPU class and hourly expressions a 15-minute class. Any further physical-trigger growth requires consolidation or an ADR-backed rebalance.

Cron expressions are source-owned in `worker/wrangler.toml`. The canonical logical schedule-key mapping, physical trigger aliases, status-tracked jobs, and connection-budget metadata live in `shared/lib/cron-jobs.ts`; dispatch chains live in `shared/lib/scheduled-runner-registry.ts`. Several logical lanes intentionally map multiple hourly physical expressions to one logical slot identity so their invocations use Cloudflare's hourly CPU class; the aliases rebalance existing logical work rather than adding scheduled work. `runScheduledSlotWithFence()` in `worker/src/lib/scheduled-slot-fence.ts` owns claim, heartbeat, takeover, and terminal ownership fencing. Stale child progress/lease cleanup, synthetic run persistence, attempt abandonment, and the operator event marker live behind the narrow `scheduled-slot-reconciliation.ts` internal stage. Abandonment child rows preserve bounded `progressSnapshot` evidence before cleanup; slot events expose the same evidence in shallow `abandonedProgress` entries. Validated counts, reserve coin/adapter identifiers and aggregate I/O/timing counters survive, while `metadataStatus` identifies missing, malformed or oversized input. Last-known progress is diagnostic context, not attribution of the interruption. The fence stores compact child-job summaries in `cron_slot_executions.metadata` (`jobsRun`, `jobsSkipped`, `jobsNeutralSkipped`, `jobsDegraded`, `jobsErrored`, `budgetOnlyJobs`, and per-job outcomes) and marks the slot `degraded` or `error` when children skip/degrade/fail even if best-effort execution lets later jobs continue. A stale child is `skipped_neutral` only when its child and slot heartbeats stopped together, the slot's Worker version differs from the reconciler's version (falling back to the version the dying invocation stamped into `cron_run_progress.metadata.workerVersion` when the slot row records none), and both the child's last progress write and the slot's own last heartbeat fall between 15 seconds before and 120 seconds after CI's write-once `worker-version-activated:<version>` timestamp, with activation no later than the sweep. Zero progress past the child's start is not required: a deploy evicts a child at any point in its run. The marker timestamp comes from Cloudflare deployment `created_on`, never CI wall time. The scheduled first-seen marker remains diagnostic evidence, retained alongside an out-of-window activation reading; a missing or out-of-window activation marker stays `error`/`abandoned`. Neutral expected no-op children, such as an idle `digestTriggerPoll` with no force-run request, are counted separately and never synthesize a missing `daily-digest` failure; durable started digest progress is still reconciled if its owner is abandoned. A job normally owns exactly one schedule key. `SHARED_SCHEDULED_JOB_IDENTITIES` in `shared/lib/scheduled-runner-registry.ts` is the closed allowlist of jobs that intentionally run from more than one scheduled slot (`cron-sentinel` runs from four) under one `cron_runs.job` identity; a job dispatched from a slot it has neither a definition for nor an allowlist entry in makes the descriptor build throw, so anything not listed must use a distinct job name. Sharing an identity also shares one lease, one status row, and one freshness reading, so `/api/status` reports such a job against its registered interval rather than against any single slot's cadence. Run `npm run check:cron-sync` and `npm run check:cron-connections` after schedule or job-chain changes.

`cron-sentinel` is the shared control-plane identity for the quarter-hour status lane and daily 03:00 lane. Its declarative rule table records each legacy condition's source, severity, cooldown, and sustained window. The status invocation evaluates freshness and digest publication; the daily invocation evaluates duration. DEX turnover and reserve post-sync remain producer-adjacent, while mint/burn growth and repair debt retain compatibility job rows consumed by existing health/status readers.

Scheduled consumers that need only lifecycle, identity, contract, or price-sanity metadata use `shared/lib/stablecoins/worker-runtime-registry.ts`, never the evidence-rich full registry. This includes daily housekeeping, blacklist, DEX discovery, detail-cache prune, supply snapshot, Telegram, and extended mint/burn. Daily housekeeping uses its own daily-only cron-sentinel entrypoint, so it does not initialize reserve-post-sync code. `npm run check:runtime-reachability` bundles these memory-constrained entrypoints and rejects paths back to the full registry. The quarter-hourly sync still legitimately consumes evidence-rich metadata elsewhere in its graph, so its entrypoint is not falsely classified as registry-free.

Cloudflare invocation analytics attributed the 2026-09-03 aligned `:00` failures to `exceededMemory`: the quarter-hourly core, active measured-execution lane, and daily housekeeping were invoked together while the service's 128 MB isolate was already near its limit. Their logical slot identities remain unchanged, but physical dispatch is staggered: measured execution uses paired hourly `5 * * * *` and `35 * * * *` triggers for logical `:00/:30` slots, and daily housekeeping uses `3 3 * * *` for its logical 03:00 slot. The P2-09 topology review raises the physical-expression gate from 40 to 41 because the RPC-heaviest measured-execution graph must receive Cloudflare's hourly CPU class; the split adds no logical work, fetch-capable entry, or connection pressure. Warm-isolate reuse still means the first production executions are required acceptance evidence.

Cron sentinel source state is persisted independently in the existing cache table under `cron-sentinel:source:<source>`. Every sentinel mode returns the worst latest evaluated source status across freshness, digest publication, growth, duration, repair debt, turnover, and reserve post-sync. A healthy run in another mode cannot erase a current daily or reserve warning; only a newer evaluation of that same source clears it. Neutral/locked skips and cancellation retain the prior state, source exceptions persist an error, and monotonic cache writes reject older completions. Retained state stops contributing after two producer intervals, capped at 48 hours, so an abandoned source cannot degrade every mode forever. Metadata includes each current source's own `observedAt`, so aggregation does not make old evidence appear newly measured. Missing source cache state is seeded from its latest evaluated source entry in retained `cron_runs` history, subject to the same age bound.
...
The Workflow runs four replay-safe steps: load the fixed-input identity, compile through the existing publication runner against a D1 facade that permits reads but captures and suppresses canonical cache writes through every D1 terminal method, gate the captured result against the loaded generation, then atomically write `safety-score-v9:shadow:<generation>` plus a `cron_runs`-compatible terminal row for `compute-safety-score-v9-workflow`. The facade rejects DDL, pragmas, non-cache writes, and malformed cache-write bindings before they can reach D1. That 30-minute producer is registered in `CRON_JOB_DEFINITIONS` with watch status impact and the compiler's zero-outbound-fetch connection budget, so status freshness and connection accounting include every shadow run. Each step has three exponential-backoff retries and a 14-minute per-attempt timeout; compiler or admission failures are recorded as error terminal rows and returned as Workflow errors without changing the authoritative cron publication.

Shadow retention is generation-bounded: each committed publication writes `safety-score-v9:shadow:<generation>` only when no newer shadow exists, then deletes sibling keys no newer than its own generation in the same D1 batch. Exactly one newest shadow envelope remains for the pilot's latest-versus-current comparison, even when an older Workflow finishes later. The terminal `cron_runs` row is derived from that same admission result inside the batch: a generation the monotonic guard rejects commits neither the shadow row nor a non-error terminal row, so the instance's own failure handler records the conflict as its single `status='error'` row instead of leaving durable history reporting a rejected publication as ok or degraded.

Run shadow for approximately seven days before any cutover decision. For every slot, compare the cron and shadow publication identity/digest, confirm one effective instance per deterministic ID, inspect per-step CPU/duration and retry history, verify no outbound-fetch admission, confirm the terminal row remains visible to the status oracle, and calculate active-CPU/storage cost. Also compare Cloudflare `exceededMemory` outcomes for the quarter-hourly and chart lanes against the ADR-26 baseline. Cutover remains a separate documented go decision on all six ADR-26 parity questions; until then the cron fence, leases, memory-lane gate, `22`/`52` expressions, and live writer remain authoritative.

### Deployed Trigger Topology

Do not maintain a second schedule table in this document:

`worker/wrangler.toml` owns the deployed expressions and `CRON_GROWTH_HEADROOM_POLICY` (`shared/lib/cron-jobs.ts`) owns the reviewed growth gate; run `npm run check:cron-sync` / `npm run check:cron-connections` for the live topology instead of reading a count from this document. The hourly aliases for `quarterHourly`, `v9SupplyAttributionOffset`, `depegResolverOffset`, `v9PublicationOffset`, `statusSelfCheckOffset`, and both mint/burn lanes, together with the existing DEX pairs, intentionally map multiple physical expressions to one logical slot identity for Cloudflare's hourly CPU class. DDR is the one deliberate new logical slot: it prevents the supply-attribution and DDR run-scoped graphs from sharing one invocation while retaining zero outbound connections; the same service may still reuse a warm isolate.
...
The four-hour reserve slot uses two chains: `sync-live-reserves → sync-redemption-backstops → cron-sentinel` and `sync-kinesis-supply`. Both consumers of the generation the head writes stay ordered behind it — the backstop computation reads the live-reserve snapshot metadata, and the post-sync watchdog publishes a reserve-drift envelope stamped with the current time, so a watchdog launched beside the producer would observe and re-publish the previous generation before the slot's rows landed. Kinesis supply reads no reserve output and still runs beside the long-running head. A head the slot fence abandons delays the watchdog rather than losing it: the five-minute `reserve-recovery` lane replays the abandoned checkpoint in chain order, and the children whose producer never completed are recorded as not-started with `upstream-incomplete:sync-live-reserves` instead of refreshing the envelope from stale rows. The trigger's declared peak drops to `3/6`.

The hourly yield slot (`hourlyYieldSync`) also runs two chains: the publication chain `sync-yield-supplemental → fetch-tbill-rate → sync-yield-data`, and beside it the Dwellir trial's observation lane `observe-rpc-provider-parity`. The second chain is independent — it reads no yield output, so it neither delays the publication nor inherits its stop-on-failure — and it is strictly serial (`maxConnections: 1`, one request in flight, one chain at a time) under a four-minute internal deadline inside its default five-minute wrapper, so it contributes exactly one steady-state outbound connection to the slot. Per chain, the probe compares Dwellir with that chain's first registry operator or reviewed public pin: head lag, `totalSupply()` read at a common block, a short address-filtered log window, and a pruned-log probe for chains whose Dwellir `logsHistory` is `none`. It is the health probe for the `dwellir-evm` circuit, so it re-tests the provider even while that circuit is open, and it is watch-tier with `freshnessSurface: none` — a parity finding is evidence for the provider decision, never a public data surface, so it is not availability- or public-impacting and is surfaced only through the operator-only `GET /api/rpc-provider-trial` report.

The daily `snapshot-psi` and `snapshot-public-dataset` jobs keep their preferred 08:00 UTC chain and also have a same-day catch-up at the cache-safe tail of every quarter-hourly core slot. The catch-up defers to the 08:00 attempt: slots whose `slotStartedAt` precedes the day's 08:00 UTC record `before_daily_slot` as a neutral no-op, so a midnight slot can never claim the write-once public snapshot with a stale cache. After 08:00 each catch-up checks the target UTC-day row, records `same_day_snapshot_exists` as a neutral no-op when present, and invokes the idempotent writer only for a hole; the public-dataset catch-up carries the same `daily0800Utc` stablecoins-cache freshness minimum as the 08:00 chain. A successful repair records `metadata.reason = "same_day_catch_up"`; PSI replaces its midnight-keyed row atomically and the public dataset remains write-once by date. These not-due checks run before the stablecoins-cache capability gate: only a catch-up that still has a hole to fill records the degraded `stablecoins-cache-unsafe` skip, so an unsafe core slot cannot misreport an already-completed day. The existence reads themselves — the scheduled not-due checks and the writer's own write-once precheck (`loadExistingSnapshot`, read before the fill and again when the fenced insert loses a race) — run through `runWithOverloadRetry()`, so a transient D1 overload retries instead of recording an error run; a read that still fails is recorded as the job's error because it proves nothing about the day (R2). Because `same_day_snapshot_exists` is written only after that read succeeded and found the row, cron health treats a fresh such skip as proven-satisfied evidence that the period's artifact exists and lets availability recover past an earlier transient error (see [Cron health model](./status-dashboard.md#cron-health-model)).

Safety Score V9 supply attribution uses its dedicated `+8` trigger. DDR uses the independent D1-only `depegResolverOffset` trigger five minutes later, reads the latest `sync-stablecoins` capability metadata, and runs under the same core-slot/publication and V9 memory-lane admission fence in a separate invocation. A warm service isolate may be reused, but the two large run-scoped graphs no longer coexist within one invocation; observed clean production executions remain required. Exact V9 input preparation runs after each `16,46` DEX consumer result in the same serial half-hourly chain and is passed the exact current DEX generation ID; `:46` obtains that identity without rewriting DEX surfaces, and a `:16` DEX error recovers the last accepted exact publication while it remains inside the reviewed four-hour DEX evidence budget (`DEX_LIQUIDITY_EVIDENCE_MAX_AGE_SEC`, recorded as `dexPublicationRecovery`), failing closed with the refusal in `upstreamRecovery` beyond it or when no single consistent accepted generation can be read. After building that input, preparation observes the reviewed transfer-materiality cohort at blocks no later than the input clock with at most three chains in parallel, then writes the input, peg-provenance seed, and successful transfer-materiality generation in one D1 batch. This keeps the transfer observation bound to the exact base-input generation consumed by the later `22,52` compiler instead of binding it to the prior half-hour input available to the earlier `+8` supply-attribution trigger. Per-chain RPC failures become explicit rejected observations. An unexpected observer-level failure reports the preparation as degraded but preserves report-card availability by writing the fixed input and peg seed without replacing the prior transfer cache; the compiler rejects that stale row on its exact base-input or registry identity and emits bounded-unknown transfer facts. Genuine cancellation still aborts before the D1 batch. The preparation step rejects a DEX generation mismatch, while compilation fences the fixed input against the latest accepted DEX generation both before enrichment and immediately before evaluation. Before capture, preparation waits up to 3 minutes when an active `sync-stablecoins` lease/progress row is newer than the published `stablecoins` cache; if that producer remains pending, preparation degrades without replacing the fixed input. Compilation admits an `ok` quarter-hour core slot from the current Worker directly, and otherwise requires that slot's `stablecoins` generation joined to the current cache in `surface_publication_generations` — the publishing Worker version need not still be current, so a deploy landing between the core slot and the trigger cannot deny a slot whose publication is still the live cache generation. The compiler separately rejects a fixed input whose stablecoin timestamp no longer matches the live cache generation. Thus an isolated active-row coverage gap can proceed to asset-local V9 handling, while stale/no-write stablecoin runs remain fail-closed. Since a distinct expression alone cannot prevent overlap in one Worker service, V9 lanes retain the D1-backed memory-lane lease: a prior active invocation of the same V9 schedule lane remains fail-closed, while unrelated scheduled slots cannot suppress canonical publication and later scheduled invocations wait at the lightweight dispatcher boundary before loading their runner graphs. The publication lane requests three minutes so its existing two-minute end-to-end compiler timeout can settle, but `runV9AfterCoreWithinWindow()` mechanically clamps every V9 lane to the next quarter-hour boundary; the core/version/input/publication fences are unchanged. Delayed or competing V9 work skips neutrally; a complete same-fixed-input supply-attribution generation that finished just after the scoring clock also defers neutrally instead of publishing an aggregate-only partial candidate. Producer due intervals are shortened below the consumer acceptance window so the 15-minute trigger grid captures healthy generations roughly every 30 minutes instead of drifting into a 45-minute beat; stale or identity-incompatible generations report clause-specific reasons in cron metadata. XAUT `transparency-stale` is a bounded diagnostic producer rejection when the expected inventory was observed and the aggregate-only fallback was journaled; other rejected assets remain blocking for producer health. Missing Worker-version metadata degrades. Solana account snapshots resolve the newest produced finalized block at or before the reported bank context within a bounded 64-slot window, so skipped context slots do not become false `deployment-state-unavailable` rejections; failure to resolve that anchor remains fail-closed. The canonical compiler receives publication-exact peg provenance and transfer materiality from the fixed-input producer's atomic D1 batch rather than reconstructing either from mutable state.

Reviewed Solana deployment reads prefer the configured keyed Alchemy/dRPC roster, then fail over across the two public cluster aliases, Pocket Network's public Solana gateway, and PublicNode. Each RPC method retains bounded sequential failover, so the additional fallbacks do not increase the trigger's peak outbound connection count.

The same atomic preparation batch includes `safety-score-v9:supply-attribution-source:v1`, a compact projection of the exact input's base identity, source generation, registry fingerprint, scoring clock, active attribution cohort, and cohort-only aggregate/chain supply rows. The `+8` producer parses this projection rather than the full compiler input. Missing, malformed, or older-than-30-minute projections fail closed before observer modules perform RPC work. Canonical publication still consumes and validates the full exact input; its already-normalized cache object is passed into the publication runner without a second whole-input normalization, and base immutability is checked through the normalized content-addressed base generation plus source generation instead of allocating two whole-input JSON strings. The publication gate inflates and validates the prior accepted response before compiling the replacement, immediately projects only identity, deterioration bindings, route/capacity deltas, and pillar scores, and releases the full prior graph. Held-state persistence validates that compact identity against the uncompressed storage envelope, so neither accepted nor held paths recreate a second full publication graph while the candidate is resident.

V9 publication admission is gated solely by quarter-hourly core-slot evidence through `runV9AfterCoreWithinWindow()` on the `v9PublicationOffset` lane; sibling lanes, including DDR on `depegResolverOffset` and `sync-v9-supply-attribution` on `v9SupplyAttributionOffset`, are not admission gates and cannot suppress publication. Holds come only from the publication assessment's own reason codes (coverage floors, `dex`/`redemption`/`liveReserves` input health, quarantines, or assessment failure); sibling failures can still affect the assessment indirectly by degrading its inputs.

### Cron Slot Capacity and Connection Pool Budget

The platform header-wait limit and Pharos's stricter trigger-wide budget are stated once in [Worker and API Limits: Connection-budget operating assumption](./worker-and-api-limits.md#connection-budget-operating-assumption). What follows is how that budget is measured and applied per slot.
...
The `halfHourlyChartsOffset` trigger peaks at `3/6`: DEX consumption, the D1-only exit-route turnover watchdog, V9 input preparation, and the failure-independent chart writer run serially, so transfer-materiality observation is the peak child. Moving the existing observation into that exact-input stage adds no trigger; the D1-only duration source now runs inside `cron-sentinel` with zero outbound connections.
...
- `daily-digest` freshness-gates the current Safety Score map through the Pages manifest and dated PNG routes. Telegram persists it as a large link preview inside the immutable digest payload; X uploads and attaches the PNG before creating the post (one bounded upload retry, then the tweet aborts). The map is a required attachment: an unready map defers the whole edition through the `digest:safety-map-deferral` intent, and the digest-trigger poll retries until the map is live. A date rollover with the intent still pending means that day's digest deliberately stayed unsent.
...
- Expiring operator pauses live in `telegram_delivery_pauses` with independent `fresh`, `pending`, and `admin` generations. The state helpers generation-fence pause/resume and conditionally write the permanent operator audit, while webhook user replies stay outside the admin-delivery pause. The Access-protected control route is retired, so the current runbook permits read-only inspection and requires a reviewed audited control/script before any new manual pause or resume.
- Each dispatch run sends up to 3,600 Telegram message attempts in parallel batches of 4 under a 4.5-minute hard timeout; pending-drain and fresh-send batches stop after a 4-minute soft deadline. Existing due rows drain before authoritative source-specific candidate capture and target planning. Candidate pages union relevant direct subscriptions, resolved preset targets, and global-family flags; capture and planning reuse page fan-out inputs only while preference generations match. Target pages pack complete idempotent plan units into bounded D1 transactions, then hand off bounded target pages with set-based suppression/enqueue/state changes and one grouped confirmation instead of per-target database loops. When a backlog exists, newly planned targets remain queued for the next five-minute run. Unattempted pending claims are released, and untouched fresh tails are queued. Overflow and retryable fresh-send failures are enqueued to claim-based `telegram_pending_alerts` rows for subsequent runs.
...
async function logCronRun(
  ...
  options?: CronRunLoggerOptions, // { slotStartedAt?, timeoutBudget?, abortSignal?, producer? }
...
- Exposes a `reportProgress(...)` callback; for leased jobs, wrapper-owned milestones (`started`, `lease-acquired`, `completed`, timeout/skip states when applicable) are emitted only after the job lease has been acquired, before any cron-specific progress stages
...
- `worker_producer_history` and `worker_producer_heads` distinguish invocation completion from productive output and publication for every schedule/job/path/kind, including shared paths and budget-only surfaces. Calendar work keeps its UTC-month identity. History rows retain compact scalar metadata, outcomes, and errors only; rich nested metadata remains in the 7-day `cron_runs` drill-down, and latest publication detail remains on the producer head and generation ledger.
...
Long-running leased jobs can now surface active progress through `cron_run_progress`, which powers `/api/status` while the run is still live. The slot-fence heartbeat runs every 60 seconds for every slot policy; the five-minute stale-heartbeat window (six minutes for long slots) and the 16-minute wall-clock dead-line remain unchanged. Heartbeat writes retry transient D1 overloads inline, and a write that has not settled within one heartbeat period — a request queued behind a sustained D1 overload rather than fast-failing — no longer suppresses later ticks: the fence starts a replacement attempt (an idempotent CAS on owner/generation/state), so a live isolate keeps its slot row fresh and is not falsely reconciled as platform-abandoned; a late-landing attempt can stamp at most one heartbeat period in the past, which the stale window always absorbs. Long-running child leases retain their separate 30-second renewal cadence and tolerate at most three consecutive renewal failures before the wrapper aborts controlled work. Progress UPSERTs preserve a row backed by another still-active lease, and terminal cleanup is fenced to the reporting invocation's start and slot identity. The status handler cross-checks that progress row against an active matching `cron_leases` entry before surfacing it as in-flight, so an expired lease converts its progress row into a stale artifact instead of a phantom runner.

Progress writes are coalesced in memory: a stage transition always writes, while a same-stage update within 10 seconds of the last write is skipped. Known short-lived control-plane and maintenance jobs skip `cron_run_progress` entirely; the skip set is explicit in `worker/src/lib/cron-timeouts.ts` (including DB-only PSI/DEWS/Tape, fence reconciliation, supply snapshots, pruning jobs, and watchdog sources), rather than inferred from timeout overrides because those overrides are wall-clock safety caps, not p95 measurements.
...
Progress-reporting leased jobs emit wrapper-owned progress milestones; known short-lived jobs in the explicit skip set do not create an in-flight row. Current producers with cron-specific detailed stages include:
...
The per-job wrapper-timeout roster — every job's ceiling plus its per-job rationale and internal-budget notes — is owned by [`Per-Job Cron Timeouts` in `worker-and-api-limits.md`](./worker-and-api-limits.md#per-job-cron-timeouts) and is not repeated here. Configuration is derived in `worker/src/lib/cron-timeouts.ts` from `CRON_JOB_DEFINITIONS` plus `CRON_TIMEOUT_OVERRIDES_MS`; jobs without an override use the 5-minute default, and the scheduled-runner contract test fails when a new status-tracked job would fall back implicitly.

### Circuit Breakers

Most high-risk external integrations are protected by per-source circuit breakers (`worker/src/lib/circuit-breaker.ts`). State is persisted in the D1 `cache` table under keys like `circuit:defillama-stablecoins`. Breaker writes also maintain the aggregate `cache["provider:circuit:index"]` row as best-effort telemetry; individual `circuit:<source>` rows remain the execution and `/api/status.providerCircuitHealth` source of truth. Persisted records are admitted through the shared `CircuitRecordSchema` (`shared/types/status/public-health.ts`): a malformed or nonfinite-numeric record is ignored and the source resets to the closed default instead of poisoning execution, while a valid record retains unknown additive cache fields for forward compatibility. The `POST /api/reset-circuit-breaker` endpoint was retired on 2026-08-09; clearing a breaker is now a scoped delete of its `circuit:<source>` row, documented in [`lease-and-breaker-recovery.md`](./runbooks/lease-and-breaker-recovery.md). Bounded low-volume fallbacks such as gold-api.com metal spot quotes, the secondary FX mirror, and ExchangeRate-API daily reference snapshots use explicit retry/timeout/cooldown behavior but are not currently circuit-gated.
...
- Source-specific semantics: `aznd-curve-pool` fails its circuit on thrown provider work while guarded no-quote results stay neutral. `vaults-fyi` is disabled by default in code (`worker/src/lib/env.ts`), but the checked-in deployment sets `VAULTS_FYI_ENABLED="true"` (`worker/wrangler.toml`) so production enables it when the API key is present; `uniswap-v3-bsc-shadow` is target-only staging recovery; some live-reserve adapters opt into source-invariant within-run result sharing behind their scoped key.
...
- DexScreener discovery records aggregate `dexscreener-liquidity` success when at least one request in the run succeeds. A later HTTP 429 or WAF 1015 stops remaining requests but does not convert a partially successful run into a breaker failure; a zero-success refusal still records failure.
...
| `worker-version-activated:<version>`                       | Deploy workflow                  | Write-once JSON activation evidence keyed by the Cloudflare Worker version ID; `updated_at` is the matched Cloudflare deployment's `created_on` epoch, not CI wall time.                                                                                                                                                   |
| `worker-version-first-seen:<version>`                      | Scheduled Worker                 | Write-once first scheduled-execution diagnostic for a Worker version; it is not an authoritative deploy boundary.                                                                                                                                                                                                         |
...
| `safety-score-v9:transfer-materiality-generation:v1`       | `prepareSafetyScoreV9Input`      | Private raw-unit deployment observations bound to the same base-input generation, registry fingerprint, and scoring clock as the fixed input; unsupported or incomplete deployment inventories remain fail-closed. |
...
| `safety-score-v9:supply-attribution-source:v1`              | `prepareSafetyScoreV9Input`      | Compact identity-linked supply-attribution source written atomically with the exact input; the `+8` producer rejects missing, malformed, or stale rows before RPC fan-out. |
...
| `detail:<id>`                                              | stablecoin detail API            | Cached detail-page body per stablecoin; served for 24h, then refreshed synchronously on access. The daily `prune-detail-cache` job is its only deletion path (see [Append-only D1 Retention Policy](#append-only-d1-retention-policy))                                                                                     |
| `detail-write-failure:<id>`                                | stablecoin detail API            | Marker written when a `detail:<id>` cache write fails or is oversized; the staleness watchdog degrades on markers fresher than 24h and prunes them after 7-day retention                                                                                                                                                 |
...
| `cron-staleness-watchdog:producer-state:v1`                | `cron-sentinel` freshness source | Last observed `ok` / `stale` state for each registry-derived consumer-data producer; used to emit alerts only on state transitions                                                                                                                                                                                        |
| `cron-staleness-watchdog:alert:direct:v1`                  | `cron-sentinel` freshness source | Successful consolidated operator-alert delivery timestamp used for the 30-minute flap cooldown; delivery targets `TELEGRAM_OPERATOR_CHAT_ID` and is skipped entirely when that binding is unset                                                                                                                            |
| `ddr:quarantine-alerted-event-ids`                         | `computeDepegResolver`           | Permanent ledger of depeg event ids that already raised the repair-required quarantine alert; each id alerts at most once, so clearing the row is the only way to re-arm it                                                                                                                                               |
| `dex-exit-route-turnover-watchdog:snapshot:v1`             | `cron-sentinel` turnover source | Compact per-coin `routeId -> evidenceKind` baselines (held at the last confirmed set while a divergence is unconfirmed) plus per-coin `candidates` for divergences seen in exactly one published generation, used to detect substantial sustained published-route turnover across missed producer cycles; payloads written before the sustain window existed parse as candidate-free baselines. Carries `pendingAlert` for a confirmed alert (the per-coin baseline advances in the confirming run) and is cleared only by a later run that observes no alerting coin; an unreadable value is replaced with a fresh baseline instead of wedging the watchdog                                                                                                                                               |
| `status-self-check:bootstrap-observed:<producer>`          | `runStatusSelfCheck`             | Permanent marker written the first time a bootstrap-tracked producer is observed to have run. `cron_runs` retention is one week, so a zero run count cannot distinguish "never bootstrapped" from "dead for a week"; once this marker exists the producer's 503 probes fail                                                                                                                                                |
...
| `setCacheIfAbsent(db, key, value, updatedAt)`   | Write-once insert using `ON CONFLICT DO NOTHING`; preserves the first durable timestamp for deployment/diagnostic markers.   |
...
async function batchExecute(
  ...
  optionsOrChunkSize: number | { chunkSize?: number; signal?: AbortSignal } = 100, // D1_BATCH_SIZE
): Promise<number>;
```

Chunks statements into batches of 100 (Pharos's conservative `D1_BATCH_SIZE` cap), executes sequentially, and returns the summed D1 `meta.changes` count across all batched statements. This is distinct from D1's 100-bound-parameter limit per query; the platform does not document 100 as a statement-count ceiling for `batch()`.
...
This means duplicate trigger deliveries for the same slot are skipped before shared-slot fan-out can reorder downstream jobs, while individual jobs inside the accepted slot still use their existing per-job leases. The slot fence also races controlled-deadline expiry against the runner promise: cooperative jobs still receive the abort signal, and a non-cooperative or stuck runner is finalized as an error before the platform has to reconcile it as an abandoned slot.

Fence admission pre-sweeps stale prior `cron_slot_executions` rows for the same schedule key, and a same-slot takeover performs the same reconciliation before new work starts. The five-minute reserve-recovery lane additionally retains a global sweep. A `running` row older than the 15-minute scheduled-event wall clock plus a minute of skew is treated as provably dead regardless of its heartbeat column, and a child lease whose heartbeat has been silent for five minutes no longer shields its slot even while the lease TTL is unexpired. Before touching child artifacts, reconciliation atomically transitions the exact stale owner/generation/heartbeat row from `running` to `reconciling`; a concurrent heartbeat makes that claim change zero rows and leaves all child state untouched. If the claiming process dies, a later admission can reclaim the stale `reconciling` row with the same owner/generation/heartbeat compare-and-swap and a new generation, while a fresh reconciliation claim remains protected for the full stale window. Takeovers increment `execution_generation`, and heartbeat/finalization updates require the current owner, generation, and state. A zero-change heartbeat aborts controlled work, while a late original finalizer cannot overwrite the takeover or synthetic error. Reconciliation writes idempotent synthetic child `cron_runs` evidence and the compact abandonment event marker. This keeps platform-killed scheduled events from remaining operationally `running` or `reconciling` without letting a stale observer corrupt live artifacts.
...
Supplemental yield publication is family-owned: the eight `yield:supplemental-sources:v1:<family>` rows are written independently, and a successful family that resolves to zero deduplicated candidates may publish an explicit empty row to clear a previous non-empty family snapshot. Failed or malformed families leave their prior row untouched; fresh valid family-row presence is the loader's availability signal, including when every current family row is explicitly empty. The legacy aggregate `yield:supplemental-sources:v1` row is retained for coordinated cleanup but is no longer written, read, or used as a fallback.
A per-run outcome row, `yield:supplemental-source-run:v1`, records each family's write result (`published`, `skipped-newer`, `empty`, `empty-published`, or `retained-previous`) plus the `degradedFamilies` list. A family whose upstream fetch failed mid-run skips its cache write so the previous snapshot is retained (`retained-previous`) and is named in that outcome row; the post-V9 publisher surfaces those families through `supplementalMeta.degradedFamilies` instead of letting a fresh empty write mask a partial outage. The supplemental cron also reports `degraded` whenever any family is retained after failure, while keeping its successful write count.

Health freshness checks for mint/burn major symbols and scheduler stale alerts use the same shared resolver in `worker/src/lib/mint-burn-health-config.ts`, including env overrides (`MINT_BURN_MAJOR_SYMBOLS`, `MINT_BURN_STALE_WARN_SEC`, `MINT_BURN_STALE_CRIT_SEC`, `MINT_BURN_ALERT_COOLDOWN_SEC`). The public `/api/health` status itself now follows critical-lane sync freshness (`lastSuccessfulSyncAt` + latest run status) rather than raw event recency, so quiet majors do not produce false stale health. The critical producer prunes safely valued, aggregated, and Tape-projected event rows after 8 days and hourly aggregates after 95 days in bounded oldest-first batches; retention errors degrade the producer and remain visible in its cron metadata. The advisory `mintBurn.totalEvents` value comes from the growth source in the latest daily `cron-sentinel` `cron_runs` result instead of scanning `mint_burn_events`/`mint_burn_hourly` or reading `sqlite_sequence`; rollout can fall back to the retired `mint-burn-growth-watchdog` row. The unchanged 2.3M-row threshold remains a fail-safe for non-converging cleanup or unexpectedly large protected debt.
...
Public query feeds preserve dependency-read failures separately from healthy absence. `/api/peg-summary` still serves its usable peg rows when the optional `dex_prices` read fails, but adds `degradedReason: "dex-prices-read-failed"`, a `Warning` header, and `Cache-Control: no-store`; it never publishes that failure as an ordinary empty DEX observation. `/api/safety-score-history-v2` validates each persisted history row independently, omits malformed rows, and reports `malformedRows` without allowing one bad row to fail the remaining feed; if persisted rows exist but none validate, the endpoint returns `503`.

### GET /api/status

The mint/burn integrity section consumes per-contract raw-conservation evidence produced by the existing mint/burn lanes. It performs bounded D1 cache reads, not archive RPC work, and distinguishes verified mismatches from unverified source or audit conditions. Matched block ranges and integer residuals own the integrity verdict; the legacy circulating-supply difference remains contextual. See [Mint/Burn Integrity](./status-dashboard.md#mintburn-reconciliation-card).
...
The serial `data-invariant-canary` evaluates freshness against its actual execution clock, clamped no earlier than the scheduled slot time. The scheduled clock remains attached to cron/slot telemetry, but a producer that legitimately publishes while earlier status tasks are still running is not misclassified as a future pointer. Its blacklist identity completeness check is the durable regression guard for the completed 2026-08-26 legacy identity repair.
...
Operational Telegram booleans parsed from cron metadata are tri-state. Missing or malformed suppression flags remain `null` in the status supplement rather than becoming `false`. The `priceSourceHealth` supplement is schema-validated before publication; malformed metadata yields `priceSourceHealth: null` plus `sectionErrors.priceSourceHealth`, while valid metadata can still be enriched with the independently loaded source-depth distribution.
...
| ------------------------------- | ----------------------------------- | ------------------------------------- |
| `D1_BATCH_SIZE`                 | 100                                 | Pharos statements-per-batch cap       |
...
Owner risk acceptance (2026-09-12): the broad `api-rate-limit-ip` WAF rule remains intentionally disabled and the drift manifest continues to reflect that setting. Broad API floods can therefore reach Worker ingress; route-specific gates and per-key controls do not replace an edge-wide volumetric limit. The free Safety Grades route has a separate native Worker limiter.
