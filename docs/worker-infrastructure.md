...
`worker/wrangler.toml` currently deploys 35 physical expressions. The hourly aliases for `quarterHourly`, `v9SupplyAttributionOffset`, `v9PublicationOffset`, and `statusSelfCheckOffset`, together with the existing DEX pairs, intentionally map multiple physical expressions to one logical slot identity for Cloudflare's hourly CPU class.
...
| `AZND_CURVE_POOL`                    | `aznd-curve-pool`             | `enrich-prices` exact thin AZND Curve route; thrown provider work fails the circuit, guarded no-quote results stay neutral |
...
| `UNISWAP_V3_BSC_SHADOW`              | `uniswap-v3-bsc-shadow`       | `sync-dex-liquidity-stage` target-only BSC Uniswap V3 staging recovery                                                    |
