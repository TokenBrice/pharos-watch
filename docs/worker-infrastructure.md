...
| `worker/src/lib/chain-registry.ts` | `ALCHEMY_AUTHORIZATION_BY_URL` | Alchemy RPC bearer-auth routing | Repopulated on every buildAlchemyRpcUrl call for the configured key; resets with the isolate. | Env ALCHEMY_API_KEY is authoritative; the map only pairs key-free URLs with their Authorization header. |
