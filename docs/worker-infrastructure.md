...
The daily `snapshot-psi` and `snapshot-public-dataset` jobs keep their preferred 08:00 UTC chain and also have a same-day catch-up at the cache-safe tail of every quarter-hourly core slot. Each catch-up first checks the target UTC-day row, records `same_day_snapshot_exists` as a neutral no-op when present, and invokes the idempotent writer only for a hole. A successful repair records `metadata.reason = "same_day_catch_up"`; PSI replaces its midnight-keyed row atomically and the public dataset remains write-once by date.
