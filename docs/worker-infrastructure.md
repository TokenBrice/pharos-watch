...
The generation-fenced D1 target and quote tables are additive. The measured lane consumes the prior scoring generation. `sync-dex-liquidity-stage` then loads sources and builds the exact pool graph at `10,40`, storing it one bounded chunk per D1 batch in `dex_liquidity_scoring_stages` / `dex_liquidity_scoring_stage_chunks` and reporting cumulative chunk, record, and byte progress after every persisted batch. The D1-only `sync-dex-liquidity` consumer at `16,46` prefers that source slot and otherwise consumes the newest still-ready generation from the preceding 55 minutes, consumes the prior measured generation, publishes scoring surfaces and the next target inventory, and runs before charts in the same serial chain.
