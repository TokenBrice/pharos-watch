...
The safety-grade snapshot reads the immutable V8 full artifact named by the active Safety Score publication manifest and dual-writes organic rows to legacy `safety_grade_history` plus `safety_score_history_v2`. V2 preserves model/methodology/policy/evaluation identities, input and publication generations, publication epoch, and transition kind. The current API and Tape project V2 and unmatched legacy rows through compatibility views; methodology-boundary, rollback, and restoration baselines are not treated as organic movements. V9 activation remains blocked until a separate authorized cutover writer can insert one explicit non-comparable V9 baseline per active asset.
