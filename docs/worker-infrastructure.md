...
The `halfHourlyChartsOffset` trigger peaks at `3/6`: DEX consumption, the D1-only exit-route turnover watchdog, V9 input preparation, and the failure-independent chart writer run serially, so transfer-materiality observation is the peak child. Moving the existing observation into that exact-input stage adds no trigger and keeps the reviewed fetch-capable-entry count unchanged by correcting the D1-only `cron-duration-watchdog` declaration to zero outbound connections.
...
| `cron-staleness-watchdog:producer-state:v1`                | `runCronStalenessWatchdog`       | Last observed `ok` / `stale` state for each registry-derived consumer-data producer; used to emit alerts only on state transitions                                                                                                                                                                                        |
| `cron-staleness-watchdog:alert:direct:v1`                  | `runCronStalenessWatchdog`       | Successful consolidated operator-alert delivery timestamp used for the 30-minute flap cooldown; delivery targets `TELEGRAM_OPERATOR_CHAT_ID` and is skipped entirely when that binding is unset                                                                                                                            |
...
| `dex-exit-route-turnover-watchdog:snapshot:v1`             | `runDexExitRouteTurnoverWatchdog` | Compact previous DEX publication generation and per-coin `routeId -> evidenceKind` sets used to detect substantial published-route turnover across missed producer cycles                                                                                                                                                |
