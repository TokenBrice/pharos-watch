...
Fresh, independently revalidated route identities with at least two successful cycles may remain in the bounded V9 route compiler for one evidence-freshness window after rotating out of the current pool shortlist; they never rejoin liquidity, pricing, display, or target-publication surfaces. The six owner-ratified Uniswap V3/PancakeSwap V3 deployment cohorts, the reviewed Base Aerodrome Slipstream cohort, and the reviewed Tron SunSwap V2 factory/pair/Smart Router profile are score-eligible. Optimism Uniswap V3, Fluid, Raydium CLMM, Orca Whirlpool, and every other unratified or paused cohort capture only in shadow and retain an `activation-pending` capability gate. Fluid's bounded target-only copy is hydrated from canonical tracked-contract metadata after direct-source compaction, and only exact pools on the pinned Ethereum, Arbitrum, Base, or Polygon resolver deployments can become shadow targets; unsupported chains and identity-poor discovery fingerprints fail closed. Native registries encode reviewed active/shadow policy explicitly. A valid native profile enters P4 capacity and physical-pool completeness only when that policy is active; shadow, stale, malformed, failed, or identity-drifted profiles remain gated, and non-EVM pool identities preserve case through selection and validation. SunSwap's first activation followed two complete production generations with 20 current direct profiles and the same multi-hop-only pool gated in each, but the first two active DEX-scoring invocations exceeded the Worker memory limit; the registry returned to shadow. The phase-split handoff now removes source/provider graphs from the scoring invocation, and SunSwap was reactivated only after six complete current 21/21 shadow generations plus healthy `16,46` consumers. No stale proof or manual capacity is substituted for a current direct route; the first active post-deploy consumers remain rollback evidence for memory, stale-heartbeat, completeness, or identity/proof drift. Degraded native quote diagnostics stay in the merged cron metadata without changing active EVM health; a native invocation error still fails the merged job, and a native cursor failure with deferred or rate-limited work remains degraded. The Solana producer binds native case-sensitive pool/mint identities and exact raw amounts to Raydium CLMM state or Jupiter context-slot proof inside a bounded RPC slot window. The Tron producer verifies canonical SunSwap factory/pair identities and exact constant-product output against one direct SUN Smart Router route; because TronGrid rejects numeric historical tags, its latest-only state reads must fit a bounded before/after block bracket. This lane does not make a deployment score-eligible by registry presence alone.
...
- `GET /api/report-cards/v9` is an additive activation-gated contract backed by the accepted `cache["report-cards:v9-shadow"]` envelope and `cache["report-cards:v9-shadow:publication-health"]`. It remains dark (`404`) until the identity-bound owner activation marker matches the canonical snapshot. The reader validates and projects the canonical V9 envelope into the owned public schema with complete model, policy, build, base-input, publication identity, and matching health. Missing, malformed, incomplete, or mismatched state returns `503`; this route has no V8 fallback and does not affect the unversioned V8 route. While held it serves the last accepted snapshot with `X-Safety-Score-Status: held`, accepted-timestamp freshness headers, and `Cache-Control: no-store`.
- `GET /api/report-cards/v9-preview` is the owner-approved read-only feedback exception. It reads the latest canonical validated projection, forces `lifecycle: "shadow"`, bypasses the Worker edge cache, returns `Cache-Control: no-store`, and never reads or changes the activation marker. The stable path and original opaque compatibility alias are explicitly exempt from the public API-key gate while remaining denied on the site-data lane. The former unlinked frontend preview page was retired after V9 activation; the compatibility API aliases remain read-only and expose no admin diff, history, review, or mutation data.
...
| `report-cards:v9-fixed-input:exact`                         | V9 shadow compiler               | Fully prepared V9 accepted input written atomically with the accepted envelope after the compiler validates the compact seed, matching V8 identity, publication health, coverage floors, and producer-failure assessment. Held attempts never replace it. |
| `report-cards:v9-shadow`                                   | V9 shadow sidecar                | Canonical checksum-verified accepted V9 envelope projected by the activation-gated `/api/report-cards/v9` route and the read-only feedback route; strict consumers require full identity/completeness and never fall back to V8. |
| `report-cards:v9-shadow:publication-health`                | V9 shadow sidecar                | Strict bounded current/held state with accepted generation/time, latest attempt time, first hold time, and stable hold reasons. The V9 publication clock provides ordering; equal-time conflicting writes fail. |
