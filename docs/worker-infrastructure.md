...
`worker/wrangler.toml` owns the deployed expressions and `CRON_GROWTH_HEADROOM_POLICY` (`shared/lib/cron-jobs.ts`) owns the reviewed growth gate; run `npm run check:cron-sync` / `npm run check:cron-connections` for the live topology instead of reading a count from this document. The hourly aliases for `quarterHourly`, `v9SupplyAttributionOffset`, `v9PublicationOffset`, and `statusSelfCheckOffset`, together with the existing DEX pairs, intentionally map multiple physical expressions to one logical slot identity for Cloudflare's hourly CPU class.
