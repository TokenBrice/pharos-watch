...
- `GET /api/report-cards/v9-preview-412d818c031b7bc5` is the owner-approved read-only feedback exception. It reads the same validated projection, forces `lifecycle: "shadow"`, uses the standard cache profile, and never reads or changes the activation marker. Its opaque companion page is unlinked and `noindex`; opacity is discoverability control, not authentication. The preview exposes no admin diff, history, review, or mutation data and does not select V9 for any active consumer.
