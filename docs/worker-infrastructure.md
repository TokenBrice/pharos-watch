...
The worker fingerprints method + path + sorted query + body for a given action key. Replays return the stored response with `X-Idempotent-Replay: true`; conflicting reuse returns `409` with `X-Idempotency-Conflict: request-mismatch` and cannot replace the original intent's audit. Reservations have an owner/generation and a separate durable execution-start transition. An unstarted reservation may be taken over after 20 minutes. A started reservation normally returns `503` without rerunning the mutation; if it remains `PENDING` for more than 24 hours, takeover first compare-and-swap records the abandoned attempt as `execution_unknown`, emits a structured warning, and only then reserves a new generation. Terminal writes remain owner/generation compare-and-swap operations. A thrown handler, an explicit unknown-certainty response, or any handler HTTP 5xx returns `503` with `error = "execution_unknown"` plus `X-Execution-Certainty: unknown`. Later requests with an explicitly stored `execution_unknown` key replay the same operator-reconciliation state without running the mutation again. Confirmed terminal responses expire after seven days; `PENDING` and `execution_unknown` rows are excluded from that prune, so uncertainty is never converted into an automatic re-execution by TTL expiry.
