...
Fetch-heavy lanes are split across the expressions in `worker/wrangler.toml` so they do not compete with the quarter-hourly core pipeline for Pharos's conservative six-connection-per-invocation fetch budget or share CPU budget with DB-only availability jobs. The DEX source and publication lanes retain paired hourly physical expressions (`10` + `40`, and `16` + `46`) for Cloudflare's hourly CPU class, but expensive source work occurs only at `:10`, price publication at `:16`, and score publication on even-hour `:16`. The `:40` path is neutral and `:46` only reuses the current generation for the dependent half-hour V9 preparation. `[limits].cpu_ms = 300000` remains the repo cap. Any expression added near the trigger soft cap requires consolidation or an ADR-backed rebalance.
