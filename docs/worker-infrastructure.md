...
Reviewed Solana deployment reads fail over across the two public cluster aliases, Pocket Network's public Solana gateway, and PublicNode. Each RPC method retains bounded sequential failover, so the added independent provider does not increase the trigger's peak outbound connection count.
