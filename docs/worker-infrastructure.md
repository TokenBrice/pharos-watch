...
Fetch-heavy lanes are split across the expressions in `worker/wrangler.toml` so they do not compete with the quarter-hourly core pipeline for the trigger-wide fetch budget or share CPU budget with DB-only availability jobs. The DEX source and publication lanes retain paired hourly physical expressions (`10` + `40`, and `16` + `46`) for Cloudflare's hourly CPU class, and `quarterHourly` (`0`, `15`, `30`, `45`), `v9SupplyAttributionOffset` (`8`, `23`, `38`, `53`), `v9PublicationOffset` (`22`, `52`), and `statusSelfCheckOffset` (`9`, `24`, `39`, `54`) now use the same hourly-alias form. Expensive source work occurs only at `:10`, price publication at `:16`, and score publication on even-hour `:16`; the `:40` path is neutral and `:46` only reuses the current generation for the dependent half-hour V9 preparation. `[limits].cpu_ms = 300000` remains the repo cap, while Cloudflare's [Workers limits](https://developers.cloudflare.com/workers/platform/limits/) give sub-hourly expressions a 30-second CPU class and hourly expressions a 15-minute class. Any further physical-trigger growth requires consolidation or an ADR-backed rebalance.
