...
| `safety-score-v9:supply-attribution-generation:v1`         | `syncSafetyScoreV9SupplyAttribution` | Private, bounded, content-addressed exact attribution generation containing the reviewed expected/observed asset inventory, accepted raw observations, explicit rejected outcomes, and source identity; the report-card publisher re-derives current USD partitions from this row without provider or RPC fan-out |
| `report-cards:v9-fixed-input:exact`                         | V9 shadow sidecar                | Private V9 attempt input retaining only a compatible fresh attribution generation plus the latest prior reserve and supply-attribution journal rows; rejected supply attempts retain an allowlisted exact leaf code and known rejected source time, while pre-diagnostic V1 rows remain parse-compatible; journal content changes this envelope checksum but not the V8 base-input generation, V9 fact-set/result identities, or public payloads                                                     |
...
| `sync-v9-supply-attribution`      | 900s (15min)     | `*/15 * * * *` (30min accepted / 15min rejected cooldown)   |
