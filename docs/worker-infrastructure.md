...
- `GET /api/yield-rankings` hydrates Safety Score fields from the same published report-card envelope, using compute-on-read only when the published snapshot cannot be loaded. The hourly publisher stages a yield publication generation, writes current/history rows and compact selected-source decisions against that generation, then marks it `published` only after the `yield-rankings` cache CAS succeeds; failed or CAS-skipped generations stay in D1 as operator evidence while public reads keep using the previous cache.
