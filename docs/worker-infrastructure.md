...
Scheduled job attempt ledger: `WORKER_JOB_LEDGER_MODE` defaults to `off`. Set it to `shadow` after migration `0165_worker_job_attempts.sql` has applied to record best-effort scheduled-job attempts in `worker_job_attempts` without changing cron execution, leases, or `cron_runs`; `write` is reserved for later promotion and currently behaves like `shadow`. `WORKER_JOB_LEDGER_ALLOWLIST` can narrow recording to a CSV of job names during rollout; leave it unset to record all scheduled jobs once the mode is enabled.
