...
| `sync-dex-liquidity-stage`        | 3,600s (1h)      | work at `:10`; retained hourly `:40` trigger is neutral       |
| `sync-dex-liquidity`              | 7,200s (2h)      | prices hourly at `:16`; score even-hour `:16`; `:46` reuses current generation |
...
| `sync-v9-supply-attribution`      | 900s (15min)     | `8,23,38,53 * * * *` (12min accepted / 14min rejected cooldown; the `22,52` publication consumes the `:08`/`:38` capture, which must precede the `16,46` prepare slot that stamps its fixed-input clock) |
| `prepare-safety-score-v9-input`   | 1800s (30min)    | after `:16` publication or `:46` exact-generation reuse; includes exact transfer-materiality observation |
