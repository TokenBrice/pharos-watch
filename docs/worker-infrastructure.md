...
The EVM lane first reserves at most one published score-bearing direction packet closest to its adapter-specific expiry, bounded to 20 estimated requests inside the same 720-request ceiling. The legacy Curve 3pool packet remains atomic, the reservation does not advance the durable cursor, and the remaining inventory keeps the existing whole-coin rotation.
...
Fresh, independently revalidated route identities with at least two successful cycles may remain in the bounded V9 route compiler for one evidence-freshness window after rotating out of the current pool shortlist; they never rejoin liquidity, pricing, display, or target-publication surfaces. The six owner-ratified Uniswap V3/PancakeSwap V3 deployment cohorts, the reviewed Base Aerodrome Slipstream cohort, and the reviewed Tron SunSwap V2 factory/pair/router profile are score-eligible. Optimism Uniswap V3, the reviewed wM/USDC Raydium CLMM direction, Fluid, generic Raydium CLMM, Orca Whirlpool, Meteora, and every other unratified or paused cohort capture only in shadow and retain an `activation-pending` capability gate. Fluid's bounded target-only copy is hydrated from canonical tracked-contract metadata after direct-source compaction, and only exact pools on the pinned Ethereum, Arbitrum, Base, or Polygon resolver deployments can become shadow targets; unsupported chains and identity-poor discovery fingerprints fail closed. Native registries encode reviewed active/shadow policy explicitly. A valid native profile enters P4 capacity and physical-pool completeness only when that policy is active; the reviewed Raydium direction remains shadow-only but still requires a captured single-segment replay matching the strict direct quote. Shadow, stale, malformed, failed, or identity-drifted profiles remain gated, and non-EVM pool identities preserve case through selection and validation. Solana reuses a still-fresh exact profile only after explicit operational collection failure; semantic or malformed history blocks that fallback. SunSwap's first activation followed complete production generations that retained one multi-hop-only gap, but the first two active DEX-scoring invocations exceeded the Worker memory limit; the registry returned to shadow. The phase-split handoff now removes source/provider graphs from the scoring invocation, and SunSwap was reactivated only after complete current shadow generations plus healthy `16,46` consumers. Optimism and the reviewed Raydium direction returned to shadow after their first score-facing consumers produced memory termination, stale-heartbeat, and incomplete-stage evidence. No stale proof or manual capacity is substituted for a current direct route. Shadow-only Solana quote degradation stays diagnostic in the merged cron metadata; attempted failures from the active Tron policy, any native invocation error, and non-durable native cursor deferrals degrade the merged job. The Solana producer binds native case-sensitive pool/mint identities and exact raw amounts to Raydium CLMM state or Jupiter context-slot proof inside a bounded RPC slot window. The Tron producer verifies canonical SunSwap factory/pair identities and exact constant-product output against a direct SUN Smart Router route or, only when every returned candidate is a clean V2 multi-hop path, the pinned official V2 Router's factory-bound two-token quote. Because TronGrid rejects numeric historical tags, latest-only state reads must fit a bounded before/after block bracket. This lane does not make a deployment score-eligible by registry presence alone.
...
| `report-cards:v9:last-attempt`                              | `computeSafetyScoreV9`           | Additive internal clean/partial/held attempt result with quarantined and affected asset IDs; written atomically with the accepted publication and health state. |
