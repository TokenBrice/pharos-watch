...
Cloudflare Worker serving the Pharos API. Handles HTTP routing, edge caching, CORS, admin auth, and scheduled runtime work across 19 cron expressions / runner slots. `CRON_INTERVALS` / `/api/status` track the 46 `CRON_JOB_DEFINITIONS` jobs across 18 job-bearing slots; `CRON_CONNECTION_BUDGET_ENTRIES` also includes budget-only scheduled surfaces such as Telegram registration reconciliation and the separate `*/5 * * * *` digest-trigger poll slot. The digest-trigger poll is the 19th runner slot and executes manual digest requests under the `daily-digest` lease rather than registering as its own status job.
...
Scheduled job attempt ledger: `WORKER_JOB_LEDGER_MODE` defaults to `off`. Set it to `shadow` after migration `0165_worker_job_attempts.sql` has applied to record best-effort scheduled-job attempts in `worker_job_attempts` without changing cron execution, leases, or `cron_runs`. `WORKER_JOB_LEDGER_ALLOWLIST` can narrow recording to a CSV of job names during rollout; leave it unset to record all scheduled jobs once the mode is enabled.

Repair task runner: `WORKER_REPAIR_RUNNER_MODE` defaults to `off`. The first rollout only dual-writes DDR repair-required events into `worker_repair_tasks` and keeps the existing DDR cache marker as status fallback; no repair task is claimed or executed until a later runner slice explicitly enables shadow/active processing.

Data-invariant canaries: `WORKER_CANARY_MODE` defaults to `off`. Set it to `shadow`, `status`, or `alert` after migration `0167_worker_canary_runs.sql` has applied to run the DB/cache-only `data-invariant-canary` job in the status self-check slot. The current slice records structural findings in `worker_canary_runs` and `/api/status.canaries`; it does not change producer publication behavior.
...
| `WORKER_JOB_LEDGER_MODE` | `string` | optional | - | - | Scheduled Worker job-attempt ledger mode. Unset or `off` disables writes; `shadow` records best-effort telemetry without changing execution. |
...
| `WORKER_REPAIR_RUNNER_MODE` | `string` | optional | - | - | Worker repair-task runner mode. Unset or `off` disables repair processing; `shadow` may record reconciliation telemetry before an enabled runner claims tasks. |
...
Most high-risk external integrations are protected by per-source circuit breakers (`worker/src/lib/circuit-breaker.ts`). State is persisted in the D1 `cache` table under keys like `circuit:defillama-stablecoins`. Breaker writes also maintain the aggregate `cache["provider:circuit:index"]` row as a status-read optimization; individual `circuit:<source>` rows remain the execution source of truth, and `/api/reset-circuit-breaker` removes both the source row and its index entry. Bounded low-volume fallbacks such as gold-api.com metal spot quotes, the secondary FX mirror, and ExchangeRate-API daily reference snapshots use explicit retry/timeout/cooldown behavior but are not currently circuit-gated.
