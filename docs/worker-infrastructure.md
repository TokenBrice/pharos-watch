...
| `worker/src/cron/depeg-resolver/utils.ts` | `v9DependencyImpairmentByCoin`<br>`v9MintPostureBandByCoin` | Depeg resolver V9 dependency and mint-posture projection | Both hydrated from the current V9 cards at resolver-run start; cleared when V9 publication is unavailable and reset with the isolate. | The current Safety Score V9 publication inputs and stablecoin registry statuses are authoritative; this only avoids repeated per-coin dependency and mint-component scans inside one run. |
