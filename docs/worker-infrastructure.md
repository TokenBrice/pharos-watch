...
Fetch-heavy provider phases should use `worker/src/lib/provider-execution.ts` for lane permits, provider-local permits, timeout signal composition, circuit gating, and response-body policy. `createProviderExecutionContextForJob()` derives the lane ceiling from `CRON_CONNECTION_BUDGET_ENTRIES` and refuses budgets above the repo's 5/6 headroom-full limit. The first production pilot is the `sync-dex-liquidity` direct API phase: protocol families run serially, each completed result is immediately compacted to tracked pools and bounded source evidence before the next family starts, each provider remains inside its provider-local policy, and circuit outcomes are recorded through the existing `circuit:<source>` breaker keys. Consumed Curve response trees are also released before this phase. The static trigger declaration remains conservatively `5/6` because one provider may still use its nested request width.
...
- Each dispatch run sends up to 3,600 Telegram message attempts in parallel batches of 4 under a 14-minute hard timeout; pending-drain and fresh-send batches stop after a 4-minute soft deadline. Existing due rows drain before authoritative subscriber capture and target planning. Target pages pack complete idempotent plan units into bounded D1 transactions, then reconcile each committed batch with one grouped read instead of issuing a database round trip per subscriber. When a backlog exists, newly planned targets remain queued for the next five-minute run. Unattempted pending claims are released, and untouched fresh tails are queued. Overflow and retryable fresh-send failures are enqueued to claim-based `telegram_pending_alerts` rows for subsequent runs.
...
| `report-cards:v9-shadow`                                   | V9 shadow sidecar                | Canonical checksum-verified V9 shadow envelope projected by `/api/report-cards/v9`; strict consumers require full identity/completeness and never fall back to V8                                                                                                                                                       |
...
The dedicated quarter-hour `cron-slot-sweeper` reconciles stale `cron_slot_executions` rows whose heartbeat has not advanced within the 35-minute slot stale window. Scheduled slot admission first pre-sweeps stale prior rows for the same schedule key. Before touching child artifacts, the sweeper atomically transitions the exact stale owner/generation/heartbeat row from `running` to `reconciling`; a concurrent heartbeat makes that claim change zero rows and leaves all child state untouched. If the claiming process dies, a later sweep can reclaim the stale `reconciling` row with the same owner/generation/heartbeat compare-and-swap and a new generation, while a fresh reconciliation claim remains protected for the full stale window. Takeovers increment `execution_generation`, and heartbeat/finalization updates require the current owner, generation, and state. Claim, takeover, metadata, and finalization retries accept a zero-change result only when a readback exactly matches the intended post-state, which distinguishes an ambiguous committed D1 response from ownership loss. Finalization also atomically requires that no expected child progress remains; progress committed after reconciliation but before that update makes the compare-and-swap lose, and the same claim reruns reconciliation before trying to finish again. A zero-change heartbeat aborts controlled work, while a late original finalizer cannot overwrite the takeover.

Child reconciliation is evidence-based rather than treating every abandoned outer slot as a failed job. Existing real terminal rows remain authoritative, including a distinct neutral `skipped_locked` disposition. For `sync-dex-liquidity`, persistence-stage progress metadata carries the exact `generationId`; that identity is authoritative only when the generation falls inside the child's bounded run window and its expected, written, and current row counts are equal and its state is `published`. When older progress lacks the identity, lookup is restricted to the same `[started_at, min(updated_at, started_at + 13 minutes)]` window and succeeds only for one unambiguous persistence generation. Reconciliation snapshots the exact progress row, latest terminal row, and DEX generation fields; ambiguous DEX lookup also snapshots the exact candidate set instead of treating ambiguity as a stable sentinel. A no-progress classification carries an exact `NOT EXISTS` predicate, and progress that appears after the initial scan is promoted into the progress reconciliation path rather than being sealed as not-started. The reconciler checks again after a synthetic not-started terminal commits; if progress appeared at that boundary, it compare-and-swaps the reconciliation-owned cron row and producer history to the progress-derived disposition and clears the progress through the normal fenced path. Each child mutation revalidates that snapshot together with the current slot state/owner/generation claim. Proven publication records a synthetic `degraded` child result and completes the matching attempt with the published row count. If complete publication appears after a reconciliation-owned synthetic failure was committed, the reconciler compare-and-swaps that same synthetic terminal and its producer outcome to the published disposition before correcting the attempt; an unrelated real terminal is never rewritten. A missing or staged generation after the persistence boundary is a publication failure; malformed, mismatched, or multiple generation evidence is terminal-accounting unknown; and an expected child with neither progress nor a terminal row is not-started. Those categories are retained in slot metadata and watchdog telemetry instead of being collapsed into a generic runtime error. The outer slot finishes `degraded` when every observed child is a preserved success, a lock skip, or proven published work; publication failures, real child failures, not-started work, or unmatched accounting finish it `error`.

Worker-attempt terminalization is a strict prerequisite for cleanup. The update requires one unique active attempt and compare-and-swaps its attempt/idempotency key, schedule, slot, job, producer identity, invocation, attempt number, owner, state, and prior update timestamp; it merges reconciliation metadata and records terminal time, duration, and item count. Terminalization and retirement of the exact expired old-owner lease execute in one D1 batch. Both statements are fenced on the observed lease owner/expiry, active-attempt state, reconciliation claim, progress row, and terminal/publication evidence. A zero-change retry is successful only when the attempt and retired lease exactly match the intended post-state. If authoritative evidence changes after that batch, the current reconciler may correct only a terminal attempt carrying reconciliation ownership metadata, using another exact claim/evidence CAS; ordinary terminal attempts remain immutable. An old owner that renews before the batch therefore leaves the child retryable, while a later renewal cannot revive the retired lease row. Any identity, lease, evidence, or terminal-write failure aborts before that child's progress deletion or slot finalization; progress deletion revalidates the final post-synthetic evidence in its own mutation. Producer history, producer heads, generic publication rows, and the operator event marker are also guarded by the current reconciliation claim, and deterministic keys make partial replay idempotent. Producer history resolves conflicts on both the idempotency key and the schema's `(schedule, job, path, kind, invocation)` uniqueness identity: real terminal logging may replace reconciliation-owned synthetic history for the same invocation, while a guarded synthetic write cannot overwrite existing real history. This keeps platform-killed events from remaining operationally active without allowing a stale observer or transient D1 error to corrupt live ownership.
