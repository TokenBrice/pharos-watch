...
`worker/wrangler.toml` currently deploys 34 physical expressions. The hourly aliases for `quarterHourly`, `v9SupplyAttributionOffset`, and `statusSelfCheckOffset`, together with the existing DEX pairs, intentionally map multiple physical expressions to one logical slot identity for Cloudflare's hourly CPU class.
