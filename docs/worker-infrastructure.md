...
- `POST /api/donor-key-claims` (`worker/src/api/donor-key-claims.ts`) is exempt from `X-API-Key`. It returns `403` before anything else while `DONOR_KEY_CLAIMS_OPEN` (`shared/lib/public-api-contract.ts`) is `false`, then spends `DONOR_KEY_CLAIM_RATE_LIMIT` (`10` per `60` seconds per client IP) before the body is read, returning `429` with `Retry-After: 60` on denial and `503` when the binding is absent. It then parses the Sign-In-With-Ethereum message, validates domain `pharos.watch`, URI `https://pharos.watch/api/`, and a 5-minute freshness window, recovers the externally-owned signer, and checks the donor threshold against `shared/data/funding/donations.json`. Issuance is one `db.batch` writing the `api_key_donor_claims` row and the `api_keys` row together, so a losing concurrent claim rolls back instead of orphaning a key. Responses are no-store and the plaintext token is returned once. Signatures, tokens, and wallet addresses are never logged.
