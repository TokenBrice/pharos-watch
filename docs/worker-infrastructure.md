...
The four-hour reserve slot uses three independent chains: `sync-live-reserves → sync-redemption-backstops`, `sync-kinesis-supply`, and `cron-sentinel`. Only redemption backstops consume the live-reserve result, so the other producer and the watchdog start independently instead of waiting behind the long-running reserve head. In particular, `cron-sentinel` must never be abandoned by the producer it watches; keeping it in its own chain preserves watchdog coverage when reserve synchronization reaches the slot deadline.
