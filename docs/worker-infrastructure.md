...
- `GET /api/safety-grades` is exempt from `X-API-Key` (`worker/src/api/safety-grades.ts`): a grade-only projection of the accepted V9 publication with the report-cards cache profile, so almost all traffic is an edge-cache hit. It has no per-caller limiter in the Worker; anonymous abuse control belongs in a Cloudflare WAF rate-limiting rule on that path.
...
| `shared/lib/iana-local-time.ts` | `localFormatterCache` | IANA local-time scheduling helpers | One Intl.DateTimeFormat per recognized IANA timezone until isolate recycle; unrecognized zones are never cached. | The subscriber timezone in D1 and the runtime timezone database are authoritative; cached formatters carry no business state. |
