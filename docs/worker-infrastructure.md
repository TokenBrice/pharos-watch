...
Cloudflare invocation analytics attributed the 2026-09-03 aligned `:00` failures to `exceededMemory`: the quarter-hourly core, active measured-execution lane, and daily housekeeping were invoked together while the service's 128 MB isolate was already near its limit. Their logical slot identities remain unchanged, but physical dispatch is staggered: measured execution uses `5,35 * * * *` for logical `:00/:30` slots, and daily housekeeping uses `3 3 * * *` for its logical 03:00 slot. This preserves cadence, freshness accounting, physical-trigger count, and connection budgets while preventing the three rich run-scoped graphs from starting simultaneously. Warm-isolate reuse still means the first production executions are required acceptance evidence.

Cron sentinel source state is persisted independently in the existing cache table under `cron-sentinel:source:<source>`. Every sentinel mode returns the worst latest evaluated source status across freshness, digest publication, growth, duration, repair debt, turnover, and reserve post-sync. A healthy run in another mode cannot erase a daily or reserve warning; only a newer evaluation of that same source clears it. Neutral/locked skips and cancellation retain the prior state, source exceptions persist an error, and monotonic cache writes reject older completions. Metadata includes each source's own `observedAt`, so aggregation does not make old evidence appear newly measured. Missing source cache state is seeded from its latest evaluated source entry in retained sentinel history, preserving pre-rollout daily warnings. Existing cron schedules and tables are unchanged.
