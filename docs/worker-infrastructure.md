...
- `daily-digest` freshness-gates the current Safety Score map through the Pages manifest and dated PNG routes. Telegram persists it as a large link preview inside the immutable digest payload; X uploads and attaches the PNG before creating the post. Map failures degrade to text-only delivery.
