...
| `report-cards:v9-shadow:publication-health`                | V9 shadow sidecar                | Strict bounded current/held state with accepted generation/time, latest attempt time, first hold time, and stable hold reasons. The candidate clock provides ordering; equal-time conflicting writes fail. |
