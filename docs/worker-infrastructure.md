...
Scheduled consumers that need only lifecycle or identity membership use `shared/lib/stablecoins/worker-runtime-registry.ts`, never the evidence-rich full registry. This includes the daily detail-cache prune and supply snapshot; `npm run check:mint-burn-runtime-imports` retains its legacy command name and checks those direct membership boundaries in addition to bundling the Telegram and extended mint/burn entrypoints. The quarter-hourly sync still legitimately consumes evidence-rich metadata elsewhere in its graph, so its entrypoint is not falsely classified as registry-free.
...
The `safety-score-v9-publication` Workflow is a shadow-only execution substrate for the existing `compute-safety-score-v9` compiler. `WORKER_V9_WORKFLOW_MODE=off` is the checked-in kill switch and preserves the cron-only path. Setting it to `shadow` makes the authoritative `v9PublicationOffset` cron create the deterministic instance `v9-publication:<slotStartedAt>` only after the cron publication settles. A duplicate create is treated as an existing instance, while other trigger errors are warnings and cannot undo or replace the live cron result.
...
| `reserve-recovery`         | 13 min  | Isolated five-minute reserve interruption-recovery lane: bounded candidate/claim windows plus suffix and sidecar replay                                                                                                |
...
| `snapshot-public-dataset`  | 10 min  | `PUBLIC_DATASET_CRON_TIMEOUT_MS`: the 8-minute stablecoins-cache retry window plus 2 minutes for D1 reads, gzip/hash work, and the immutable insert                                                                    |
| `sync-cl-exit-depth`       | 9 min   | Score-bearing measured-execution lane; the producer stops RPC work at eight minutes, leaving a minute to publish a complete quote generation and record the cron result                                                |
...
Most high-risk external integrations are protected by per-source circuit breakers (`worker/src/lib/circuit-breaker.ts`). State is persisted in the D1 `cache` table under keys like `circuit:defillama-stablecoins`. Breaker writes also maintain the aggregate `cache["provider:circuit:index"]` row as best-effort telemetry; individual `circuit:<source>` rows remain the execution and `/api/status.providerCircuitHealth` source of truth. The `POST /api/reset-circuit-breaker` endpoint was retired on 2026-08-09; clearing a breaker is now a scoped delete of its `circuit:<source>` row, documented in [`lease-and-breaker-recovery.md`](./runbooks/lease-and-breaker-recovery.md). Bounded low-volume fallbacks such as gold-api.com metal spot quotes, the secondary FX mirror, and ExchangeRate-API daily reference snapshots use explicit retry/timeout/cooldown behavior but are not currently circuit-gated.
...
- Source-specific semantics: `aznd-curve-pool` fails its circuit on thrown provider work while guarded no-quote results stay neutral. `jusd-citrea-bridge` records success when Citrea transport and every attempted route's identity, schema, and public burn capability validate but the routes are underfunded; it leaves the price missing, while no-trusted-parent skips before RPC remain neutral. `vaults-fyi` is disabled by default in code (`worker/src/lib/env.ts`), but the checked-in deployment sets `VAULTS_FYI_ENABLED="true"` (`worker/wrangler.toml`) so production enables it when the API key is present; `uniswap-v3-bsc-shadow` is target-only staging recovery; some live-reserve adapters opt into source-invariant within-run result sharing behind their scoped key.
