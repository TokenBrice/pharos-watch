...
| `dex-exit-route-turnover-watchdog:snapshot:v1`             | `cron-sentinel` turnover source | Compact previous DEX publication generation and per-coin `routeId -> evidenceKind` sets used to detect substantial published-route turnover across missed producer cycles. Carries `pendingAlert` for an alerting run (the baseline advances in the same run) and is cleared only by a later run that observes no alerting coin; an unreadable value is replaced with a fresh baseline instead of wedging the watchdog                                                                                                                                                |
