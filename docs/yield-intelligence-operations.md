...
- `sync-yield-data` now runs on a dedicated post-V9 trigger at `24,54 * * * *`, after the `10,40 * * * *` DEX source stage, the `16,46 * * * *` scoring-then-charts chain, and each `22,52 * * * *` V9 publication lane have had time to settle, while leaving a buffer before the heavy `:00/:30` lanes. It does not depend on the separate `26,56 * * * *` DEWS / PSI lane.
