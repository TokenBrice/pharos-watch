...
- `sync-yield-data` now runs on a dedicated post-V9 trigger at `28,58 * * * *`, after the `10,40 * * * *` DEX source stage, the `16,46 * * * *` scoring-then-charts chain, and each `22,52 * * * *` V9 publication lane have had time to settle. It does not depend on the separate `26,56 * * * *` DEWS / PSI lane.
