...
- read-time `yield-rankings` freshness warnings are now source-cadence-aware: hourly families trip after three hourly publish cycles, all `protocol-api` rows — supplemental families plus the hourly single-coin adapters (BIMA, Etherfuse, Hashnote, Midas, Ondo, Zephyr) — and the optional Aave/Compound rows wait 6 hours so they do not false-positive during the supplemental lane's normal 4-hour cache cycle, `price-derived` rows wait 36 hours because their observations come from daily `supply_history` snapshots, and derived-source comparison anchors trip after 14 days.
...
- Configure vaults.fyi credentials only as Worker runtime secrets. Do not commit a credential or put one in docs. The production four-hour lane caps runs at 13 credits, reserves that allowance before fetching, and dynamically lowers it when the remaining UTC-month budget cannot sustain the configured cadence. Provider telemetry exposes actual/reserved credits, sustainable and unthrottled month-end forecasts, utilization, remaining runs, and `coverageBudgetState`; a warning begins before 75 percent projected or actual utilization. Provider quota/errors fail open and should not block the hourly publisher. In `sourceCoverage.sourceFamilySummaries.vaultsFyi.provider.vaultsFyi`, `skipReason: "disabled"` means the enable flag is off/unset, `"no-key"` means the flag is true but the runtime secret is absent, and `"invalid-config"` includes a malformed enable flag.
...
| Benchmark fallback | `sync-yield-data` benchmark provenance | Fallback active, missing benchmark, or age above 48h | Age above 24d | No | Retained or old benchmark data degrades Yield Health | [benchmark fallback](./runbooks/yield-benchmark-fallback-stale.md) |
...
The monthly coverage audit is an operator queue, not a persistent workflow store. The cached report carries `operatorQueue.persistence="deferred"`, the allowed action labels, headline gaps (`manifestMissingIds`, `yieldBearingMissingFromRankings`, unmatched high-TVL pools, missing protocols), and recommendation candidates (native exact-pool, source-family adapter, and lending allowlist candidates). `/api/status` exposes the bounded queue under `yieldHealth.coverageAudit.headlineGaps` and `yieldHealth.coverageAudit.recommendationCandidates` so the admin card can show representative items without loading the full cache payload. Operators should classify each item in the monthly audit note:
