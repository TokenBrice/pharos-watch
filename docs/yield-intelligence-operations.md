...
- `sync-yield-data` runs on a dedicated hourly trigger at `55 * * * *`, after the `:52` Safety Score V9 publication. Recent V9 publications completed within 56 seconds, so this leaves two full settling minutes before yield loads the active safety identity and 27 minutes before the next V9 publication. The lane remains independent of the separate `26,56 * * * *` DEWS / PSI lane and does not collide with the `25 */4 * * *` supplemental refresh.
...
- `sync-yield-supplemental` runs on its own slower `25 */4 * * *` trigger and feeds a cache snapshot that the post-V9 publisher consumes.
...
- The four-hour supplemental publisher writes the eight per-family snapshots. The post-V9 publisher uses valid fresh family rows directly; when required family coverage is partial it keeps the valid families and marks `partial-family-cache` without backfilling from the retained aggregate row.
