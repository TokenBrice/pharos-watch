...
- `sync-yield-data` runs on a dedicated hourly trigger at `20 * * * *`, after the hourly `10 * * * *` DEX source stage and the `16,46 * * * *` scoring-then-charts chain have had time to settle. It does not depend on the separate `26,56 * * * *` DEWS / PSI lane.
