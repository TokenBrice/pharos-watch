...
- post-V9 publication writes selected-source decision rows after D1 staging and before cache publication. The per-row alternatives JSON blob is bounded to 4 KB and records compact selected, rejected, and retained-alternative reasons for operator debugging.
...
| --- | --- | --- | --- | --- | --- | --- |
| Rankings freshness | `sync-yield-data` -> `cache['yield-rankings']` | Age above 8 post-V9 producer intervals | Missing payload or age above 12 post-V9 producer intervals | Yes, when stale or missing | Degraded-but-not-stale rankings remain watch-only | [stale or missing rankings](./runbooks/yield-rankings-stale-or-missing.md) |
