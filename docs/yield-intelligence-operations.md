...
- read-time safety hydration is compatible across ordinary quarter-hourly report-card input/publication generations only when the V8 model, schema, methodology, and evaluation-build digest match the hourly yield snapshot. This keeps the 15-minute report-card cadence compatible with the hourly yield cadence while still failing closed during a scoring-code or methodology rollout until `sync-yield-data` republishes.
