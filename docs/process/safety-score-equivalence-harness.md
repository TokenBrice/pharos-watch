...
- The producer publishes with `publishedAt = fixedInput.clockSec` (`worker/src/cron/compute-safety-score-v9.ts:364`). Replaying with `--published-at <capture clockSec>` therefore reproduces the exact publication clock, not an approximation.
