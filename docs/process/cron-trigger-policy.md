...
The worker declares **~19 cron expressions** in `worker/wrangler.toml`. Each expression maps to one Cloudflare scheduled-trigger invocation, routed in `worker/src/handlers/scheduled/context.ts` to the jobs configured for that slot.
...
The soft cap is **20 trigger expressions** before re-architecting batched dispatch. Adding the 20th trigger should prompt a design pass on whether routing logic in `context.ts` can fan out instead of growing the cron table.
...
2. **Fit into an existing trigger.** Map the new job through `worker/src/handlers/scheduled/context.ts` to share an existing trigger that has headroom. Jobs sharing a slot must consume or cancel any fetch response bodies before opening more fetches; see `docs/worker-and-api-limits.md`.
...
- Re-architecting batched dispatch (one trigger fanning out to many logical jobs in `context.ts`) is the supported path past 20 cron expressions; bespoke new triggers should be the exception.
