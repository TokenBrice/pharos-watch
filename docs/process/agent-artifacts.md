...
- **Byte-identical pairs** (currently `annotations-refresh`, `stablecoin-addition-orchestrator`, `stablecoin-runtime-price-marketcap-gate`, and any newly mirrored skill such as `mica-research` when it is promoted to both surfaces): the `.claude/skills/<name>/SKILL.md` file can be a relative symlink to `../../../.codex/skills/<name>/SKILL.md`. Edit the codex copy; both surfaces pick up the change automatically.
