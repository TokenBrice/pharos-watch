...
The **historical admin replay** treats a depeg as active for any UTC day whose window overlaps the event interval. When rebuilding a prior UTC day, it uses same-day `supply_history.price` to derive replayed deviation when that day price is usable, but on the UTC day a depeg begins it keeps `peak_deviation_bps` as a floor if the daily snapshot understates the move. Later days fall back to `peak_deviation_bps` only when a usable historical day price is unavailable. This preserves intraday crisis windows without permanently peak-anchoring recovery days.
