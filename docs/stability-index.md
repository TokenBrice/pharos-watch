...
- **Canonical source:** `shared/lib/methodology-versions/stability-index.ts` (re-exported from `shared/lib/methodology-versions/stability-index.ts`, with changelog entries in `shared/data/methodology-changelogs/stability-index/`)
...
| DEWS stress breadth | Exact core-universe `stress_signals` rows from the published DEWS generation pointer; DEWS still monitors all active listings, but only core/cash/shadow warning bands (`ALERT`, `WARNING`, `DANGER`) contribute after the generation is proven non-empty and fresh |
