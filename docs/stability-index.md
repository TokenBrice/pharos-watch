...
The **historical admin replay** treats a depeg as active for any UTC day whose window overlaps the event interval. When rebuilding a prior UTC day, it uses same-day `supply_history.price` to derive replayed deviation when that day price is usable, but on the UTC day a depeg begins it keeps `peak_deviation_bps` as a floor only if the depeg was still active through that UTC close and the daily snapshot understates the move. Same-day wicks that fully recover before the UTC close use the daily historical price instead of the intraday peak. Later days fall back to `peak_deviation_bps` only when a usable historical day price is unavailable. This preserves real crisis windows without permanently peak-anchoring recovery days or turning transient intraday noise into synthetic daily collapses.
...
- **Historical admin backfill**: `handleBackfillStabilityIndex()` replays completed UTC days only. The replay path bounds market-cap denominators to the PSI-eligible universe, treats any depeg overlapping the UTC day as active, derives same-day depeg severity from `supply_history.price` when available, keeps `peak_deviation_bps` as a start-day floor only when the event remained active through that UTC close and the daily snapshot misses the move, and otherwise falls back to `peak_deviation_bps` only for missing/invalid historical prices. For methodology `v3.0+`, it also derives historical `stressBreadth` from same-day `stress_signal_history` warning bands (`ALERT`, `WARNING`, `DANGER`). When a rebuild day cannot be replayed because archival inputs are missing, the endpoint preserves the existing stored row instead of deleting that day.
