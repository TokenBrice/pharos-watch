...
Mint Authority coverage counts curated review breadth first, then appends standalone score-band breakdowns for reviewed rows. `Unknown` means no compact mint-authority review is available; stablecoin detail pages always render the Mint Authority section, showing an explicit `Not reviewed by Pharos` / `Mint Authority Score: NR` state when no reviewed data exists, while score-oriented aggregate surfaces treat the row as `NR`. Compact projections include cap-mutability evidence so coverage score bands remain equivalent to full metadata scoring.
