...
`telegram-retention-cleanup` deletes retained Telegram audit/analytics rows in ordered capped batches instead of uncapped table DELETEs. Processed updates are capped at 5,000 rows and the other table/cache passes at 10,000 rows per daily 03:00 UTC run; per-table `cappedAtLimit` metadata exposes remaining backlog pressure.
