...
`telegram-retention-cleanup` deletes retained Telegram audit/analytics rows, including source-resolution, freeze event/target, and target-item lineage, in ordered capped batches instead of uncapped table DELETEs. Processed updates run in 1,000-row batches with a 2-second internal time budget and a 5,000-row ceiling; the other table/cache passes are capped at 10,000 rows per daily 03:00 UTC run. Alert source, resolution, freeze, target-item, job, and dead-letter audit rows use the same 90-day window. Usage, adoption, and adoption-retention aggregates use 400 days; CTA quota buckets use two days; the Mini App open-to-first-mutation cache uses 30 minutes and is deleted immediately by `/forget`. A bounded 5,001-row processed-update probe reports the remaining count exactly below that limit or as a lower bound at the limit. Remaining processed-update debt sets `runBudgetTruncated`, while per-table `cappedAtLimit` metadata distinguishes the row ceiling from time-budget exhaustion.
