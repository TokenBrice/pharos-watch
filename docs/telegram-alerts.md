...
| `telegram_pending_alerts` | Overflow and retry delivery queue | `id`, `chat_id`, `message_html`, `disable_notification`, `created_at`, `attempts`, retry/dedupe/priority fields, claim fields, `delivery_state`, `delivery_started_at`, `delivery_completed_at`, risk `source_event_id`, `alert_scope_json`, `preference_generation`, `markup_policy_json` |
| `telegram_alert_jobs` / `telegram_alert_job_targets` | Durable discovery manifests and target-level delivery/effect audit | `job_id`, `source_event_id`, `alert_type`, `severity`, `status`, `target_count`, `last_cursor`, target `chat_id`, `chunk_index`, `pending_dedupe_key`, target `status`, `sent_at`, `enqueued_at`, `failed_at`, `error_class`, `cancelled_at`, `cancellation_reason`, `effect_state`, `effect_owner`, `effect_generation`, effect timestamps |
| `telegram_alert_source_events` / `telegram_alert_source_resolution_pages` | Immutable detected event plus cursorable preset-resolution lifecycle | versioned `source_event_id`, exact event/baseline payloads, `resolving/planned/baseline_committed/complete/expired` state, family/page cursor, attempts, terminal timestamps |
...
`telegram-retention-cleanup` deletes retained Telegram audit/analytics rows, including source-resolution and target-item lineage, in ordered capped batches instead of uncapped table DELETEs. Processed updates run in 1,000-row batches with a 2-second internal time budget and a 5,000-row ceiling; the other table/cache passes are capped at 10,000 rows per daily 03:00 UTC run. Alert source, resolution, target-item, job, and dead-letter audit rows use the same 90-day window. A bounded 5,001-row processed-update probe reports the remaining count exactly below that limit or as a lower bound at the limit. Remaining processed-update debt sets `runBudgetTruncated`, while per-table `cappedAtLimit` metadata distinguishes the row ceiling from time-budget exhaustion.
...
The webhook claims individual Telegram update IDs, durably marks `effect_state = 'started'` before command handling, and owner/generation-fences the terminal marker. Stale `unstarted` claims can be recovered. Once effects start, a missing terminal marker is treated as uncertain and duplicate deliveries are acknowledged without rerunning effects; `/api/status.telegramBot.webhookEffectUnknown` exposes the count for operator reconciliation.
...
| `/export` | Emits a copy-paste base64url **watchlist token** of your explicit coin follows, the alert types you use, and your followed presets (quiet hours / snooze are intentionally excluded). If the token or its Telegram code block would exceed the import/message limits, the command refuses the export instead of emitting a split, unusable token; existing follows remain unchanged. Read-only; works in any chat. |
| `/import <token>` | Decodes an `/export` token, validates each coin id against the subscribable registry (skipping frozen, unknown, or otherwise unavailable coins and reporting the count), then stages a bulk subscribe through the standard `[ Confirm ] [ Cancel ]` flow (single pending slot, initiator-locked). The token is versioned (`v:1`) and capped at 4000 chars; a malformed, oversized, or newer-version token is rejected without applying anything. In groups, `/import` is admin-gated like `/subscribe`. Copy-paste artifacts (a leading `/import`, surrounding backticks, whitespace) are tolerated. Import applies one uniform alert-type set to the imported coins (per-coin tuning is not part of the portable token). |
...
Telegram may redeliver the same `update_id`. The webhook owner/generation-claims each update and persists a deterministic effect key plus `effect_state = 'started'` before command handling. Only stale `unstarted` claims are reclaimable. Already processed duplicates return `200 ok`; fresh unstarted in-flight duplicates return `503 retry`. A retry that finds effects started but no terminal marker returns `200 ok` without rerunning the command. This is a deliberate at-most-once ambiguity: it can require manual recovery after a crash between the effect-start marker and the command, but it prevents a successful reply or mutation from being repeated when the final D1 marker fails. Operators use `telegramBot.webhookEffectUnknown` and the processed-update row before any explicit retry/repair.
...
- Detection first inserts an immutable `telegram_alert_source_events` row. Dynamic preset followers are resolved in normalized 100-row cursor pages; completed pages are reused and only pending pages are queried on recovery. Subscriber/preset existence, the current family flag, chat snooze, per-coin snooze, and explicit local-off state are revalidated before a persisted page can route an alert.
- Snapshot baselines remain unchanged while any resolution page is pending. Once every page and resulting target manifest is durable, the fixed snapshot cache writes and `baseline_committed` transition share one D1 batch; `complete` is later bookkeeping. A crash before that batch resumes the event, while a crash after it can only replay target-deduped work. Expiry atomically advances the stored baseline and marks untouched page/target work expired so one permanently failing page cannot block newer diffs or send stale alerts.
- Consolidated target chunks have normalized item junction rows. Recovery subtracts an item only after every chunk in its earlier message group is terminal, so successful direct/global items are not repeated when a previously unresolved preset page becomes available.
- Overflow subscribers are enqueued to `telegram_pending_alerts` and drained in subsequent runs. New risk rows persist their immutable source event, the exact coin/family pairs in the consolidated target group, the chat preference generation observed during planning, and the original markup/link-preview policy. Every split chunk in one message group carries the same conservative group scope, so one newly ineligible pair cancels every remaining chunk rather than delivering a partial stale group. Pending rows are claimed by `processing_owner` before send and only rows owned by the current invocation are delivered.
...
When the chunked subscriber queue exceeds the per-run cap (3,600), overflow messages are written
...
Terminal pending drops are dead-lettered before deletion with `reason` values `ttl_expired`, `permanent_failure`, `max_attempts`, `blocked_disabled`, or `preference_changed`. Preference cancellation also marks the target's legacy status `failed` and records `cancelled_at` plus a bounded `cancellation_reason`; it is not misclassified as TTL expiry. Expired-row cleanup logs an error-level bypass event and still removes expired live rows when dead-letter insertion fails, so an audit-table outage cannot let the live delivery queue grow without bound.
...
Use staged rollout for any durable sender or alert-job migration:

1. Metrics-only and dry-run job creation: measure fan-out and target counts without changing authoritative sends.
2. Dual-write old pending rows and new alert jobs: old sender remains authoritative.
3. Shadow-drain new jobs without sending: verify cursoring, dedupe keys, and D1 operation volume.
4. Canary a small alert type or internal chats: keep old pending queue available for rollback.
5. Promote the new sender for all alert types after `npm run check:telegram-load -- --enforce-target-slo` passes for the 5,000-watcher target.

Rollback must not require schema rollback. Keep the old pending sender readable until the new sender has drained or intentionally dead-lettered all in-flight risk alerts.
