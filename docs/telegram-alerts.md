...
The freeze outbox is intentionally separate from `telegram_alert_target_plans`, whose deployed CHECK constraint admits only the original five families. It still creates canonical source/job/job-target/item lineage and hands every rendered chunk to `telegram_pending_alerts`; the freeze-specific tables preserve the independent Tape/blacklist provenance and resumable cohort without a destructive table rebuild.
