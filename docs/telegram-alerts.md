...
The webhook validates the configured secret from `X-Telegram-Bot-Api-Secret-Token`. During rotation it accepts either `TELEGRAM_WEBHOOK_SECRET` or `TELEGRAM_WEBHOOK_SECRET_PREVIOUS`. Invalid secrets and malformed JSON return `200 ok` without side effects so Telegram does not keep retrying. A missing bot token is acknowledged but emits a rate-limited configuration warning instead of silently discarding updates. If every D1-backed command flood check is unavailable, ingress fails closed with the standard busy reply and does not dispatch the command. Non-command messages also return `200 ok` without side effects only when no active pending flow exists; pending/setup handlers run first and can clear expired rows or execute a disambiguation selection from plain text.
...
| `/status <ticker>` | Returns a compact snapshot: current price freshness, supply, DEWS band, safety grade, active-depeg state, DEX liquidity, best yield context (including the live PYS when the ranking resolves; an unavailable PYS is reported only for that coin), and — for coins covered by the mint/burn tracker — a `Flow 24h: <signed compact USD>` line (24h net mint/burn flow with data age). The flow line reads the existing per-coin 24h flow cache (`perCoinFlowCacheKey(id, 24)`) and never recomputes; untracked coins omit it. No subscription required. The reply carries a `[ Why? ] [ Coverage ] [ Subscribe ]` inline keyboard so users can drill down or quick-subscribe (DEWS + depeg) without retyping a command. The `Subscribe` button is gated by the same group admin check as `/subscribe`. |
...
| `/subscribe <types> <targets>` | Enables one or more alert types for explicit coins; preset watchlists support DEWS, depeg, and safety only |
...
| Payload | Behavior |
|---------|----------|
| `sub_<types>_<targets>` (e.g. `sub_depeg_usdc-circle` or `sub_dews-depeg_usd-top25`) | Translates to `/subscribe <types> <targets>` and dispatches the existing subscribe path. Both non-empty segments are required; multiple alert types are joined by `-`, so `sub_usdc` is malformed and remains read-only. Valid private-chat subscription links are classified as mutating before dispatch so the effect fence records the subscription write and confirmation reply. Group deep-links fall back to the standard onboarding reply with no mutation. |
...
Freeze alerts retain two immutable identities: the projected `tape_events.event_id` and its source `blacklist_events.id`. New Tape projections include the canonical stablecoin id from the verified blacklist contract configuration; historical symbol-only rows are accepted only when the symbol maps to one tracked coin. Malformed, unknown, or ambiguous rows fail closed: the cursor remains immediately below the first such row so a later registry update can make it deliverable. After three consecutive observations of the same poison row, Telegram emits a structured error and skips that row before continuing, preventing one bad projection from wedging the feed.
...
- `launch`, `reserve`, and `freeze` do not accept presets; use explicit ticker/coin-id targets or the special `all` target.
...
Global all-stablecoin depeg follows can also carry a severity threshold through `/set all depeg-step 100|250|500`. A configured value gates fresh depeg and resolution notifications below that peak deviation and also controls worsening follow-up milestones. The current model intentionally couples the severity floor and worsening cadence; they cannot be tuned independently. On `/set <target> depeg-step`, the value `off` means what the settings keyboard's Depeg toggle means: it turns the depeg family **off** for that target. It does not clear the step while leaving depeg alerts enabled — writing the step alone used to turn depeg alerts on, which is the opposite of what the command's own error text promised. The settings screen renders a stored step only while the family flag is on, so `Depeg: OFF` never carries a `(+Nbps)` suffix. Preset subscriptions can set the same per-coin threshold in one command, for example `/subscribe usd-top-50 depeg-step 250`.
...
- Depeg resolutions by checking which prior active depegs disappeared and then loading the corresponding closed event rows; only rows classified as recovered emit "Depeg Resolved" — an explicit recovery close reason (`recovered-primary`, `recovered-dex`, `recovered-native`), or a legacy row with no `close_reason` but a recorded `recovery_price`
...
The package-level `npm run check:telegram-load` command is blocking: it passes `--enforce-target-slo` and fails when the SLO-enforced tier (`sloEnforcedTarget`, 1,000 watchers since 2026-09-22; production is ~855) misses the normal under-15-minute risk-alert SLO or breaches the spike bound. The 5,000-watcher `requiredTarget` still bounds CPU, TTL margin and the status-path budgets; its SLO rows are advisory until the planner is redesigned (the 5-minute planner cadence caps a single-depeg fan-out at ~40 minutes there). For an advisory local report that only fails critical query-plan regressions, run `npx tsx scripts/ci/check-telegram-load.ts` directly without `--enforce-target-slo`.
...
The pulse publication is generation-fenced: a run writes `telegram:pulse:snapshot` only when its build-start generation is at least as new as the stored generation, so a slower older run cannot replace a newer payload. Lifecycle history is limited to the latest 90 complete UTC days from `telegram_watcher_lifecycle_daily`; the current UTC day is never plotted as history, and no subscriber-cohort fallback is synthesized from current state.

Daily new, churned, and reactivated watcher counts come from atomic subscribe, unsubscribe, and reactivate transitions in `telegram_watcher_lifecycle_events_daily`, not from the net change between two active-watcher snapshots. Preset resolution or lifecycle-event read failures publish the affected pulse fields as `null` and name them in `quality.unavailableFields`; unavailable adoption data is never represented as zero. Migration `0246_telegram_watcher_lifecycle_events.sql` must be deployed before the Worker version that reads these counters.
...
Subscriber alert messages end with a `View on Pharos` link. Telegram digest posts carry a `Read on Pharos →` link after the editorial body and any cemetery or tracking appendices. Under `public` recap rollout — the current production setting — the post then ends with the private-recap CTA (`Open @PharosWatchBot for a private /recap →`); in `off`, `dark`, and `canary` the `Read on Pharos →` link is last. A current daily Safety Score map contributes a dated summary block (`Today’s map`, or `<date> map` when the map is carried forward, plus mapped supply, A-tier and C/D/F-tier counts) rendered at the top of the digest message, above the edition kicker; the map image itself is delivered as a separate photo message from the durable digest outbox, with the caption `Safety Score map · <date>`, before the text chunks.
...
- The command reconciliation issues two scoped `setMyCommands` calls: the full list under `scope: { type: "all_private_chats" }` and a group-safe list under `scope: { type: "all_group_chats" }`. Both lists derive from the runtime-neutral command reference in `shared/lib/telegram-bot-registration.ts`, which also owns the per-command syntax variants, concise `/help` lines, public-reference examples, and deprecated aliases (`/market` → `/brief`) that the bot's `/help` reply and the `/pharoswatchbot/` command reference render. The group menu includes read-only commands and group-valid subscription/settings controls, but intentionally omits `/start`, `/forget`, and `/recap` because setup deep links, destructive data deletion, and the private daily recap stay private-chat only. Both scopes share a single cache key (`telegram:commands-reconciled`); a fresh cache hit skips both round trips, and bumping `TELEGRAM_COMMANDS_CACHE_VERSION` forces every deployment to reconcile once.
