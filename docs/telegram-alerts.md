...
`telegram-retention-cleanup` deletes retained Telegram audit/analytics rows in ordered capped batches instead of uncapped table DELETEs. Processed updates run in 1,000-row batches with a 2-second internal time budget and a 5,000-row ceiling; the other table/cache passes are capped at 10,000 rows per daily 03:00 UTC run. A bounded 5,001-row processed-update probe reports the remaining count exactly below that limit or as a lower bound at the limit. Remaining processed-update debt sets `runBudgetTruncated`, while per-table `cappedAtLimit` metadata distinguishes the row ceiling from time-budget exhaustion.
