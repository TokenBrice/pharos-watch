...
3. A successful protected merge triggers the production deploy classifier. The deploy workflow does not rerun the full PR suite: Worker mutation keeps a focused migration check and Wrangler activation proof; Pages publication keeps build-artifact checks and a release-marker proof. Details live in [Deployment Process](./deployment-process.md).
