...
Local discovery shares one machine, unlike independent GitHub runners. When failures appear load-dependent, reproduce the exact build or shard alone and lower `MERGE_GATE_DISCOVERY_MAX_PARALLEL` before changing timeouts, worker counts, or product code. A serial focused pass plus a parallel-only local failure is contention evidence, not permission to weaken a test budget.
