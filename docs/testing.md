...
- `check:oracle-risk-coverage:enforce` remains the deploy-impacting prebuild content guardrail for direct-CDP oracle profiles.
...
- `npm run check:redemption-coverage-audit` is a manual Redemption Backstop backlog audit against `scripts/lib/redemption-coverage-audit-baseline.json`, so default-inferred active gaps, total active gaps, and heuristic route counts cannot grow while the backlog is being researched.
