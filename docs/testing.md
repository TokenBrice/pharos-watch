...
CI **does not** run a full-suite coverage gate. The PR workflow runs `coverage:critical` when an enrolled source or critical-coverage plumbing changes; the compare ref scopes the no-regression ratchet to touched sources. Each PR coverage shard runs only the owners of touched enrolled sources, while the weekly/manual Critical Coverage Ratchet workflow runs the full derived owner set:
