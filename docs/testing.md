...
npm run test:smoke-ui:mobile -- --url http://localhost:3000 # Mobile-focused rendered smoke; strict common-control touch targets, table geometry checks, console reporting, and desktop spot checks
```
...
6. `pages-release`:
   ...
   - serves the just-built `out/` export locally, proxies direct `/api/*` and `/_site-data/*` calls to the selected API base, injects `SITE_API_SHARED_SECRET` for the site-data proxy hop, and runs `npm run test:smoke-ui -- --url http://127.0.0.1:4173 --mode local` plus `npm run test:smoke-ui:mobile -- --url http://127.0.0.1:4173`
