...
CI **does not** run a full-suite coverage gate. The PR workflow runs `coverage:critical` when an enrolled source or critical-coverage plumbing changes; the compare ref scopes the no-regression ratchet to touched sources. Every coverage shard runs the full derived owner test set so transitive imports can exercise a touched source. The weekly/manual Critical Coverage Ratchet workflow uses the same full derived owner set:
...
- PR coverage shards pass the changed file set to `buildCriticalCoverageArgs`; they include only touched enrolled sources for V8 remapping and invoke every derived owner test. A plumbing-only change falls back to the full derived source set.
