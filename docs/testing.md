...
`npm run test:merge-gate` now mirrors the deploy-path validate contract locally. If the changed-file set is not deploy-impacting, it prints the diff and exits successfully. For deploy-impacting diffs, it always runs `audit:deps`, lint, the root source `typecheck`, worker-boundary, shared-cycle detection, migrations, cron schedule/connection checks, doc sync checks, duplicate-export and redemption-backstop guards, unused-code, hotspot-ratchet, the full test suite, and critical coverage. It adds `npm run build` + `npm run seo:check` when Pages-impacting files changed, and adds `cd worker && npx tsc --noEmit` when worker-impacting files changed. It still skips deploy-time smoke suites.

`npm run check:unused-code` now scans all runtime code under `src/`, `shared/`, `worker/src/`, and `functions/`, with explicit module/export allowlists for intentional exceptions. `npm run check:hotspot-ratchet` now guards the current decomposition queue plus the maintained shell/facade files, including the extracted methodology section body modules under `src/app/methodology/sections/core/` and `src/app/methodology/sections/monitoring/`, the split scoring-changelog content modules under `src/app/methodology/scoring-changelog/`, `src/app/coverage/client.tsx`, `src/app/chains/[chain]/client.tsx`, `src/components/contagion-graph.tsx`, `shared/lib/report-cards.ts`, `worker/src/cron/daily-digest.ts`, `worker/src/cron/dispatch-telegram-alerts.ts`, `worker/src/cron/dex-liquidity/orchestrator.ts`, `worker/src/cron/dex-liquidity/scoring.ts`, `worker/src/cron/sync-live-reserves.ts`, `worker/src/cron/sync-mint-burn.ts`, `worker/src/cron/sync-stablecoins.ts`, `worker/src/cron/sync-stablecoins/enrich-prices.ts`, `worker/src/cron/sync-blacklist.ts`, `worker/src/cron/sync-fx-rates.ts`, `worker/src/cron/yield-config.ts`, `worker/src/cron/yield-sync/sources.ts`, `worker/src/lib/live-reserves-store.ts`, and `worker/src/lib/status-reliability.ts`. Each baseline entry now also declares a `disposition`, `targetBudget`, and implementation note so the ratchet doubles as a decomposition backlog rather than a blind ceiling list. The matching backlog note in [`agents/plans/2026-03-29-hotspot-decomposition-backlog.md`](/Users/ahirice/Documents/git/stablecoin-dashboard/agents/plans/2026-03-29-hotspot-decomposition-backlog.md) must track the owning lane, next split target, and validation commands for each queued hotspot. Refresh the baseline only after an intentional refactor with `npm run check:hotspot-ratchet:update-baseline`, and update the matching backlog metadata at the same time.
...
- `npm run test:smoke-ops` performs private post-deploy checks against the operator surfaces through Cloudflare Access. For direct `ops-api.pharos.watch` requests, service-token mode sends `CF-Access-Client-Id` / `CF-Access-Client-Secret` and the worker verifies the injected Access JWT there. For the same-origin Pages proxy path, the script first bootstraps an Access session on `ops.pharos.watch`; when a `CF_Authorization` cookie is returned, it reuses that cookie to smoke `https://ops.pharos.watch/api/admin/status`. If the UI host exposes only the interactive Access redirect, the script keeps the shell/access-gate assertion and direct `ops-api` smoke but skips the same-origin proxy assertion.
