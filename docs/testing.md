...
Every generated-artifact registry entry declares a build lifecycle: `compile-input` for files the static export needs before compilation, `post-refresh` for projections rebuilt after release data refresh, or `maintenance-only` for explicitly maintained outputs. Plain `prebuild` selects only compile inputs; the Pages workflow selects compile inputs plus post-refresh outputs once after refreshing data. The separate `inputState: "build-time"` artifacts (`sitemap-dates` and `docs-metadata`) still derive timestamps from commit history, remain gitignored and `checkable: false`, and are materialized by `npm run bootstrap:generated:history` for non-build consumers. Committed outputs marked `autoStage` are still regenerated and staged by `.githooks/pre-commit`, while `npm run check:generated-artifacts` retains full-registry freshness semantics for every checkable artifact.
