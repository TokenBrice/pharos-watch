...
2. Other PRs run `check:pr:static` plus two shards of `test:pr`. The static runner always checks changed-file lint, source types, environment and import contracts; root `package.json` or `package-lock.json` changes also run the production-scope dependency audit. It adds data/generated, Worker, and Telegram checks only for relevant paths. `test:pr` unions the critical API contract list with Vitest's dependency-selected changed tests. PRs that change GitHub workflows or composite actions also run the path-scoped Zizmor analysis before merge.
