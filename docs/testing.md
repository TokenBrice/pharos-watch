...
2. A dependency-free `preflight` first checks out the exact PR base SHA and runs its pinned Gitleaks scanner, helpers, configuration, and ignore policy against the base-to-head commit range. Recurring fixture false positives belong in narrow rules that pair the owning path with the exact reviewed value; `.gitleaksignore` retains only findings not covered by such a rule. Only after that required scan succeeds does preflight check out the PR merge result and run the second required lane, `run-gitleaks.ts --tree`, over the bytes the range scan cannot see: the combined-diff `++` lines (conflict-resolution content present in neither parent) of the checked-out merge commit *and of every merge commit inside the PR range* — a resolution-only secret from an earlier merge is invisible to the `--no-merges` range lane and already present in the checked-out merge's PR-head parent — fed through `stdin`. Fingerprints are commit-pinned, so a directory scan could never honour them; the resolution lines are new by construction and any finding there fails closed. A non-merge checkout with no merge commits in range scans nothing. Preflight then runs the deploy-impact classifier and manifest preflight without `npm ci`, and generate the selected matrix from `scripts/lib/pr-lanes.mts`. One `prepare` job performs `npm ci` and both generated-artifact bootstraps; its SHA-keyed cache of `node_modules` and bootstrap outputs is restored by every matrix and coverage-merge job. Setup checks registered tracked, checkable bootstrap outputs against the checkout’s `HEAD` after both fresh generation and restore-only setup, and rejects nonignored registered output changes from either path before validation starts. PR jobs restore only and never save cache state. The tests job fans out to four stable hash shards from `scripts/lib/shard-timings.mts`, while the all-critical coverage job starts as soon as `prepare` completes and merges one shard per configured critical file with fail-closed validation of shard count, distinct shard indices, artifact identity, manifest identity, test-file uniqueness, valid percentages, and exact expected-file coverage.
...
- `npm run audit:coverage -- --domain=oracle-risk` blocks on missing or structurally incomplete CDP oracle profiles and required branch evidence by default. Use `--advisory` only while backfilling to print those findings without failing; stale-review reminders and the reviewed applicability queue remain advisory, while explicit unresolved dispositions remain V9 blockers rather than silently passing as profile-only evidence.
