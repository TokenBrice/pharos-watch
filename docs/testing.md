...
2. `validate` (runs before any deployment):
   ...
   - after `validate:prebuild`, fans out onto separate runners for three non-critical Vitest shards, critical coverage, optional Worker runtime typecheck, optional Worker operational-script typecheck, and optional Pages build/SEO
   ...
   - runs `npm run build` + `npm run seo:check` in PR validation when `pages_changed=true` and `run_pages_build_and_seo=true`
   - production deploy calls set `run_pages_build_and_seo=false`; Pages deploys still prove the exact publish artifact in `pages-prepare` with build, SEO, artifact upload, local serve, and local browser smoke before `pages-publish`
...
4. `upload-worker-version` (needs `validate` and `detect-changes`):
   ...
   - Re-run `npm run check:migrations` on the release runner immediately before any remote D1 change so the exact deploy environment replays the full migration tree and enforces rollout-safety guardrails again
   - Upload a candidate Worker version with `wrangler versions upload` before touching production D1 so the preview-smoke target is already prepared when the schema change lands
...
5. `deploy-worker` (needs `upload-worker-version` when Worker promotion is required):
   ...
   - On combined Worker + Pages deploys, waits for `pages-prepare` to build, SEO-check, upload, serve, and local-smoke the Pages artifact before production Worker promotion
...
7. `rollback-worker`:
   ...
   - Does not roll back D1 schema or data; use the D1 time-travel runbook in `worker/migrations/MANIFEST.md` when a migration itself must be undone
...
8. `pages-prepare`:
   ...
   - on combined worker + Pages deploys, uses the uploaded Worker's preview URL for digest sync and for the local `/_site-data/*` smoke lane so the static export is rehearsed against the exact candidate API before Worker production promotion
   ...
   - local artifact `smoke-ui` keeps the full overflow route set but splits route checks across two browser sessions by default; set `SMOKE_UI_OVERFLOW_WORKERS=1` to force serial overflow checks or up to `3` for more parallelism
...
`npm run test:merge-gate` now mirrors the deploy-path validate contract locally. If the changed-file set is not deploy-impacting, it prints the diff and exits successfully. For deploy-impacting diffs, it always runs `audit:deps`, `audit:pricing-providers`, lint, the root source `typecheck`, worker-boundary, shared-cycle detection, migrations, cron schedule/connection checks, `check:doc-counts`, `check:verified-doc-links`, `check:doc-source-paths`, `check:doc-sync`, `check:env-contract`, `check:frozen-invariants`, duplicate-export and redemption-backstop guards, unused-code, hotspot-ratchet, SQL-safety, `check:stablecoin-data`, `npm run test:noncritical`, and critical coverage. The critical coverage lane owns the critical test files, so the merge gate keeps the full deploy test surface without rerunning those files in the bare Vitest lane. The cycle step now blocks on cycles in `shared/`, `worker/src`, and `src`. It adds `npm run build` + `npm run seo:check` when Pages-impacting files changed, and adds both `npm run typecheck:worker` and `npm run typecheck:worker-scripts` when worker-impacting files changed. After `validate:prebuild`, independent build/non-critical-test/critical-coverage/typecheck groups run in parallel by default; a failing parallel group aborts siblings and reports the failing command explicitly. Set `MERGE_GATE_SERIAL=1` for the older serial execution shape. It still skips deploy-time smoke suites.
...
export default defineConfig({
  ...
  test: {
    ...
    exclude: [...configDefaults.exclude, ...worktreeExcludes, ".claude/**", ".next/**", "out/**", "coverage/**"],
    coverage: {
      provider: "v8",
      reporter: ["text", "lcov"],
      exclude: [
...
- `npm run test:critical-contracts` covers the explicitly enumerated critical handler suites (`cache-passthrough`, `peg-summary`, `report-cards`, `stability-index`, `dex-liquidity`, `stress-signals`, `mint-burn-flows`) plus shared strict-path registry tests and router mapping tests.
- `npm run test:invariants` covers numerical/schema invariants and cache-write validation guards in critical cron paths.
- `npm run test:merge-gate` runs a delta-aware local gate for merged worktree changes. It skips cleanly when no deploy surfaces changed, runs the shared validate core for deploy-impacting diffs, always adds the common postbuild `test:noncritical` and `coverage:critical` phase for deploy-impacting diffs, adds `build` + `seo:check` for Pages-impacting changes, and adds Worker runtime plus Worker operational-script typechecks for worker-impacting changes. Useful controls: `npm run test:merge-gate -- --staged`, `MERGE_GATE_BASE_REF=<ref>`, `MERGE_GATE_DRY_RUN=1`, and `MERGE_GATE_SERIAL=1`.
- `npm run test:smoke-api` performs HTTP-level smoke checks for `/api/health` plus every strict contract path mirrored from `shared/lib/api-endpoints/` and guarded by `src/lib/__tests__/api-endpoints.test.ts` (currently including `stablecoins`, `peg-summary`, `report-cards`, `stability-index`, `dex-liquidity`, `redemption-backstops`, `stress-signals`, and `mint-burn-flows`) with shape/range assertions, sequential endpoint execution, and bounded retries for transient failures.
...
- `npm run test:smoke-ui` performs a fast browser smoke check in either local or live mode. Homepage/GA checks run once; overflow checks use the same route source and assertions as before but are chunked across browser sessions (`SMOKE_UI_OVERFLOW_WORKERS`, default `2` local and `1` live, capped at `3`). Local mode keeps the full tracked mobile overflow route sweep against the built artifact, while live mode keeps the homepage/GA checks and a narrow mobile canary route set against the real host. Both modes fail on homepage outage/empty states (`Failed to load data` or `Failed to load this dataset`, `stablecoins:404`, `Data not yet available` or `Waiting for first sync`, `Connection issue` or `Unable to reach the Pharos data API right now.`, `No stablecoin data available`).
