...
6. `pages-release`:
   ...
   - serves the just-built `out/` export locally, proxies direct `/api/*` and `/_site-data/*` calls to the selected API base, injects `SITE_API_SHARED_SECRET` for the site-data proxy hop, runs `npm run test:smoke-ui -- --url http://127.0.0.1:4173 --mode local`, and runs a strict mobile canary pass (`npm run test:smoke-ui:mobile -- --url http://127.0.0.1:4173`) only when `pages_ui_changed=true`
