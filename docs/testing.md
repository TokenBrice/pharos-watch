...
2. `validate` (runs before any deployment):
   ...
   - `npm run check:shared-cycles`
     This now blocks on cycles in `shared/`, `worker/src`, and `src`.
...
| -------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `supply.test.ts`                       | `shared/lib/supply.ts`                                                | `sumPegBuckets`, `getCirculatingRaw`, `getPrevDayRaw`, `getPrevWeekRaw`, `getPrevMonthRawOrNull`                                                                                                                    |
...
| `api-endpoints.test.ts`                | `shared/lib/api-endpoints/`                                         | Endpoint registry invariants: probe groups, status actions, cache/method flags, strict contract path uniqueness, smoke assertion alignment                                                                          |
...
| `api-reference-doc.test.ts`            | `src/lib/api-reference-doc.ts`                                        | API reference document loading, section / subsection parsing, and endpoint organization                                                                                                                             |
| `blacklist-status-buckets.test.ts`     | `src/lib/blacklist-status-buckets.ts`                                 | Blacklist status bucket resolution, report-card overrides, and stablecoin filtering by status                                                                                                                       |
| `blacklist-status.test.ts`             | `src/lib/blacklist-status.ts`                                         | Resolved blacklist status, inherited-coin fallback, and report-card status override                                                                                                                                 |
| `cache-health.test.ts`                 | `shared/lib/cache-health.ts`                                          | Cache freshness and impact status, degradation handling, and source staleness                                                                                                                                       |
| `chart-utils.test.ts`                  | `src/lib/chart-utils.ts`                                              | Y-domain computation with padding and series merging by timestamp                                                                                                                                                   |
| `compare-config.test.ts`               | `src/lib/compare-config.ts`                                           | Canonical ID resolution, legacy symbol mapping, and ambiguous-symbol handling                                                                                                                                       |
| `compare-derive.test.ts`               | `src/lib/compare-derive.ts`                                           | Comparison coin derivation, supply/flow series building, card data mapping, and color cycling                                                                                                                       |
| `compare-pages.test.ts`                | `src/lib/compare-pages.ts`                                            | Comparison page blacklist status and at-a-glance row building                                                                                                                                                       |
| `contagion-layout.test.ts`             | `src/lib/contagion-layout.ts`                                         | Graph node layout, percentile computation, normalization, jitter, supernode state, and pruning                                                                                                                      |
| `explorer-url.test.ts`                 | `shared/lib/explorer.ts`                                              | Blockchain explorer URL building for TRON, Solana, and Starknet transactions / contracts                                                                                                                            |
| `mint-burn-coin-helpers.test.ts`       | `src/lib/mint-burn-coin-helpers.ts`                                   | 24h activity inference, pressure score / state resolution, and net direction derivation                                                                                                                             |
| `nav-config.test.ts`                   | `src/lib/nav-config.ts`                                               | Primary nav ordering, nav group structure, legacy Risk Lab exclusion, and Digest placement                                                                                                                          |
| `psi-history-events.test.ts`           | `src/lib/psi-history-events.ts`                                       | PSI chart event alternation, label positioning, and date-based filtering / ordering                                                                                                                                 |
| `public-status.test.ts`                | `src/lib/status/public-status.ts`                                     | Mint/burn status degradation, cache impact, circuit-breaker filtering, and worst-cache summary                                                                                                                      |
| `route-error-recovery.test.ts`         | `src/lib/route-error-recovery.ts`                                     | Hard-reload detection for stale chunks and dynamic-import failures                                                                                                                                                  |
| `sidebar-signals.test.ts`              | `src/lib/sidebar-signals.ts`                                          | Depeg, PSI, blacklist, health, and digest signal wiring with sidebar unread tracking                                                                                                                                |
| `stablecoin-url-codec.test.ts`         | `src/lib/stablecoin-url-codec.ts`                                     | Stablecoin ID encoding / decoding, legacy symbol mapping, and ambiguous-symbol rejection                                                                                                                            |
| `status-dashboard-model.test.ts`       | `src/lib/status-dashboard-model.ts`                                   | Browser probe summarization, freshness floor, top-causes filtering, and notice generation                                                                                                                           |
| `table-comparator.test.ts`             | `src/lib/table-comparator.ts`                                         | Table sorting by column, null value positioning, and ascending / descending direction                                                                                                                               |
| `total-mcap-chart.test.ts`             | `src/lib/total-mcap-chart.ts`                                         | Total market cap chart rows, per-coin history alignment, and future-snapshot prevention                                                                                                                             |
| `yield-benchmark.test.ts`              | `src/lib/yield-benchmark.ts`                                          | Yield benchmark frame selection, mixed-currency handling, and benchmark fallback resolution                                                                                                                         |
| `yield-constants.test.ts`              | `src/lib/yield-constants.ts`                                          | Yield warning signal formatting, PYS color mapping, and PYS breakdown computation                                                                                                                                   |

### Shared Library Tests (`shared/lib/__tests__/`)

| File                                          | Module Under Test                                                     | What It Covers                                                                                                                                       |
| --------------------------------------------- | --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `blacklist-active-records.test.ts`            | `shared/lib/blacklist-active-records.ts`                              | Active blacklist record building, balance snapshot integration, and summary stat derivation                                                         |
| `blacklist-aggregates.test.ts`                | `shared/lib/blacklist-aggregates.ts`                                  | Chart data construction, per-coin aggregation, and summary statistic composition                                                                    |
| `blacklist.test.ts`                           | `shared/lib/blacklist.ts`                                             | Amount valuation, price-asset ID mapping, and gold/commodity stablecoin handling                                                                     |
| `centralized-custody.test.ts`                 | `shared/lib/centralized-custody.ts`                                   | Centralized custody fraction computation and crypto-exchange custodial categorization                                                               |
| `chain-aggregator.test.ts`                    | `shared/lib/chain-aggregator.ts`                                      | Multi-chain supply aggregation and historical availability handling                                                                                 |
| `chain-circulating.test.ts`                   | `shared/lib/chain-circulating.ts`                                     | Chain alias normalization plus current and historical circulating supply derivation                                                                 |
| `chain-health.test.ts`                        | `shared/lib/chain-health.ts`                                          | Backing diversity, peg stability, and supply quality components for the Chain Health Score                                                          |
| `chains.test.ts`                              | `shared/lib/chains.ts`                                                | Chain metadata invariants, ordering, and explorer URL building                                                                                       |
| `classification-invariants.test.ts`           | `shared/lib/stablecoins.ts` + classification                          | Governance, backing, and custodial-exposure consistency across the tracked roster                                                                   |
| `commodity-median.test.ts`                    | `shared/lib/commodity-median.ts`                                      | Peer commodity median price series and per-ounce normalization                                                                                      |
| `dependency-graph.test.ts`                    | `shared/lib/dependency-graph.ts`                                      | Coin dependency resolution, graph construction, and cycle detection                                                                                 |
| `format.test.ts`                              | `shared/lib/format.ts`                                                | `formatCurrency`, `formatBps`, `formatPegDeviation`, `formatPercentChange`, `formatSupply`, `formatAddress`, `formatDuration`, and related helpers   |
| `math.test.ts`                                | `shared/lib/math.ts`                                                  | Math helpers: clamp, sum, weighted average, and percentile computation                                                                               |
| `mint-burn-signals.test.ts`                   | `shared/lib/mint-burn-signals.ts`                                     | Flow composite state, minting-pressure scoring, and pressure-band derivation                                                                        |
| `peg-rates.test.ts`                           | `shared/lib/peg-rates.ts`                                             | Peg bucket derivation, rate computation, and median price aggregation                                                                               |
| `peg-score.test.ts`                           | `shared/lib/peg-score.ts`                                             | Peg score composition, tracking-start semantics, and lookback window behavior                                                                        |
| `peg-utils.test.ts`                           | `shared/lib/peg-utils.ts`                                             | Depeg event merging, worst-deviation helpers, and duration derivation                                                                                |
| `pricing-source-registry.test.ts`             | `shared/lib/pricing-source-registry.ts`                               | Pricing-source registry ordering, invariants, and provider metadata                                                                                  |
| `psi-view-model.test.ts`                      | `shared/lib/psi-view-model.ts`                                        | PSI display composition, historical sample selection, and component breakdown                                                                       |
| `rate-series.test.ts`                         | `shared/lib/rate-series.ts`                                           | Rate-series interpolation, clamping, and time-based lookup semantics                                                                                 |
| `redemption-backstop-confidence.test.ts`      | `shared/lib/redemption-backstop-confidence.ts`                        | Capacity confidence inference, semantic resolution, and sourcing propagation                                                                         |
| `redemption-backstop-consistency.test.ts`     | `shared/lib/redemption-backstops.ts` + adapters                       | Live-adapter configuration and cross-module consistency checks                                                                                       |
| `redemption-backstop-scoring.test.ts`         | `shared/lib/redemption-backstop-scoring.ts`                           | Effective-exit score, capacity score, and cost/coverage balance math                                                                                  |
| `redemption-backstops.test.ts`                | `shared/lib/redemption-backstops.ts`                                  | Backstop configuration resolution, route modeling, and capacity tracking                                                                             |
| `report-cards.test.ts`                        | `shared/lib/report-cards.ts`                                          | Grade computation, dimension scorers, peg multiplier, dependency risk, and stress test                                                              |
| `reserve-asset-risk.test.ts`                  | `shared/lib/reserve-asset-risk.ts`                                    | Asset risk tier mapping, liquidity classification, and strategy keyword filters                                                                      |
| `reserve-risk-consistency.test.ts`            | `shared/lib/reserve-templates.ts` + risk                              | Reserve consistency checks, risk tier assignment, and category validation                                                                            |
| `runtime-origins.test.ts`                     | `shared/lib/runtime-origins.ts`                                       | Hostname canonicalization, site-data routing, and origin validation                                                                                  |
| `site-data-routes.test.ts`                    | `shared/lib/site-data-routes.ts`                                      | Path matching, upstream path resolution, and site-data route mapping                                                                                 |
| `stablecoin-id-registry.test.ts`              | `shared/lib/stablecoin-id-registry.ts`                                | ID consistency, alias handling, and dead / shadow coin tracking                                                                                      |
| `stablecoins.test.ts`                         | `shared/lib/stablecoins.ts`                                           | Metadata loading, canonical ordering, and asset data integrity                                                                                       |
| `status-metadata.test.ts`                     | `shared/lib/status-metadata.ts`                                       | Metadata type coercion, Telegram dispatch parsing, and primitive handling                                                                            |
| `status-thresholds.test.ts`                   | `shared/lib/status-thresholds.ts`                                     | Blacklist gap status, health band assignment, and threshold logic                                                                                    |
| `supply.test.ts`                              | `shared/lib/supply.ts`                                                | Peg bucket summation, circulating supply derivation, and time-window helpers                                                                         |
| `timeout-signal.test.ts`                      | `shared/lib/timeout-signal.ts`                                        | Timeout signal creation, race-with-timeout helper, and abort coordination                                                                             |
| `yield-rankings.test.ts`                      | `shared/lib/yield-rankings.ts`                                        | Ranking deduplication, variant consolidation, and pool normalization                                                                                |
| `yield-scoring.test.ts`                       | `shared/lib/yield-scoring.ts`                                         | PYS constants, weight thresholds, variance scoring, and risk penalties                                                                              |
...
| `admin-job.test.ts`                   | `worker/src/lib/admin-job.ts`                              | Admin job helpers, auth guards, parameter parsing, and response formatting                                                            |
| `api-freshness.test.ts`               | `worker/src/lib/api-freshness.ts`                          | Latest successful cron timestamp tracking and freshness evaluation state                                                              |
| `api-keys.test.ts`                    | `worker/src/lib/api-keys.ts`                               | API key lifecycle: creation, rotation, rate limiting, usage tracking, and authentication                                              |
| `blacklist-gaps.test.ts`              | `worker/src/lib/blacklist-gaps.ts`                         | Blacklist gap metrics, recoverable-gap telemetry, and temporal analysis                                                               |
| `cex-tickers.test.ts`                 | `worker/src/lib/cex-tickers.ts`                            | Binance, Bitstamp, Coinbase, and Kraken price fetching and symbol mapping                                                             |
| `cg-ticker.test.ts`                   | `worker/src/lib/cg-ticker.ts`                              | CoinGecko ticker fetching, symbol normalization, and price mapping                                                                     |
| `chainlink-feeds.test.ts`             | `worker/src/lib/chainlink-feeds.ts`                        | Chainlink price feed resolution and mainnet / fallback chain selection                                                                |
| `collateral-drift.test.ts`            | `worker/src/lib/collateral-drift.ts`                       | Reserve collateral drift detection and change significance thresholds                                                                 |
| `cron-metadata.test.ts`               | `worker/src/lib/cron-metadata.ts`                          | Cron run metadata, execution context, and dispatch tracking                                                                            |
| `curve-onchain.test.ts`               | `worker/src/lib/curve-onchain.ts`                          | Curve protocol on-chain pool data fetching and normalization                                                                           |
| `depeg-helpers.test.ts`               | `worker/src/lib/depeg-helpers.ts`                          | Depeg logic helpers: open / close decisions and event validation                                                                      |
| `depeg-pending.test.ts`               | `worker/src/lib/depeg-pending.ts`                          | Pending depeg state machine, confirmation paths, and state transitions                                                                 |
| `depeg-signals.test.ts`               | `worker/src/lib/depeg-signals.ts`                          | Depeg signal detection, aggregation, and severity computation                                                                          |
| `depeg-trust-policy.test.ts`          | `worker/src/lib/depeg-trust-policy.ts`                     | Trust policy for depeg events, source verification, and event credibility                                                             |
| `env.test.ts`                         | `worker/src/lib/env.ts`                                    | Environment variable contract validation, type coercion, and defaults                                                                  |
| `evm-rpc.test.ts`                     | `worker/src/lib/evm-rpc.ts`                                | EVM RPC node selection, fallback behavior, and call batching / rate handling                                                          |
| `fetch-retry.test.ts`                 | `worker/src/lib/fetch-retry.ts`                            | HTTP fetch with retries, backoff strategy, and transient failure handling                                                             |
| `fx-rate-state.test.ts`               | `worker/src/lib/fx-rate-state.ts`                          | FX rate state machine, cache expiry, and rate staleness detection                                                                      |
| `fx-realtime.test.ts`                 | `worker/src/lib/fx-realtime.ts`                            | Real-time FX rate fetching, rate freshness, and fallback mechanics                                                                     |
| `geckoterminal-price-probe.test.ts`   | `worker/src/lib/geckoterminal-price-probe.ts`              | GeckoTerminal DEX pool probing, price discovery, and liquidity scanning                                                                |
| `jwt-verify.test.ts`                  | `shared/lib/cloudflare-access-jwt.ts`                      | JWT signature verification, expiry checks, and claim validation                                                                        |
| `mint-burn-canonical-chain.test.ts`   | `worker/src/lib/mint-burn-canonical-chain.ts`              | Canonical chain resolution for mint/burn events and multi-chain deduplication                                                          |
| `mint-burn-health-config.test.ts`     | `worker/src/lib/mint-burn-health-config.ts`                | Mint / burn health threshold configuration and band assignment                                                                         |
| `mint-burn-parse.test.ts`             | `worker/src/lib/mint-burn-pipeline/parse.ts`               | Event log parsing, contract event decoding, and parameter extraction                                                                   |
| `mint-burn-roundtrip-sweep.test.ts`   | `worker/src/lib/mint-burn-pipeline/roundtrip-sweep.ts`     | Roundtrip detection sweep, transaction pairing, and matching logic                                                                     |
| `native-peg-implied-prices.test.ts`   | `worker/src/lib/native-peg-implied-prices.ts`              | Native-peg implied price calculation and backed-asset normalization                                                                    |
| `native-peg-quotes.test.ts`           | `worker/src/lib/native-peg-quotes.ts`                      | Native-peg quote generation and conversion rate computation                                                                             |
| `paged-token-pools.test.ts`           | `worker/src/lib/paged-token-pools.ts`                      | Paginated token pool fetching, cursor management, and response aggregation                                                             |
| `price-consensus.test.ts`             | `worker/src/lib/price-consensus.ts`                        | Multi-source price consensus, outlier rejection, and median computation                                                                |
| `price-publish-policy.test.ts`        | `worker/src/lib/price-publish-policy.ts`                   | Price publishing rules, stale-price suppression, and confidence gating                                                                 |
| `psi-benchmark-scenarios.test.ts`     | `worker/src/lib/stability-index.ts`                        | PSI benchmark scenarios for stablecoin stress patterns and regime changes                                                              |
| `psi-replay.test.ts`                  | `worker/src/lib/psi-replay.ts`                             | PSI historical replay, methodology-version handling, and DEWS stress inclusion                                                        |
| `pyth.test.ts`                        | `worker/src/lib/pyth.ts`                                   | Pyth protocol price feed retrieval and data validation                                                                                 |
| `rate-limit.test.ts`                  | `worker/src/lib/rate-limit.ts`                             | Rate limiter state machine, quota tracking, and window management                                                                      |
| `redemption-backstop-capacity.test.ts`| `worker/src/lib/redemption-backstop-capacity.ts`           | Backstop capacity estimation and protocol-specific capacity models                                                                    |
| `redemption-backstop-cost.test.ts`    | `worker/src/lib/redemption-backstop-cost.ts`               | Redemption cost modeling, fee estimation, and slippage calculation                                                                     |
| `redemption-backstop-sources.test.ts` | `worker/src/lib/redemption-backstop-sources.ts`            | Backstop source detection, protocol identification, and capability inference                                                          |
| `redemption-backstops-store.test.ts`  | `worker/src/lib/redemption-backstops-store.ts`             | Completed-run snapshot manifests, generation filtering, and row persistence                                                            |
| `redstone.test.ts`                    | `worker/src/lib/redstone.ts`                               | Redstone oracle integration and price data validation                                                                                  |
| `report-card-cache.test.ts`           | `worker/src/lib/report-card-cache.ts`                      | Report-card caching, snapshot freshness, and cache invalidation                                                                        |
| `report-cards-snapshot-topo.test.ts`  | `worker/src/lib/report-cards-snapshot.ts`                  | Report-card snapshot topology and dependency ordering for computation                                                                   |
| `request-source-attribution.test.ts`  | `worker/src/lib/request-source-attribution.ts`             | Request source attribution and API consumer tracking                                                                                   |
| `stablecoins-cache-validation.test.ts`| `worker/src/lib/stablecoins-cache.ts`                      | Stablecoins cache structure validation and schema compliance                                                                            |
| `status-evaluation-state.test.ts`     | `worker/src/lib/status/evaluation-state.ts`                | Status evaluation state tracking and probe result persistence                                                                          |
| `telegram-bot-stats.test.ts`          | `worker/src/lib/status/telegram-bot-stats.ts`              | Telegram bot statistics, subscription metrics, and usage tracking                                                                       |
| `telegram-digest-appendices.test.ts`  | `worker/src/lib/telegram-digest-appendices.ts`             | Digest appendix generation, cemetery / snapshot diffing, and deferred commits                                                          |
| `telegram-presets.test.ts`            | `worker/src/lib/telegram-presets.ts`                       | Telegram preset alert configuration and subscription templates                                                                          |
| `telegram-webhook-registration.test.ts` | `worker/src/lib/telegram-webhook-registration.ts`        | Telegram webhook registration, subscription management, and auth flow                                                                   |
| `tron-address.test.ts`                | `worker/src/lib/tron-address.ts`                           | TRON address parsing, validation, and format normalization                                                                              |
| `yield-source-links.test.ts`          | `worker/src/lib/yield-source-links.ts`                     | Yield source link resolution, protocol URL generation, and dapp mapping                                                                 |

### API Contract Tests (`worker/src/api/__tests__/`)

| File                                  | Handler                                                | Modes Tested                                                                                                                                         |
| ------------------------------------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `router-contract.test.ts`             | `route` + strict frontend contract paths               | All strict paths resolve in `worker/src/router.ts`, unknown paths return null, mutating admin GET guards hold (with audit/backfill repair preview exceptions) |
...
| `backfill-dews.test.ts`               | `handleBackfillDEWS`                                   | Admin auth, DEWS backtest response shape, current-row refresh preview/execution, and bounded history prune preview/execution                         |
...
| `api-key-audit-log.test.ts`           | `handleApiKeyAuditLog`                                 | Admin audit log endpoint, key event tracking, and pagination                                                                                         |
| `api-keys.test.ts`                    | `handleApiKeys`                                        | Admin API key management endpoint, authentication, and key lifecycle operations                                                                      |
| `backfill-depegs-dry-run.test.ts`     | `handleBackfillDepegs` (dry-run path)                  | Depeg backfill dry-run mode, preview behavior, and validation                                                                                        |
| `backfill-price-sources.test.ts`      | `handleBackfillPriceSources`                           | Admin price-source backfill and sourcing metadata updates                                                                                            |
| `blacklist-summary.test.ts`           | `handleBlacklistSummary`                               | Blacklist summary endpoint and per-coin aggregate derivation                                                                                         |
| `chains.test.ts`                      | `handleChains`                                         | Chains metadata endpoint and per-chain stablecoin derivation                                                                                         |
| `dews-history-repair.test.ts`         | `handleDewsHistoryRepair`                              | Admin DEWS history repair endpoint, gap filling, and recomputation                                                                                   |
| `dex-liquidity-response.test.ts`      | `formatDexLiquidityResponse`                           | DEX liquidity response formatting, field mapping, and serialization                                                                                  |
| `mint-burn-flows-shared.test.ts`      | `buildMintBurnFlowsShared`                             | Shared mint/burn flow helpers, aggregate construction, and per-coin modes                                                                            |
| `non-usd-share.test.ts`               | `handleNonUsdShare`                                    | Non-USD stablecoin market share calculation and reporting                                                                                            |
| `og.test.tsx`                         | `handleOgImage`                                        | OG image generation: layout, fallback, and Satori rendering                                                                                          |
| `public-status-history.test.ts`       | `handlePublicStatusHistory`                            | Public status history paginated endpoint and probe timeline querying                                                                                 |
| `reclassify-atomic-roundtrips.test.ts`| `handleReclassifyAtomicRoundtrips`                     | Admin roundtrip reclassification, atomic-swap detection, and state updates                                                                           |
| `remediate-blacklist-amount-gaps.test.ts` | `handleRemediateBlacklistAmountGaps`               | Admin blacklist remediation endpoint, gap healing, and amount correction                                                                             |
| `request-source-stats.test.ts`        | `handleRequestSourceStats`                             | Admin request-source statistics, API consumer metrics, and usage analytics                                                                           |
| `telegram-pulse.test.ts`              | `handleTelegramPulse`                                  | Telegram market-pulse endpoint, sentiment data, and flow summaries                                                                                   |
| `telegram-webhook-auth.test.ts`       | `handleTelegramWebhook` (auth path)                    | Telegram webhook auth validation, signature verification, and timing-attack prevention                                                               |
| `telegram-webhook-messages.test.ts`   | `handleTelegramWebhook` (message parsing)              | Telegram webhook message parsing, command extraction, and format handling                                                                            |
| `telegram-webhook-parsing.test.ts`    | `parseTelegramUpdate`                                  | Telegram update JSON parsing and callback / message / subscription handling                                                                          |
| `telegram-webhook-resolution.test.ts` | `resolveTelegramCommand`                               | Telegram command resolution and routing to subscription / admin handlers                                                                             |
...
| `sync-redemption-backstops.test.ts`     | `sync-redemption-backstops.ts`     | Stablecoins-cache gating, thresholded unresolved/missing-capacity status semantics, source-mode metadata, and current-row pruning                               |
...
| `challenger-persistence.test.ts`        | `challenger-persistence.ts`        | DEX price challenger publication, statement building, and snapshot metadata                                                                                    |
| `cron-stage-contracts.test.ts`          | `cron-stage-contracts.ts`          | Cron stage orchestration, slot / fan-out contracts, and dependency ordering                                                                                     |
| `dex-api-common.test.ts`                | `dex-api-common.ts`                | DEX API common helpers, request building, and response normalization                                                                                            |
| `dex-liquidity-direct-api.test.ts`      | `dex-liquidity-direct-api.ts`      | DEX liquidity direct API fetching, pool enumeration, and data collection                                                                                        |
| `dex-liquidity-fetch-primary.test.ts`   | `dex-liquidity-fetch-primary.ts`   | Primary DEX liquidity source fetching, retry behavior, and payload handling                                                                                     |
| `dex-liquidity-price-bridge.test.ts`    | `dex-liquidity-price-bridge.ts`    | DEX price bridge, price discovery across pools, and median consensus                                                                                            |
| `dex-liquidity-series-stability.test.ts`| `dex-liquidity-series-stability.ts`| DEX liquidity series stability analysis, spike detection, and trend validation                                                                                  |
| `mint-burn-run-state-rotation.test.ts`  | `mint-burn-run-state-rotation.ts`  | Mint / burn run-state rotation, checkpoint advancement, and sync progress                                                                                       |
| `pool-filter.test.ts`                   | `pool-filter.ts`                   | DEX pool filtering, quality thresholds, and relevance scoring                                                                                                   |
| `reserve-adapters.test.ts`              | `reserve-adapters.ts`              | Live reserve adapter testing, orchestration, and protocol integration                                                                                           |
| `reserve-adapters/__tests__/concurrency.test.ts` | `reserve-adapters/concurrency.ts` | Per-attempt reserve adapter I/O limiter, queue release after failures, and connection-budget guardrail coverage                                                 |
| `reserve-adapter-validate.test.ts`      | `reserve-adapter-validate.ts`      | Reserve adapter validation, contract compliance, and schema checks                                                                                              |
| `reserve-sync-integration.test.ts`      | `reserve-sync-integration.ts`      | Reserve sync end-to-end integration, adapter chaining, and result merging                                                                                       |
| `snapshot-chain-supply.test.ts`         | `snapshot-chain-supply.ts`         | Per-chain supply snapshots and chain-specific calculations                                                                                                       |
| `sync-kinesis-supply.test.ts`           | `sync-kinesis-supply.ts`           | Kinesis supply syncing and XAU stablecoin handling                                                                                                               |
| `sync-stablecoins-post-enrichment.test.ts` | `sync-stablecoins/post-enrichment.ts` | Post-enrichment stablecoin data processing, augmentation, and finalization                                                                              |
| `sync-yield-supplemental.test.ts`       | `sync-yield-supplemental.ts`       | Supplemental yield data syncing, secondary sources, and fallback behavior                                                                                        |
| `telegram-alert-snapshots.test.ts`      | `telegram-alert-snapshots.ts`      | Telegram alert snapshot generation, state persistence, and alert triggering                                                                                      |
| `telegram-pending-queue.test.ts`        | `telegram-pending-queue.ts`        | Telegram pending message queue, delivery state, and retry scheduling                                                                                             |
| `weekly-recap.test.ts`                  | `weekly-recap.ts`                  | Weekly recap generation, digest composition, and historical summary                                                                                              |
| `yield-aave-onchain-source.test.ts`     | `yield-aave-onchain-source.ts`     | Aave on-chain yield source, lending-pool query, and APY calculation                                                                                              |
| `yield-beefy-source.test.ts`            | `yield-beefy-source.ts`            | Beefy yield source integration and vault APY fetching                                                                                                            |
| `yield-bima-source.test.ts`             | `yield-bima-source.ts`             | Bima yield source, RWA pool integration, and yield provision                                                                                                     |
| `yield-cache.test.ts`                   | `yield-cache.ts`                   | Yield ranking cache, snapshot storage, and freshness management                                                                                                  |
| `yield-compound-v3-source.test.ts`      | `yield-compound-v3-source.ts`      | Compound v3 yield source, supply-rate retrieval, and market integration                                                                                          |
| `yield-config-registry.test.ts`         | `yield-config-registry.ts`         | Yield source configuration registry, protocol metadata, and pool mapping                                                                                         |
| `yield-coverage-audit.test.ts`          | `yield-coverage-audit.ts`          | Yield coverage audit, tracked-pool validation, and stablecoin coverage                                                                                           |
| `yield-evaluation.test.ts`              | `yield-evaluation.ts`              | Yield metric evaluation, APY / APR normalization, and performance scoring                                                                                        |
| `yield-hashnote-source.test.ts`         | `yield-hashnote-source.ts`         | HashNote yield source and tokenized-fund APY retrieval                                                                                                           |
| `yield-history-backfill.test.ts`        | `yield-history-backfill.ts`        | Historical yield data backfill, gap filling, and retroactive snapshots                                                                                           |
| `yield-history-snapshots.test.ts`       | `yield-history-snapshots.ts`       | Daily yield history snapshots, time-series persistence, and archive management                                                                                   |
| `yield-morpho-source.test.ts`           | `yield-morpho-source.ts`           | Morpho yield source and vault / market APY fetching                                                                                                              |
| `yield-ondo-source.test.ts`             | `yield-ondo-source.ts`             | Ondo yield source, product integration, and yield rates                                                                                                          |
| `yield-pendle-source.test.ts`           | `yield-pendle-source.ts`           | Pendle yield source, PT / YT market integration, and implied yields                                                                                              |
| `yield-publication.test.ts`             | `yield-publication.ts`             | Yield rankings publication, cache writes, and snapshot finalization                                                                                              |
| `yield-resolve.test.ts`                 | `yield-resolve.ts`                 | Yield pool resolution, pool matching, and stablecoin integration                                                                                                 |
| `yield-scrvusd-source.test.ts`          | `yield-scrvusd-source.ts`          | scrvUSD yield source and Curve vault integration                                                                                                                 |
| `yield-variant-scanner.test.ts`         | `yield-variant-scanner.ts`         | Yield variant scanning, pool-duplicate detection, and normalization                                                                                              |
| `yield-yearn-kong-source.test.ts`       | `yield-yearn-kong-source.ts`       | Yearn Kong yield source, vault APY retrieval, and token integration                                                                                              |
