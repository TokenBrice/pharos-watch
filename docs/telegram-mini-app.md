...
- **Private command replies.** `/start`, `/help`, `/presets`, `/settings`, `/list`, `/status <ticker>`, `/why <ticker>`, `/coverage <ticker>`, `/set`, `/mute`, `/unmutehours`, `/timezone`, `/unsnooze`, and `/health` attach Web App buttons where the reply can open a matching panel. Quick-subscribe confirmations in private chats also attach a per-coin tuning button. Group and supergroup replies keep their existing command and callback keyboards.
