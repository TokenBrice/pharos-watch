...
`src/app/pharoswatchbot/page.tsx` is the product-facing main page for PharosWatchBot and the wider Telegram feature set. It is linked from the top-nav `Resources` menu's `Updates & Alerts` column ("Alert Bot") and the homepage status strip.
