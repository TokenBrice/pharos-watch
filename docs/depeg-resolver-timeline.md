...
- **Degraded publication hardening.** Health-deferral runs still publish a degraded cache artifact. Valid previous DDR rows are preserved with stale live overlays and the current degraded reason; missing/invalid previous snapshots degrade to empty rows.
...
- **Source-row adoption and repairs.** Nearby same-coin/same-direction source rows can be adopted into an unsealed canonical incident through append-only links/revisions. Sealed incidents still require explicit repair authorization; strict live same-coin/same-peg/same-direction tail rows inside the merge window may be ledgered automatically, while ambiguous repairs remain manual.
