...
- Use `oracleRisk` only for reviewed oracle / collateral price-feed setup. Safety Score v8.1 consumes it only for crypto-backed CDP assets (`mechanismArchetype: "cdp"` plus `flags.backing: "crypto-backed"`), and missing reviews are neutral. Source reviewed profiles with protocol docs, audits, or verified contract references.
