...
- Use `oracleRisk` only for reviewed oracle / collateral price-feed setup. Safety Score v8.1 consumes it only for crypto-backed CDP assets (`mechanismArchetype: "cdp"` plus `flags.backing: "crypto-backed"`), and missing reviews are neutral. Direct non-variant CDPs can receive the oracle blend; tracked variants can carry research metadata for coverage but resolvable wrappers display inherited parent exposure instead of taking a duplicate direct blend. Source reviewed profiles with protocol docs, audits, or verified contract references; include `reviewedAt`, `reviewer`, `confidence`, and branch rows when collateral markets or chains use materially different oracle paths.
