...
7. An amountless Tron freeze is instead replayed from the token's own confirmed transfer ledger: the frozen balance is the address's cumulative signed TRC20 flow through the freeze millisecond, persisted as `amount_source=derived` with `provenance_source=trongrid-transfer-replay` only when the freeze receipt proves the stored event, no transfer shares the freeze millisecond, the ledger paginates to completion, and the cumulative flow reconciles exactly with a raw confirmed `balanceOf` read. Replay never overwrites an existing amount, a current-balance snapshot, or an operator-repaired value.

The scheduled replay lane (`worker/src/lib/blacklist/tron-amount-recovery.ts`) runs in the `sync-blacklist` maintenance tail under the same runtime window and subrequest budget as the EVM repair lane, capped at 24 rows and 40 history pages per row, with 120 history pages per run. Its candidates come from the same durable `blacklist_amount_repair_queue` rows as every other chain, so retry backoff and priority apply unchanged. A freeze younger than 15 minutes, a truncated ledger, an unmatched receipt, or a shared-millisecond transfer leaves the row unresolved for the next run instead of writing a guessed amount.
