...
Per-coin subscription writes have two deliberate modes: subscribe-style follows bump alert flags with `MAX(...)`, while settings-style overrides replace exactly one setting and mark the matching `alert_*_override` column so dispatch can tell an explicit off from a default zero. Settings-style depeg writes share one rule: `depeg on` preserves an existing worsening step, `depeg off` clears it, and `depeg-step <bps>` enables depeg while setting that step. `/subscribe ... depeg-step off` is still a follow with depeg enabled and no worsening-step threshold.
