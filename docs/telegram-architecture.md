...
| `TELEGRAM_RECAP_ROLLOUT_MODE` | No | Personalized recap rollout mode: `off` (default when unset or malformed), `dark` (DB-only projection), `canary` (exact chat-ID allowlist), `public` |
| `TELEGRAM_RECAP_ROLLOUT_CHAT_IDS` | No | Comma-separated exact chat IDs eligible in `canary` mode; ignored in every other mode |
