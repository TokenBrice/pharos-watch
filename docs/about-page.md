...
- On-chain reads now include Sim by Dune wallet-balance and DeFi-position APIs plus reviewed DefiLlama treasury adapters for protocol treasury coverage, alongside selected public chain RPC endpoints, Etherscan v2, TronGrid, Alchemy, and dRPC when a direct reserve probe needs chain-native access, including Frankencoin's ZCHF -> VCHF StablecoinBridge balance read for redemption-capacity telemetry.
