...
- The DEX data source group reflects the live liquidity and yield pipeline: DeFiLlama Yields and DeFiLlama Protocols, protocol-native yield APIs where needed (currently BIMA Earn), alongside Curve Finance API, The Graph, Fluid API + DexReservesResolver, Balancer API, Raydium API, Orca API, GeckoTerminal, and DexScreener.
