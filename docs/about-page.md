...
- The `Ratings & Reference` source group now explicitly includes Bluechip, Frankfurter, Open Exchange Rates, `fawazahmed0/exchange-api`, gold-api.com, and FRED DGS3MO. Keep this list aligned with `DATA_SOURCE_GROUPS` in `src/app/about/page.tsx` when the FX or benchmark stack changes.
- The DEX data source group reflects the live liquidity pipeline: DeFiLlama Yields and DefiLlama Protocols alongside Curve Finance API, The Graph, GeckoTerminal, and DexScreener.
