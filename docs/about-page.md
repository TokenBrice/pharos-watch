...
- The `/portfolio/` page now also surfaces a read-only `Protocol Treasury Stable Exposure` section sourced from reviewed public DefiLlama treasury adapters on GitHub plus Sim by Dune wallet-balance and DeFi-position reads. Keep that feature copy aligned with the route implementation when the treasury seed/model changes.
...
- Reserve-transparency sources now include issuer/protocol reserve APIs, dashboards, proof-of-reserve portals, and direct on-chain vault/accounting reads used for live reserve composition where available, with current examples including Anzen, Ethena, Falcon, infiniFi, M0, Mento Reserve, OpenEden, Re, USDD, USD.AI, Accountable, Tether, Frax, Circle, First Digital Labs, SG-FORGE, Paxos, Sky/MakerDAO, Chainlink PoR/NAV feeds, and Aave GHO.
- On-chain reads now include Sim by Dune wallet-balance and DeFi-position APIs plus reviewed DefiLlama treasury adapters for protocol treasury coverage, alongside selected public chain RPC endpoints, Etherscan v2, TronGrid, Alchemy, and dRPC when a direct reserve probe needs chain-native access.
