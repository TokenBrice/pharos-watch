...
- The `On-chain Reads & Events` roster names Dwellir as a supplemental last-position RPC operator: it is reachable only after every registry operator and adapter-pinned endpoint, it never replaces an existing provider, it is excluded from mint/burn and blacklist event scans, and an internal provider-parity monitor is the only lane that reads it by design.
...
- The pricing roster includes guarded CHFm FPMM and AUDm/CADm/COPm Broker sell quotes from Mento; USDm parent pricing and executable pool-state checks are required, without assumed FX parity. BD can use its reviewed Aerodrome BD/USDC sell route with fresh trusted USDC pricing, pinned runtime code, exact pool identity, current inventory and quote-depth checks. Legacy USDv can use exact-mint Jupiter direct quotes only with independent confirmed Meteora pool-state, inventory, depth and slot-freshness checks and a fresh trusted USDC parent.
