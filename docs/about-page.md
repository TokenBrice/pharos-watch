...
- The pricing roster includes guarded CHFm FPMM and AUDm/CADm/COPm Broker sell quotes from Mento; USDm parent pricing and executable pool-state checks are required, without assumed FX parity. BD can use its reviewed Aerodrome BD/USDC sell route with fresh trusted USDC pricing, pinned runtime code, exact pool identity, current inventory and quote-depth checks.
