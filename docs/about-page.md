...
- The `/portfolio/` page now also surfaces a read-only `Protocol Treasury Stable Exposure` section sourced from reviewed public DefiLlama treasury adapters on GitHub plus Sim by Dune wallet-balance reads. Keep that feature copy aligned with the route implementation when the treasury seed/model changes.
...
- On-chain reads now include Sim by Dune wallet-balance APIs and reviewed DefiLlama treasury adapters for protocol treasury coverage, plus selected public chain RPC endpoints alongside Etherscan v2, TronGrid, Alchemy, and dRPC when a direct reserve probe needs chain-native access.
