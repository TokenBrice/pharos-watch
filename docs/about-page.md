...
- On-chain reads now include selected public chain RPC endpoints, Etherscan v2, TronGrid, Alchemy, and dRPC when a direct reserve probe needs chain-native access, including Frankencoin's ZCHF -> VCHF StablecoinBridge balance read for redemption-capacity telemetry.
