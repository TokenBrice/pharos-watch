...
- The `Ratings & Reference` source group now explicitly includes Bluechip, Chainlink Data Feeds, Frankfurter, Open Exchange Rates, `fawazahmed0/currency-api`, ExchangeRate-API, gold-api.com, FRED `DGS3MO`, FRED `ECBESTRVOLWGTTRMDMNRT`, and the SNB current-rates page. Keep this list aligned with `DATA_SOURCE_GROUPS` in `src/app/about/page.tsx` when the FX or benchmark stack changes.
