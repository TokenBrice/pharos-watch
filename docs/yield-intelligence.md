...
The sync also records comparison-anchor freshness under `sourceCoverage.comparisonAnchorFreshness`. The summary includes the anchored row count, stale anchor count, oldest stale anchor age/source, bounded stale examples, and a truncation flag. `/api/status` exposes the same summary at `yieldHealth.comparisonAnchorFreshness` for operators. This is observability-only: stale-anchor warnings still follow the read-time `data-stale` rules above, and the freshness summary does not change source arbitration or scoring.
