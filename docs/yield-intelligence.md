...
- `sourceRisk.dependencyConcentration` is a reviewer-set, stablecoin-id-keyed signal (not auto-derived) capturing cross-venue concentration that per-venue tiering structurally misses. It adds `+0.10` (`medium`) or `+0.20` (`high`) to the source-risk penalty (`low` is a zero-penalty informational severity). The one penalty-bearing seed is `yvusdc-yearn` = Sky (`medium`): the vault's funded debt sits ~100% behind Sky governance (sUSDS + Spark), matching Yearn's own risk report. The map also carries informational `low`-severity (zero-penalty) entries for single-curator Morpho vaults (`gtusdc-gauntlet`, `gtusdcp-gauntlet`, `steakusdc-steakhouse`, `steakusdt-steakhouse`) that surface the curator coupling without an added penalty. Unseeded coins stay neutral.
- DEWS Yield Anomaly reads the derived `venueRiskTier` directly, so its `structured-medium-risk-venue` (+10) and `structured-high-risk-venue` (+25) branches now fire across the wider 61-venue registry as of DEWS v6.091; no DEWS threshold changed.
...
- DEWS methodology (introduced in v5.99, current v6.091) consumes a bounded subset of populated structured yield evidence inside the existing Yield Anomaly sub-signal: reward-heavy rows, thin or stale sources, source switches, high source-risk penalties, reviewed medium/high-risk venue tiers, and rank-attribution drivers for source-risk or source-switch moves. Missing, malformed, or neutral source-risk fields and legacy warning-only rows remain explicit no-ops.
...
8. Batch preload source-aware `yield_history` datasets (previous best source, previous TVL by source, 30d APY history by source) and legacy best-history fallbacks, chunking stablecoin ID lists so each D1 statement stays under the 100-bind limit; latest previous-best/previous-TVL rows are selected in SQL rather than loading all historical candidates into Worker memory
...
**Batch query policy:** No per-coin query loops are used for the three high-volume `yield_history` reads (previous exchange rate, previous TVL, 30d APY history). Previous-best and previous-TVL reads select one latest row per coin/source bucket in SQL, while 30d APY history remains chunked by stablecoin IDs and indexed by stablecoin ID in memory. Before loading previous-row candidates, the cron estimates the candidate count in D1; above the guardrail it returns a degraded result with `preservedPreviousRanking: true` instead of risking Worker `exceededMemory`. Cleanup deletes also chunk ID lists to stay under D1's 100-bound-parameter cap.
...
```json
{
  ...
  "benchmarks": {
    ...
    "RUB": { "key": "RUB", "label": "RUB CBR key rate", "currency": "RUB", "rate": 14.5, "recordDate": "2026-06-11", "source": "cbr-key-rate", "isFallback": false, "fallbackMode": null, "isProxy": false },
