...
- **Current methodology version:** `v8.291`
...
- `worker/src/cron/yield-sync/source-risk.ts` owns the sparse typed `yieldRiskConfig` home for reviewed venue tiers. The reviewed venue set now assigns `aave-v3`, `compound-v3`, `sparklend`, `spark-savings`, `yearn`, `yearn-finance`, and `pendle` to `low`; `maple`, `morpho`, `morpho-v1`, `morpho-blue`, and `beefy` to `medium`. Unknown or unreviewed venues remain neutral. Review is tied to the monthly yield coverage audit; guessed venue penalties are explicitly out of scope.
- Low remains a no-op in PYS because no negative source-risk bonus exists. Medium contributes the existing +0.15 source-risk penalty. DEWS Yield Anomaly adds a bounded `structured-medium-risk-venue` branch at +10 for reviewed medium venues in DEWS v6.07, while the existing high-risk venue branch remains +25.
