...
- Freshness eligibility is applied before confidence arbitration. Expired candidates stay auditable, and every benchmark key used by published rows is evaluated independently; the hard benchmark scoring TTL is 48 hours of fetch age plus a per-key observation-age bound (`YIELD_BENCHMARK_RECORD_MAX_AGE_SEC` in `worker/src/cron/yield-sync/benchmarks.ts`: 5 days for daily/overnight series, 7 days CHF, 10 days TRY, 12 days RUB, 45 days for CAD's monthly series), evaluated as `max(fetch age, record age)` — a fresh fetch carrying a stale observation is also stale. Post-RF1 the bound is enforced at the producer (`fetch-tbill-rate` USD/USD_EFFR observation-age guards), in the registry health (`yieldHealth.benchmarkRegistry` per-key classification in `worker/src/lib/status/yield-health.ts`), and in the evaluation/publication classifiers (`prepareYieldEvaluation` per-row freshness in `worker/src/cron/yield-sync/evaluation.ts`, the publication NR classifier in `worker/src/cron/yield-sync/publication-ranking-payload.ts`, and the read-path live-safety recompute in `worker/src/api/yield-rankings-cache.ts`). The USD reference the v8.43 re-base consumes is gated by the same classifier before it touches any row.
...
Coordinator evaluation reuses one `computePysComponents()` result for the score and null explanation. The standalone full-input APIs retain their finite-positive APY/scaling guards before component computation. Two fail-closed guards bound the inputs: a nominal `apy30d` above `PYS_APY_SANITY_MAX` (300) is a source-math failure and scores 0 instead of clamping toward a perfect 100, and a non-finite variance fails closed to maximum variance (sustainability multiplier at its 0.3 floor) while a *missing* variance keeps the documented neutral 0. `apyVarianceScore` is derived from the published two-decimal `yieldStability`, so served and published scores agree.
...
- `sourceRisk.rewardShare` resolves once per publication from the same payload the penalty uses: a base-only row (`apyReward` null with `apyBase` at `currentApy`) publishes `rewardShare: 0` instead of null, and the never-expiring stored-value fallback is gone, so the published field can no longer disagree with the penalty input on the same row.
- `sourceRisk.sourceSwitchCount30d` counts only publications where the previous winner was a live, non-rejected candidate in the current set: a transiently missing previous source records the `previous-source-transiently-missing` anomaly instead of a switch, and runs shorter than two publications contribute no count.
...
**Observation-age bounds:** Each key carries a per-key bound on its own observation (`recordDate`) age — `YIELD_BENCHMARK_RECORD_MAX_AGE_SEC` in `worker/src/cron/yield-sync/benchmarks.ts` — combined with the 48-hour fetch TTL as `max(fetch age, record age)` and enforced at the producer, in the registry health, and in the evaluation/publication classifiers. Daily/overnight series get 5 days: `USD`, `USD_EFFR`, `EUR`, `GBP`, `JPY`, `MXN`, `BRL`, `AUD`, `SGD`. The calendar-matched bounds are `CHF` 7 days (SAR3MC publishes T+1 and the Good Friday + Easter Monday cluster lands the newest print exactly 5d old), `TRY` 10 days (TCMB Kurban/Bayram holiday clusters can leave the newest print past 5d), `RUB` 12 days (CBR key-rate prints sit ~9-10 days old across the Jan 1-8 holiday cluster), and `CAD` 45 days (the Bank of Canada's monthly print).
...
| `reference-benchmark-degraded` | The USD reference the v8.43 hurdle re-base consumes is retained or otherwise degraded | The row's PYS is estimated until the USD reference refreshes |
...
- Public rankings expose only a validated published generation. Responses carry an optional strict `_meta` freshness envelope (`updatedAt`, `ageSeconds`, `status: fresh|degraded|stale`). Failed validation, stale-writer, or publication attempts leave the previous public snapshot intact.
- Publication validates candidates before building alternate arrays and decision summaries, then validates each completed asset ranking independently. A candidate with non-finite required APY, alternate TVL, or derived spread is omitted from row counts and attribution; an invalid asset cannot freeze unrelated rankings. Non-finite stored history APY is likewise excluded from trailing-window statistics.
- Rejected diagnostic winners are excluded from selected-source and benchmark health, matching their exclusion from public rankings. Their rejection evidence and the tracked/published coverage guards remain active. A blocked publication carries the run's `inputDiagnostics` counters on the guard result, because that result replaces the run metadata.
- Expired selected-source evidence remains a documented publication fallback: the validated ranking generation is written and the cron result stays degraded with the source key in its `yield-source:expired-selected:*` reason. Such a publication does not advance `freshness:yield-data`; publication continuity is not a clean-freshness claim. `/api/health` therefore treats a fresh `yield_data` table fallback as degraded when the sentinel is absent, and treats a recent last-clean sentinel as degraded while cron history records later degraded runs; a clean recovery run advances the sentinel and clears that quality verdict.
...
- Missing, incomplete, or evaluation-incompatible live safety hydration falls back to the cached payload's own publish-time safety values — coherent by construction, published unchanged with `yield-safety-hydration-stale` and `liveSafetyHydration.fallback: "publish-time-snapshot"` — for up to the 24-hour stale-coherent window (`shared/lib/yield-safety-fallback.ts`). `liveSafetyHydration.reason` is a comma-joined list naming every applicable reason, including the upstream snapshot's own reason, and the publish-time-snapshot fallback path counts safety coverage as 0 under the provenance rule. Safety-derived public fields are cleared to explicit NR only when the payload carries no stamped safety identity or the fallback ages past that window; both variants preserve the immutable hourly snapshot provenance. The publish-time fallback is a body-level freshness note and emits no HTTP `Warning` header (clients render that header as a data-quality banner on every page that reads yield rankings); the NR variant and partial live-hydration degradation still emit `Warning: 199`.
