...
| `aging`            | Observation age past the midpoint of its class's cadence-to-stale window (2h hourly, 5h supplemental, 27h price-derived, 30h rate-derived, 48h slow NAV) | Read-time-only: the row missed its refresh slot; a refresh is expected before the stale bound |
...
- Rejected diagnostic winners are excluded from selected-source and benchmark health, matching their exclusion from public rankings. Their rejection evidence and the tracked/published coverage guards remain active.
