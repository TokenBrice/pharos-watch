...
- A final resolve-stage eligibility pass runs after linked-variant projection and before evaluation/arbitration. It removes every `lending-opportunity`, `fixed-yield`, and `structured-tranche` candidate whose measured `sourceTvlUsd` is null, non-finite, or below `max(chain absolute floor, 0.1% of current tracked supply)`, across tracked, explicit, auto-discovered, supplemental, and linked-variant paths. Structured-tranche rows additionally retain Royco's bespoke market/vault floors.
