...
- Source-risk coverage ratios are measured over per-field eligible rows: depth and tier exclude rows whose venue is the asset itself (`price-derived`/`rate-derived`), and `rewardShare` counts only lanes that can publish a split. The `venueProtocol`/`venueChain` denominators intentionally stay whole-corpus so the resolver change reads as a real drop, and best-row and alt-row ratios are reported separately.
