...
| DEX liquidity + DEX implied prices | DefiLlama Yields, Curve, Uniswap/Aerodrome subgraphs, plus `dex_pool_staging` rows produced from CoinGecko Onchain/GeckoTerminal/DexScreener/CoinGecko Tickers discovery | `worker/src/cron/dex-liquidity/*` orchestrator + staged-pool merge | `dex_pool_staging` -> `dex_liquidity`, `dex_liquidity_history`, `dex_prices` | `GET /api/dex-liquidity`, `GET /api/dex-liquidity-history` | `useDexLiquidity`, `useDexLiquidityHistory` | Liquidity page, Compare, report-card inputs |
