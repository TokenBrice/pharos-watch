...
|-------|------------------|--------------------------|---------------|-------------|------------------|------------|
| Stablecoin core list + prices | DefiLlama, CoinGecko, CMC, DexScreener, protocol redemption quotes, FX sources | `worker/src/cron/sync-stablecoins.ts` (+ `worker/src/cron/enrich-prices.ts`, `worker/src/cron/protocol-price-overrides.ts`, `worker/src/lib/price-validation.ts`) | `cache` key: `stablecoins` | `GET /api/stablecoins` | `useStablecoins` | Homepage, Compare, Dependency Map, many cards |
