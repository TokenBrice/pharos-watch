...
|-------|------------------|--------------------------|---------------|-------------|------------------|------------|
| Stablecoin core list + prices | DefiLlama, CoinGecko, CMC, DexScreener, Pyth Network, Binance, Coinbase, RedStone, Curve on-chain, protocol redemption quotes, FX sources (Frankfurter, Open Exchange Rates) | `worker/src/cron/sync-stablecoins.ts` (+ `worker/src/cron/enrich-prices.ts`, `worker/src/lib/authoritative-price-sources.ts`, `worker/src/lib/price-consensus.ts`, `worker/src/lib/price-validation.ts`) | `cache` key: `stablecoins` | `GET /api/stablecoins` | `useStablecoins` | Homepage, Compare, Dependency Map, many cards |
