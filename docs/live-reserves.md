...
`hive-hbd-protocol` reads Hive consensus state (dynamic global properties, median price feed and treasury account) from two distinct public JSON-RPC node hosts against a pinned chain configuration. It brackets each node's material reads with identical head state and requires cross-node agreement on supplies, thresholds, the median feed, treasury HBD, and the derived debt ratio. The second URL is a mandatory corroborating node, so the generic fallback path cannot substitute it for the primary. A coherence failure permits one complete two-node resample within the same 19-second signal; both first samples settle before retry, and Pharos request caching is bypassed for every bracket read. Each sample uses six POSTs across the two nodes with at most two concurrent requests. Upstream node/proxy caching remains a source limitation; matching results do not establish a cache-free historical state proof. Treasury-adjusted debt arithmetic uses bigint fixed-point; the adapter emits one unlinked 100% high-risk endogenous-HIVE mechanism slice with `not-applicable` freshness and deliberately no USD reserve totals, collateralization ratio, or redemption-capacity claims. Print-stop state is an informational warning and hard-limit state degrades the snapshot.
...
`sky-makercore` preserves positive-debt unrecognized groups inside the high-risk `Other modules` slice and reports their combined `unknownExposurePct`. Discovering a new group is informational by itself; the shared 5% material-unknown policy determines whether the snapshot degrades. Zero-debt unknown groups do not emit warnings.
