...
`frax-balance-sheet` now covers both `frxusd-frax` and legacy `frax-frax` through the Frax v2 balance-sheet API. Its diagnostic source origin is `issuer-attested` because Frax publishes the API; that label is separate from the adapter's unchanged score-bearing `evidenceClass`. Known Frax ecosystem assets are classified explicitly, including the Erebor reserve account, locked VELO position, mixed sfrxUSD/scrvUSD Curve LP, and small USDT, BNB, and MATIC balances; the mixed LP remains unlinked rather than being misrepresented as pure crvUSD exposure. Any future unmapped balance-sheet exposure is aggregated and only degrades the run when material. For frxUSD redemption modeling, the adapter emits current stablecoin capacity as a USD amount and intentionally avoids reusing reserve-composition ratios as supply-relative redemption capacity.

`frax-fpi-collateral` parses Frax's `/v2/fpifpis/fpi-collateral` endpoint for FPI. The reserve evidence origin is `issuer-attested` because the admitted composition comes from Frax's API; the nested Controller Pool observation does not relabel the reserve payload as independent on-chain evidence. It excludes self-held FPI from reserve slices, nets self-held FPI against liabilities, classifies known Frax ecosystem assets, and degrades only when unmapped non-FPI collateral exposure becomes material. The same adapter run also records a V9-only Controller Pool redemption attempt from one pinned Ethereum block. The attempt validates dependency bytecode, oracle round freshness and agreement, CPI tracker identity and age, pause and peg-band state, fee, quote, FRAX balance, and maximum input capacity. A rejected attempt remains diagnostic and adds an informational warning; an accepted attempt is available only to the V9 extension route. The issuer collateral response and route attempt are published together, so an issuer API failure publishes neither.
