...
Adapters that can emit mapped dependency targets are enrolled in the reviewed mapping registry in `scripts/lib/dependency-target-dispositions.ts`. Each record names the adapter, review provenance, and mapped canonical targets; the dependency coverage gate rejects a missing, stale, or source-free mapping review. This registry reviews the mapping rule rather than duplicating provenance onto every live slice.
