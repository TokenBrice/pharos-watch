...
`hive-hbd-protocol` reads Hive consensus state (dynamic global properties, median price feed, treasury account, chain config) from two public JSON-RPC nodes, brackets each node's reads with identical head IDs, and requires cross-node agreement on supplies, thresholds, the median feed, treasury HBD, and the derived debt ratio before accepting a snapshot. Treasury-adjusted debt arithmetic uses bigint fixed-point; the adapter emits one unlinked 100% high-risk endogenous-HIVE mechanism slice with `not-applicable` freshness and deliberately no USD reserve totals, collateralization ratio, or redemption-capacity claims. Print-stop state is an informational warning and hard-limit state degrades the snapshot.
