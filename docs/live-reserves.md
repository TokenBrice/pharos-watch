...
`flying-tulip-ftusd` consumes Flying Tulip's timestamped ftUSD dashboard API, requires the reviewed Ethereum and Sonic chain set and exact USDC/USDT/USSD collateral addresses, aggregates collateral by underlying, and retains borrow/stake leverage and health-factor diagnostics. It is deliberately `weak-live-probe` with `issuer-attested` origin: the snapshot is visible on reserve detail and status surfaces but never substitutes for independent collateral assurance or scoring input.
