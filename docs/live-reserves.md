...
the `liquity-v2-branches` adapter reads branch ActivePool collateral balances, DefiLlama prices, ActivePool debt, optional branch shutdown status, and optional live redemption-fee telemetry. `bold-liquity`, `feusd-felix`, `usdq-quill`, `nect-beraborrow`, and `cdp-enosys` use this path so their reserve slices remain branch-collateral based while nested redemption telemetry uses aggregate branch debt as `capacityUsd` with `capacityKind = "live-direct-bounded"` and `freshnessKind = "same-run-onchain"`. BOLD additionally configures protocol-priced mechanism telemetry: one bounded Multicall3 packet reads BOLD total supply, each TroveManager's fresh price/redeemability tuple, and each branch Stability Pool's committed BOLD deposits. A supply/debt reconciliation gate must pass before the adapter publishes `collateralizationRatio` and `liquidationCapacityRatio`; unavailable or unreconciled optional telemetry is omitted with an informational warning so the reviewed detail-page values remain the fallback, while an observed ratio below 1.0 degrades the snapshot. The Beraborrow binding also supports ERC-4626-style Collateral Vault share branches, branch `fetchPrice()` fallback, and per-branch `getRedemptionRateWithDecay()` fee telemetry.
