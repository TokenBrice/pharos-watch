...
`frax-fpi-collateral` parses Frax's `/v2/fpifpis/fpi-collateral` endpoint for FPI. It excludes self-held FPI from reserve slices, nets self-held FPI against liabilities, classifies known Frax ecosystem assets, and degrades only when unmapped non-FPI collateral exposure becomes material. The same adapter run also records a V9-only Controller Pool redemption attempt from one pinned Ethereum block. The attempt validates dependency bytecode, oracle round freshness and agreement, CPI tracker identity and age, pause and peg-band state, fee, quote, FRAX balance, and maximum input capacity. A rejected attempt remains diagnostic and adds an informational warning; an accepted attempt is available only to the V9 extension route. The issuer collateral response and route attempt are published together, so an issuer API failure publishes neither.
