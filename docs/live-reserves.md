...
The single lightweight declaration in `shared/types/live-reserve-adapter-declarations.ts` owns each adapter key, its params-schema identifier and accepted primary input kinds, config validation policy, public source definition, provenance status, and display badge metadata. `shared/lib/live-reserve-adapter-descriptors.ts` resolves those identifiers to Zod schemas and publishes `LIVE_RESERVE_ADAPTER_DEFINITIONS` as the single enriched map for config, provenance, display, and adapter consumers. `LIVE_RESERVE_ADAPTER_KEYS` and `LiveReserveAdapterKey` remain declaration-derived key views; key-only consumers do not initialize the Zod schema catalog. Five important definition properties are not user-configured per coin:
...
- `single-bucket`: one-slice live proofs/attestations. Some are true independent evidence (`anzen-usdz`, `astherus-earn-wrapper`, `blast-usdb-yield-manager`, `btcfi`, `chainlink-nav`, `chainlink-por`, `chronicle-nav`, `erc4626-single-asset`, `escrow-balance`, `hive-hbd-protocol`, `initia-wrapper-vault`, `liquity-native-active-pool`, `liquity-v1`, `m0`, `m0-wrapper-underlying`, `pusd-vault`, `ripple-transparency`, `sgforge-coinvertible`, `sgho-wrapper`, `spiko-api`, `superstate-liquidity`, `united-por`, `usd1-bundle-oracle`, `usdai-hub`, `yamato`), while weak liveness-only or proof-class summary feeds such as `single-asset`, `solstice-attestation`, and `river-protocol-info` are tagged `weak-live-probe`.
...
These policy warnings still preserve the last-known live snapshot for detail/status surfaces, but they keep it out of current V9 Backing/dependency fact compilation because scoring accepts only independent snapshots whose latest sync state is `ok`.
...
`usdai-hub` promotes USDai from curated-validated to independent evidence: one Arbitrum run reads the hub's PYUSD balance, canonical USDai `totalSupply()`, `bridgedSupply()`, pause state, and the proxy's EIP-1967 implementation, requires PYUSD balance ≥ bridge-safe liabilities, and fails closed on identity, implementation-pin, or reconciliation drift. It emits one measured 100% PYUSD slice (`pyusd-paypal` dependency) with `not-applicable` freshness and retains the existing whitelisted redemption-capacity telemetry.
...
`usdai-proof-of-reserves` consumes USD.AI's public on-chain reserve index, preserves oversized fixed-point `share` and `amount` integers from the raw JSON payload, groups the many hardware-loan `DEAL` rows into a single high-risk loan slice, and exposes liquid reserve buckets such as PYUSD separately. The accounting can be independently reconstructed from Arbitrum: the liquid PYUSD amount is the USDai contract's token balance, while LoanRouterV2 events enumerate loan hashes and `loanState(bytes32)` exposes current outstanding principal. That does not independently prove the physical GPU mapping, appraisal, insurance, borrower-SPV structure, or legal enforceability represented by those on-chain loans. Because Pharos still ingests USD.AI's first-party index and its off-chain collateral descriptions, the adapter's diagnostic source origin remains `issuer-attested`; this describes the ingestion path and residual off-chain claims, not opaque token or loan balances. Its separate score-bearing `evidenceClass` is unchanged, while the detail page uses the proof-style badge so it does not describe the entire mixed feed as independently measured. The adapter prefers explicit `share` weights when the share-bearing rows already cover the full published mix, ignores auxiliary amount-only rows in that case with an informational warning, and falls back to `amount` weighting only if the feed stops publishing share values entirely. Because the API endpoint itself does not publish a trustworthy disclosure timestamp, the adapter hydrates verified freshness from USD.AI's public reserves page when available, using the oldest scoped proof-row timestamp as the freshness bound while retaining oldest/latest spread metadata for review; otherwise snapshots stay `freshnessMode = "unverified"`. As of April 4, 2026, Pharos binds this mixed reserve feed to `susdai-usd-ai`, not to base `usdai-usd-ai`, because the public API is protocol/yield-side collateral rather than a clean base-token reserve proof.
...
Both adapters fail closed on identity drift, non-positive backing, zero supply, malformed required payloads, and required source failure: RPC failure for Astherus and LCD request failure for Initia.
...
Solstice [documents weekly independent verification](https://docs.solstice.finance/legal-documents/attestations-and-proof-of-solvency). Its adapter source-age limit is 700,000 seconds, matching USX's reviewed weekly reporting allowance with grace. This replaces the generic three-day dashboard limit for this adapter only. A report older than that bound still degrades, and a successful fetch does not refresh its report timestamp or change its weak-live-probe evidence class.
...
- DefiLlama spot-price loading for valuation (`fetchDefiLlamaPrices`), with fixed-price overrides supported for wrapper branches in `evm-branch-balances`; tracked branch assets can also reuse a fresh stablecoins cache price when DefiLlama address pricing is missing. Within a run the upstream price fetch is deduped by requested asset identity, while resolved prices are cached per caller-key set and returned as a caller-owned map, so two callers pricing the same address each get their own logical keys and callers that extend the result with fallback prices cannot pollute the cache
...
To register a new adapter for a coin's `liveReservesConfig.adapter`, edit these surfaces in order. The shared schema and Worker registry tests fail if definition or fetcher coverage drifts.

1. **Params schema, when needed** — define a reusable Zod schema in `shared/lib/live-reserve-adapter-param-schemas.ts` and expose it through `LIVE_RESERVE_PARAM_SCHEMAS`. Reuse `none` or another existing schema when the shape already matches.
2. **Descriptor declaration** — add one entry to `LIVE_RESERVE_ADAPTER_DESCRIPTOR_DECLARATIONS` in `shared/types/live-reserve-adapter-declarations.ts`. Declare accepted primary input kinds, params-schema identifier, source/evidence class, source-sharing policy, supported semantics/versions, redemption telemetry, validation policy, and only non-default provenance or display metadata. The key union and enriched `LIVE_RESERVE_ADAPTER_DEFINITIONS` map derive from this entry.
...
- [Safety Score V9](./report-cards.md) uses fresh authoritative independent live reserve slices as Backing reserve exposures when available. `dynamic-mix` adapters can qualify when their latest sync state is `ok` **and** the snapshot carries scoring-eligible freshness evidence; only a subset of `single-bucket` adapters carry `evidenceClass = independent`, while `validated-static` / `weak-live-probe` feeds remain detail/status data only. The compiler reconciles accepted tracked slices to effective dependency edges, and falls back to reviewed static/curated rows only when no accepted live rows were captured.
