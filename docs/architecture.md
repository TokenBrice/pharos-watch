...
- **ADR-5 — Historical penalty-only score blends.** Safety Score V8 applied reviewed Mint Authority, CDP oracle, and bridge-route scores to its Decentralization dimension through penalty-only blends. Canonical V9 retires that numeric blend and compiles the underlying reviewed evidence into typed Backing, Exit, and Economic Control facts, gaps, and caps. The historical rule remains documented so archived V8 results are reproducible; it is not an active V9 scoring path. See `report-cards.md` and `classification.md`.
...
- **ADR-10 — Cron trigger soft cap is 32 with static ownership.** The former 20-expression soft cap predated the fully source-owned schedule registry, slot fencing, per-trigger connection-budget validation, and compact producer-history read model. The audited topology remains below Cloudflare's account trigger limit and every expression still has one `worker/wrangler.toml` declaration, a `shared/lib/cron-jobs.ts` budget, a `shared/lib/scheduled-runner-registry.ts` path, lease/status ownership, and sync tests. The soft cap is 32; crossing it requires another ADR and consolidation/rebalance plan. This cap is an internal operability guard, not permission to add unaudited triggers.
