...
| `GET /api/request-source-stats`              | Admin machine-readable total site-vs-external demand summary, including Pages `/_site-data/*` delivery telemetry plus `public-api` / `site-api` worker-lane load. Preferred access is `ops-api.pharos.watch/api/request-source-stats` with Access service-token headers |
...
| `GET /api/treasury-stable-exposure`          | Daily snapshot of reviewed protocol / DAO treasury stablecoin sleeves with explicit direct-wallet totals, treasury-denominator status, decentralized stable exposure, and stable-sleeve weighted grades                       |
...
└── src/
    ...
    │   ├── reserve-adapters/    # Per-protocol live reserve adapters (33 adapters)
