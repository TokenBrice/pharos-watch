...
| `POST /selector-snapshot`          | Pages Function (`functions/selector-snapshot/[[path]].ts`): stores an exact, tracked-identity-derived, semantically normalized `SelectorOutput` projection with `provenance: "client-unverified"` under a server-recomputed `sid`. The 100 KB cap is streaming; HMAC-IP rate/quota keys require a dedicated pepper. |
| `GET /selector-snapshot/:sid`      | Pages Function: returns the client-unverified frozen projection or `404`. It recomputes the sid, returns `502` for corrupt/mismatched values, and returns `503` when a first-read five-year retention extension cannot be confirmed.                                                                                |
