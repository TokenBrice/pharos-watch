...
- `src/app/sitemap.ts` owns sitemap output for indexable routes. `/compare/` and `/compare/[slug]/` static comparison pages are included; `/digest/` is included while digest detail sitemap promotion is limited to all weekly recaps plus the newest bounded daily slice from `selectSitemapDigestEntries(...)`; methodology changelog sitemap membership is the explicit `METHODOLOGY_CHANGELOG_SITEMAP_PATHS` allowlist. `/portfolio/`, `/admin/`, `/admin-api/`, and `/pharoswatchbot/app/` are omitted. `/funding/` uses the latest of route edit time and checked-in funding data timestamps for `lastModified`. `LAST_EDITED` dates are auto-generated from git history during prebuild (`scripts/maintenance/generate-sitemap-dates.ts`) and written to a generated JSON file (gitignored). Public docs use `scripts/maintenance/generate-docs-metadata.ts` for git-derived first/last modified dates.
