...
Since Safety Score methodology 9.48, wholly unmapped live compositions contribute no dependency edges rather than restoring old curated weights. Compiled dependency facts retain per-slice `rejectionReasons` (`no-match`, `expired`, or reviewed `non-link`) with zero-based `sliceIndex`; curated fallback is considered only when no live composition exists.
