...
- scales nodes up and shows ticker labels when the neighborhood is small — 1.5x at ≤10 visible nodes, 2x at ≤5. The ceiling is deliberately low: raster logos in `public/logos` are 50px sources, and `MAX_RASTER_LOGO_RADIUS` in `contagion-graph-svg.tsx` additionally caps the drawn image so a sparse map never upscales one into pixelation. Vector (`.svg`) logos are exempt and keep filling the node;
