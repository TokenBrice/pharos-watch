...
The reviewed set lives in the committed evidence sidecar `worker/src/lib/mint-burn-conservation-reviewed.json`: one entry per config identity (chain, stablecoin id, lowercase address, decimals) carrying the reviewer's identity evidence (explorer-verified source, external address match, on-chain decimals), supply-path review and at least one audited window proving `mint − burn = totalSupply delta` exactly. 15 identities across 14 assets (the thirteen investigated on September 15 plus both BUIDL Ethereum share classes) are currently admitted; a config with no entry falls back to `unreviewed-contract-or-event-semantics`. Admission additionally requires ordinary zero-address `Transfer` semantics and the `transfer-zero-address` adapter. An `unsupported` entry carries a reason from the fixed vocabulary — `rebasing-supply-without-events`, `unpaired-supply-path:<fn>`, `total-supply-override`, `deprecated-upgrade-forwarding`, `zero-address-transfer-without-supply-change:<fn>`, `unverified-implementation-source` — so rebasing, custom-event and unreviewed configs are explicitly unsupported rather than falsely passing or triggering an invented conservation failure. `npm run audit:mint-burn-conservation-admission` produces, replays and merges this evidence. The Worker runtime never parses the evidence file: eligibility resolves from the generated lookup `worker/src/lib/mint-burn-conservation-runtime.generated.json` (identity, disposition, reason and runtime params projected from the sidecar by a registered generator; `npm run check:generated-artifacts -- --only=mint-burn-conservation-runtime` guards drift), keeping the evidence out of the isolate bundle.
...
12. **Invalidate flow API caches** — on successful runs (`status ∈ {ok, degraded}`), purge flow cache rows from the shared `cache` table using PK-range predicates over the versioned `FLOW_CACHE_PREFIX`. The critical lane (and manual `all` runs) purges the whole `mint-burn-flows:v3:*` range and its scheduled sidecar immediately republishes the aggregate windows (`24` and `168`); the extended lane has no republish sidecar, so it purges only the per-coin `mint-burn-flows:v3:coin:*` rows and leaves the published aggregate (Bank Run Gauge) rows in place. The gauge therefore ages at most one 30-minute lane window between critical-lane republications, well inside its 2 h staleness tolerance, and stale per-coin payloads rebuild on the next `/api/mint-burn-flows` request against the freshly written buckets.
