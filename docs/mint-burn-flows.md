...
2. Prefers an exact UTC event-day `supply_history` price. If absent, it fetches a bounded event-day CoinGecko market-chart range, then DefiLlama CoinGecko-identity and exact contract-chart ranges. Each accepted price records its source and source timestamp.
