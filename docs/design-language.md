...
- `LongformScrollspyNav` renders once as a sticky horizontal pill banner; on `lg+` the banner stays full-width with the `Jump to` label and section pills centered above the full-width dossier stream instead of reserving a side rail column
