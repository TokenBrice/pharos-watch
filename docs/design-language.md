...
- A "verification passport" strip docks at the bottom of the hero card behind a hairline `border-t`: identity-document fields (Mechanism, Attestor, Jurisdiction, Issued, MiCA, GENIUS, Redeemability, Minting, Freeze, Record, Chains) with the field name in small muted letters above a mono all-caps value, each linking to the section that proves the fact. Flat document fields, not pills; data-driven text tones only (attestor tier ladder via `POR_TIER_STYLES.textCls`, freeze amber/emerald); snap-scroll carousel below `lg`, wrapping row on `lg+`. The card's final row is the passport's machine-readable zone (`HeroPassportMrz`): two 44-char TD3-style OCR mono lines re-encoding the dossier facts (every character data-driven, real ICAO check digit), acting as a single copy-citation button with `aria-hidden` glyphs. See `docs/stablecoin-detail-page.md` `### Hero passport strip` and `### Hero machine-readable zone (MRZ)` for the contracts.
