...
- Market Pulse expand affordance: card headers use a compact filled square button (`CardExpandButton`) rather than a bare maximize glyph. The control keeps a muted grey fill in light and dark mode, a hairline border, and a 12px maximize icon so the action reads as a button without competing with card data.
