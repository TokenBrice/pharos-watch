...
- Reference primitives (`--p-blue-500`) directly in components, except for documented local visualizations that intentionally need a primitive ramp before a semantic token exists. Current exceptions include the PSI lead card's red/amber primitive ramp in `src/components/kpi-bar-parts.tsx`, the contagion-graph edge stroke in `src/components/contagion-graph/contagion-graph-svg.tsx` (`--p-frost-blue`), and the homepage discovery group accents in `src/lib/homepage-discovery.ts`.
