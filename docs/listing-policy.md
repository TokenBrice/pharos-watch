...
A missing live price does not by itself move an active asset to quarantine or delist, and it is never papered over with a substitute quote. When every admissible price lane is empty and the asset's lifecycle is otherwise sound, the gap may instead carry a reviewed price-gap acknowledgement (`STABLECOIN_PRICE_GAP_REVIEWS` in `worker/src/lib/stablecoin-publication-coverage.ts`): a dated registry entry naming an owner, a concrete reason, at least one public `https` source, and an expiry no more than roughly 30 days after the review. An acknowledged gap stays published as missing — missing counts, affected market-cap exposure, and per-asset rejection detail are unchanged — but it stops producing public health warnings and admin warning causes. Renewal requires a fresh review with new evidence; expiry is automatic, so the gap alerts again until the asset is re-sourced, frozen, or delisted. Acknowledgement never applies to a depeg, never applies to an asset that already has a price, and never substitutes for the lifecycle disposition, which keeps its own dated review.
