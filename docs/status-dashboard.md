...
- `publicationHealth`: admin-only read-only publication generation summary for surfaces with existing ledgers or cache-backed publication metadata. It reports `dex-liquidity`, `yield-rankings`, and `stablecoins`; stablecoins prefers migrated `surface_publication_generations` rows when present and otherwise derives the current published generation from the validated canonical `stablecoins` cache plus response-ready companion timestamp, without changing publisher behavior.
