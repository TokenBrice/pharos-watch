...
The reviewed set lives in the committed evidence sidecar `worker/src/lib/mint-burn-conservation-reviewed.json`, with per-identity reviewer evidence and audited windows; it is the only list of admitted and explicitly unsupported identities. Exact identities and Transfer semantics are reviewed explicitly; an `unsupported` entry carries a reason from the sidecar's fixed vocabulary, and unsupported configs remain visible and unverified. Adding another asset requires proving compatible totalSupply/zero-address event semantics and adding the exact contract identity to the sidecar. A reviewed coin does not automatically admit a replacement contract.
