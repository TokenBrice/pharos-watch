...
- Data-quality causes now include `reserve_sync_budget_truncated`, `reserve_sync_write_uncertain`, and `reserve_sync_history_write_gap`; one-off low-share truncation is warning-level observability, while high-share truncation and uncertain writes can degrade reserve health before freshness collapses
