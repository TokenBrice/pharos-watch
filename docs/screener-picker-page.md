...
Since `selector-v2.3`, yield confidence treats `yieldObservationDays30d` as the distinct UTC observation-day count in the trailing 30-day window. Fewer than 21 observed days triggers `short-yield-history` and caps confidence at 80; a fully observed 30-day rail does not. The explicit field name is part of the dataset hash and snapshot replay contract, replacing the misleading `yieldHistoryDays` span label.

Since `selector-v2.4`, the rail-depth key of the yield-source comparator ranks `sourceDepthRatio` on the published depth bands — `deep` at `>= 1%` of tracked supply, `moderate` at `>= 0.1%`, `thin` below it — instead of scaling the supply fraction to a 0–100 score. Scaling it mapped the deepest measurable rail to `<= 10` while a rail with no depth evidence took a neutral 45, so an unmeasured rail won the depth key against every measured one; a rail nothing sized now ranks below all three bands. `shared/lib/selector/yield-source.ts` owns the band thresholds and the `/yield` depth lens imports them. Venue preference, source risk, freshness, and the source-key tie-break are unchanged, and `selector-v2.3` snapshots stay on the current-generation replay path.

Since `selector-v2.5`, an unresolved venue field degrades the rail it belongs to instead of the coin. Rails whose `venueChain` the yield domain never resolved are filtered out **before** the comparator runs, so a chain-less top rail no longer removes every rail for that coin and no longer publishes it under `missingSignals: ["recommendedSource"]` — which could push a whole run into the `sparse`/`uneven` coverage banners. A coin keeps its coverage whenever any rail resolves. In the same object, `RecommendedSource.sourceRiskTier` is now nullable and an unsourced venue tier publishes as `null` — rendered "tier not sourced" — rather than as a measured `"mid"`; the comparator keeps scoring an unknown tier at the neutral 55, so no ordering changes. `selector-v2.4` snapshots stay on the current-generation replay path, and the snapshot shape guard accepts both the null and the three-tier forms.
