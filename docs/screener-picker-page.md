...
| `s:{sid}`                          | KV value (`SELECTOR_SNAPSHOTS`) | Content-addressed snapshot projection. New writes carry trusted `pharos-server-recomputed-v1` KV metadata; legacy values without that metadata remain client-unverified. Written with a 90-day unread TTL; the first read returns the snapshot only after extending it to the full 5-year retention TTL (KV metadata `extended: true`). |
...
**Snapshot share URL:** `/screener/picker/?sid={32-hex}`. Legacy `/screener/selector/?sid={32-hex}` links are preserved through the Pages redirect table and must keep the query string intact. The sid is content-addressed — two server recomputations that produce identical canonical output (modulo timestamp and freshness fields) share the same sid. The frozen artifact contains the form answers and server-recomputed output; the sid is the lookup key. New replays carry `provenance: "pharos-verified"` and must render a `Pharos-verified snapshot` banner. Historical values without trusted KV metadata are projected as `provenance: "client-unverified"`; their banner must state that Pharos did not reproduce their scores from canonical source data. The share UI must disclose before or during link creation that answers and shortlist output are stored for up to 5 years and that anyone with the link can view the snapshot. Snapshots that are never opened expire after 90 days; a read returns `503` if the full-retention extension cannot be confirmed.
...
| ----------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /selector-snapshot`     | `POST` | Accepts allowlisted `SelectorInput`, recomputes output from canonical sources, and stores a verified schema-v3 projection. Returns `{ sid, ev }`.                   |
| `GET /selector-snapshot/:sid` | `GET`  | Returns the stored verified or explicitly legacy-unverified `SelectorOutput`, or `404`. `private, no-store` so every read re-enters the same-origin gate.           |
...
**Trusted provenance:** POST ignores caller output, dataset hashes, scores, ranks, identities, and display copy. After server recomputation, it writes `provenance: "pharos-verified"`, `snapshotSchemaVersion: 3`, a `verification.kind` of `pharos-server-recomputed-v1`, and KV metadata with the same trust kind. GET treats the KV metadata as the authority: a self-claimed verified body without trusted metadata is downgraded to the legacy client-unverified projection, while a metadata-attested body that fails the exact verified contract returns `502`. The verification object binds `datasetHash` and `engineVersion` into the content-addressed sid.

**Validation and recomputation:** shared selector snapshot code owns the runtime-neutral input and replay contracts. POST projects `SelectorInput` onto an exact allowlist, then the Pages helper fetches stablecoins, peg summary, report cards, stress signals, DEX liquidity, yield rankings, Bluechip ratings, and redemption backstops from the authenticated site-data lane. Every body is bounded, fully consumed, and parsed by its shared response schema before the shared dataset adapter and selector engine run. The eight requests are split into two batches of four to respect Cloudflare's per-trigger connection pool. Missing configuration, any non-2xx source, invalid JSON, or any source schema failure aborts the write with `503`. Legacy GET validation still derives tracked identities and recomputes locally provable relationships, but those artifacts remain explicitly unverified.

**Canonicalization:** the strip rule covers `timestamp`, `debug`, `provenance`, `snapshotSchemaVersion`, `perInputStaleness`, plus any field whose name matches the suffixes `ageSeconds`, `capturedAt`, `stalenessMs`, `updatedAt`, `fetchedAt`. The schema-v3 `verification` binding is not stripped and therefore separates verified artifacts from legacy projections. `coverageWarnings.newListingCount` is **not** stripped; the engine derives it from content-level recent-listing flags, so it contributes to the sid.
...
**Failure modes:** malformed JSON / wrong input shape → `400`; oversized → `413`; origin mismatch → `404`; canonical-source, KV, quota-store, or unconfirmed retention-extension outage → `503`; corrupt KV value → `502`; missing KV or HMAC-pepper configuration → `500`.
...
| POST includes caller scores, identities, prose, `debug`, or an arbitrary 64-hex hash | Ignore all output claims; project the input and recompute from canonical data.              |
| POST input has unknown enum or misses a required answer                          | `400`.                                                                                       |
| Any canonical source is unavailable or fails its shared schema                   | `503`; do not write KV.                                                                      |
| Stored body self-claims verified provenance without trusted KV metadata          | Replay only as `client-unverified`; never show the verified banner.                          |
| Trusted KV metadata points to a tampered verified payload                        | `502`.                                                                                       |
...
- Result summary renders distinct banners for `lowConfidence`, sparse coverage, uneven coverage, `usedRelaxedFallback`, stale Trading share blockers, and methodology/version drift. Frozen schema-v3 snapshots render `Pharos-verified snapshot`; legacy schema-v2 replays render `Unverified client snapshot` and explain that Pharos did not reproduce their scores from canonical source data.
- `[Copy share link]` uses POST-then-copy: POST only the allowlisted selector input, let the Pages Function recompute the output, copy `/screener/picker/?sid={sid}&ev=pharos-server-recomputed-v1` on `200`, and leave the button disabled with a notice on failure. Active Trading share copy is blocked when relevant `perInputStaleness` exceeds the configured freshness ceiling; Treasury and Yield are not blocked by Trading-only staleness. Disabled/error reasons are associated with the button and announced through status/alert regions.
...
The live Picker engine remains deterministic and client-side. Snapshot creation is a Pages-only surface that independently fetches the authenticated Worker site-data lane and runs the same shared adapter and engine before writing KV. Picker UI and selector-engine changes are Pages-impacting; source-schema or site-data endpoint changes can also affect snapshot creation and must preserve the shared contracts.
...
| `shared/lib/selector/data-adapter.ts`           | Shared canonical API-to-selector dataset adapter (frontend + Pages Function)          |
