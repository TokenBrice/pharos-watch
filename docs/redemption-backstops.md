...
- **Current methodology version:** <!-- GENERATED-START: methodology-version-redemption-backstop -->`v4.43`<!-- GENERATED-END: methodology-version-redemption-backstop -->
...
Latest `v4.43` update: live route provenance now requires a same-run route-specific executable predicate. Capacity, collateral, TVL, backing, or static configuration alone cannot publish `onchain` / `protocol-api` openness, and the route-status confidence component reaches 100 only when current open evidence is paired with positive executable capacity.
