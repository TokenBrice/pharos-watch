...
- **Current methodology version:** `v4.14`
...
Latest `v4.14` update: Steakhouse Prime USDT (`steakusdt-steakhouse`) reserve-sync telemetry now uses Morpho V2 vault `liquidity` as current executable redemption capacity after validating the exact vault address, underlying USDT address, listed status, and Ethereum chain id. The score still caps capacity by the ERC-4626 asset base, keeps Morpho `forceDeallocatableLiquidity` as context only, and leaves the standalone DEX liquidity score unchanged.
...
ERC-4626 single-asset wrappers such as fxSAVE, Spark savings wrappers, sUSDS, sDAI, scrvUSD, sfrxUSD, and stcUSD use the live adapter's idle underlying ERC-20 balance as current direct redemption capacity when fresh reserve telemetry is available, rather than treating the full wrapper supply as immediately executable. Reviewed Morpho V2 vault configs can additionally use Morpho's vault `liquidity` field as same-run API capacity after validating the exact vault, underlying asset, listed status, and chain id; `forceDeallocatableLiquidity` remains contextual and is not scoring capacity.
