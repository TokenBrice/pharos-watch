...
Safety Score methodology **9.7** supports an explicit `physical-commodity-delivery` output only with reviewed `physicalCommodityDelivery` terms: XAU/XAG, deliverable troy ounces per token, minimum delivery tokens, bounded bps/flat-USD/delivery-USD fees, and `sameNotionalEligible: false`. An omitted fee is not zero. Prose-only or unbounded delivery charges leave the legacy commodity route unstructured and unresolved. Valuation uses the captured unscaled USD/oz commodity reference, never the token peg value; the minimum-lot threshold and fees reduce retained value. Physical delivery reuses blue-chip tier 65 but never earns fiat or same-notional settlement credit. A fresh peg producer cycle and redemption observation are required for old captures that lack this contract.
