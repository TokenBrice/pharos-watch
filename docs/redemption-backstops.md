...
`BOLD`, Base Dollar `BD`, `feUSD`, `USDQ`, `NECT`, and `CDP` use the live `liquity-v2-branches` onchain adapter for bounded current direct capacity rather than the old static full-supply model. Configs without a mechanism-redeemability packet conservatively use aggregate ActivePool branch debt when the 4-hourly reserve snapshot is fresh and clean, while BOLD and BD use their same-run TroveManager packet to include only branches explicitly reporting `redeemable = true`. A false branch is excluded from immediate capacity and degrades route status; an unreadable required redeemability packet leaves the route unrated instead of publishing aggregate debt. Shutdown/sunsetting reads remain an additional degraded or unknown route-status signal. BD's reviewed route covers its five Base launch branches and carries the current CollateralRegistry redemption fee into the nested telemetry.
...
M0 wrappers `wM`, `USDSC`, and `USDnr` now use `m0-wrapper-underlying` capacity telemetry from the underlying M token balance; the `m-extension` wrappers (`USDSC` on Soneium, `USDnr` on Ethereum) also require their reviewed SwapFacility and approved swapper route before emitting whitelisted-primary direct capacity.
