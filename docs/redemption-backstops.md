...
- **Current methodology version:** <!-- GENERATED-START: methodology-version-redemption-backstop -->`v4.44`<!-- GENERATED-END: methodology-version-redemption-backstop -->
...
Latest `v4.44` update: Noon's USN route is modeled as an issuer-processed, days-settled rail with an exact 7-calendar-day bound taken from the five-Business-Day processing clause, and sUSN's request/claim rail publishes a live on-chain settlement bound read from the WithdrawalHandler's own `withdrawPeriod()` instead of excluding capacity as an unproven asynchronous request. A documented-bound exit-route observation now carries its same-run chain-read time as `observedAt` when the observation holds a direct freshness kind.
...
Safety Score methodology **9.8** values reviewed `physicalCommodityDelivery` terms (XAU/XAG, deliverable troy ounces per token, minimum delivery tokens, published bps/flat fees, and `sameNotionalEligible: false`) even when delivery terms are unbounded. `deliveryTermsUnbounded: true` means unpriced delivery/handling is not deducted or represented as a zero-cost promise: an explicit `unboundedDeliveryCap` lowers output quality to **55**, below bounded physical tier **65**. Bounded delivery retains its published USD deduction. Valuation uses the captured unscaled USD/oz reference, never the token peg value; minimum lots and published fees reduce retained value. No physical route earns fiat or same-notional settlement credit. Historical captures lacking raw USD/oz require a fresh peg producer cycle and redemption observation before output gaps close.
...
| USDm (`cusd-celo`)      | Stable basket: USDC + USDT, matching the two reviewed direct Celo V3 FPMM output pools measured by live reserve sync.                                                                                                                                                     | [Mento V3 reserve](https://docs.mento.org/mento-v3/dive-deeper/the-reserve), [Mento V3 FPMM](https://docs.mento.org/mento-v3/dive-deeper/fpmm)                                              |
...
| SILK | Rechecked 2026-08-27: remains unresolved. Shade Lend lets a holder choose a vault and receive pro-rata vault collateral, but public issuer docs do not publish a canonical current vault whitelist. The set is governance-mutable, so the former sSCRT/wBTC/USDC subset was not a complete payout composition. | [Shade Lend stability mechanisms](https://docs.shadeprotocol.io/shade-protocol/advanced-topics-apps/lend/stability-mechanisms) |
...
| satUSD | Rechecked 2026-08-27: remains unresolved. River's redemption docs describe a $1 collateral exit from least-collateralized positions, while its FAQ names BTC, ETH, BNB, and other liquid staking tokens as collateral. The public materials do not enumerate the complete eligible LST inventory, so a BTC/ETH/BNB subset is not a complete payout composition. | [River satUSD redemption docs](https://docs.river.inc/products/editor/redemption), [River FAQ](https://docs.river.inc/intro/faq) |
| srUSDe | Stable basket: USDe + sUSDe. Strata's current FAQ identifies both as redemption outputs, with instant sUSDe settlement and a seven-day USDe cooldown. | [Strata srUSDe market](https://docs.strata.markets/markets/ethena-usde/srusde), [Strata FAQ](https://docs.strata.markets/resources/faqs) |
| UTY | Rechecked 2026-08-27: remains unresolved. XSY says the issuance contract redeems UTY for approximately $1 in value, but does not name the payout asset; its UTY overview says holders have no ownership rights over specific underlying assets. | [XSY UTY peg-arbitrage docs](https://xsy-1.gitbook.io/xsy-main/open-market-peg-arbitrage.md), [XSY UTY overview](https://xsy-1.gitbook.io/xsy-main/unity-uty-overview.md) |
...
| Route | Ruling | Primary evidence |
| ----- | ------ | ---------------- |
| USN (`usn-noon`) | Issuer-processed rail: `settlementModel: "days"` with an exact 7-calendar-day bound (`settlementDelaySec: 604_800`) read from Terms-of-Service §6's five-Business-Day processing commitment, `rules-based-nav` execution, and the §29 discretion to gate redemptions. Redemption is KYC-gated and the claim is unsecured debt of the issuer. Fees stay `undisclosed-reviewed` (the fees page is non-binding and publishes no schedule against §7's right to charge), and capacity keeps its 15% supply-ratio heuristic with the majority private-credit reserve rationale. | [USN Terms of Service](https://docs.noon.capital/additional-resources/terms-and-policies/asset-terms-usn-terms-of-service), [Noon fees](https://docs.noon.capital/built-for-high-yields/fees-and-other-charges), [Noon liquidity](https://docs.noon.capital/noon-the-basics/liquidity) |
| sUSN (`susn-noon`) | Request/claim rail with a live on-chain settlement bound: the registry `async-request` marker is removed, the reviewed `settlementModel` is `days`, and `settlementDelaySec` is the handler's live `withdrawPeriod()` (604,800 seconds today), with capacity = measured idle USN published as `live-direct-bounded`. The one-transaction whitelisted path is a documented fast path only; the modeled rail is the holder request/claim. Access and the 65-point route cap are unchanged. | [Noon minting and redemption](https://docs.noon.capital/built-for-high-yields/our-stablecoin-usn-and-susn/minting-and-redemption), [sUSN staking vault](https://etherscan.io/address/0xE24a3DC889621612422A64E6388927901608B91D#readContract), [Noon WithdrawalHandler](https://etherscan.io/address/0x0DaBc0D9B270c9B0C4C77AaCeAa712b56D0F9178#readContract), [WithdrawalHandler admin timelock](https://etherscan.io/address/0x36857EF0B10A61A68d58C29eE256990fa9699722#readContract) |
...
- `routeStatusSource`:
  ...
  - reviewed adapters may emit `protocol-api` or `onchain` only when the current run evaluated a route-specific executable predicate; capacity, balance, collateral, TVL, backing, or configuration evidence alone does not qualify
...
- `modelConfidence`:
  ...
  - the route-status freshness component reaches 100 only for `open` current `onchain` / `protocol-api` evidence with positive executable capacity; an open route with zero or missing capacity is capped at 70
