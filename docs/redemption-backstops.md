...
| pmUSD, reUSD (Re Protocol), srUSD, wsrUSD, USD3 (Reserve) | Already declare complete outputs; they stay unresolved only because an output leg lacks a current price (susds-sky, susde-ethena, steakusdc-steakhouse have no peg row; rusd-reservoir's deviation is null). Blocked on price coverage, not on output curation. | — |
