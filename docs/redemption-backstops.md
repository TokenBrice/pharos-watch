...
Latest `v4.38` update: Base Dollar gains a reviewed five-branch collateral-redemption route, and mechanism-enabled Liquity V2 adapters now count only branches whose same-run TroveManager tuple reports `redeemable = true`. A false branch is excluded and degrades the route; unreadable required redeemability leaves capacity unrated instead of crediting aggregate ActivePool debt. Optional solvency telemetry can fail independently after branch eligibility is known. Scoring weights, ladders, and the exit engine are unchanged.
...
- optional `notes`
- optional V9-only projection overlays `v9RouteCostTerms`, `v9RouteReviewTerms`, and `v9ComposedDexExit`, which the Safety Score V9 route adapter reads without changing the standalone redemption producer
