...
Latest `v4.34` update: a third exit-credit pass bounds ZSD (Zephyr) at a fixed 10 bps — the pinned RingCT consensus source deducts `exchange_128 / 1000` on every `REDEEM_STABLE`, and mainnet has run hard fork 11 since block 536000, so the pre-fork 200 bps path is unreachable. USDPT (Western Union) gains an `offchain-issuer` route on the Anchorage Digital Bank rail, lifting coverage to 315 configured coins; it is client-gated (the Covered Stablecoin Terms redeem "exclusively from Clients") and modeled as `days` settlement because the terms promise only "commercially reasonable efforts" and reserve discretion to refuse, suspend, or limit. USSD (Sonic) moves onto live escrow telemetry with no fallback — the BrandedCustodian's `totalAssets()` is the executable exit bound, and at Sonic block 77432523 it and the custodian's frxUSD balance both read 2,081,316.47, with `redeemFee()` still 0 and `paused()` reverting. satUSD (River), USDz (Anzen) and USDai also drop full-supply models with no fallback in favour of live reads — per-chain trove debt via `getGlobalSystemBalances()` for satUSD, the SPCT pool's own USDC for USDz, and the live PYUSD float for USDai, whose 174.3M read against a ~172.6M supply shows why no fixed fraction of supply could stand in. satUSD and USDz additionally read their redemption fee live rather than against a reviewed ceiling. USDz's access model is also corrected from whitelisted to permissionless against its deployed source — `redeem()` enforces only a caller blacklist, and the SPCT whitelist covers the USDz contract itself rather than holders — and its first live read is an honest negative in the DOLA class: the SPCT pool held 0.006695 USDC against a supply of 806,422.80, with `redeem()` requiring `spct.reserveUSD() * 1e12 >= _amount` so that ceiling is exact. DOC (Money On Chain) consumes `MoCState.freeDoc()` identity-bound through the MoC connector, keeping a 95% documented ratio as fallback because only the Rootstock-local balance is redeemable and Rootstock has a single public RPC. eUSD migrates to the Reserve DTF adapter and becomes live-only like USD3, reading the `redemptionAvailable()` throttle — 5,000,000.00 against a 22,834,920.56 supply at the verified read, with the basket SOUND — and HONEY moves to live-direct HoneyFactory vault telemetry (7,831,557.70 held, basket mode off, all four assets pegged, live rates 0–5 bps). USBD and xUSD are deliberately left unmodeled: USBD's immediate capacity is trove-set-dependent and a pinned system-debt view would overstate it, and BabelFish's deployed v5 aggregator manager departs from its documented interface. Routes recorded as honest skips: satUSD takes no static fee bound (its cost model is corrected from undisclosed to the documented `min(redemptionFeeFloor + baseRate, maxRedemptionFee)` formula, unbounded only because `maxRedemptionFee` is a governance parameter capped at 100%); dEURO keeps no bound and is flagged for a remodel, because primary docs document no holder redemption into position collateral at all and the one documented exit is a fee-free `StablecoinBridge` burn bounded by per-bridge inventory; ZeUSD (Zoth) and bnUSD (Balanced) stay unconfigured, the first because the route belongs to the ZeDP position holder on a paused V1 router, the second because the tracked v1 ICON token's Stability Fund swap now runs through SODAX on distinct token identities. Scoring weights, ladders, and the exit engine are unchanged — routes move only through better evidence.

Previous `v4.33` update: a second exit-credit pass configures three previously unconfigured PSM rails and bounds four more redemption fees against primary sources. FXD (Fathom), HOLLAR (Hydration), and USDH (Hubble) gain `psm-swap` routes, lifting coverage to 314 configured coins; USDai is bounded at its published 10 bps redemption fee and corrected from permissionless to whitelisted access; ZYS takes a fixed 10 bps bound from Zephyr's consensus RingCT source; USDnr is bounded at 0 bps for the modeled USDnr → M leg. USDH carries `routeStatus` unknown because no current Hubble source proves the module still executes, and HOLLAR deliberately keeps no fee bound because its buy-back fee is a per-collateral on-chain parameter with no documented ceiling. MAI (QiDao) and iUSD (Initia) stay unconfigured — QiDao's PSM pages now 404, and Initia's bridge documentation does not mention iUSD. Four routes also move onto live capacity telemetry: OUSG reads its InstantManager router capacity (with Ondo's published $50M 24-hour global instant-redemption limit as fallback), USD3 reads the RToken `redemptionAvailable()` throttle with no fallback, USDD reads the Tron PSM GemJoin balance behind its reviewed 16% fallback, and DOLA is corrected downward — its reviewed 8% PSM share was phantom capacity in a contract drained in December 2025, so it is now live-only with no fallback. Scoring weights, ladders, and the exit engine are unchanged — routes move only through better evidence.
