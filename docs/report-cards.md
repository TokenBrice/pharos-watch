...
The stablecoin registry currently contains 340 tracked metadata entries. Report-card snapshots score the 312 active tracked assets plus the 91 cemetery assets; frozen-archive tracked entries are emitted as stub `F` cards, while pre-launch entries remain outside the snapshot until they launch.
