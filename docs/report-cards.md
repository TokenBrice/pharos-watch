...
The same publication cron also persists a private exact fixed-input artifact at `report-cards:fixed-input:exact`; it is an operator/replay contract, not a public API response. The artifact captures the in-memory V8 inputs used for that publication, omits the additive V9-only supply-attribution and diagnostic evidence-journal fields for strict rollback compatibility, and is written in the same D1 batch as the full snapshot, compact score map, and Telegram safety source. The current parser restores both omitted fields to empty schema defaults. All four projections carry the same V8 identity. Publication fails before the batch if active DEX rows do not belong to one complete published generation/timestamp, redemption rows cannot bind to one completed producer run, the fixed-input generation differs from the report-card publication generation, or the compressed artifact exceeds its bounded cache size.

The quarter-hour chain runs `sync-v9-supply-attribution` before the report-card publisher. That isolated producer reads the prior publication-exact V8 input, performs the bounded attribution RPC work, appends its immutable attempt journal, and atomically writes a private content-addressed generation. V9 preparation itself is D1-only: it accepts the generation only when schema, registry fingerprint, expected asset inventory, source clock, and freshness match, re-derives accepted partitions from raw observations against the current aggregate liabilities, and otherwise supplies an empty attribution map. It then persists the resulting attempt input separately at `report-cards:v9-fixed-input:exact`. The reserve `evidenceJournalById` projection records only strict content-addressed attempt/admission/fallback metadata, hashes, timestamps or blocks, adapter-registry source origin, and sidecar identity; it accepts no raw response, URL, credential, or free-form diagnostic. The separate `supplyAttributionJournalById` projection also records the allowlisted exact observer leaf rejection code and reuses `sourceObservedAtSec` for the rejected source timestamp when a freshness or clock-skew predicate has a known source time. The rejection field is optional on read so immutable V1 rows written before leaf diagnostics retain their original content-addressed IDs; all newly created rejected rows require it. Source origin records who produced the reserve evidence (`issuer-attested`, `onchain-observation`, `independent-assurance`, `reviewed-curation`, or explicit `unknown`); it is separate from score-bearing adapter `evidenceClass` and cannot change admission or scores. D1 retains 45 days and at most 32 records per asset, while each exact input carries at most the latest two. Journal content changes the private V9 envelope checksum but is deliberately absent from base-input identity, fact compilation, result identity, the V8 cache, and every public response. The V9 artifact retains the V8 publication identity that anchors its base input, while score-bearing V9 source, fact-set, result, and candidate identities bind only admitted evidence. For wM, reviewed EVM deployment state remains at or before the source fixed clock; the finalized Solana observation may land just after it only when the complete packet stays within the 120-second cross-chain envelope, otherwise the attempt records explicit skew provenance and falls back to aggregate-only evidence. Any generation read, journal read, artifact, compile, or shadow-state failure remains inside the caught V9 sidecar and cannot roll back the already committed V8 publication.
