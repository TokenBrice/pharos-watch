...
The stablecoin registry currently contains 256 tracked metadata entries. Report-card snapshots score the 242 active tracked assets plus the 88 cemetery assets; pre-launch and frozen-archive tracked entries are emitted as stub `F` cards (defunct; pre-launch remain outside the snapshot until they launch).
