...
The stablecoin registry currently contains 410 tracked metadata entries. Report-card snapshots score the 365 active tracked assets plus the 88 cemetery assets; frozen-archive tracked entries are emitted as stub `F` cards, while pre-launch entries remain outside the snapshot until they launch.
