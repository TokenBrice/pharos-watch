...
- `src/components/stablecoin-detail/stablecoin-safety-score-v9-card.tsx` renders detail-page score, pillars, evidence, and breakdowns.
  - The Economic Control breakdown leads with its binding components and folds the rest into a `Diagnostic inputs` disclosure. The pillar scores on the lowest binding control, but the producer emits components alphabetically, so payload order buried the deciding rows behind per-chain bridge diagnostics on the widest deployments.
