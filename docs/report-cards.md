...
When the post-commit V9 shadow refresh is due, its preparation step may add bounded V9-only evidence to the same normalized base generation and persist the resulting attempt input separately at `report-cards:v9-fixed-input:exact`. The reserve `evidenceJournalById` projection records only strict content-addressed attempt/admission/fallback metadata, hashes, timestamps or blocks, adapter-registry source origin, and sidecar identity; it accepts no raw response, URL, credential, or free-form diagnostic. Source origin records who produced the reserve evidence (`issuer-attested`, `onchain-observation`, `independent-assurance`, `reviewed-curation`, or explicit `unknown`); it is separate from score-bearing adapter `evidenceClass` and cannot change admission or scores. D1 retains 45 days and at most 32 records per asset, while each exact input carries at most the latest two. Journal content changes the private V9 envelope checksum but is deliberately absent from base-input identity, fact compilation, result identity, the V8 cache, and every public response. The V9 artifact retains the V8 publication identity that anchors its base input, while score-bearing V9 source, fact-set, result, and candidate identities bind only admitted evidence. For wM, reviewed EVM deployment state remains at or before the fixed clock; the finalized Solana observation may land just after it only when the complete packet stays within the 120-second cross-chain envelope, otherwise the attempt records explicit skew provenance and falls back to aggregate-only evidence. Preparation happens only after the atomic V8 batch, and any journal read, enrichment, artifact, compile, or shadow-state failure remains inside the caught V9 sidecar.
