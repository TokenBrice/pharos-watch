...
V9-only bridge attribution runs on the dedicated `+8` expression. The V9 fixed input is then prepared immediately after successful half-hourly DEX publication, carrying that exact DEX generation ID, and canonical compilation runs at `+22` and `+52`. When the current half-hourly DEX run errors, preparation does not skip: it consumes the last accepted exact DEX publication while that generation is inside the shared four-hour DEX evidence budget (`DEX_LIQUIDITY_EVIDENCE_MAX_AGE_SEC`, the same budget a `:46` cadence-reuse slot consumes it under) and records `dexPublicationRecovery` — upstream status, reused generation, age, and budget — in the run metadata; beyond the budget, or when no single consistent accepted generation can be read, it fails closed with that refusal in `upstreamRecovery`. The compiler rejects an input when its DEX dependency is older than the latest accepted DEX generation. The isolated attribution and compilation lanes acquire a D1-backed memory-lane lease, bind to the immutable current Worker version ID and matching finished core slot, decline admission while an earlier scheduled slot is still active, and enforce absolute deadlines. An `ok` core slot admits directly. A degraded, non-terminal, or post-deploy core slot admits only when the durable publication ledger proves that slot's `sync-stablecoins` run published the current `stablecoins` cache generation — whichever Worker version wrote it — and the compiler separately requires the fixed input's stablecoin timestamp to match the live cache generation. This keeps partial active-row coverage visible and asset-local without admitting stale or no-write stablecoin data. Delayed or competing deliveries skip neutrally; missing Worker-version metadata degrades visibly. The critical quarter-hour production lane is stablecoins → DDR; the atomic report-card publication that feeds V9 runs separately, on the half-hourly `16,46` chart slot (`prepare-safety-score-v9-input`, after DEX publication). That atomic publication includes a compact, publication-exact V9 peg-provenance seed; the later V9 compiler rejects missing, partial, or identity-mismatched seeds rather than reconstructing provenance from mutable event rows.
...
For a DefiLlama asset with exactly one missing tracked chain, supply-gap reconciliation uses CoinGecko only when the current market-cap observation is timestamped and fresh. The `coingecko-gap-fill` policy raises each aggregate supply bucket to the greater of its existing DefiLlama value and the corresponding CoinGecko history value, then assigns only that increase to the missing chain. This keeps the current `chainCirculating` sum equal to `getCirculatingRaw(asset)` instead of publishing the CoinGecko remainder only on chain-level surfaces. The reconciled aggregate and chain packet therefore follow the same sum-equals-aggregate contract enforced when curated aggregate packets are restored. NAV and yield-bearing assets are excluded from par-valued on-chain supply-gap fallback.
...
The 2026-09-24 coverage-review pass quarantined four more records after intake could not establish a defensible supply or market-cap path, each with `changedAt: 2026-09-24` and `reviewBy: 2026-10-24`: `hlusd-hela` (no tracked contract on any supported chain; CoinGecko stale since 2026-08-14 when Biconomy suspended HLUSD trading), `vusd-virtue` (native VUSD is an IOTA Move coin that no configured supply-probe family can read, and reviewed aggregates refuse a partial roster, so the IOTA EVM leg's raw supply is not a defensible circulating figure), `luausd-lumi-finance` (NAV token with no admissible price lane: no CoinGecko page, no DefiLlama price, and one imbalanced Arbitrum Curve pool at ~$3.4K effective TVL with zero 24h volume), and `bib01-backed` (NAV tracker in issuer wind-down whose CoinGecko row has been frozen since 2026-09-17 while DefiLlama only re-stamps the same 121.86 mirror value, so the reviewed seven-chain on-chain aggregate cannot be valued and no positive market cap can be published). The same pass fixed `vcred-vcred` rather than quarantining it: Hemi joins the public RPC registry (`rpc.hemi.network/rpc` with `hemi.drpc.org` as the independent second operator) and a `CURATED_ONCHAIN_SUPPLY_EXCLUSIONS` entry subtracts the protocol SafeProxy's 987,400,000 units from the 1,000,000,000-unit total supply — verified over the registered RPC on 2026-09-24 — so the published supply is the 12,600,000-unit circulating basis CoinGecko reports instead of a ~79x overstated raw total. `usda-avalon` stays active with a dated price-gap acknowledgement instead of quarantine, because its DefiLlama supply path is sound and its only price lane (DefiLlama's per-deployment quote, e.g. `nibiru` 0xf4e0…2003) is re-stamped intermittently rather than absent.
...
Solomon's replacement `usdv-solomon-v2` is a separate active CoinGecko supplemental asset. Its current market cap belongs to the new six-decimal Chancery mint; the original `usdv-solomon` retains DefiLlama 261 supply. The recycled `solomon-usdv` CoinGecko ID supplies no imported historical price or market-cap series to either generation. Detail charts fall back to forward local supply history/cache, and existing legacy history is preserved. This separation also prevents the replacement's small current market cap from being used as a supply fallback for the original token.
