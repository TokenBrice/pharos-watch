...
-- Link the latest USDXL below-peg sealed DDR incident tail.
--
-- Events 90092 and 90094 reopened inside DDR's sticky incident window after
-- event 90090 had a public prediction sealed. Link both events through the
-- sealed repair authorization path and advance the canonical current source to
-- the still-open event 90094.
...
SELECT
  ...
  NULL,
  NULL,
  'USDXL closed tail event reopened within DDR incident merge window after sealed prediction',
  unixepoch(),
  4102444800,
  'migration-0149'
WHERE EXISTS (
...
SELECT
  ...
  a.id,
  unixepoch(),
  'sealed incident tail event linked through explicit repair authorization'
FROM depeg_resolver_event_repair_authorizations a
WHERE a.event_id = 90092
  AND a.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
  AND a.operation = 'incident_link'
...
SELECT
  ...
  NULL,
  NULL,
  'USDXL closed tail event is an intermediate source event for the sealed canonical incident',
  unixepoch(),
  4102444800,
  'migration-0149'
WHERE EXISTS (
...
SELECT
  ...
  90090,
  90092,
  'sealed incident tail event adopted as intermediate source event',
  a.id,
  NULL,
  unixepoch(),
  'migration-0149'
...
WHERE a.event_id = 90092
  ...
  AND NOT EXISTS (
    ...
    WHERE existing.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
      AND existing.previous_event_id = 90090
      AND existing.current_event_id = 90092
      AND existing.reason = 'sealed incident tail event adopted as intermediate source event'
  );

UPDATE depeg_resolver_incidents
SET current_event_id = 90092,
    current_started_at = 1780689116,
    updated_at = unixepoch()
WHERE incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
  AND current_event_id = 90090
  AND EXISTS (
    SELECT 1
    FROM depeg_resolver_incident_event_links l
    WHERE l.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
  ...
  AND EXISTS (
    ...
    WHERE r.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
      AND r.previous_event_id = 90090
      AND r.current_event_id = 90092
      AND r.reason = 'sealed incident tail event adopted as intermediate source event'
  );
...
WHERE EXISTS (
  ...
  AND NOT EXISTS (
    ...
    FROM depeg_resolver_incident_event_links l
    WHERE l.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
      AND l.event_id = 90094
  )
  AND NOT EXISTS (
    SELECT 1
    FROM depeg_resolver_event_repair_authorizations a
    WHERE a.event_id = 90094
      AND a.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
      AND a.operation = 'incident_link'
      AND a.created_by = 'migration-0149'
  );
...
WHERE a.event_id = 90094
  ...
  AND NOT EXISTS (
    ...
    FROM depeg_resolver_incident_revisions existing
    WHERE existing.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
      AND existing.previous_event_id = 90092
      AND existing.current_event_id = 90094
      AND existing.reason = 'sealed incident live tail adopted as current source event'
  );

UPDATE depeg_resolver_incidents
SET current_event_id = 90094,
    current_started_at = 1780691797,
    updated_at = unixepoch()
WHERE incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
  AND current_event_id = 90092
  AND EXISTS (
    SELECT 1
    FROM depeg_resolver_incident_event_links l
    WHERE l.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
  ...
  AND EXISTS (
    ...
    FROM depeg_resolver_incident_revisions r
    WHERE r.incident_key = 'ddr2:191d8a3d2d947537a22e8b973ba54b9d'
      AND r.previous_event_id = 90092
      AND r.current_event_id = 90094
