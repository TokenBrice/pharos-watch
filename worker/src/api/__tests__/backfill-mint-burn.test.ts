...
vi.mock("../../lib/alchemy-logs", () => ({
  ...
  getAlchemyTransactionByHash: vi.fn(async () => null),
  getAlchemyTransactionReceipt: vi.fn(async () => null),
...
describe("handleBackfillMintBurn", () => {
  ...
  it("updates flow_type on existing rows when classifier output differs", async () => {
    ...
    vi.mocked(getAlchemyTransactionByHash).mockReset().mockResolvedValue({
      hash: TX_HASH,
      to: "0x28b5a0e9c621a5badaa536219b3a228c8168cf5d",
      input: "0x6fd3504e",
    });
    vi.mocked(getAlchemyTransactionReceipt).mockReset().mockResolvedValue({
      transactionHash: TX_HASH,
      to: "0x28b5a0e9c621a5badaa536219b3a228c8168cf5d",
      logs: [
        {
