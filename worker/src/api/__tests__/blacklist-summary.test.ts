...
describe("handleBlacklistSummary", () => {
  it("returns summary stats, chart payload, and chain options", async () => {
    const db = mockD1([
      ...
      {
        match: "amount_status IN",
        rows: [],
        first: { n: 0 },
      },
      {
        match: "MAX(timestamp) AS t",
        rows: [],
        first: { t: 1_777_000_100 },
      },
      {
        match: "AND timestamp >= ?",
        rows: [],
      ...
      {
        match: "COUNT(*) AS n FROM blacklist_events WHERE suppression_reason IS NULL",
        rows: [],
        first: { n: 2 },
      },
      {
  ...
  it("derives perCoinBlacklistCounts and preserves required stats", async () => {
    const db = mockD1([
      ...
      { match: "amount_status IN", rows: [], first: { n: 0 } },
      { match: "MAX(timestamp) AS t", rows: [], first: { t: 1_700_200_000 } },
      { match: "AND timestamp >= ?", rows: [], first: { n: 0 } },
      {
        match: "COUNT(*) AS n FROM blacklist_events WHERE suppression_reason IS NULL",
        rows: [],
        first: { n: 3 },
  ...
  it("excludes suppression_reason != null from public aggregates", async () => {
    ...
    const db = mockD1([
      ...
      { match: "amount_status IN", rows: [], first: { n: 0 } },
      { match: "MAX(timestamp) AS t", rows: [], first: { t: null } },
      { match: "AND timestamp >= ?", rows: [], first: { n: 0 } },
      {
        match: "COUNT(*) AS n FROM blacklist_events WHERE suppression_reason IS NULL",
        rows: [],
