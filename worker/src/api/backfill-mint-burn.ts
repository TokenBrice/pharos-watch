...
export async function handleBackfillMintBurn(
  ...
  const chunkSizeParam = (typeof body.chunkSize === "number" ? Math.trunc(body.chunkSize) : null) ?? parsedChunkSize;
  const maxChunks = (typeof body.maxChunks === "number" ? Math.trunc(body.maxChunks) : null) ?? parsedMaxChunks;
