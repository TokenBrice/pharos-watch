...
export const handleDigestSnapshot = withErrorHandler("digest-snapshot", async (
  ...
  const targetRow = await db
    .prepare(
      `SELECT generated_at, input_data FROM daily_digest
       WHERE generated_at >= ? AND generated_at < ? AND (${targetFilter}) AND (${NON_INTERNAL_DIGEST_SQL_FILTER})
  ...
  const prevRow = isWeekly
    ...
    : await db
      .prepare(
        `SELECT generated_at, input_data FROM daily_digest
         WHERE generated_at < ? AND (${NON_WEEKLY_DIGEST_SQL_FILTER}) AND (${NON_INTERNAL_DIGEST_SQL_FILTER})
         ORDER BY generated_at DESC LIMIT 1`,
      )
      .bind(dayStart)
      .first<DigestRow>();
