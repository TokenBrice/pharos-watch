...
async function staleSnapshotResponse(db: D1Database, snapshot: DdrResponse): Promise<DdrResponse> {
  ...
  return {
    ...
    _meta: {
      ...
      degradedReasonDetail: snapshot._meta.degradedReasonDetail ?? null,
      lastRefreshAttemptAt: snapshot._meta.lastRefreshAttemptAt ?? null,
      readOverlay: {
  ...
  try {
    ...
    const response = DdrResponseSchema.safeParse({
      _meta: {
        ...
        degradedReasonDetail: parsed._meta?.degradedReasonDetail ?? null,
