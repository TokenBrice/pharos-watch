...
export async function computeAndStoreDEWS(
  ...
  const assembled = await assembleDewsScoringInput({
    ...
    onStablecoinsLoaded: async (eligibleAssetCount) => {
      await reportDewsProgress(reportProgress, "stablecoins-cache-loaded", {
        rowsComputed: eligibleAssetCount,
        validationFailures,
      });
    },
    onBeforeSourceHydration: async () => {
      await reportDewsProgress(reportProgress, "source-hydration", { validationFailures });
    },
    onSourceHydrationLoaded: async () => {
      await reportDewsProgress(reportProgress, "source-hydration-loaded", {
