...
const context = () => ({ assetsById: new Map([["usdc-circle", { id: "usdc-circle", price: 1, priceSource: "coingecko", priceConfidence: "single-source", priceObservedAt: NOW - 20, priceObservedAtMode: "upstream" } as PeggedAsset]]) });
...
function setup(options: { pool?: unknown; timestamp?: number | null; quoteChange?: (q: (typeof quoteFixture)[number]) => void } = {}) {
  fetchJson.mockImplementation(async (url: string, init: RequestInit) => {
    ...
    if (url.includes("jup.ag")) {
      ...
      body = { result: request.method === "getMultipleAccounts" ? options.pool ?? state() : options.timestamp === undefined ? NOW - 30 : options.timestamp };
    }
