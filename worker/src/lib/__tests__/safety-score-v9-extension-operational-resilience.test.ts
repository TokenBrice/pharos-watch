...
describe("Safety Score v9 operational-resilience overlays", () => {
  ...
  it("resolves assurance history exactly once without upgrading assurance to audit", () => {
    const overlay = getSafetyScoreV9OperationalResilienceOverlay("usdt-tether", CURRENT_CLOCK_SEC);
    const reconciliation = overlay?.reserveReconciliation;
    expect(reconciliation).toBeDefined();
    expect(reconciliation!.historySourceIds.length).toBeGreaterThan(0);
    expect(new Set(reconciliation!.historySourceIds).size).toBe(reconciliation!.historySourceIds.length);
    for (const sourceId of reconciliation!.historySourceIds) {
      expect(overlay!.sources.filter((source) => source.sourceId === sourceId)).toMatchObject([
        { confidence: "independent-assurance" },
      ]);
    }
    expect(reconciliation!.latestAssurance).toMatchObject({
      level: "reasonable-assurance",
      periodEnd: reconciliation!.latestReportPeriodEnd,
    });
    expect(reconciliation!.latestAssurance.sourceIds.length).toBeGreaterThan(0);
    for (const sourceId of reconciliation!.latestAssurance.sourceIds) {
      expect(reconciliation!.historySourceIds).toContain(sourceId);
    }
    expect(overlay!.sources.filter((source) => source.confidence === "audited")).toEqual([]);
  });
