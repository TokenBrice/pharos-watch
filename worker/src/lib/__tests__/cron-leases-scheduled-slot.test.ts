...
describe("runScheduledSlotWithFence", () => {
  ...
  it("does not reconcile child progress from a different schedule with a colliding slot timestamp", async () => {
    ...
    // P2-10 reconciles every registered chain member; P2-03 makes this quarter-hourly fixture six jobs.
  ...
  it("reconciles orphaned owner progress without deleting newer leases and remains fence-safe and idempotent", async () => {
    ...
    expect(
      JSON.parse(missingLeaseDb.getRuns().find((run) => run.job === "sync-yield-data")?.metadata ?? "{}"),
    ).toMatchObject({
      leaseOwner: "yield-owner-a",
      leaseUntil: null,
    });
    ...
    expect(
      JSON.parse(newerLeaseDb.getRuns().find((run) => run.job === "sync-yield-data")?.metadata ?? "{}"),
    ).toMatchObject({
    ...
    // Chain members that already got not-started runs before the fence was lost
    // stay; the abandoned child must not get one.
    expect(fenceLostDb.getRuns().map((run) => run.job)).not.toContain("sync-yield-data");
  ...
  it("uses a 60-second default scheduled-slot heartbeat cadence", async () => {
    ...
    expect(db.getSlot("daily0800Utc", slotStartedAt)?.updated_at).toBe(slotStartedAt + 60);
