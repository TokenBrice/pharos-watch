...
import { COMPOUND_V3_COMETS } from "./sources-optional-protocols-constants";
...
async function fetchCompoundV3SupplyRateCandidates(
  ...
  const { results, telemetry } = await runOptionalSourceFamily(
    ...
    signal,
    () => fetchCompoundV3SupplyRates([...COMPOUND_V3_COMETS], signal, chainRpcs),
    {
      results: [],
...
async function fetchAaveV3SupplyRateCandidates(
  ...
  const { rates, telemetry } = await runOptionalSourceFamily(
    ...
    signal,
    () => fetchAaveV3SupplyRates(targets, signal, chainRpcs),
    {
      rates: new Map(),
      telemetry: EMPTY_OPTIONAL_RPC_TELEMETRY,
    },
