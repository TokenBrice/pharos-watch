...
describe("fetchPendleMarketSources", () => {
  ...
  it("filters expired and implausibly high implied APY markets", async () => {
    mockFetch([{
      match: "api-v2.pendle.finance",
      body: { total: 2, limit: 100, skip: 0, results: [
        {
          id: "1-0xexpired", address: "0xexpired", chainId: 1, isActive: true,
        ...
        {
          id: "1-0xhigh", address: "0xhigh", chainId: 1, isActive: true,
          expiry: new Date(Date.now() + 30 * 86400 * 1000).toISOString(),
          impliedApy: 1.5, underlyingApy: 0.031, aggregatedApy: 1.5,
          underlyingAsset: { symbol: "USDG", address: "0xdef" },
          assetRepresentation: "USDG", protocol: "Global Dollar",
          liquidity: { usd: 82_000_000 },
          categoryIds: ["stables"],
