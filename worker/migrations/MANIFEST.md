...
| 0200     | `0200_safety_score_v9_shadow_history.sql`                | Add immutable compressed V9 replay artifacts, complete shadow-attempt history, canonical daily summaries, and fenced model-selection state          |
...
| `safety_score_v9_artifacts` | V9 release evidence - retain through V9-15 | Counted candidate inputs and matching policy/build/result artifacts must remain checksum-verifiable through release and the complete post-release observation window; archive or prune only under an explicit V9 retirement procedure. |
| `safety_score_v9_shadow_attempts` | V9 release evidence - retain through V9-15 | Every scheduled, missed, failed, retried, and successful attempt is part of the prospective release gate and cannot be reconstructed from latest-cache state. |
| `safety_score_v9_shadow_days` | V9 release evidence - retain through V9-15 | Canonical day selection and blockers prove the uninterrupted shadow window; retain with its underlying attempts and artifacts. |
...
- `0200_safety_score_v9_shadow_history.sql`: roll back V9 shadow publication by restoring the prior Worker. Keep the additive artifacts, attempts, daily summaries, and publication-state row for replay and forensic inspection; do not delete counted release evidence.
