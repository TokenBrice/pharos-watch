...
Analytics dashboard tracking 192 stablecoins (+2 shadow assets for PSI). Static Next.js 16 export → Cloudflare Pages. API: Cloudflare Worker + D1.
...
```
src/app/         — Representative pages (homepage, admin, blacklist, cemetery, chains, chains/[chain], changelog, compare, coverage, dependency-map, depeg, digest, flows, liquidity, methodology, portfolio, privacy, safety-scores, stability-index, start, status, telegram, treasuries, about, yield, stablecoin/[id], stablecoins/[peg], stablecoins/backing/[backing], stablecoins/governance/[governance], stablecoins/infrastructure/[infrastructure])
