...
export const YIELD_METHODOLOGY_V8: readonly MethodologyChangelogEntry[] = [
  {
    ...
    impact: [
      "`effectiveYield` becomes `max(0, apy30d + 0.25 * benchmarkSpread + (usdBenchmarkRate - benchmarkRate))`, equivalently `max(0, usdBenchmarkRate + 1.25 * benchmarkSpread)`. The 25% spread slice is retained; the new `hurdleRebase` term is zero for every row benchmarked at a USD-currency rate. Re-scoring the final v8.42 publication generation (156 scored rows) through the released scorer — the replay includes this release's venue-registry, rewardShare and switch-count (churn-term) rules, with switch counts as published — 140 of the 156 scored rows are identical to v8.42 (top of the board unchanged: DUSD 85, BOLD 69, USDC 59); the NAV seed-filter rewrite of rolling APY history cannot be replayed from the payload, so the first post-release publication finalizes the count",
      "Wrapped iTRY (`witry-brix`) falls from PYS 44 (rank 5) to 6 (rank ~95). CHF and EUR rows rise because their local hurdles sit below the USD rate (ZCHF 26 → 51, rank 17 → 5; EURC 20 → 33) — this is the covered-interest-parity reading of excess yield on a USD footing, which assumes a frictionless hedge and ignores FX basis; PYS does not model FX carry or hedging cost. Exactly three rows move more than 10 points (wiTRY −38, ZCHF +25, EURC +13) and the `yield.pys_dropped` tape event fires once, for wiTRY; five USD-benchmarked wrapper rows pick up their first venue penalty from the child-id venue map (gtUSDC 39 → 34, bbqUSDC 28 → 25, stcUSD 19 → 17, syrupUSDC 14 → 13, syrupUSDT 10 → 9), included in the identical count above",
