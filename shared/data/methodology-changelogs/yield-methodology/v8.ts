...
export const YIELD_METHODOLOGY_V8: readonly MethodologyChangelogEntry[] = [
  {
    ...
    summary:
      "Pharos Yield Score credited high-rate pegs for paying their own central-bank rate: Wrapped iTRY at 38% APY, only +1.3 points over the BIST TLREF overnight rate, ranked beside USDC at +0.8 over T-bills. Effective yield now adds the USD risk-free rate minus the row's own benchmark, so a peg's policy-rate compensation is no longer scored as excess yield; the re-base term itself is zero for USD-currency rows. The same release hardens the scoring inputs it depends on: the USD reference is gated on benchmark health, arbitration can no longer be won by the churn it creates, variance and APY-envelope failures fail closed, and reward share and venue resolution are re-derived from one payload so published and scored values agree — five USD wrapper rows pick up their first venue penalty from that resolution.",
    impact: [
      "`effectiveYield` becomes `max(0, apy30d + 0.25 * benchmarkSpread + (usdBenchmarkRate - benchmarkRate))`, equivalently `max(0, usdBenchmarkRate + 1.25 * benchmarkSpread)`. The 25% spread slice is retained; the new `hurdleRebase` term is zero for every row benchmarked at a USD-currency rate. Re-scoring the final v8.42 publication generation (156 scored rows) through the released scorer alone — the re-base plus this release's venue-registry, rewardShare and switch-count (churn-term) rules, with switch counts as published — leaves 140 rows identical; the first publication under the release moved more, because a payload replay cannot see the release's source-adapter fixes — a row whose adapter previously could not qualify now wins arbitration and republishes from its own source (BOLD, below). The finalized publication therefore stands as the record: 86 of 155 scored rows unchanged, four rows down more than 10 points and two up (below), with the top of the board otherwise unchanged (DUSD 85, USDC 59)",
      "Under the finalized publication, rows moving more than 10 PYS points are BOLD 70 → 28 (−42), wiTRY 44 → 6 (−38), sDOLA 18 → 6 (−12) and CUSD 32 → 22 (−10) down, and ZCHF 26 → 51 (+25) and EURC 20 → 34 (+14) up. CHF and EUR rows rise because their local hurdles sit below the USD rate — the covered-interest-parity reading of excess yield on a USD footing, which assumes a frictionless hedge and ignores FX basis; PYS does not model FX carry or hedging cost. BOLD's fall is neither the re-base nor a history rewrite: it is the v8.42 source fix completing. Its row previously resolved through `linked-variant:ybold-yearn:protocol-api:yearn:ybold` and so published the tracked yBOLD wrapper's 7.78% APY; with this release's adapter fix the direct `onchain:bold-liquity` Stability Pool reader finally carries a valid observation time, wins arbitration, and BOLD publishes its own 3.34% Stability Pool yield — a correctness improvement, at `partial` qualification because the deterministic row's evidence is not yet complete. The `yield.pys_dropped` tape event fires for the four drops (wiTRY at warning severity, BOLD severe). Five USD-benchmarked wrapper rows also pick up their first venue penalty from the child-id venue map (gtUSDC 39 → 34, bbqUSDC 28 → 25, stcUSD 19 → 17, syrupUSDC 14 → 13, syrupUSDT 10 → 9)",
      "Only the 14 rows whose selected benchmark is not a USD-currency benchmark (7 EUR, 3 GBP, 2 CHF, 1 TRY, 1 RUB) carry a non-zero re-base. The 6 USD rows benchmarked against `USD_EFFR` take no re-base (same USD currency), so they score identically to v8.42",
      ...
      "`usdBenchmarkRate` is populated only while the USD benchmark entry classifies healthy on its own feed evidence; otherwise it is null (rebase 0, row scores as under v8.42) and affected non-USD rows publish `estimated` (degraded) or NR / `benchmark-stale` (stale) with the `reference-benchmark-degraded` warning instead of a silently mis-based score",
      "Scoring guards fail closed: a nominal `apy30d` above the new `PYS_APY_SANITY_MAX = 300` envelope scores 0 instead of clamping toward 100, a non-finite APY variance resolves to maximum variance (sustainability multiplier at its 0.3 floor) while a missing variance keeps the documented neutral 0, `apyVarianceScore` is derived from the published two-decimal `yieldStability` so served and published scores agree, and the persisted `pys_inputs_at_publish` snapshot (schemaVersion 2) now carries `usdBenchmarkRate`/`hurdleRebase` so history replays the published score exactly (`pysReproducibility`: exact / not-scored / legacy-partial / invalid)",
      "Arbitration charges every candidate whose selection would be a real switch a flat 0.1 margin on the pre-run switch count, with a deterministic source-key tie-break; a switch is counted only when the previous winner is a live non-rejected candidate (otherwise `previous-source-transiently-missing`); the churn term is published on the chosen row only, so alternate rows publish no switch count and their stored penalty excludes the term",
      "`sourceRisk.rewardShare` is re-derived every publication from the same payload the penalty uses: base-only rows publish 0 instead of null, and the never-expiring stored-value fallback is gone",
      "`sourceRisk.venueProtocol` never records the derivation method (former `price-derived`/`rate-derived` venues publish null); wrapper rows resolve their venue through a child-id map (e.g. `susds-sky`/`stusds-sky` → `spark-savings`, `stcusd-cap` → `cap`, `scrvusd-curve` → `curve-llamalend`) plus the `pendle-v2` → `pendle` and `sdai` → `spark-savings` aliases, lifting known venue-tier coverage on wrapper rows",
      "A non-finite `benchmarkRate` on one measurement now omits that row-level field and degrades only that row instead of failing the whole publication generation",
      "The CAD benchmark entry is relabelled 'CAD Bank rate (policy, monthly)' — series `V122530` is the monthly administered Bank of Canada policy rate, not CORRA; user-visible on benchmark chips",
    ],
    commits: [],
    reconstructed: false,
  },
