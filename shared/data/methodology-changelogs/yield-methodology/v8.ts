...
export const YIELD_METHODOLOGY_V8: readonly MethodologyChangelogEntry[] = [
  {
    ...
    impact: [
      "`effectiveYield` becomes `max(0, apy30d + 0.25 * benchmarkSpread + (usdBenchmarkRate - benchmarkRate))`, equivalently `max(0, usdBenchmarkRate + 1.25 * benchmarkSpread)`. The 25% spread slice is retained; the new `hurdleRebase` term is zero for every row benchmarked at a USD-currency rate. Re-scoring the final v8.42 publication generation (156 scored rows) through the released scorer alone — the re-base plus this release's venue-registry, rewardShare and switch-count (churn-term) rules, with switch counts as published — leaves 140 rows identical; the first publication under the release moved more, because the release also rewrites rolling APY history (the bootstrap-seed filter) and that cannot be replayed from a payload. The finalized publication therefore stands as the record: 86 of 155 scored rows unchanged, four rows down more than 10 points and two up (below), with the top of the board unchanged (DUSD 85, BOLD's replacement ordering intact, USDC 59)",
      "Under the finalized publication, rows moving more than 10 PYS points are BOLD 70 → 28 (−42), wiTRY 44 → 6 (−38), sDOLA 18 → 6 (−12) and CUSD 32 → 22 (−10) down, and ZCHF 26 → 51 (+25) and EURC 20 → 34 (+14) up. CHF and EUR rows rise because their local hurdles sit below the USD rate — the covered-interest-parity reading of excess yield on a USD footing, which assumes a frictionless hedge and ignores FX basis; PYS does not model FX carry or hedging cost. BOLD's fall is the history rewrite, not the re-base: it is benchmarked in USD, so its hurdle is untouched, but its rolling `apy30d` drops once the bootstrap-seed rows leave the window. The `yield.pys_dropped` tape event fires for the four drops (wiTRY at warning severity, BOLD severe). Five USD-benchmarked wrapper rows also pick up their first venue penalty from the child-id venue map (gtUSDC 39 → 34, bbqUSDC 28 → 25, stcUSD 19 → 17, syrupUSDC 14 → 13, syrupUSDT 10 → 9)",
