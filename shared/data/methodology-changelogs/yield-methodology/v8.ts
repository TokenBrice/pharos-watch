...
export const YIELD_METHODOLOGY_V8: readonly MethodologyChangelogEntry[] = [
  {
    ...
    summary:
      "Pharos Yield Score credited high-rate pegs for paying their own central-bank rate: Wrapped iTRY at 38% APY, only +1.3 points over the BIST TLREF overnight rate, ranked beside USDC at +0.8 over T-bills. Effective yield now adds the USD risk-free rate minus the row's own benchmark, so a peg's policy-rate compensation is no longer scored as excess yield. USD-currency rows are unchanged. The same release hardens the scoring inputs it depends on: the USD reference is gated on benchmark health, arbitration can no longer be won by the churn it creates, variance and APY-envelope failures fail closed, and reward share and venue resolution are re-derived from one payload so published and scored values agree.",
    impact: [
