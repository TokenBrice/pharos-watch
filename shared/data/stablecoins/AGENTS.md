...
- Supported sidecar domains are `reserves` (`reserves`), `mint-authority` (`mintAuthority`), `compliance` (`mica`, `genius`), and `risk-review` (`canBeBlacklisted`, `blacklistabilityReview`, `oracleRisk`, `bridgeRouteRisk`). See `docs/process/stablecoin-research-sidecars.md`.
