...
- The supported sidecar domains and the fields each one owns are defined by `STABLECOIN_SOURCE_DOMAIN_FIELDS` in `shared/lib/stablecoins/schema.ts`. Domain semantics, including which sidecar owns a given control and the `bridgeRouteRisk` route/control rules, are owned by `docs/process/stablecoin-research-sidecars.md`.
