// Shared by frontend, Pages Functions, script, and Worker tests; keep this out of a runtime boundary.
...
function cloneRequest(request: Request): Request {
  // Cloudflare's Request.clone() preserves generic host metadata that is not
  // part of this runtime-neutral test helper's public contract.
...
async function routeMatches(
  ...
  const requestMatches = typeof route.match === "function"
    ? await route.match(cloneRequest(request))
    : strictUrl
...
export function mockFetch(routes: MockRoute[] = [], options: MockFetchOptions = {}): MockFetchSpy {
  ...
  const fetchImplementation: MockFetchFn = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if ("respond" in route) {
      outcome = await route.respond(cloneRequest(normalized.request));
