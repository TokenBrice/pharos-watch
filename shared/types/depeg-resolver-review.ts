...
export const DdrrV2SummaryMetricsSchema = z.object({
  ...
  predictionRatePct: RatioSchema.min(0).max(1).nullable(),
  invalidationAdjustedPredictionRatePct: RatioSchema.min(0).max(1).nullable(),
  decisionProgressPct: RatioSchema.min(0).max(1).nullable(),
  operationalMissRatePct: RatioSchema.min(0).max(1).nullable(),
  noCallRatePct: RatioSchema.min(0).max(1).nullable(),
  preLockRecoveredPct: RatioSchema.min(0).max(1).nullable(),
  preLockTerminalPct: RatioSchema.min(0).max(1).nullable(),
  missedLockPct: RatioSchema.min(0).max(1).nullable(),
  stateAssignedPct: RatioSchema.min(0).max(1).nullable(),
  finalizedCoveragePct: RatioSchema.min(0).max(1).nullable(),
  ...
  invalidatedPct: RatioSchema.min(0).max(1).nullable(),
