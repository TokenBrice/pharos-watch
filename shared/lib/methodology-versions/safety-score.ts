import { SAFETY_SCORE_V1 } from "../../data/methodology-changelogs/safety-score/v1";
import { SAFETY_SCORE_V2 } from "../../data/methodology-changelogs/safety-score/v2";
import { SAFETY_SCORE_V3 } from "../../data/methodology-changelogs/safety-score/v3";
import { SAFETY_SCORE_V4 } from "../../data/methodology-changelogs/safety-score/v4";
import { SAFETY_SCORE_V5 } from "../../data/methodology-changelogs/safety-score/v5";
import { SAFETY_SCORE_V6 } from "../../data/methodology-changelogs/safety-score/v6";
import { SAFETY_SCORE_V7 } from "../../data/methodology-changelogs/safety-score/v7";
import { SAFETY_SCORE_V8 } from "../../data/methodology-changelogs/safety-score/v8";
import {
  SAFETY_SCORE_V9_BRIDGE_AND_COMMODITY_DEFECT_CORRECTIONS,
  SAFETY_SCORE_V9_VALIDATOR_QUORUM_AUTHORITY,
  SAFETY_SCORE_V9_ADJUDICATED_MECHANISM_NON_DISCLOSURE,
  SAFETY_SCORE_V9_UNPROVEN_SETTLEMENT_BOUND,
  SAFETY_SCORE_V9_EXIT_CAPACITY_TRUTH,
  SAFETY_SCORE_V9_TETHER_DISCLOSURE_FRESHNESS,
  SAFETY_SCORE_V9_AUDITED_STALE_RESERVE_CEILING,
  SAFETY_SCORE_V9_AUDITED_RESERVE_FALLBACK,
  SAFETY_SCORE_V9_EVIDENCE_SCOPE_AND_FRESHNESS,
  SAFETY_SCORE_V9_CAP_REASON_PRECEDENCE,
  SAFETY_SCORE_V9_CAP_AND_EVIDENCE_CLARIFICATIONS,
  SAFETY_SCORE_V9_EXIT_ROUTE_STABILITY,
  SAFETY_SCORE_V9_MINT_POSTURE_LADDER_REFINEMENT,
  SAFETY_SCORE_V9_ACTIVATION,
  SAFETY_SCORE_V9_NONE_RESOLVED_TOP_RUNG,
  SAFETY_SCORE_V9_CAUSAL_RESPONSIBILITY,
  SAFETY_SCORE_V9_COMMODITY_CLAIM_ARCHETYPE,
  SAFETY_SCORE_V9_CONTROL_SECTION_APPLICABILITY,
  SAFETY_SCORE_V9_CURATED_DEPENDENCY_GATE,
  SAFETY_SCORE_V9_EXIT_BOUNDARY_UNIFICATION,
  SAFETY_SCORE_V9_INCIDENT_DECAY_SEVERITY,
  SAFETY_SCORE_V9_EXIT_REDUNDANCY,
  SAFETY_SCORE_V9_MERGED_MINT_GRADER,
  SAFETY_SCORE_V9_MINT_BRIDGE_SCOPE_BOUNDARY,
  SAFETY_SCORE_V9_POLICY_GATE_PROVENANCE,
  SAFETY_SCORE_V9_NATIVE_INPUT,
  SAFETY_SCORE_V9_ROUTE_CAPACITY,
  SAFETY_SCORE_V9_STRESS_STATE_DIGEST_REMOVAL,
  SAFETY_SCORE_V9_ACCESS_STRUCTURAL_APPLICABILITY,
  SAFETY_SCORE_V9_UNRECOGNIZED_CHAIN_LABEL_TOLERANCE,
  SAFETY_SCORE_V9_UNSUPPORTED_EXIT_COVERAGE,
  SAFETY_SCORE_V9_WRAPPER_LOCAL_CONTROL_EVIDENCE,
  SAFETY_SCORE_V9_WRAPPER_OPERATOR_CLASSIFICATION,
  SAFETY_SCORE_V9_ORACLE_APPLICABILITY,
  SAFETY_SCORE_V9_RESERVE_FRESHNESS_PROVENANCE,
  SAFETY_SCORE_V9_RESERVE_SLICE_IDENTITY,
  SAFETY_SCORE_V9_ROUTE_BUDGET_COMPLETENESS,
  SAFETY_SCORE_V9_REVIEWED_BOUNDED_RESIDUALS,
  SAFETY_SCORE_V9_ATTRIBUTION_RECONCILIATION,
  SAFETY_SCORE_V9_EVIDENCE_ATTRIBUTION,
  SAFETY_SCORE_V9_TRANSFER_MATERIALITY_OBSERVATION,
  SAFETY_SCORE_V9_UNDISCLOSED_PRIMARY_EXIT,
  SAFETY_SCORE_V9_BRIDGE_RESIDUE_MATERIALITY,
  SAFETY_SCORE_V9_SCOPED_CONTROL_QUESTIONS,
  SAFETY_SCORE_V9_BRIDGE_SCOPED_QUESTIONS,
} from "../../data/methodology-changelogs/safety-score/v9-activation";
import { createMethodologyVersion } from "./base";
import {
  SAFETY_SCORE_METHODOLOGY_CHANGELOG_PATH,
  SAFETY_SCORE_METHODOLOGY_VERSION,
} from "./constants";

const safetyScore = createMethodologyVersion({
  currentVersion: SAFETY_SCORE_METHODOLOGY_VERSION,
  changelogPath: SAFETY_SCORE_METHODOLOGY_CHANGELOG_PATH,
  changelog: [
    SAFETY_SCORE_V9_BRIDGE_AND_COMMODITY_DEFECT_CORRECTIONS,
    SAFETY_SCORE_V9_VALIDATOR_QUORUM_AUTHORITY,
    SAFETY_SCORE_V9_ADJUDICATED_MECHANISM_NON_DISCLOSURE,
    SAFETY_SCORE_V9_UNPROVEN_SETTLEMENT_BOUND,
    SAFETY_SCORE_V9_EXIT_CAPACITY_TRUTH,
    SAFETY_SCORE_V9_TETHER_DISCLOSURE_FRESHNESS,
    SAFETY_SCORE_V9_AUDITED_STALE_RESERVE_CEILING,
    SAFETY_SCORE_V9_AUDITED_RESERVE_FALLBACK,
    SAFETY_SCORE_V9_EVIDENCE_SCOPE_AND_FRESHNESS,
    SAFETY_SCORE_V9_CAP_REASON_PRECEDENCE,
    SAFETY_SCORE_V9_CAP_AND_EVIDENCE_CLARIFICATIONS,
    SAFETY_SCORE_V9_EXIT_ROUTE_STABILITY,
    SAFETY_SCORE_V9_MINT_POSTURE_LADDER_REFINEMENT,
    SAFETY_SCORE_V9_CURATED_DEPENDENCY_GATE,
    SAFETY_SCORE_V9_NONE_RESOLVED_TOP_RUNG,
    SAFETY_SCORE_V9_BRIDGE_SCOPED_QUESTIONS,
    SAFETY_SCORE_V9_SCOPED_CONTROL_QUESTIONS,
    SAFETY_SCORE_V9_BRIDGE_RESIDUE_MATERIALITY,
    SAFETY_SCORE_V9_UNDISCLOSED_PRIMARY_EXIT,
    SAFETY_SCORE_V9_CONTROL_SECTION_APPLICABILITY,
    SAFETY_SCORE_V9_MINT_BRIDGE_SCOPE_BOUNDARY,
    SAFETY_SCORE_V9_POLICY_GATE_PROVENANCE,
    SAFETY_SCORE_V9_RESERVE_SLICE_IDENTITY,
    SAFETY_SCORE_V9_ROUTE_BUDGET_COMPLETENESS,
    SAFETY_SCORE_V9_RESERVE_FRESHNESS_PROVENANCE,
    SAFETY_SCORE_V9_REVIEWED_BOUNDED_RESIDUALS,
    SAFETY_SCORE_V9_ATTRIBUTION_RECONCILIATION,
    SAFETY_SCORE_V9_EVIDENCE_ATTRIBUTION,
    SAFETY_SCORE_V9_TRANSFER_MATERIALITY_OBSERVATION,
    SAFETY_SCORE_V9_ORACLE_APPLICABILITY,
    SAFETY_SCORE_V9_STRESS_STATE_DIGEST_REMOVAL,
    SAFETY_SCORE_V9_ACCESS_STRUCTURAL_APPLICABILITY,
    SAFETY_SCORE_V9_COMMODITY_CLAIM_ARCHETYPE,
    SAFETY_SCORE_V9_WRAPPER_OPERATOR_CLASSIFICATION,
    SAFETY_SCORE_V9_INCIDENT_DECAY_SEVERITY,
    SAFETY_SCORE_V9_EXIT_BOUNDARY_UNIFICATION,
    SAFETY_SCORE_V9_MERGED_MINT_GRADER,
    SAFETY_SCORE_V9_NATIVE_INPUT,
    SAFETY_SCORE_V9_EXIT_REDUNDANCY,
    SAFETY_SCORE_V9_WRAPPER_LOCAL_CONTROL_EVIDENCE,
    SAFETY_SCORE_V9_UNSUPPORTED_EXIT_COVERAGE,
    SAFETY_SCORE_V9_UNRECOGNIZED_CHAIN_LABEL_TOLERANCE,
    SAFETY_SCORE_V9_CAUSAL_RESPONSIBILITY,
    SAFETY_SCORE_V9_ROUTE_CAPACITY,
    SAFETY_SCORE_V9_ACTIVATION,
    ...SAFETY_SCORE_V8,
    ...SAFETY_SCORE_V7,
    ...SAFETY_SCORE_V6,
    ...SAFETY_SCORE_V5,
    ...SAFETY_SCORE_V4,
    ...SAFETY_SCORE_V3,
    ...SAFETY_SCORE_V2,
    ...SAFETY_SCORE_V1,
  ],
});

export {
  SAFETY_SCORE_METHODOLOGY_CHANGELOG_PATH,
  SAFETY_SCORE_METHODOLOGY_VERSION,
  SAFETY_SCORE_METHODOLOGY_VERSION_LABEL,
} from "./constants";
export const SAFETY_SCORE_METHODOLOGY_CHANGELOG = safetyScore.changelog;
export const SAFETY_SCORE_METHODOLOGY_CHANGELOG_NAV_VERSIONS = safetyScore.versionLabels;
