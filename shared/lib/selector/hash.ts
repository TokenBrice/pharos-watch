/**
 * Small synchronous SHA-256 helper for selector dataset hashing.
 *
 * `buildSelectorRows()` is intentionally synchronous because the Selector
 * runs inside React `useMemo`; Web Crypto's async digest would force a wider
 * frontend contract. This implementation is deterministic in browser and
 * Node runtimes and avoids Node-only `crypto` imports in the client bundle.
 */
