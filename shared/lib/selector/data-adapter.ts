...
import { COLLATERAL_QUALITY_SCORE } from "../report-card-policy";
import type {
  AltYieldSource,
  BluechipRatingsMap,
  DexLiquidityMap,
...
export function buildSelectorRows(args: BuildSelectorRowsArgs): BuildSelectorRowsResult {
  ...
  for (const meta of CLIENT_TRACKED_STABLECOINS) {
    ...
    const currentDeviationBps = peg?.currentDeviationBps ?? rawInputs?.activeDepegBps ?? dex?.dexDeviationBps ?? null;
    ...
    const row: MergedRow = {
      ...
      collateralQuality:
        rawInputs?.collateralQuality != null ? COLLATERAL_QUALITY_SCORE[rawInputs.collateralQuality] : null,
      ...
      effectiveExitScore: rawInputs?.effectiveExitScore ?? redemption?.effectiveExitScore ?? null,
