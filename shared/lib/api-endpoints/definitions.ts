...
const BASE_ENDPOINT_DEFINITIONS = [
  ...
  adminAction({
    key: "trigger-yield-coverage-audit",
    path: API_PATHS.triggerYieldCoverageAudit(),
    cacheBypass: false,
    opsProxyTimeoutMs: 330_000,
    routeDependencies: ["chainRpcs"],
  }, {
      label: "Run Yield Coverage Audit",
      confirm: "Recompute the yield coverage audit using current configuration and cached inputs?",
      group: "audit",
      kind: "repair",
      risk: "low",
      scope: globalActionScope("Yield coverage audit cache"),
      expectedDuration: "Synchronous; keep the connection open until completion",
      preconditions: ["Deploy reviewed yield configuration first and verify yield rankings are available."],
      resultMode: "immediate",
      rollback: "No automatic rollback; the next audit replaces this report.",
