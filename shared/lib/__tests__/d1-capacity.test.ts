...
describe("assessD1Capacity", () => {
  ...
  it("forecasts the next threshold and exhaustion from a 30-day linear trend", () => {
    const current = { observedAt: NOW, databaseSizeBytes: 7 * GB };
    const result = assessD1Capacity(current, [
      { observedAt: NOW - 20 * 86_400, databaseSizeBytes: 5 * GB },
      { observedAt: NOW - 10 * 86_400, databaseSizeBytes: 6 * GB },
      current,
    ]);

    expect(result.forecastBasis).toBe("linear-30d");
