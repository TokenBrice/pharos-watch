...
def verify_coin(coin: dict, dl_gecko: str | None, api_key: str | None) -> dict:
    ...
    if our_gecko:
        ...
        if our and "id" in our:
            ...
            price = our.get("market_data", {}).get("current_price", {}).get("usd")
...
def print_result(result: dict) -> None:
    ...
    if result.get("dl_name"):
        ...
        print(
            f"  [DL]       {dl_gecko:<30} -> {result['dl_name']:<30} addr_match={status}"
        )

    if result["contract_gecko"]:
        print(
            f"  [CONTRACT] {result['contract_gecko']:<30} -> {result.get('contract_name', '?')}"
        )
