...
2. Research with `WebSearch` / `WebFetch` (agent-browser on 403), in descending authority: Federal Register → federal regulators (OCC/Fed/FDIC/NCUA/FinCEN/OFAC/Treasury) → state regulators → issuer filings/disclosures → auditor reports → news (never alone for an official status).
