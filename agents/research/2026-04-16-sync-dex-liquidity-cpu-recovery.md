...
- 2026-04-16 08:40 UTC, 09:10 UTC, 09:40 UTC, and 10:10 UTC: the half-hourly slot entered `sync-dex-liquidity` but did not write a final `cron_runs` row.
- `wrangler tail` during the 09:40 UTC and 10:10 UTC slots reported `"10,40 * * * *" ... Exceeded CPU Limit`.
...
- After gas normalization was deployed, the 10:10 UTC run no longer emitted the leading-zero RPC error, but still exceeded CPU after fetching 8.5K direct-API pools and merging 1.8K staged pools.
...
The no-reporting root cause was Cloudflare CPU termination. The recent direct-API liquidity enhancements expanded the in-memory direct-source surface to thousands of pools, most of which cannot match any tracked stablecoin. The scoring cron still hydrated metadata, built identities, counted identity keys, and ran dedupe over that full untracked set before discovering it could not contribute.

The malformed Fluid gas quantity was a contributing bug. The Fluid direct-API enrichment path sent `eth_call` with `gas: "0x0F4240"`. JSON-RPC quantities must not contain leading zero digits. Alchemy and fallback RPCs rejected those calls repeatedly during the DEX liquidity run, which burned additional CPU/logging budget inside the already-heavy direct-API/staged-pool phase.

Because Cloudflare killed the isolate, `logCronRun()` could not catch the failure or insert an error row.
...
- Filter direct-API pools to tracked-token candidates before metadata hydration, identity counting, dedupe, conversion, and price-observation extraction. Sub-threshold tracked direct pools still reserve their exact identity so staged discovery cannot re-add the same physical pool.
...
- `npm test -- worker/src/cron/dex-liquidity/__tests__/orchestrator-phases.test.ts worker/src/cron/__tests__/sync-dex-liquidity.test.ts worker/src/lib/__tests__/evm-rpc.test.ts`
