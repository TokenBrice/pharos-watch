...
| Named risk-water areas | `systems/risk-water-areas.test.ts`; `systems/area-labels.test.ts`; `systems/pharosville-world.test.ts` `names DEWS water areas from live band counts`; `maps warning and danger DEWS ships...`; Playwright `pharosville exposes all named risk water areas in browser details` | DEWS bands plus Data Fog and Ledger Mooring map to named water areas, printed labels, placement terrain, browser-selectable details, risk-water labels, and risk zones | `npm test -- src/app/pharosville/systems/risk-water-areas.test.ts src/app/pharosville/systems/area-labels.test.ts src/app/pharosville/systems/pharosville-world.test.ts`; `npx playwright test tests/visual/pharosville.spec.ts --grep "named risk water"` |
...
| Motion route behavior | `systems/motion.test.ts` | Normal-motion routes, route cycles, all seven risk-water zones, meaningful dockless patrols, ordered DEWS dwell/wake/drift, storm/fog proximity, and water-only samples | `npm test -- src/app/pharosville/systems/motion.test.ts` |
