...
1. Read these files for context:
   ...
   - `shared/lib/stablecoins.ts` (stablecoin metadata with `contracts: ContractDeployment[]` — array of `{ chain, address, decimals }`)
...
e) **Chain-aware source routing**: For each coin, build `coinChains` from its `contracts` array (type `ContractDeployment[]`): `new Map((coin.contracts ?? []).map(c => [c.chain, c.address]))`. This gets passed to the crawl function.
...
2. Also read `shared/lib/stablecoins.ts` and `shared/types/index.ts` — understand the `contracts` field shape: `ContractDeployment[]` (array of `{ chain, address, decimals }`), NOT a `Record`.
...
3. Add tests to `worker/src/cron/dex-discovery/__tests__/orchestrator.test.ts`:
   - **Chain-aware routing**: Coin with `contracts: [{ chain: "ethereum", address: "0x...", decimals: 6 }, { chain: "arbitrum", address: "0x...", decimals: 6 }]` only generates crawl calls for those 2 chains, not all 30+
