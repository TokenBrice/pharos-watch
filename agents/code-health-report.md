...
- **Pre-push follow-up:** done 2026-06-13 — regenerated `docs/agent-code-map.md` with `node scripts/maintenance/generate-agent-code-map.mjs` after the localized refactors landed.

### Wave 2 — Localized Refactors

| Theme | Members | Combined action | Effort | Risk |
| --- | --- | --- | --- | --- |
| `[x]` `isRecord` guard consolidation | `f-xcut-1`, `slu-4`, `worker-store-5`, `worker-infra-4` | Done 2026-06-13: selector snapshot, frontend bootstrap/API guards, worker status/cache/auth/raw snapshot, address-price provider barrel, alert-safety cache, depeg publication, and yield cleanup wrappers now route through canonical `isRecord`. Scripts and divergent `UnknownRecord`/utility helpers intentionally stayed out. | small | low |
...
| `[x]` Worker error-message coercion helper | `f-xcut-2` | Done 2026-06-13: added `worker/src/lib/error-utils.ts`, folded redstone/pricing/safe wrappers, and migrated the same-identifier worker ternaries to `toErrorMessage(error)`. Kept the helper worker-local, not in `shared/lib`. | medium | low |
...
| `[x]` Shared types derive from Zod schemas / enums | `types-1`, `types-5`, `types-6`, `types-2`, `types-3`, `types-4`, `types-7`, `fe-6`, `chains-9` | Done 2026-06-13: prior rows already exported the other shared schema/enum-derived types; this pass tightened `CronRun.status` with `CronRunStatusSchema` and preserves unknown raw statuses in metadata. `TelegramDispatchCronMetadata` and `DeadStablecoin.contracts` remain explicit. | small | low |
| `[x]` Single-source cross-file constant lists | `chains-7`, `vm-4`, `rbc-10`, `pc-2`, `wapi-6`, `worker-store-7`, `types-8` | Verified done 2026-06-13: `ACTIVE_BACKING_DIVERSITY_TYPES` is exported and aggregator `backingTotals` derives from it; remaining members are comment-only cross-references or intentional holds. `DeadStablecoin.contracts` stayed out. | trivial | low |
| `[x]` Pure helpers out of component/client files | `fe-10`, `f-compare-2`, `f-stablecoin-detail-2`, `sd-1` | Done 2026-06-13: `gradeBandLabel`, `buildCompareSelectionInsights`, and `buildYieldStoryCallouts` live in `src/lib`, tests import the lib helpers directly, and the single-use detail-section alias is gone. | trivial | none |
...
| `[x]` God-file decomposition (Mint Authority VM) | `f-xcut-6`, `vm-11`, `frontend-7` | Done 2026-06-13: moved the Mint Authority view-model slice, labels, `labelFromMap`, and unchanged `shortenAddress` into `stablecoin-detail-mint-authority-view-model.ts`; `stablecoin-detail-view-model.ts` re-exports the public API. | medium | low |
...
- Done 2026-06-13: moved both pure helpers into `src/lib` modules and repointed compare/yield tests away from the client components.
