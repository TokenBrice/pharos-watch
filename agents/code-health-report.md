...
| --- | --- | --- | --- | --- |
| `[ ]` Trivial `sleep(ms)` helper consolidation | `f-xcut-8`, `w-cron-2`, `scr-8`, `scr-1` | Export plain `sleep` from `worker/src/lib/abort.ts`; import in cron-lease + cron-logger (source from abort.ts, NOT cron-logger — would cycle). Leave `sleepWithSignal` / `sleepWithAbort`. In scripts, import `sleep` from `smoke-runtime.mjs` in run-pages-smoke, wait-pages-release-marker, rollback-pages-deployment, smoke-ui. | trivial | low |
| `[ ]` Time-unit & basis-point magic numbers | `slu-2`, `slu-3`, `slu-11`, `pc-7`, `pc-8`, `wapi-7`, `worker-infra-6` | Shared `time-constants` drop-ins (relative-time, format, og.tsx 86400->DAY_SECONDS, fetch-tbill-rate 86_400_000->DAY_MS). Per-file consts for cache-age 1200 and INT32 2147483647. `BPS_PER_UNIT` in a neutral math-constants module (NOT format.ts), skip the dollar literal at redemption-backstop-cost.ts:46. | small | low |
| `[ ]` Dead exports masked from check:unused-code | `cls-1`, `sel-1`, `sel-10`, `api-2`, `frontend-5`, `f-pharoswatchbot-1`, `vm-5`, `w-dispatch-3`, `cls-6`, `api-3` | Delete the genuinely-dead exports/aliases; regenerate agent-code-map after site-data-origin. Borderline (flag, don't blind-delete): classification-pegs.ts facade and getStrictContractPaths (in-file constant dedup only — allowlisted + mocked). | trivial | none |
| `[ ]` Unreachable code & no-op recomputes (worker) | `worker-store-1`, `worker-6`, `pc-4`, `pc-9`, `w-dispatch-2`, `wrouter-3`, `wrouter-4`, `w-cron-3`, `worker-11`, `w-dispatch-1`, `w-status-1`, `w-status-3`, `ci-2`, `ci-3` | Delete unreachable functions + redundant recomputes; `parseBoeSoniaCsv` is flag-don't-delete (comment + owner confirm). | small | low |
| `[ ]` Repeated single-array scans collapse to one pass | `w-status-2`, `wapi-2`, `chains-8`, `worker-8`, `chains-4`, `sel-5`, `sel-6`, `f-hooks-4`, `f-hooks-6`, `f-hooks-7`, `scr-10`, `scr-5` | Cache `ADMIN_PROBE_PATHS.includes`, hoist `Math.floor(Date.now()/1000)`, single-pass aggregator/buildSymbolLookups, `continue`->`break`, inline Set literals, single lazy useState, drop redundant `enabled:true`, hoist helpers. | small | none |
| `[ ]` Cross-boundary type re-derivations & import nits | `sd-4`, `types-3`, `vm-9`, `slu-2` | Export `HeroSectionBaseProps` (intersect `reportCard` back), import `DdrAssessmentCheckpoint` from `@shared/types/depeg-resolver`, merge split type-import lines in compare-derive.ts. | trivial | none |
...
| Theme | Members | Combined action | Effort | Risk |
| --- | --- | --- | --- | --- |
| `[ ]` Number-coercion parsers onto number-utils.ts | `f-xcut-4`, `worker-infra-5` | Consolidate ONLY the two identical `Number()`-based `parsePositiveNumber` copies. Leave the `parseFloat` CEX/etherfuse variants (lenient trailing-garbage parsing on a live price path) or verify per-site first. | small | medium |
| `[ ]` Hand-rolled median/percentile (frontend + offline) | `fviz-2`, `vm-1` | Frontend median: radar -> `median`, yield-view-model -> `median(apys) ?? 0` (preserves typed-number 0-on-empty). Percentile: consolidate contagion + yield-scatter only (dead manual branch); leave status-dashboard-model. Pin with stats tests. Overlaps scores audit. | small | medium |
| `[ ]` GRADE_RANK / BLUECHIP_GRADE_VALUES tables | `sel-2`, `sel-7`, `types-5` | selector `GRADE_RANK` = negation of shared rank -> use `getReportCardGradeRank` negated. snapshot.ts BLUECHIP_GRADES: derive from `BluechipGradeSchema.options` (const not exported). Coordinate with scores `cross-math-1`. | small | low |
| `[G]` Local stableJsonStringify (hash-input boundary) | `worker-store-2` | Import `stableJsonStringifyV1`; byte-identical for current integer/string payloads but feeds persisted incident_key/source_fingerprint. Snapshot-verify keys via ddrv2-store tests. A future non-integer payload would hash instead of throw. | small | medium |
| `[ ]` Inline ternary/value duplication (worker + components) | `wapi-1`, `tw-7`, `wapi-10`, `f-stablecoin-detail-1`, `w-recap-1`, `w-cron-6`, `w-cron-8`, `worker-store-6`, `worker-infra-2`, `worker-infra-3`, `worker-infra-7`, `worker-infra-9`, `fe-5`, `fe-7` | Safe: hoist confidence-tier ternary, dead props, hoist helper. Needs care: w-recap-1 import existing DEWS_BAND_RANK (NOT THREAT_BAND_ORDER — breaks typecheck); worker-infra-7 keep line-894 toLowerCase; f-stablecoin-detail-1 verify-first (code-splitting). | medium | medium |
| `[ ]` Chart-component consistency | `fviz-4`, `fviz-3`, `fviz-5`, `fviz-6`, `fviz-1`, `f-pharoswatchbot-2`, `f-pharoswatchbot-4`, `sd-7` | Safe: CHART_HEIGHT import (skip psi-history 336px branch), comparison-chart externalRange. Needs care: gradient-id must use SANITIZED useId; usePrefersReducedMotion dedup loses live `change` listener — decide explicitly. Leave ResponsiveContainer migration optional. sd-7 design judgment vs memory. | medium | medium |
| `[G]` Divergent-hue design flags (owner sign-off) | `cls-9`, `cls-10` | VAR badge(sky)/chart(slate) and GOVERNANCE centralized yellow-600/700 change rendered pixels — flag, don't auto-fix. | trivial | medium |
| `[ ]` Sentinel/SQL-param readability (scoped) | `tw-10`, `worker-store-6`, `f-pharoswatchbot-3`, `rbc-12`, `f-pharoswatchbot-1` | GATED_SENTINEL refactor touches a separate module + tests (medium risk, defer). Safe: rename `whereSql`->`filterSql`, cohort comments, optional carousel .css extraction. | small | low |
| `[G]` Design-consistency flags (left-stripe) | `sd-7` | (Also listed under chart consistency.) Cross-check `docs/design-language.md` before removing the MobileRiskSnapshot stripe. | trivial | low |
