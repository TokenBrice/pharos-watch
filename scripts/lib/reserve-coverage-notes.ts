...
export const REVIEWED_LIVE_RESERVE_SOURCE_NOTES: Record<string, LiveReserveSourceQualityNote> = {
  ...
  "eursafo-spiko": {
    sourceUrl: "https://public-api.spiko.io/share-classes/eurSAFO/totals",
    sourceQuality: "independent",
    expectedAdapterFamily: "spiko-api (implemented and live)",
    freshnessEvidence: "The spiko-api adapter reads Spiko's public share-classes totals endpoint directly, which publishes a CACEIS-administered netAssetValue.updatedAt timestamp on every business day.",
    scoreGradePlausible: true,
    note: "Superseded: EURSAFO is now wired to the spiko-api live reserve adapter (independent, single-bucket) instead of the curated-only static bucket this note originally reviewed.",
  },
  "moveusd-cfx": {
  ...
  "safo-spiko-usd": {
    sourceUrl: "https://public-api.spiko.io/share-classes/SAFO/totals",
    sourceQuality: "independent",
    expectedAdapterFamily: "spiko-api (implemented and live)",
    freshnessEvidence: "The spiko-api adapter reads Spiko's public share-classes totals endpoint directly, which publishes a CACEIS-administered netAssetValue.updatedAt timestamp on every business day.",
    scoreGradePlausible: true,
    note: "Superseded: SAFO is now wired to the spiko-api live reserve adapter (independent, single-bucket) instead of the curated-only static bucket this note originally reviewed.",
  },
  "usda-avalon": {
  ...
  "eurspkcc-spiko": {
    sourceUrl: "https://public-api.spiko.io/share-classes/eurSPKCC/totals",
    sourceQuality: "independent",
    expectedAdapterFamily: "spiko-api (implemented and live)",
    freshnessEvidence: "The spiko-api adapter reads Spiko's public share-classes totals endpoint directly, which publishes a CACEIS-administered netAssetValue.updatedAt timestamp on every business day.",
    scoreGradePlausible: true,
    note: "Superseded: EURSPKCC is now wired to the spiko-api live reserve adapter (independent, single-bucket, 100% cash-and-carry strategy exposure) instead of the curated 80/20 static split this note originally reviewed.",
  },
  "hbd-hive": {
