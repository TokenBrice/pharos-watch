...
describe("weekly curation coverage", () => {
  it("covers the current authored catalog", () => {
    ...
    expect([attestor.total - attestor.missing.length, attestor.total]).toEqual([77, 77]);
    expect([oneLiner.total - oneLiner.missing.length, oneLiner.total]).toEqual([365, 365]);
