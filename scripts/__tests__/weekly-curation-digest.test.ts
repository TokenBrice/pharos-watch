...
describe("weekly curation coverage", () => {
  it("covers the current authored catalog", () => {
    ...
    expect([attestor.total - attestor.missing.length, attestor.total]).toEqual([83, 83]);
    expect([oneLiner.total - oneLiner.missing.length, oneLiner.total]).toEqual([394, 394]);
