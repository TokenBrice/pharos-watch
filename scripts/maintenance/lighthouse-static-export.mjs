...
function printScoreSummary(report, reportPath, threshold, throttlingMethod) {
...
async function main() {
  ...
  const reportBase = path.join(outputDir, `${timestampSlug()}-${routeSlug(route)}`);
  const lighthouseArgs = [
    ...
    `--throttling-method=${options.throttlingMethod}`,
    "--chrome-flags=--headless=new --no-sandbox",
    "--quiet",
  ];

  try {
    console.log(`[lighthouse-static] Auditing ${targetUrl} (threshold ${options.threshold})`);
    ...
    printScoreSummary(report, jsonPath, options.threshold, options.throttlingMethod);
  } finally {
    if (server) {
