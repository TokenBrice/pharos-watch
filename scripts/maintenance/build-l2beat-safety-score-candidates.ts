...
interface CliOptions extends CandidateReportCliOptions {
  chainIds: string[];
}
...
export async function runCli(
  ...
  const output = renderCoverageAuditReport(audit, options.format, renderL2BeatStablecoinSafetyAuditMarkdown);
  ...
  writeCandidateReportCliOutput({
    ...
    protectedMessage: "L2BEAT candidate reports are advisory; write them under agents/ or another scratch path.",
    missingMessage: (target) => `Failed to write L2BEAT candidate report: ${target}`,
    writtenMessage: (target) => `wrote ${audit.reviewRows.length} L2BEAT Safety Score candidate row(s) to ${target}`,
  });
  return 0;
}
