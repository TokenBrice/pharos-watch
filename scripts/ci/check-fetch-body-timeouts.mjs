...
const KNOWN_FETCH_BODY_TIMEOUT_DEBT = new Set([
  ...
  "worker/src/cron/dex-liquidity/fetch-fallbacks.ts::const res = await fetchWithRetry(url, {::const data = (await res.json()) as { tickers?: CgTicker[] };",
  "worker/src/cron/dex-liquidity/fetch-fluid.ts::const res = await fetchWithRetry(::const tickers = await res.json() as unknown;",
  "worker/src/cron/dex-liquidity/fetch-pancakeswap.ts::const res = await fetchWithRetry(subgraphUrl, {::const rawBody = await res.text();",
  "worker/src/cron/dex-liquidity/geckoterminal-shared.ts::const res = await fetchWithRetry(::const json = (await res.json()) as { data?: unknown };",
  "worker/src/cron/dex-liquidity/subgraph-helpers.ts::const res = await fetchWithRetry(config.subgraphUrl, {::const json = (await res.json()) as {",
